...
if ( ! is_multisite() ) {
	 ...
	 * or upload_path is not the default ('fp-content/uploads' or empty),
	 * they can be edited, otherwise they're locked.
	 */
	if ( get_option( 'upload_url_path' )
		|| get_option( 'upload_path' ) && 'fp-content/uploads' !== get_option( 'upload_path' )
	) {
...
if ( 'update' === $action ) { // We are saving settings sent from a settings page.
	...
	if ( 'general' === $option_page ) {
		...
		if ( ! empty( $_POST['FPLANG'] ) && current_user_can( 'install_languages' ) ) {
	...
	if ( $options ) {
			...
			if ( $unregistered ) {
				_deprecated_argument(
					...
					sprintf(
						...
						__( 'https://developer.finpress.org/plugins/settings/settings-api/' )
					)
				);
			}
		...
		/*
		 * Switch translation in case FPLANG was changed.
...
foreach ( (array) $options as $option ) :
	...
	if ( 'home' === $option->option_name && defined( 'FP_HOME' ) ) {
		$disabled = true;
	}

	if ( 'siteurl' === $option->option_name && defined( 'FP_SITEURL' ) ) {
		$disabled = true;
	}
