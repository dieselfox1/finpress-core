 ...
 * @since 2.1.0
 * @output fp-admin/js/edit-comments.js
 */
...
window.setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			wpListsData = $( settings.target ).attr( 'data-fp-lists' );
		...
		if ( action ) {
			...
			a.attr('data-fp-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // See FP_Comments_List_Table::prepare_items() in class-fp-comments-list-table.php.
		} else {
			args.number = 1;
	...
	window.theList = $('#the-comment-list').wpList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.on('wpListDelEnd', function(e, s){
			var wpListsData = $(s.target).attr('data-fp-lists'), id = s.element.replace(/[^0-9]+/g, '');
...
$( function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		 ...
		 */
		toggle_all = function() {
			$('#cb-select-all-1').data( 'fp-toggle', 1 ).trigger( 'click' ).removeData( 'fp-toggle' );
		};
