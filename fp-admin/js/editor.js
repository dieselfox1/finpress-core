...
( function( $, fp ) {
	...
	function SwitchEditors() {
		...
		function init() {
			if ( ! tinymce && window.tinymce ) {
				...
				$$( document ).on( 'click', function( event ) {
					...
					if ( target.hasClass( 'fp-switch-editor' ) ) {
						id = target.attr( 'data-fp-editor-id' );
		...
		function getShortCodePositionsInText( content ) {
			...
			var shortcodeDetailsRegexp = fp.shortcode.regexp( allShortcodes.join( '|' ) ),
		...
		function scrollVisualModeToStartElement( editor, element ) {
			var elementTop = editor.$( element ).offset().top,
				...
				edTools = $( '#fp-content-editor-tools' ),
				edToolsHeight = 0,
				edToolsOffsetTop = 0,
			 ...
			 * experience.
			 *
			 * In order to find the top of the editor, we calculate the offset of `#fp-content-editor-tools` and
		 ...
		 * `fp-includes/js/tinymce/plugins/finpress/plugin.js`.
		 *
		...
		function removep( html ) {
			...
			if ( html.indexOf( '[caption' ) !== -1 ) {
				...
				html = html.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
					return a.replace( /<br([^>]*)>/g, '<fp-temp-br$1>' ).replace( /[\r\n\t]+/, '' );
				});
			}
			...
			if ( preserve.length ) {
				html = html.replace( /<fp-preserve>/g, function() {
		...
		function autop( text ) {
			...
			if ( preserve_linebreaks ) {
				text = text.replace( /<fp-line-break>/g, '\n' );
			}

			// Restore the <br> tags in captions.
			if ( preserve_br ) {
				text = text.replace( /<fp-temp-br([^>]*)>/g, '<br$1>' );
			}
		...
		fp.editor.autop = wpautop;
		fp.editor.removep = pre_wpautop;
	 ...
	 * with custom jQuery events on the document element, fp-before-tinymce-init and fp-before-quicktags-init.
	 *
	 * @since 4.8.0
	 *
	 ...
	 * }
	 */
	fp.editor.initialize = function( id, settings ) {
		var init;
		var defaults;

		if ( ! $ || ! id || ! fp.editor.getDefaultSettings ) {
			return;
		}

		defaults = fp.editor.getDefaultSettings();
		...
		if ( settings.tinymce && settings.quicktags ) {
			...
			var $wrap = $( '<div>' ).attr( {
					'class': 'fp-core-ui fp-editor-wrap tmce-active',
					id: 'fp-' + id + '-wrap'
				} );

			var $editorContainer = $( '<div class="fp-editor-container">' );

			var $button = $( '<button>' ).attr( {
					type: 'button',
					'data-fp-editor-id': id
				} );

			var $editorTools = $( '<div class="fp-editor-tools">' );
			...
			if ( settings.mediaButtons ) {
				...
				$addMediaButton.append( '<span class="fp-media-buttons-icon"></span>' );
				$addMediaButton.append( document.createTextNode( ' ' + buttonText ) );
				$addMediaButton.data( 'editor', id );

				$editorTools.append(
					$( '<div class="fp-media-buttons">' )
			...
			$wrap.append(
				$editorTools
					.append( $( '<div class="fp-editor-tabs">' )
						.append( $button.clone().attr({
							id: id + '-tmce',
							'class': 'fp-switch-editor switch-tmce'
						}).text( window.tinymce.translate( 'Visual' ) ) )
						.append( $button.attr({
							id: id + '-html',
							'class': 'fp-switch-editor switch-html'
		...
		if ( window.tinymce && settings.tinymce ) {
			...
			$( document ).trigger( 'fp-before-tinymce-init', init );
		...
		if ( window.quicktags && settings.quicktags ) {
			...
			$( document ).trigger( 'fp-before-quicktags-init', init );
	 ...
	 *
	 * Intended for use with editors that were initialized with fp.editor.initialize().
	 *
	 * @since 4.8.0
	 *
	 * @param {string} id The HTML id of the editor textarea.
	 */
	fp.editor.remove = function( id ) {
	 ...
	 */
	fp.editor.getContent = function( id ) {
		var editor;
