/**
 * @output fp-admin/js/code-editor.js
 */

if ( 'undefined' === typeof window.fp ) {
	/**
	 * @namespace fp
	 */
	window.fp = {};
}
if ( 'undefined' === typeof window.fp.codeEditor ) {
	/**
	 * @namespace fp.codeEditor
	 */
	window.fp.codeEditor = {};
}
...
( function( $, fp ) {
	...
	/**
	 * @typedef {object} fp.codeEditor~CodeEditorInstance
	 ...
	 */
	fp.codeEditor.initialize = function initialize( textarea, settings ) {
		...
		instanceSettings = $.extend( {}, fp.codeEditor.defaultSettings, settings );
		instanceSettings.codemirror = $.extend( {}, instanceSettings.codemirror );

		codemirror = fp.CodeMirror.fromTextArea( $textarea[0], instanceSettings.codemirror );
		...
		if ( codemirror.showHint ) {
			codemirror.on( 'keyup', function( editor, event ) { // eslint-disable-line complexity
				...
				innerMode = fp.CodeMirror.innerMode( codemirror.getMode(), token.state ).mode.name;
