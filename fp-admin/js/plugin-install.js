 ...
 *
 * @output fp-admin/js/plugin-install.js
 */
...
jQuery( function( $ ) {
	...
	function handleTabbables() {
		...
		$firstAndLast.off( 'keydown.fp-plugin-details' );
		// Attach again the keydown event on the first and last focusable elements.
		$firstAndLast.on( 'keydown.fp-plugin-details', function( event ) {
	...
	$( '.wrap' ).on( 'click', '.thickbox.open-plugin-details-modal', function( e ) {
		...
		tbWindow
			.attr({
				'role': 'dialog',
				'aria-label': fp.i18n.__( 'Plugin details' )
			})
