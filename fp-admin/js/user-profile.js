/**
 * @output fp-admin/js/user-profile.js
 */
...
(function($) {
	...
	function bindToggleButton() {
		...
		$toggleButton = $pass1Row.find('.fp-hide-pw');
	...
	function bindPasswordResetLink() {
		$( '#generate-reset-link' ).on( 'click', function() {
			var $this  = $(this),
				...
				var resetAction =  fp.ajax.post( 'send-password-reset', data );
	...
	function bindPasswordForm() {
		...
		$submitButton = $( '#submit, #fp-submit' ).on( 'click', function () {
			updateLock = false;
		});
		...
		$passwordWrapper = $pass1Row.find( '.fp-pwd' );
		$generateButton  = $pass1Row.find( 'button.fp-generate-pw' );
		...
		$generateButton.on( 'click', function () {
			...
			fp.ajax.post( 'generate-password' )
				.done( function( data ) {
					$pass1.data( 'pw', data );
				} );
		} );

		$cancelButton = $pass1Row.find( 'button.fp-cancel-pw' );
	...
	function check_pass_strength() {
		...
		strength = fp.passwordStrength.meter( pass1, fp.passwordStrength.userInputDisallowedList(), pass1 );
	...
	clipboard.on( 'success', function( e ) {
		...
		fp.a11y.speak( __( 'Application password has been copied to your clipboard.' ) );
	} );
	...
	$( function() {
		var $colorpicker, $stylesheet, user_id, current_user_id,
			...
			greeting     = $( '#fp-admin-bar-my-account' ).find( '.display-name' );
		...
		$colorpicker.on( 'click.colorpicker', '.color-option', function() {
			...
			if ( user_id === current_user_id ) {
				...
				// Repaint icons.
				if ( typeof fp !== 'undefined' && fp.svgPainter ) {
					try {
						colors = JSON.parse( $this.children( '.icon_colors' ).val() );
					} catch ( error ) {}

					if ( colors ) {
						fp.svgPainter.setColors( colors );
						fp.svgPainter.paint();
					}
				}
	...
	$( function() {
		if ( $( '.reset-pass-submit' ).length ) {
			$( '.reset-pass-submit button.fp-generate-pw' ).trigger( 'click' );
		}
	});
