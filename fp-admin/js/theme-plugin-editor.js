/**
 * @output fp-admin/js/theme-plugin-editor.js
 */
...
fp.themePluginEditor = (function( $ ) {
	'use strict';
	var component, TreeLinks,
		__ = fp.i18n.__, _n = fp.i18n._n, sprintf = fp.i18n.sprintf;
	...
	component.init = function init( form, settings ) {
		...
		component.noticeTemplate = fp.template( 'fp-file-editor-notice' );
		...
		if ( false !== component.codeEditor ) {
			/*
			 * Defer adding notices until after DOM ready as workaround for FP Admin injecting
	...
	component.showWarning = function() {
		...
		setTimeout( function() {
			fp.a11y.speak( fp.sanitize.stripTags( rawMessage.replace( /\s+/g, ' ' ) ), 'assertive' );
		}, 1000 );
	};
	...
	component.submit = function( event ) {
		...
		request = fp.ajax.post( 'edit-theme-plugin-file', data );
	...
	component.initCodeEditor = function initCodeEditor() {
		...
		editor = fp.codeEditor.initialize( $( '#newcontent' ), codeEditorSettings );
 ...
 * @type {object}
 */
fp.themePluginEditor.l10n = fp.themePluginEditor.l10n || {
	saveAlert: '',
	saveError: '',
	lintError: {
		alternative: 'fp.i18n',
...
fp.themePluginEditor.l10n = window.fp.deprecateL10nObject( 'fp.themePluginEditor.l10n', fp.themePluginEditor.l10n, '5.5.0' );
