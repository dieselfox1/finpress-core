/**
 * @output fp-admin/js/widgets/media-video-widget.js
 */
...
(function( component ) {
	 ...
	 *
	 * @class    fp.mediaWidgets.controlConstructors~VideoDetailsMediaFrame
	 * @augments fp.media.view.MediaFrame.VideoDetails
	 *
	 * @private
	 */
	VideoDetailsMediaFrame = fp.media.view.MediaFrame.VideoDetails.extend(/** @lends fp.mediaWidgets.controlConstructors~VideoDetailsMediaFrame.prototype */{
		...
		createStates: function createStates() {
			this.states.add([
				new fp.media.controller.VideoDetails({
					media: this.media
				}),
				...
				new fp.media.controller.MediaLibrary({
					type: 'video',
					id: 'add-video-source',
					title: fp.media.view.l10n.videoAddSourceTitle,
				...
				new fp.media.controller.MediaLibrary({
					type: 'text',
					id: 'add-track',
					title: fp.media.view.l10n.videoAddTrackTitle,
					toolbar: 'add-track',
					media: this.media,
					menu: 'video-details'
				})
	 ...
	 * Video widget model.
	 *
	 * See FP_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @class    fp.mediaWidgets.modelConstructors.media_video
	 ...
	 * See FP_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @class    fp.mediaWidgets.controlConstructors.media_video
	 * @augments fp.mediaWidgets.MediaWidgetControl
	 */
	VideoWidgetControl = component.MediaWidgetControl.extend(/** @lends fp.mediaWidgets.controlConstructors.media_video.prototype */{
		...
		fetchEmbed: function fetchEmbed() {
			...
			control.fetchEmbedDfd = fp.apiRequest({
		...
		renderPreview: function renderPreview() {
			...
			if ( mime && attachmentId ) {
				if ( ! _.contains( _.values( fp.media.view.settings.embedMimes ), mime ) ) {
				...
				if ( matches ) {
					if ( ! _.contains( _.keys( fp.media.view.settings.embedMimes ), matches[1] ) ) {
						error = 'unsupported_file_type';
					}
				} else {
					isOEmbed = true;
			...
			previewTemplate = fp.template( 'fp-media-widget-video-preview' );
