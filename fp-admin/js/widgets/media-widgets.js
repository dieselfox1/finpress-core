/**
 * @output fp-admin/js/widgets/media-widgets.js
 */

/* eslint consistent-this: [ "error", "control" ] */

/**
 * @namespace fp.mediaWidgets
 * @memberOf  fp
 */
fp.mediaWidgets = ( function( $ ) {
	'use strict';
	 ...
	 *
	 * @memberOf fp.mediaWidgets
	 *
	 * @type {Object.<string, fp.mediaWidgets.MediaWidgetModel>}
	 */
	...
	component.PersistentDisplaySettingsLibrary = fp.media.controller.Library.extend(/** @lends fp.mediaWidgets.PersistentDisplaySettingsLibrary.prototype */{

		/**
		 * Library which persists the customized display settings across selections.
		 *
		 * @constructs fp.mediaWidgets.PersistentDisplaySettingsLibrary
		 * @augments   fp.media.controller.Library
		 *
		...
		initialize: function initialize( options ) {
			...
			fp.media.controller.Library.prototype.initialize.call( this, options );
		},
		...
		display: function getDisplaySettingsModel( model ) {
			...
			display = fp.media.controller.Library.prototype.display.call( this, model );
	 ...
	 *
	 * @class    fp.mediaWidgets.MediaEmbedView
	 * @augments fp.media.view.Embed
	 */
	component.MediaEmbedView = fp.media.view.Embed.extend(/** @lends fp.mediaWidgets.MediaEmbedView.prototype */{
		...
		initialize: function( options ) {
			...
			fp.media.view.Embed.prototype.initialize.call( view, options );
		 ...
		 * Refresh embed view.
		 *
		 * Forked override of {fp.media.view.Embed#refresh()} to suppress irrelevant "link text" field.
		 *
		 * @return {void}
		 */
		refresh: function refresh() {
			/**
			 * @class fp.mediaWidgets~Constructor
			 */
			var Constructor;

			if ( 'image' === this.controller.options.mimeType ) {
				Constructor = fp.media.view.EmbedImage;
			} else {

				// This should be eliminated once #40450 lands of when this is merged into core.
				Constructor = fp.media.view.EmbedLink.extend(/** @lends fp.mediaWidgets~Constructor.prototype */{
					...
					updateoEmbed: function() {
						...
						fp.media.view.EmbedLink.prototype.updateoEmbed.call( embedLinkView );
					},
					...
					fetch: function() {
						...
						if ( matches ) {
							fileExt = matches[1];
							if ( ! fp.media.view.settings.embedMimes[ fileExt ] ) {
								embedLinkView.renderFail();
							} else if ( 0 !== fp.media.view.settings.embedMimes[ fileExt ].indexOf( embedLinkView.controller.options.mimeType ) ) {
								embedLinkView.renderFail();
							} else {
						...
						embedLinkView.dfd = fp.apiRequest({
							url: fp.media.view.settings.oEmbedProxyUrl,
							data: {
								url: url,
	 ...
	 *
	 * @class    fp.mediaWidgets.MediaFrameSelect
	 * @augments fp.media.view.MediaFrame.Post
	 */
	component.MediaFrameSelect = fp.media.view.MediaFrame.Post.extend(/** @lends fp.mediaWidgets.MediaFrameSelect.prototype */{
		...
		createStates: function createStates() {
			...
			_.each( fp.media.view.settings.embedMimes, function( embedMime ) {
			...
			this.states.add([
				...
				new fp.media.controller.EditImage({ model: this.options.editImage }),

				// Embed states.
				new fp.media.controller.Embed({
		 ...
		 * Main insert toolbar.
		 *
		 * Forked override of {fp.media.view.MediaFrame.Post#mainInsertToolbar()} to override text.
		 *
		 * @param {fp.Backbone.View} view - Toolbar view.
		 * @this {fp.media.controller.Library}
		 * @return {void}
		 */
		mainInsertToolbar: function mainInsertToolbar( view ) {
			...
			view.set( 'insert', {
				 ...
				 * @ignore
				 *
				 * @fires fp.media.controller.State#insert()
				 * @return {void}
				 */
		 ...
		 * Main embed toolbar.
		 *
		 * Forked override of {fp.media.view.MediaFrame.Post#mainEmbedToolbar()} to override text.
		 *
		 * @param {fp.Backbone.View} toolbar - Toolbar view.
		 * @this {fp.media.controller.Library}
		 * @return {void}
		 */
		mainEmbedToolbar: function mainEmbedToolbar( toolbar ) {
			toolbar.view = new fp.media.view.Toolbar.Embed({
				controller: this,
		 ...
		 * Embed content.
		 *
		 * Forked override of {fp.media.view.MediaFrame.Post#embedContent()} to suppress irrelevant "link text" field.
		 *
		 * @return {void}
		 */
	...
	component.MediaWidgetControl = Backbone.View.extend(/** @lends fp.mediaWidgets.MediaWidgetControl.prototype */{

		/**
		 * Translation strings.
		 *
		 * The mapping of translation strings is handled by media widget subclasses,
		 * exported from PHP to JS such as is done in FP_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {Object}
		 */
		l10n: {
		 ...
		 * such as is done in FP_Widget_Media_Image::enqueue_admin_scripts(). If not,
		 ...
		 * such as is done in FP_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {string}
		 */
		mime_type: '',
		 ...
		 *
		 * @constructs fp.mediaWidgets.MediaWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.selectedAttachment = new fp.media.model.Attachment();
			 ...
			 * from the start, without having to sync with hidden fields. See <https://core.trac.finpress.org/ticket/33507>.
			 */
			...
			control.displaySettings = new Backbone.Model( _.pick(
				...
				_.keys( fp.media.view.settings.defaultProps )
			) );
		},
		...
		updateSelectedAttachment: function updateSelectedAttachment() {
			...
			if ( 0 === control.model.get( 'attachment_id' ) ) {
				...
				attachment = new fp.media.model.Attachment({
		...
		template: function template() {
			...
			return fp.template( 'widget-media-' + control.id_base + '-control' );
		},
		...
		selectMedia: function selectMedia() {
			...
			selection = new fp.media.model.Selection( selectionModels, { multiple: false } );
			...
			fp.media.frame = mediaFrame; // See fp.media().
			...
			// Disable syncing of attachment changes back to server (except for deletions). See <https://core.trac.finpress.org/ticket/40403>.
			defaultSync = fp.media.model.Attachment.prototype.sync;
			fp.media.model.Attachment.prototype.sync = function( method ) {
				if ( 'delete' === method ) {
			...
			mediaFrame.on( 'close', function onClose() {
				fp.media.model.Attachment.prototype.sync = defaultSync;
			});
		 ...
		 *
		 * @param {fp.media.view.MediaFrame.Select} mediaFrame - Select frame.
		 * @return {Object} Props.
		 */
		getModelPropsFromMediaFrame: function getModelPropsFromMediaFrame( mediaFrame ) {
			...
			_.each( fp.media.view.settings.embedExts, function( ext ) {
	 ...
	 * Media widget model.
	 *
	 * @class    fp.mediaWidgets.MediaWidgetModel
	 * @augments Backbone.Model
	 */
	component.MediaWidgetModel = Backbone.Model.extend(/** @lends fp.mediaWidgets.MediaWidgetModel.prototype */{
		 ...
		 * @return {fp.mediaWidgets.MediaWidgetModel} This model.
		 */
	 ...
	 * @memberOf fp.mediaWidgets
	 *
	 * @type {Object.<string, fp.mediaWidgets.MediaWidgetControl>}
	 */
	...
	component.handleWidgetAdded = function handleWidgetAdded( event, widgetContainer ) {
		 ...
		 * and replaced with the rendered output of FP_Widget::form() which is
	 ...
	 * attached which calls fp.mediaWidgets.init().
	 *
	 * @memberOf fp.mediaWidgets
	 *
	 * @return {void}
