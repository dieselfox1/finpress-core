/**
 * @output fp-admin/js/widgets/text-widgets.js
 */
...
/**
 * @namespace fp.textWidgets
 */
fp.textWidgets = ( function( $ ) {
	'use strict';
	...
	component.TextWidgetControl = Backbone.View.extend(/** @lends fp.textWidgets.TextWidgetControl.prototype */{
		 ...
		 * Text widget control.
		 *
		 * @constructs fp.textWidgets.TextWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.$el.html( fp.template( 'widget-text-control-fields' ) );

			control.customHtmlWidgetPointer = control.$el.find( '.fp-pointer.custom-html-widget-pointer' );
			...
			control.pasteHtmlPointer = control.$el.find( '.fp-pointer.paste-html-pointer' );
		...
		dismissPointers: function dismissPointers( pointers ) {
			_.each( pointers, function( pointer ) {
				fp.ajax.post( 'dismiss-fp-pointer', {
					pointer: pointer
				});
		...
		openAvailableWidgetsPanel: function openAvailableWidgetsPanel() {
			...
			fp.customize.section.each( function( section ) {
				if ( section.extended( fp.customize.Widgets.SidebarSection ) && section.expanded() ) {
					sidebarControl = fp.customize.control( 'sidebars_widgets[' + section.params.sidebarId + ']' );
				}
			});
			if ( ! sidebarControl ) {
				return;
			...
			setTimeout( function() { // Timeout to prevent click event from causing panel to immediately collapse.
				fp.customize.Widgets.availableWidgetsPanel.open( sidebarControl );
				fp.customize.Widgets.availableWidgetsPanel.$search.val( 'HTML' ).trigger( 'keyup' );
			});
		},
		...
		updateFields: function updateFields() {
			...
			if ( control.fields.text.is( ':visible' ) ) {
				...
				control.editor.setContent( fp.oldEditor.autop( syncInput.val() ) );
			}
		},
		...
		initializeEditor: function initializeEditor() {
			...
			triggerChangeIfDirty = function() {
				var updateWidgetBuffer = 300; // See fp.customize.Widgets.WidgetControl._setupUpdateUI() which uses 250ms for updateWidgetDebounced.
				if ( control.editor.isDirty() ) {
					 ...
					 * having to make server round-trips to call the respective FP_Widget::update()
					 * callbacks. See <https://core.trac.finpress.org/ticket/33507>.
					 */
					if ( fp.customize && fp.customize.state ) {
						fp.customize.state( 'processing' ).set( fp.customize.state( 'processing' ).get() + 1 );
						_.delay( function() {
							fp.customize.state( 'processing' ).set( fp.customize.state( 'processing' ).get() - 1 );
						}, updateWidgetBuffer );
					}
			...
			function buildEditor() {
				...
				if ( tinymce.get( id ) ) {
					...
					fp.oldEditor.remove( id );
				}

				// Add or enable the `wpview` plugin.
				$( document ).one( 'fp-before-tinymce-init.text-widget-init', function( event, init ) {
				...
				showPointerElement = function( pointerElement ) {
					...
					fp.a11y.speak( pointerElement.find( 'h3, p' ).map( function() {
						return $( this ).text();
					} ).get().join( '\n\n' ) );
				};
	 ...
	 *
	 * @memberOf fp.textWidgets
	 *
	 * @type {Object.<string, fp.textWidgets.TextWidgetControl>}
	 */
	 ...
	 * attached which calls fp.textWidgets.init().
	 *
	 * @memberOf fp.textWidgets
	 *
	 * @return {void}
