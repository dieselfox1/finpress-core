...
// Setup app structure.
_.extend( themes, { model: {}, view: {}, routes: {}, router: {}, template: fp.template });
...
themes.view.Appearance = fp.Backbone.View.extend({
	...
	search: function() {
		...
		this.searchContainer
			.find( '.search-box' )
			.append( $.parseHTML( '<label for="fp-filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el );
...
themes.Collection = Backbone.Collection.extend({
	...
	// Send request to api.finpress.org/themes.
	apiCall: function( request, paginated ) {
		return fp.ajax.send( 'query-themes', {
			data: {
				// Request data.
				request: _.extend({
					per_page: 100
...
themes.view.Theme = fp.Backbone.View.extend({
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'fp-theme-install-success', function( event, response ) {
	...
	updateTheme: function( event ) {
		...
		$( document ).on( 'fp-theme-update-success', function( event, response ) {
...
themes.view.Details = fp.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.fp-themes', function( event ) {
	...
	autoupdateState: function() {
		...
		callback = function( event, data ) {
			...
			if ( _this.model.get( 'id' ) === data.asset ) {
				...
				$( document ).off( 'fp-auto-update-setting-changed', callback );
			}
		};

		// Triggered in updates.js
		$( document ).on( 'fp-auto-update-setting-changed', callback );
	},
	...
	deleteTheme: function( event ) {
		...
		if ( ! window.confirm( fp.themes.data.settings.confirmDelete ) ) {
			return;
		}

		fp.updates.maybeRequestFilesystemCredentials( event );

		$( document ).one( 'fp-theme-delete-success', function( event, response ) {
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'fp-full-overlay expanded',
	...
	togglePreviewDeviceButtons: function( newDevice ) {
		var $devices = $( '.fp-full-overlay-footer .devices' );
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'fp-theme-install-success', function() {
			_this.model.set( { 'installed': true } );
		} );
...
themes.view.Themes = fp.Backbone.View.extend({

	className: 'themes fp-clearfix',
		...
		if ( this.model.get( 'hasUpdate' ) ) {
			...
			if ( $card.find( '.updating-message' ).length ) {
				...
				$modal.find( '.notice-warning' )
					...
					.find( 'p' ).text( fp.updates.l10n.updating );
	...
	announceSearchResults: function( count ) {
		if ( 0 === count ) {
			fp.a11y.speak( l10n.noThemesFound );
		} else {
			fp.a11y.speak( l10n.themesFound.replace( '%d', count ) );
		}
	}
});

// Search input view controller.
themes.view.Search = fp.Backbone.View.extend({

	tagName: 'input',
	className: 'fp-filter-search',
	id: 'fp-filter-search-input',
	searching: false,
...
themes.Router = Backbone.Router.extend({
	...
	search: function( query ) {
		$( '.fp-filter-search' ).val( query.replace( /\+/g, ' ' ) );
	},
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.fp-filter-search' ).trigger( 'keyup' );
		});
...
themes.view.InstallerSearch =  themes.view.Search.extend({
	...
	doSearch: function( value ) {
		...
		// Get the themes by sending Ajax POST request to api.finpress.org/themes
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	applyFilters: function( event ) {
		...
		if ( ! tags ) {
			fp.a11y.speak( l10n.selectFeatureFilter );
			return;
		}
	...
	saveUsername: function ( event ) {
		...
		return fp.ajax.send( 'save-wporg-username', {
			data: {
				_wpnonce: nonce,
				username: username
			},
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		// Bind to our global `fp.themes` object
		...
		themes.router.on( 'route:search', function() {
			$( '.fp-filter-search' ).trigger( 'focus' ).trigger( 'keyup' );
		});
...
$( function() {
	...
	$( document.body ).on( 'click', '.load-customize', function() {
		...
		urlParser.search = $.param( _.extend(
			fp.customize.utils.parseQueryString( urlParser.search.substr( 1 ) ),
			{
