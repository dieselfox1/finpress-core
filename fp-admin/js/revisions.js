 ...
 *
 * @output fp-admin/js/revisions.js
 */
...
(function($) {
	var revisions;
	/**
	 * Expose the module in window.fp.revisions.
	 */
	revisions = fp.revisions = { model: {}, view: {}, controller: {} };
	...
	revisions.model.Diffs = Backbone.Collection.extend({
		...
		load: function( comparisons ) {
			fp.revisions.log( 'load', comparisons );
			...
			return this.fetch({ data: { compare: comparisons }, remove: false }).done( function() {
				fp.revisions.log( 'load:complete', comparisons );
			});
		},
		...
		sync: function( method, model, options ) {
			if ( 'read' === method ) {
				...
				var deferred = fp.ajax.send( options ),
	...
	/**
	 * fp.revisions.model.FrameState
	 *
	 * The frame state.
	 *
	 * @see fp.revisions.view.Frame
	 *
	 ...
	 */
	revisions.view.Frame = fp.Backbone.View.extend({
		className: 'revisions',
		template: fp.template('revisions-frame'),
		...
		render: function() {
			fp.Backbone.View.prototype.render.apply( this, arguments );
	...
	revisions.view.Controls = fp.Backbone.View.extend({
		...
		ready: function() {
			...
			this.window = $(window);
			this.window.on( 'scroll.fp.revisions', {controls: this}, function(e) {
				...
				if ( scrolled >= controls.top ) {
					if ( ! frame.$el.hasClass('pinned') ) {
						...
						controls.window.on('resize.fp.revisions.pinning click.fp.revisions.pinning', {controls: controls}, function(e) {
							e.data.controls.setWidth();
						});
					}
					frame.$el.addClass('pinned');
				} else if ( frame.$el.hasClass('pinned') ) {
					controls.window.off('.fp.revisions.pinning');
	...
	// The tickmarks view.
	revisions.view.Tickmarks = fp.Backbone.View.extend({
	...
	// The metabox view.
	revisions.view.Metabox = fp.Backbone.View.extend({
	...
	revisions.view.Meta = fp.Backbone.View.extend({
		template: fp.template('revisions-meta'),
	...
	// The checkbox view.
	revisions.view.Checkbox = fp.Backbone.View.extend({
		className: 'revisions-checkbox',
		template: fp.template('revisions-checkbox'),
	...
	// The slider visually hidden help view.
 	revisions.view.SliderHelp = fp.Backbone.View.extend({
		className: 'revisions-slider-hidden-help',
		template:  fp.template( 'revisions-slider-hidden-help' )
	});
   
	// The tooltip view.
	// Encapsulates the tooltip.
	revisions.view.Tooltip = fp.Backbone.View.extend({
	...
	revisions.view.Buttons = fp.Backbone.View.extend({
		className: 'revisions-buttons',
		template: fp.template('revisions-buttons'),
	...
	// The slider view.
	revisions.view.Slider = fp.Backbone.View.extend({
		className: 'fp-slider',
		direction: isRtl ? 'right' : 'left',
		...
		start: function( event, ui ) {
			...
			$( window ).on( 'mousemove.fp.revisions', { view: this }, function( e ) {
				var handles,
					view              = e.data.view,
		...
		stop: function() {
			$( window ).off('mousemove.fp.revisions');
	...
	revisions.view.Diff = fp.Backbone.View.extend({
		className: 'revisions-diff',
		template:  fp.template('revisions-diff'),
