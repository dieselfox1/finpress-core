 ...
 *
 * @since 2.7.0
 * @output fp-admin/js/inline-edit-post.js
 */
	...
	setBulk : function(){
		...
		$( 'tbody th.check-column input[type="checkbox"]' ).each( function() {
			...
			if ( $(this).prop('checked') ) {
				c = false;
				var id = $( this ).val(),
					theTitle = $( '#inline_' + id + ' .post_title' ).html() || fp.i18n.__( '(no title)' ),
					buttonVisuallyHiddenText = fp.i18n.sprintf(
						/* translators: %s: Post title. */
						fp.i18n.__( 'Remove &#8220;%s&#8221; from Bulk Edit' ),
						theTitle
					);
		...
		$( '.inline-edit-categories input[name="post_category[]"]' ).each( function() {
			if ( categories[ $( this ).val() ] == checkedPosts.length ) {
				...
				if ( ! $( this ).parent().find( 'input[name="indeterminate_post_category[]"]' ).length ) {
					...
					$( this ).after( '<input type="hidden" name="indeterminate_post_category[]" value="' + $( this ).val() + '">' ).attr( 'aria-label', label.trim() + ': ' + fp.i18n.__( 'Some selected posts have this category' ) );
				}
			}
		} );
		...
		$( '#bulk-titles .ntdelbutton' ).click( function() {
			...
			$( '#_' + id ).parent().remove();
			fp.a11y.speak( fp.i18n.__( 'Item removed.' ), 'assertive' );
			...
			if ( $next.length ) {
				...
				fp.a11y.speak( fp.i18n.__( 'All selected items have been removed. Select new items to use Bulk Actions.' ) );
			}
		});

		// Enable auto-complete for tags when editing posts.
		if ( 'post' === type ) {
			$( 'tr.inline-editor textarea[data-fp-taxonomy]' ).each( function ( i, element ) {
				/*
	...
	edit : function(id) {
		...
		$('.tags_input', rowData).each(function(){
			var terms = $(this),
				...
				comma = fp.i18n._x( ',', 'tag delimiter' ).trim();
					...
					if ( -1 !== r.indexOf( '<tr' ) ) {
						...
						$( inlineEditPost.what + id ).hide().fadeIn( 400, function() {
							...
							$( this ).find( '.editinline' )
								...
								.trigger( 'focus' );
							fp.a11y.speak( fp.i18n.__( 'Changes saved.' ) );
						});
					} else {
						r = r.replace( /<.[^<>]*?>/g, '' );
						$errorNotice.removeClass( 'hidden' );
						$error.html( r );
						fp.a11y.speak( $error.text() );
					}
				} else {
					$errorNotice.removeClass( 'hidden' );
					$error.text( fp.i18n.__( 'Error while saving the changes.' ) );
					fp.a11y.speak( fp.i18n.__( 'Error while saving the changes.' ) );
				}
			},
		'html');
...
$( function() {
	...
	if ( typeof fp !== 'undefined' && fp.heartbeat ) {
		fp.heartbeat.interval( 10 );
	}
}).on( 'heartbeat-tick.fp-check-locked-posts', function( e, data ) {
	var locked = data['fp-check-locked-posts'] || {};

	$('#the-list tr').each( function(i, el) {
		var key = el.id, row = $(el), lock_data, avatar;

		if ( locked.hasOwnProperty( key ) ) {
			if ( ! row.hasClass('fp-locked') ) {
				...
				row.addClass('fp-locked');
			}
		} else if ( row.hasClass('fp-locked') ) {
			row.removeClass( 'fp-locked' ).find( '.locked-info span' ).empty();
		}
	});
}).on( 'heartbeat-send.fp-check-locked-posts', function( e, data ) {
	var check = [];
	...
	if ( check.length ) {
		data['fp-check-locked-posts'] = check;
	}
});
