/**
 * @output fp-admin/js/customize-controls.js
 */

/* global _wpCustomizeHeader, _wpCustomizeBackground, _wpMediaViewsL10n, MediaElementPlayer, console, confirm */
(function( exports, $ ){
	var Container, focus, normalizedTransitionendEventName, api = fp.customize;
	...
	api.OverlayNotification = api.Notification.extend(/** @lends fp.customize.OverlayNotification.prototype */{
		 ...
		 *
		 * @constructs fp.customize.OverlayNotification
		 * @augments   fp.customize.Notification
		 *
		 * @since 4.9.0
		 *
	...
	api.Notifications = api.Values.extend(/** @lends fp.customize.Notifications.prototype */{
		 ...
		 * @since 4.9.0
		 *
		 * @constructs fp.customize.Notifications
		 * @augments   fp.customize.Values
		 *
		 ...
		 * @since 4.9.0
		 *
		 * @param {string|fp.customize.Notification} notification - Notification object to add. Alternatively code may be supplied, and in that case the second notificationObject argument must be supplied.
		 * @param {fp.customize.Notification} [notificationObject] - Notification to add when first argument is the code string.
		 * @return {fp.customize.Notification} Added notification (or existing instance if it was already added).
		 */
		 ...
		 * @return {Array.<fp.customize.Notification>} Notifications.
		 */
		get: function( args ) {
		...
		render: function() {
			...
			_.each( notifications, function( notification ) {
				...
				if ( fp.a11y && ( ! previousNotificationsByCode[ notification.code ] || ! _.isEqual( notification.message, previousNotificationsByCode[ notification.code ].message ) ) ) {
					fp.a11y.speak( notification.message, 'assertive' );
				}
	...
	api.Setting = api.Value.extend(/** @lends fp.customize.Setting.prototype */{
		 ...
		 *
		 * @see PHP class FP_Customize_Setting.
		 *
		 * @constructs fp.customize.Setting
		 * @augments   fp.customize.Value
		 *
		 * @since 3.4.0
		 *
		 ...
		 * @param {Object}  [options.previewer]         - The Previewer instance to sync with. Defaults to fp.customize.previewer.
		 */
		 ...
		 *
		 * @since 4.6.0
		 * @return {fp.customize.Control[]} Controls associated with setting.
		 */
	 ...
	 *
	 * @alias fp.customize._latestRevision
	 *
	 * @since 4.7.0
	 * @type {number}
	 * @protected
	 ...
	 *
	 * @alias fp.customize._lastSavedRevision
	 *
	 * @since 4.7.0
	 * @type {number}
	 * @protected
	 ...
	 *
	 * @alias fp.customize._latestSettingRevisions
	 *
	 * @since 4.7.0
	 * @type {object}
	 * @protected
	 ...
	 *
	 * @alias fp.customize.dirtyValues
	 *
	 * @since 4.7.0
	 * @access public
	 *
	 ...
	 *
	 * @alias fp.customize.requestChangesetUpdate
	 *
	 * @since 4.7.0
	 * @access public
	 *
	...
	api.requestChangesetUpdate = function requestChangesetUpdate( changes, args ) {
		...
		// A status would cause a revision to be made, and for this fp.customize.previewer.save() should be used.
		...
		request = fp.ajax.post( 'customize_save', data );
	 ...
	 *
	 * @alias fp.customize.utils.bubbleChildValueChanges
	 *
	 * @since 4.1.0
	 *
	 * @param {fp.customize.Class} instance
	 ...
	 *
	 * @alias fp.customize.utils.prioritySort
	 *
	 * @since 4.1.0
	 *
	 * @param {(fp.customize.Panel|fp.customize.Section|fp.customize.Control)} a
	 * @param {(fp.customize.Panel|fp.customize.Section|fp.customize.Control)} b
	 * @return {number}
	 */
	 ...
	 *
	 * @alias fp.customize.utils.isKeydownButNotEnterEvent
	 *
	 * @since 4.1.0
	 *
	 ...
	 *
	 * @alias fp.customize.utils.areElementListsEqual
	 *
	 * @since 4.1.0
	 *
	 ...
	 *
	 * @alias fp.customize.utils.highlightButton
	 *
	 * @since 4.9.0
	 *
	 ...
	 *
	 * @alias fp.customize.utils.getCurrentTimestamp
	 *
	 * @since 4.9.0
	 *
	 ...
	 *
	 * @alias fp.customize.utils.getRemainingTime
	 *
	 * @since 4.9.0
	 *
	...
	Container = api.Class.extend(/** @lends fp.customize~Container.prototype */{
		 ...
		 *
		 * @constructs fp.customize~Container
		 * @augments   fp.customize.Class
		 *
		 * @since 4.1.0
		 *
		 * @borrows fp.customize~focus as focus
		 *
		 ...
		 * @param {string}  [options.type=default] - The type of the panel. See fp.customize.panelConstructor.
		...
		_animateChangeExpanded: function( completeCallback ) {
			...
			var construct = this,
				...
				overlay = content.closest( '.fp-full-overlay' ),
			...
			_.defer( function() {
				var container = content.closest( '.fp-full-overlay-sidebar-content' ),
		...
		getContainer: function () {
			...
			if ( 0 !== $( '#tmpl-' + container.templateSelector ).length ) {
				template = fp.template( container.templateSelector );
			} else {
				template = fp.template( 'customize-' + container.containerType + '-default' );
			}
	...
	api.Section = Container.extend(/** @lends fp.customize.Section.prototype */{
		...
		/**
		 * @constructs fp.customize.Section
		 * @augments   fp.customize~Container
		 *
		 * @since 4.1.0
		 *
		 ...
		 * @param {string}  [options.type=default] - The type of the section. See fp.customize.sectionConstructor.
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.headContainer.closest( '.fp-full-overlay-sidebar-content' ),
				content = section.contentContainer,
				overlay = section.headContainer.closest( '.fp-full-overlay' ),
	...
	api.ThemesSection = api.Section.extend(/** @lends fp.customize.ThemesSection.prototype */{
		currentTheme: '',
		overlay: '',
		template: '',
		...
		ready: function() {
			...
			section.template = fp.template( 'customize-themes-details-view' );
		...
		attachEvents: function () {
			...
			if ( 'local' === section.params.filter_type ) {
				...
				section.container.on( 'input', '.fp-filter-search-themes', function( event ) {
				...
				section.contentContainer.on( 'input', '.fp-filter-search', function() {
				...
				api.section.each( function ( otherSection ) {
					...
					if ( otherSection !== section ) {
						...
						if ( 'themes' === otherSection.params.type ) {
							searchTerm = otherSection.contentContainer.find( '.fp-filter-search' ).val();
							section.contentContainer.find( '.fp-filter-search' ).val( searchTerm );
		...
		loadThemes: function() {
			...
			// Load themes.
			section.headContainer.closest( '.fp-full-overlay' ).addClass( 'loading' );
			section.loading = true;
			section.container.find( '.no-themes' ).hide();
			request = fp.ajax.post( 'customize_load_themes', params );
			request.done(function( data ) {
				...
				if ( 0 !== themes.length ) {
					...
					if ( 1 === page ) {
						...
						if ( 'local' !== section.params.filter_type ) {
							fp.a11y.speak( api.settings.l10n.themeSearchResults.replace( '%d', data.info.results ) );
						}
					}
					...
					if ( 0 === section.loaded ) {
						...
						fp.a11y.speak( section.container.find( '.no-themes' ).text() );
					} else {
						section.fullyLoaded = true;
					}
				}
				...
				section.headContainer.closest( '.fp-full-overlay' ).removeClass( 'loading' );
				section.loading = false;
			});
			request.fail(function( data ) {
				if ( 'undefined' === typeof data ) {
					section.container.find( '.unexpected-error' ).show();
					fp.a11y.speak( section.container.find( '.unexpected-error' ).text() );
		...
		filterSearch: function( term ) {
			...
			if ( 0 === count ) {
				...
				fp.a11y.speak( section.container.find( noFilter ).text() );
			} else {
		 ...
		 * @since 4.9.0
		 *
		 * @param {fp.customize.ThemesSection} section - The current theme section, passed through the debouncer.
		 * @return {void}
		 */
		checkTerm: function( section ) {
			var newTerm;
			if ( 'remote' === section.params.filter_type ) {
				newTerm = section.contentContainer.find( '.fp-filter-search' ).val();
		...
		updateCount: function( count ) {
			...
			if ( 0 === count ) {
				...
				fp.a11y.speak( api.settings.l10n.announceThemeCount.replace( '%d', count ) );
			}
		},
		 ...
		 * @since 4.2.0
		 *
		 * @return {fp.customize.ThemeControl|boolean} Next theme.
		 */
		 ...
		 *
		 * @since 4.2.0
		 * @return {fp.customize.ThemeControl|boolean} Previous theme.
		 */
		...
		showDetails: function ( theme, callback ) {
			...
			fp.a11y.speak( api.settings.l10n.announceThemeDetails.replace( '%s', theme.name ) );
			if ( callback ) {
				callback();
			}
		},
	...
	api.OuterSection = api.Section.extend(/** @lends fp.customize.OuterSection.prototype */{

		/**
		 * Class fp.customize.OuterSection.
		 *
		 * Creates section outside of the sidebar, there is no ui to trigger collapse/expand so
		 * it would require custom handling.
		 *
		 * @constructs fp.customize.OuterSection
		 * @augments   fp.customize.Section
		 *
		 * @since 4.9.0
		 *
	...
	api.Panel = Container.extend(/** @lends fp.customize.Panel.prototype */{
		containerType: 'panel',

		/**
		 * @constructs fp.customize.Panel
		 * @augments   fp.customize~Container
		 *
		 * @since 4.1.0
		 *
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var panel = this,
				...
				overlay = accordionSection.closest( '.fp-full-overlay' ),
				container = accordionSection.closest( '.fp-full-overlay-sidebar-content' ),
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + panel.templateSelector + '-content' ).length ) {
				template = fp.template( panel.templateSelector + '-content' );
			} else {
				template = fp.template( 'customize-panel-default-content' );
			}
	...
	api.ThemesPanel = api.Panel.extend(/** @lends fp.customize.ThemsPanel.prototype */{

		/**
		 *  Class fp.customize.ThemesPanel.
		 *
		 * Custom section for themes that displays without the customize preview.
		 *
		 * @constructs fp.customize.ThemesPanel
		 * @augments   fp.customize.Panel
		 *
		 * @since 4.9.0
		 *
		 * @param {string} id - The ID for the panel.
		...
		attachEvents: function() {
			...
			panel.contentContainer.on( 'click', '.customize-themes-section-title, .customize-themes-mobile-back', function() {
				$( '.fp-full-overlay' ).toggleClass( 'showing-themes' );
			});
		...
		onChangeExpanded: function( expanded, args ) {
			...
			overlay = panel.headContainer.closest( '.fp-full-overlay' );
		...
		/**
		 * Install a theme via fp.updates.
		 *
		 * @since 4.9.0
		 *
		 * @param {jQuery.Event} event - Event.
		...
		installTheme: function( event ) {
			...
			request = fp.updates.installTheme( {
				slug: slug
			} );
		...
		updateTheme: function( event ) {
			...
			$( document ).one( 'fp-theme-update-success', function( e, response ) {
		...
		deleteTheme: function( event ) {
			...
			$( document ).one( 'fp-theme-delete-success', function() {
	...
	api.Control = api.Class.extend(/** @lends fp.customize.Control.prototype */{
		defaultActiveArguments: { duration: 'fast', completeCallback: $.noop },
		 ...
		 *
		 * @see PHP class FP_Customize_Control.
		 *
		 * @constructs fp.customize.Control
		 * @augments   fp.customize.Class
		 *
		 * @borrows fp.customize~focus as this#focus
		 * @borrows fp.customize~Container#activate as this#activate
		 * @borrows fp.customize~Container#deactivate as this#deactivate
		 * @borrows fp.customize~Container#_toggleActive as this#_toggleActive
		 *
		 ...
		 * @since 4.6.0
		 * @this {fp.customize.Control}
		 */
		renderNotifications: function() {
			var control = this, container, notifications, hasError = false;

			if ( 'undefined' !== typeof console && console.warn ) {
				console.warn( '[DEPRECATED] fp.customize.Control.prototype.renderNotifications() is deprecated in favor of instantiating a fp.customize.Notifications and calling its render() method.' );
			}
			...
			if ( ! control.notificationsTemplate ) {
				control.notificationsTemplate = fp.template( 'customize-control-notifications' );
			}
		...
		renderContent: function () {
			...
			if ( document.getElementById( 'tmpl-' + templateId ) ) {
				template = fp.template( templateId );
		...
		addNewPage: function () {
			...
			promise.done( function( data ) {
				...
				itemTemplate = fp.template( 'available-menu-item' );
	 ...
	 *
	 * @class    fp.customize.ColorControl
	 * @augments fp.customize.Control
	 */
	api.ColorControl = api.Control.extend(/** @lends fp.customize.ColorControl.prototype */{
		ready: function() {
			var control = this,
			...
			control.container.on( 'keydown', function( event ) {
				...
				pickerContainer = control.container.find( '.fp-picker-container' );
				if ( pickerContainer.hasClass( 'fp-picker-active' ) ) {
					picker.wpColorPicker( 'close' );
					control.container.find( '.fp-color-result' ).focus();
					event.stopPropagation(); // Prevent section from being collapsed.
				}
			} );
		}
	 ...
	 *
	 * @class    fp.customize.MediaControl
	 * @augments fp.customize.Control
	 */
	api.MediaControl = api.Control.extend(/** @lends fp.customize.MediaControl.prototype */{
		...
		ready: function() {
			...
			function setAttachmentDataAndRenderContent( value ) {
				...
				if ( 'pending' === hasAttachmentData.state() ) {
					fp.media.attachment( value ).fetch().done( function() {
						...
						fp.customize.previewer.send( control.setting.id + '-attachment-data', this.attributes );
					} );
				}
		...
		cleanupPlayer: function () {
			this.player && fp.media.mixin.removePlayer( this.player );
		},
			...
			this.frame = fp.media({
				...
				states: [
					new fp.media.controller.Library({
						...
						library:   fp.media.query({ type: this.params.mime_type }),
						multiple:  false,
						date:      false
					})
				]
	 ...
	 *
	 * @class    fp.customize.UploadControl
	 * @augments fp.customize.MediaControl
	 */
	api.UploadControl = api.MediaControl.extend(/** @lends fp.customize.UploadControl.prototype */{
	 ...
	 *
	 * @class    fp.customize.ImageControl
	 * @augments fp.customize.UploadControl
	 */
	api.ImageControl = api.UploadControl.extend(/** @lends fp.customize.ImageControl.prototype */{
		// @deprecated
	 ...
	 *
	 * @class    fp.customize.BackgroundControl
	 * @augments fp.customize.UploadControl
	 */
	api.BackgroundControl = api.UploadControl.extend(/** @lends fp.customize.BackgroundControl.prototype */{
		...
		select: function() {
			...
			fp.ajax.post( 'custom-background-add', {
	 ...
	 * @since 4.7.0
	 *
	 * @class    fp.customize.BackgroundPositionControl
	 * @augments fp.customize.Control
	 */
	api.BackgroundPositionControl = api.Control.extend(/** @lends fp.customize.BackgroundPositionControl.prototype */{
	 ...
	 *
	 * @class    fp.customize.CroppedImageControl
	 * @augments fp.customize.MediaControl
	 */
	api.CroppedImageControl = api.MediaControl.extend(/** @lends fp.customize.CroppedImageControl.prototype */{
		...
		initFrame: function() {
			...
			this.frame = fp.media({
				...
				states: [
					new fp.media.controller.Library({
						title: this.params.button_labels.frame_title,
						library: fp.media.query({ type: 'image' }),
						multiple: false,
						date: false,
						priority: 20,
					...
					new fp.media.controller.CustomizeImageCropper({
		 ...
		 * fp.media.view.Cropper.
		 *
		 * @param {fp.media.model.Attachment} attachment
		 * @param {fp.media.controller.Cropper} controller
	 ...
	 *
	 * @class    fp.customize.SiteIconControl
	 * @augments fp.customize.CroppedImageControl
	 */
	api.SiteIconControl = api.CroppedImageControl.extend(/** @lends fp.customize.SiteIconControl.prototype */{
		...
		initFrame: function() {
			...
			this.frame = fp.media({
				...
				states: [
					...
					new fp.media.controller.SiteIconCropper({
						imgSelectOptions: this.calculateImageSelectOptions,
						control: this
					})
				]
	...
	/**
	 * @class    fp.customize.HeaderControl
	 * @augments fp.customize.Control
	 */
	api.HeaderControl = api.Control.extend(/** @lends fp.customize.HeaderControl.prototype */{
		ready: function() {
			...
			fp.media.controller.Cropper.prototype.defaults.doCropArgs.wp_customize = 'on';
			fp.media.controller.Cropper.prototype.defaults.doCropArgs.customize_theme = api.settings.theme.stylesheet;
		},
		...
		openMedia: function(event) {
			...
			this.frame = fp.media({
				...
				states: [
					...
					new fp.media.controller.Cropper({
						imgSelectOptions: this.calculateImageSelectOptions
					})
				]
			});
		...
		/**
		 * Creates a new fp.customize.HeaderTool.ImageModel from provided
	...
	/**
	 * fp.customize.ThemeControl
	 *
	 * @class    fp.customize.ThemeControl
	 * @augments fp.customize.Control
	 */
	api.ThemeControl = api.Control.extend(/** @lends fp.customize.ThemeControl.prototype */{
	...
	/**
	 * Class fp.customize.CodeEditorControl
	 *
	 * @since 4.9.0
	 *
	 * @class    fp.customize.CodeEditorControl
	 * @augments fp.customize.Control
	 */
	api.CodeEditorControl = api.Control.extend(/** @lends fp.customize.CodeEditorControl.prototype */{
		...
		initEditor: function() {
			...
			if ( fp.codeEditor && ( _.isUndefined( control.params.editor_settings ) || false !== control.params.editor_settings ) ) {

				// Obtain default editor settings.
				editorSettings = fp.codeEditor.defaultSettings ? _.clone( fp.codeEditor.defaultSettings ) : {};
		...
		initSyntaxHighlightingEditor: function( codeEditorSettings ) {
			...
			control.editor = fp.codeEditor.initialize( $textarea, settings );
	...
	/**
	 * Class fp.customize.DateTimeControl.
	 *
	 * @since 4.9.0
	 * @class    fp.customize.DateTimeControl
	 * @augments fp.customize.Control
	 */
	api.DateTimeControl = api.Control.extend(/** @lends fp.customize.DateTimeControl.prototype */{
		 ...
		 * @return {fp.customize.DateTimeControl}
		 */
	 ...
	 *
	 * @since 4.9.0
	 * @class    fp.customize.PreviewLinkControl
	 * @augments fp.customize.Control
	 */
	api.PreviewLinkControl = api.Control.extend(/** @lends fp.customize.PreviewLinkControl.prototype */{
	 ...
	 *
	 * @alias fp.customize.defaultConstructor
	 */
	 ...
	 *
	 * @callback fp.customize.deferredControlsCallback
	 * @param {fp.customize.Control[]} controls Resolved controls.
	 */

	/**
	 * Collection of all registered controls.
	 *
	 * @alias fp.customize.control
	 *
	 * @since 3.4.0
	 *
	 * @type {Function}
	 ...
	 * @return {fp.customize.Control|undefined|jQuery.promise} Control instance or undefined (if function called with one id param),
	 *                                                         or promise resolving to requested controls.
	 *
	 * @example <caption>Loop over all registered controls.</caption>
	 * fp.customize.control.each( function( control ) { ... } );
	 *
	 * @example <caption>Getting `background_color` control instance.</caption>
	 * control = fp.customize.control( 'background_color' );
	 *
	 * @example <caption>Check if control exists.</caption>
	 * hasControl = fp.customize.control.has( 'background_color' );
	 *
	 * @example <caption>Deferred getting of `background_color` control until it exists, using callback.</caption>
	 * fp.customize.control( 'background_color', function( control ) { ... } );
	 *
	 ...
	 * promise = fp.customize.control( 'blogname', 'blogdescription' );
	 ...
	 * fp.customize.control( 'blogname', 'blogdescription', function( titleControl, taglineControl ) { ... } );
	 *
	 * @example <caption>Getting setting value for `background_color` control.</caption>
	 * value = fp.customize.control( 'background_color ').setting.get();
	 * value = fp.customize( 'background_color' ).get(); // Same as above, since setting ID and control ID are the same.
	 *
	 * @example <caption>Add new control for site title.</caption>
	 * fp.customize.control.add( new fp.customize.Control( 'other_blogname', {
	 *     setting: 'blogname',
	 *     type: 'text',
	 *     label: 'Site title',
	 ...
	 * fp.customize.control.remove( 'other_blogname' );
	 *
	 * @example <caption>Listen for control being added.</caption>
	 * fp.customize.control.bind( 'add', function( addedControl ) { ... } )
	 *
	 * @example <caption>Listen for control being removed.</caption>
	 * fp.customize.control.bind( 'removed', function( removedControl ) { ... } )
	 */
	 ...
	 *
	 * @callback fp.customize.deferredSectionsCallback
	 * @param {fp.customize.Section[]} sections Resolved sections.
	 */

	/**
	 * Collection of all registered sections.
	 *
	 * @alias fp.customize.section
	 *
	 * @since 3.4.0
	 *
	 * @type {Function}
	 ...
	 * @return {fp.customize.Section|undefined|jQuery.promise} Section instance or undefined (if function called with one id param),
	 *                                                         or promise resolving to requested sections.
	 *
	 * @example <caption>Loop over all registered sections.</caption>
	 * fp.customize.section.each( function( section ) { ... } )
	 *
	 * @example <caption>Getting `title_tagline` section instance.</caption>
	 * section = fp.customize.section( 'title_tagline' )
	 *
	 * @example <caption>Expand dynamically-created section when it exists.</caption>
	 * fp.customize.section( 'dynamically_created', function( section ) {
	 *     section.expand();
	 * } );
	 *
	 * @see {@link fp.customize.control} for further examples of how to interact with {@link fp.customize.Values} instances.
	 */
	 ...
	 *
	 * @callback fp.customize.deferredPanelsCallback
	 * @param {fp.customize.Panel[]} panels Resolved panels.
	 */

	/**
	 * Collection of all registered panels.
	 *
	 * @alias fp.customize.panel
	 *
	 * @since 4.0.0
	 *
	 * @type {Function}
	 ...
	 * @return {fp.customize.Panel|undefined|jQuery.promise} Panel instance or undefined (if function called with one id param),
	 *                                                       or promise resolving to requested panels.
	 *
	 * @example <caption>Loop over all registered panels.</caption>
	 * fp.customize.panel.each( function( panel ) { ... } )
	 *
	 * @example <caption>Getting nav_menus panel instance.</caption>
	 * panel = fp.customize.panel( 'nav_menus' );
	 *
	 * @example <caption>Expand dynamically-created panel when it exists.</caption>
	 * fp.customize.panel( 'dynamically_created', function( panel ) {
	 *     panel.expand();
	 * } );
	 *
	 * @see {@link fp.customize.control} for further examples of how to interact with {@link fp.customize.Values} instances.
	 ...
	 *
	 * @callback fp.customize.deferredNotificationsCallback
	 * @param {fp.customize.Notification[]} notifications Resolved notifications.
	 */

	/**
	 * Collection of all global notifications.
	 *
	 * @alias fp.customize.notifications
	 *
	 * @since 4.9.0
	 *
	 * @type {Function}
	 ...
	 * @return {fp.customize.Notification|undefined|jQuery.promise} Notification instance or undefined (if function called with one code param),
	 *                                                              or promise resolving to requested notifications.
	 *
	 * @example <caption>Check if existing notification</caption>
	 * exists = fp.customize.notifications.has( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain existing notification</caption>
	 * notification = fp.customize.notifications( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain notification that may not exist yet.</caption>
	 * fp.customize.notifications( 'a_new_day_arrived', function( notification ) { ... } );
	 *
	 * @example <caption>Add a warning notification.</caption>
	 * fp.customize.notifications.add( new fp.customize.Notification( 'midnight_almost_here', {
	 *     type: 'warning',
	 ...
	 * fp.customize.notifications.remove( 'a_new_day_arrived' );
	 *
	 * @see {@link fp.customize.control} for further examples of how to interact with {@link fp.customize.Values} instances.
	 */
	api.notifications = new api.Notifications();

	api.PreviewFrame = api.Messenger.extend(/** @lends fp.customize.PreviewFrame.prototype */{
		 ...
		 *
		 * @constructs fp.customize.PreviewFrame
		 * @augments   fp.customize.Messenger
		 *
		...
		run: function( deferred ) {
			...
			previewFrame.iframe.attr( 'onmousewheel', '' ); // Workaround for Safari bug. See FP Trac #38149.
	 ...
	 *
	 * @alias fp.customize.setDocumentTitle
	 *
	 * @since 4.1.0
	 *
	...
	api.Previewer = api.Messenger.extend(/** @lends fp.customize.Previewer.prototype */{
		refreshBuffer: null, // Will get set to api.settings.timeouts.windowRefresh.

		/**
		 * @constructs fp.customize.Previewer
		 * @augments   fp.customize.Messenger
		 *
		...
		initialize: function( params, options ) {
			...
			previewer.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Abort if URL is for admin or (static) files in fp-includes or fp-content.
				if ( /\/fp-(admin|includes|content)(\/|$)/.test( urlParser.pathname ) ) {
					return null;
				}
		...
		refreshNonces: function() {
			...
			request = fp.ajax.post( 'customize_refresh_nonces', {
				wp_customize: 'on',
				customize_theme: api.settings.theme.stylesheet
			});
	 ...
	 *
	 * @alias fp.customize._handleSettingValidities
	 *
	 * @since 4.6.0
	 * @private
	 *
	 ...
	 *
	 * @alias fp.customize.findControlsForSettings
	 *
	 * @since 4.6.0
	 * @param {string[]} settingIds Setting IDs.
	 * @return {Object<string, fp.customize.Control>} Mapping setting ids to arrays of controls.
	 */
	 ...
	 *
	 * @alias fp.customize.reflowPaneContents
	 *
	 * @since 4.1.0
	 */
	...
	$( function() {
		...
		var parent,
			body = $( document.body ),
			overlay = body.children( '.fp-full-overlay' ),
		...
		api.previewer = new api.Previewer({
			...
			save: function( args ) {
				...
				submit = function () {
					 ...
					 */
					request = fp.ajax.post( 'customize_save', query );
					api.state( 'processing' ).set( api.state( 'processing' ).get() + 1 );
			...
			trash: function trash() {
				...
				request = fp.ajax.post( 'customize_trash', {
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		api.bind( 'ready', function setUpGlobalNotificationsArea() {
			...
			sidebar = $( '.fp-full-overlay-sidebar-content' );
		...
		(function( state ) {
			...
			selectedChangesetStatus.link( changesetStatus ); // Ensure that direct updates to status on server via fp.customizer.previewer.save() will update selection.
			saved( true );
		...
		( function checkAndDisplayLockNotice() {
			...
			var LockedNotification = api.OverlayNotification.extend(/** @lends fp.customize~LockedNotification.prototype */{
				 ...
				 *
				 * @constructs fp.customize~LockedNotification
				 * @augments   fp.customize.OverlayNotification
				 *
				 * @since 4.9.0
				 *
				 * @param {string} [code] - Code.
				...
				render: function() {
					...
					takeOverButton.on( 'click', function( event ) {
						...
						request = fp.ajax.post( 'customize_override_changeset_lock', {
							wp_customize: 'on',
							customize_theme: api.settings.theme.stylesheet,
							customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function() {
			...
			function dismissAutosave() {
				...
				fp.ajax.post( 'customize_dismiss_autosave_or_lock', {
					wp_customize: 'on',
					customize_theme: api.settings.theme.stylesheet,
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function initStickyHeaders() {
			var parentContainer = $( '.fp-full-overlay-sidebar-content' ),
			 ...
			 * @access private
			 *
			 * @param {fp.customize.Panel|fp.customize.Section} container Construct.
			 * @return {void}
			 */
		...
		(function() {
			...
			function requestClose() {
				...
				if ( isCleanState() ) {
					...
					$( document ).off( 'visibilitychange.fp-customize-changeset-update' );
					$( window ).off( 'beforeunload.fp-customize-changeset-update' );
				...
				if ( dismissLock || dismissAutoSave ) {
					fp.ajax.send( 'customize_dismiss_autosave_or_lock', {
		...
		function startAutosaving() {
			...
			$( document ).on( 'visibilitychange.fp-customize-changeset-update', function() {
				if ( document.hidden ) {
					updateChangesetWithReschedule();
				}
			} );
			...
			$( window ).on( 'beforeunload.fp-customize-changeset-update', function() {
				updateChangesetWithReschedule();
			} );
		}
