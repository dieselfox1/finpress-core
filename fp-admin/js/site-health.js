 ...
 *
 * @output fp-admin/js/site-health.js
 */

/* global ajaxurl, ClipboardJS, SiteHealth, fp */
...
jQuery( function( $ ) {
	...
	var __ = fp.i18n.__,
		...
		pathsSizesSection = $( '#health-check-accordion-block-fp-paths-sizes' ),
	...
	clipboard.on( 'success', function( e ) {
		...
		fp.a11y.speak( __( 'Site information has been copied to your clipboard.' ) );
	} );
	...
	function appendIssue( issue ) {
		var template = fp.template( 'health-check-issue' ),
		...
		if ( 1 <= SiteHealth.site_status.async.length ) {
			$.each( SiteHealth.site_status.async, function() {
				...
				if ( 'undefined' !== typeof( this.has_rest ) && this.has_rest ) {
					fp.apiRequest( {
						url: fp.url.addQueryArgs( this.test, { _locale: 'user' } ),
						headers: this.headers
					} )
						.done( function( response ) {
							/** This filter is documented in fp-admin/includes/class-fp-site-health.php */
							appendIssue( fp.hooks.applyFilters( 'site_status_test_result', response ) );
						} )
					...
					$.post(
						...
						/** This filter is documented in fp-admin/includes/class-fp-site-health.php */
						appendIssue( fp.hooks.applyFilters( 'site_status_test_result', response.data ) );
					} ).fail( function( response ) {
						var description;
	...
	function addFailedSiteHealthCheckNotice( url, description ) {
		...
		/** This filter is documented in fp-admin/includes/class-fp-site-health.php */
		appendIssue( fp.hooks.applyFilters( 'site_status_test_result', issue ) );
	}
	...
	function getDirectorySizes() {
		...
		fp.apiRequest( {
			path: '/fp-site-health/v1/directory-sizes'
		} ).done( function( response ) {
			updateDirSizes( response || {} );
		} ).always( function() {
			var delay = ( new Date().getTime() ) - timestamp;

			$( '.health-check-fp-paths-sizes.spinner' ).css( 'visibility', 'hidden' );
	...
	function announceTestsProgression( type ) {
		...
		switch ( type ) {
			case 'good':
				fp.a11y.speak( __( 'All site health tests have finished running. Your site is looking good.' ) );
				break;
			case 'improvable':
				fp.a11y.speak( __( 'All site health tests have finished running. There are items that should be addressed.' ) );
				break;
			case 'waiting-for-directory-sizes':
				fp.a11y.speak( __( 'Running additional tests... please wait.' ) );
				break;
			default:
				return;
		}
