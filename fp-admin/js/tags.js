		...
		if ( r ) {
			...
			$.post(ajaxurl, data, function(r){
				...
				if ( '1' == r ) {
					...
					message = fp.i18n.__( 'The selected tag has been deleted.' );
			
				} else if ( '-1' == r ) {
					message = fp.i18n.__( 'Sorry, you are not allowed to do that.' );
					$('#ajax-response').empty().append('<div class="notice notice-error"><p>' + message + '</p></div>');
					resetRowAfterFailure( tr );

				} else {
					message = fp.i18n.__( 'An error occurred while processing your request. Please try again later.' );
					$('#ajax-response').empty().append('<div class="notice notice-error"><p>' + message + '</p></div>');
					resetRowAfterFailure( tr );
				}
				fp.a11y.speak( message, 'assertive' );
			});
		}
