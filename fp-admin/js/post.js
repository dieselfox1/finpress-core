...
( function( $ ) {
	 ...
	 * @global
	 */
	window.FPSetThumbnailHTML = function(html){
	 ...
	 * @global
	 */
	window.FPSetThumbnailID = function(id){
	 ...
	 * @global
	 */
	window.FPRemoveThumbnail = function(nonce){
		$.post(
			ajaxurl, {
	...
	$(document).on( 'heartbeat-send.refresh-lock', function( e, data ) {
		...
		data['fp-refresh-post-lock'] = send;
		...
		if ( data['fp-refresh-post-lock'] ) {
			received = data['fp-refresh-post-lock'];
				...
				if ( wrap.length && ! wrap.is(':visible') ) {
					if ( fp.autosave ) {
						...
						$(document).one( 'heartbeat-tick', function() {
							fp.autosave.server.suspend();
							wrap.removeClass('saving').addClass('saved');
							$(window).off( 'beforeunload.edit-post' );
						});

						wrap.addClass('saving');
						fp.autosave.server.triggerSave();
					}
					...
					wrap.find('.fp-tab-first').trigger( 'focus' );
				}
...
(function($) {
	...
	$( function() {
		schedule();
	}).on( 'heartbeat-send.fp-refresh-nonces', function( e, data ) {
		var post_id,
			$authCheck = $('#fp-auth-check-wrap');

		if ( check || ( $authCheck.length && ! $authCheck.hasClass( 'hidden' ) ) ) {
			if ( ( post_id = $('#post_ID').val() ) && $('#_wpnonce').val() ) {
				data['fp-refresh-post-nonces'] = {
					post_id: post_id
				};
			}
		}
	}).on( 'heartbeat-tick.fp-refresh-nonces', function( e, data ) {
		var nonces = data['fp-refresh-post-nonces'];
...
jQuery( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('fp-tab-first') && e.shiftKey ) {
			$(this).find('.fp-tab-last').trigger( 'focus' );
			e.preventDefault();
		// [Tab] on last tab cycles back to first tab.
		} else if ( target.hasClass('fp-tab-last') && ! e.shiftKey ) {
			$(this).find('.fp-tab-first').trigger( 'focus' );
			e.preventDefault();
		}
	}).filter(':visible').find('.fp-tab-first').trigger( 'focus' );

	// Set the heartbeat interval to 10 seconds if post lock dialogs are enabled.
	if ( fp.heartbeat && $('#post-lock-dialog').length ) {
		fp.heartbeat.interval( 10 );
	}
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#fp-preview'),
			target = $this.attr('target') || 'fp-preview',
		...
		if ( fp.autosave ) {
			fp.autosave.server.tempBlockSave();
		}
	...
	$document.on( 'autosave-disable-buttons.edit-post', function() {
		...
		if ( ! fp.heartbeat || ! fp.heartbeat.hasConnectionError() ) {
	...
	$( window ).on( 'beforeunload.edit-post', function( event ) {
		...
		if ( fp.autosave ) {
			changed = fp.autosave.server.postChanged();
		} else if ( editor ) {
	...
	$('.categorydiv').each( function(){
		...
		// @todo Move to jQuery 1.3+, support for multiple hierarchical taxonomies, see fp-lists.js.
		...
		$('#' + taxonomy + '-add-toggle').on( 'click', function( e ) {
			e.preventDefault();
			$('#' + taxonomy + '-adder').toggleClass( 'fp-hidden-children' );
	...
	if ( $('#submitdiv').length ) {
		...
		updateText = function() {
			...
			$('#post-status-display').text(
				...
				fp.sanitize.stripTagsAndEncodeText( $('option:selected', postStatus).text() )
			);
		...
		buttons.children( '.save' ).on( 'click', function() {
			...
			$.post(
				...
				function(data) {
					...
					fp.a11y.speak( __( 'Permalink saved' ) );
				}
			);
		});
	...
	( function() {
		...
		function dragging( event ) {
			if ( $postdivrich.hasClass( 'fp-editor-expand' ) ) {
				return;
			}
		...
		function endDrag() {
			...
			if ( mce ) {
				editor.focus();
				toolbarHeight = parseInt( $( '#fp-content-editor-container .mce-toolbar-grp' ).height(), 10 );
		...
		$handle.on( 'mousedown.fp-editor-resize', function( event ) {
			...
			$document.on( 'mousemove.fp-editor-resize', dragging )
				.on( 'mouseup.fp-editor-resize mouseleave.fp-editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.fp-editor-resize', endDrag );
	})();
	...
	$textarea.on( 'keydown.fp-autosave', function( event ) {
		// Key [S] has code 83.
		if ( event.which === 83 ) {
			if (
				event.shiftKey ||
			...
			fp.autosave && fp.autosave.server.triggerSave();
			event.preventDefault();
		}
	});
	...
	if ( $( '#original_post_status' ).val() === 'auto-draft' && window.history.replaceState ) {
		...
		$( '#publish' ).on( 'click', function() {
			...
			location += 'fp-post-new-reload=true';
	...
	copyAttachmentURLClipboard.on( 'success', function( event ) {
		...
		fp.a11y.speak( __( 'The file URL has been copied to your clipboard' ) );
	} );
} );
...
( function( $, counter ) {
	$( function() {
		var $content = $( '#content' ),
			$count = $( '#fp-word-count' ).find( '.word-count' ),
			prevCount = 0,
			contentEditor;
...
} )( jQuery, new fp.utils.WordCounter() );
