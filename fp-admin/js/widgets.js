...
window.wpWidgets = {
	...
	init : function() {
		...
		$( '#widgets-right .sidebar-name' )
			...
			.on( 'click', function() {
				...
				$document.triggerHandler( 'fp-pin-menu' );
			})
			/*
		...
		$( window ).on( 'beforeunload.widgets', function( event ) {
			...
			if ( 0 !== dirtyWidgetIds.length ) {
				...
				event.returnValue = fp.i18n.__( 'The changes you made will be lost if you navigate away from this page.' );
		...
		$(document.body).on('click.widgets-toggle', function(e) {
			...
			if ( target.parents('.widget-top').length && ! target.parents('#available-widgets').length ) {
				...
				if ( ! widget.data( 'dirty-state-initialized' ) ) {
					...
					saveButton.prop( 'disabled', true ).val( fp.i18n.__( 'Saved' ) );
					inside.on( 'input change', function() {
						self.dirtyWidgets[ widgetId ] = true;
						widget.addClass( 'widget-dirty' );
						saveButton.prop( 'disabled', false ).val( fp.i18n.__( 'Save' ) );
					});
		...
		$.post( ajaxurl, data, function(r) {
			...
			if ( del ) {
				...
				if ( r && r.length > 2 ) {
					...
					widget.find( '.widget-control-save' ).prop( 'disabled', true ).val( fp.i18n.__( 'Saved' ) );
	...
	addWidget: function( chooser ) {
		...
		window.setTimeout( function() {
			...
			window.fp.a11y.speak( fp.i18n.__( 'Widget has been added to the selected sidebar' ), 'assertive' );
		}, 250 );
	},
...
wpWidgets.l10n = window.fp.deprecateL10nObject( 'wpWidgets.l10n', wpWidgets.l10n, '5.5.0' );
