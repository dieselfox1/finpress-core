 ...
 * @param {object}  fp                                       FP object.
 * @param {object}  settings                                 FP Updates settings.
 ...
 */
(function( $, fp, settings ) {
	var $document = $( document ),
		__ = fp.i18n.__,
		_x = fp.i18n._x,
		_n = fp.i18n._n,
		_nx = fp.i18n._nx,
		sprintf = fp.i18n.sprintf;

	fp = fp || {};

	/**
	 * The FP Updates object.
	 *
	 * @since 4.2.0
	 *
	 * @namespace fp.updates
	 */
	fp.updates = {};
	...
	fp.updates.l10n = window.fp.deprecateL10nObject( 'fp.updates.l10n', fp.updates.l10n, '5.5.0' );
	 ...
	 * @type {string}
	 */
	fp.updates.ajaxNonce = settings.ajax_nonce;
	 ...
	 * @type {number}
	 */
	fp.updates.searchMinCharacters = 2;
	 ...
	 * @type {bool}
	 */
	fp.updates.shouldRequestFilesystemCredentials = false;
	 ...
	 *                                                             Default 'false'.
	 */
	fp.updates.filesystemCredentials = {
		ftp:       {
			host:           '',
			username:       '',
			password:       '',
	 ...
	 * @type {function}
	 */
	fp.updates.adminNotice = fp.template( 'fp-updates-admin-notice' );
	 ...
	 * @type {jQuery}
	 */
	fp.updates.$elToReturnFocusToFromCredentialsModal = undefined;
	 ...
	 *
	 */
	fp.updates.addAdminNotice = function( data ) {
		var $notice = $( data.selector ),
			$headerEnd = $( '.fp-header-end' ),
			$adminNotice;

		delete data.selector;
		$adminNotice = fp.updates.adminNotice( data );
		...
		$document.trigger( 'fp-updates-notice-added' );
	};
	 ...
	 */
	fp.updates.ajax = function( action, data ) {
		var options = {};

		if ( fp.updates.ajaxLocked ) {
			fp.updates.queue.push( {
				action: action,
				data:   data
			} );
		...
		options.data = _.extend( data, {
			action:          action,
			_ajax_nonce:     fp.updates.ajaxNonce,
			_fs_nonce:       fp.updates.filesystemCredentials.fsNonce,
			username:        fp.updates.filesystemCredentials.ftp.username,
			password:        fp.updates.filesystemCredentials.ftp.password,
			hostname:        fp.updates.filesystemCredentials.ftp.hostname,
			connection_type: fp.updates.filesystemCredentials.ftp.connectionType,
			public_key:      fp.updates.filesystemCredentials.ssh.publicKey,
			private_key:     fp.updates.filesystemCredentials.ssh.privateKey
		} );

		return fp.ajax.send( options ).always( fp.updates.ajaxAlways );
	};
	 ...
	 */
	fp.updates.ajaxAlways = function( response ) {
		if ( ! response.errorCode || 'unable_to_connect_to_filesystem' !== response.errorCode ) {
			fp.updates.ajaxLocked = false;
			fp.updates.queueChecker();
		}
		...
		if ( 'undefined' !== typeof response.debug && window.console && window.console.log ) {
			_.map( response.debug, function( message ) {
				...
				window.console.log( fp.sanitize.stripTagsAndEncodeText( message ) );
			} );
		}
	};
	 ...
	 * @since 4.7.0
	 */
	fp.updates.refreshCount = function() {
		var $adminBarUpdates              = $( '#fp-admin-bar-updates' ),
	 ...
	 */
	fp.updates.setCardButtonStatus = function( data ) {
	 ...
	 *                      Can be 'plugin', 'theme'.
	 */
	fp.updates.decrementCount = function( type ) {
	 ...
	 * @param {updatePluginSuccess=} args.success Optional. Success callback. Default: fp.updates.updatePluginSuccess
	 * @param {updatePluginError=}   args.error   Optional. Error callback. Default: fp.updates.updatePluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	fp.updates.updatePlugin = function( args ) {
		var $updateRow, $card, $message, message,
			$adminBarUpdates = $( '#fp-admin-bar-updates' ),
			buttonText = __( 'Updating...' ),
			isPluginInstall = 'plugin-install' === pagenow || 'plugin-install-network' === pagenow;

		args = _.extend( {
			success: fp.updates.updatePluginSuccess,
			error: fp.updates.updatePluginError
		}, args );
		...
		$document.trigger( 'fp-plugin-updating', args );

		if ( isPluginInstall && 'plugin-information-footer' === $card.attr( 'id' ) ) {
			fp.updates.setCardButtonStatus(
				{
		...
		return fp.updates.ajax( 'update-plugin', args );
	};
	 ...
	 */
	fp.updates.updatePluginSuccess = function( response ) {
		...
		fp.a11y.speak( __( 'Update completed successfully.' ) );

		if ( 'plugin_install_from_iframe' !== $updateMessage.attr( 'id' ) ) {
			fp.updates.decrementCount( 'plugin' );
		} else {
			fp.updates.setCardButtonStatus(
				{
		...
		$document.trigger( 'fp-plugin-update-success', response );
	};
	 ...
	 */
	fp.updates.updatePluginError = function( response ) {
		var $pluginRow, $card, $message, errorMessage, buttonText, ariaLabel,
			$adminBarUpdates = $( '#fp-admin-bar-updates' );

		if ( ! fp.updates.isValidResponse( response, 'update' ) ) {
			return;
		}

		if ( fp.updates.maybeHandleCredentialError( response, 'update-plugin' ) ) {
			return;
		}
		...
		fp.a11y.speak( errorMessage, 'assertive' );
		...
		$document.trigger( 'fp-plugin-update-error', response );
	};
	 ...
	 * @param {installPluginSuccess=} args.success Optional. Success callback. Default: fp.updates.installPluginSuccess
	 * @param {installPluginError=}   args.error   Optional. Error callback. Default: fp.updates.installPluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	fp.updates.installPlugin = function( args ) {
		...
		args = _.extend( {
			success: fp.updates.installPluginSuccess,
			error: fp.updates.installPluginError
		}, args );
		...
		fp.a11y.speak( __( 'Installing... please wait.' ) );

		// Remove previous error messages, if any.
		$card.removeClass( 'plugin-card-install-failed' ).find( '.notice.notice-error' ).remove();

		$document.trigger( 'fp-plugin-installing', args );
		...
		return fp.updates.ajax( 'install-plugin', args );
	};
	 ...
	 */
	fp.updates.installPluginSuccess = function( response ) {
		...
		fp.a11y.speak( __( 'Installation completed successfully.' ) );

		$document.trigger( 'fp-plugin-install-success', response );

		if ( response.activateUrl ) {
			setTimeout( function() {
				fp.updates.checkPluginDependencies( {
					slug: response.slug
				} );
			}, 1000 );
		}
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	fp.updates.installPluginError = function( response ) {
		...
		if ( ! fp.updates.isValidResponse( response, 'install' ) ) {
			return;
		}

		if ( fp.updates.maybeHandleCredentialError( response, 'install-plugin' ) ) {
			return;
		}
		...
		$document.trigger( 'fp-plugin-install-error', response );
	};
	 ...
	 * @param {Object}                          args         Arguments.
	 * @param {string}                          args.slug    Plugin identifier in the WordPress.org Plugin repository.
	 * @param {checkPluginDependenciesSuccess=} args.success Optional. Success callback. Default: fp.updates.checkPluginDependenciesSuccess
	 * @param {checkPluginDependenciesError=}   args.error   Optional. Error callback. Default: fp.updates.checkPluginDependenciesError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	fp.updates.checkPluginDependencies = function( args ) {
		args = _.extend( {
			success: fp.updates.checkPluginDependenciesSuccess,
			error: fp.updates.checkPluginDependenciesError
		}, args );

		fp.a11y.speak( __( 'Checking plugin dependencies... please wait.' ) );
		$document.trigger( 'fp-checking-plugin-dependencies', args );

		return fp.updates.ajax( 'check_plugin_dependencies', args );
	};
	 ...
	 */
	fp.updates.checkPluginDependenciesSuccess = function( response ) {
		var $message = $( '.plugin-card-' + response.slug + ', #plugin-information-footer' ).find( '.install-now' ),
		...
		fp.a11y.speak( __( 'Plugin dependencies check completed successfully.' ) );
		$document.trigger( 'fp-check-plugin-dependencies-success', response );
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	fp.updates.checkPluginDependenciesError = function( response ) {
		var $message = $( '.plugin-card-' + response.slug + ', #plugin-information-footer' ).find( '.install-now' ),
			buttonText = _x( 'Activate', 'plugin' ),
			ariaLabel = sprintf(
		...
		if ( ! fp.updates.isValidResponse( response, 'check-dependencies' ) ) {
			return;
		}
		...
		fp.a11y.speak( errorMessage, 'assertive' );
		$document.trigger( 'fp-check-plugin-dependencies-error', response );
	 ...
	 * @param {activatePluginSuccess=} args.success Optional. Success callback. Default: fp.updates.activatePluginSuccess
	 * @param {activatePluginError=}   args.error   Optional. Error callback. Default: fp.updates.activatePluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	fp.updates.activatePlugin = function( args ) {
		var $message = $( '.plugin-card-' + args.slug + ', #plugin-information-footer' ).find( '.activate-now, .activating-message' );

		args = _.extend( {
			success: fp.updates.activatePluginSuccess,
			error: fp.updates.activatePluginError
		}, args );

		fp.a11y.speak( __( 'Activating... please wait.' ) );
		$document.trigger( 'fp-activating-plugin', args );
		...
		return fp.updates.ajax( 'activate-plugin', args );
	};
	 ...
	 * @param {string} response.plugin      The plugin file, relative to the plugins directory.
	 */
	fp.updates.activatePluginSuccess = function( response ) {
		...
		fp.a11y.speak( __( 'Activation completed successfully.' ) );
		$document.trigger( 'fp-plugin-activate-success', response );
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	fp.updates.activatePluginError = function( response ) {
		var $message = $( '.plugin-card-' + response.slug + ', #plugin-information-footer' ).find( '.activating-message' ),
		...
		if ( ! fp.updates.isValidResponse( response, 'activate' ) ) {
			return;
		}
		...
		fp.a11y.speak( errorMessage, 'assertive' );
		$document.trigger( 'fp-plugin-activate-error', response );
	 ...
	 * @param {string} response.activateUrl URL to activate the just installed plugin.
	 */
	fp.updates.installImporterSuccess = function( response ) {
		fp.updates.addAdminNotice( {
			id:        'install-success',
		...
		$document.trigger( 'fp-importer-install-success', response );
	};
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	fp.updates.installImporterError = function( response ) {
		...
		$document.trigger( 'fp-importer-install-error', response );
	};
	 ...
	 * @param {deletePluginSuccess=} args.success Optional. Success callback. Default: fp.updates.deletePluginSuccess
	 * @param {deletePluginError=}   args.error   Optional. Error callback. Default: fp.updates.deletePluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	fp.updates.deletePlugin = function( args ) {
		var $link = $( '[data-plugin="' + args.plugin + '"]' ).find( '.row-actions a.delete' );

		args = _.extend( {
			success: fp.updates.deletePluginSuccess,
			error: fp.updates.deletePluginError
		}, args );
		...
		fp.a11y.speak( __( 'Deleting...' ) );

		$document.trigger( 'fp-plugin-deleting', args );

		return fp.updates.ajax( 'delete-plugin', args );
	};
	 ...
	 */
	fp.updates.deletePluginSuccess = function( response ) {
		...
		$( '[data-plugin="' + response.plugin + '"]' ).css( { backgroundColor: '#faafaa' } ).fadeOut( 350, function() {
			var $form            = $( '#bulk-action-form' ),
				...
				pluginDeletedRow = fp.template( 'item-deleted-row' ),
				/**
		...
		fp.a11y.speak( _x( 'Deleted!', 'plugin' ) );

		$document.trigger( 'fp-plugin-delete-success', response );
	};
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	fp.updates.deletePluginError = function( response ) {
		var $plugin, $pluginUpdateRow,
			pluginUpdateRow  = fp.template( 'item-update-row' ),
			noticeContent    = fp.updates.adminNotice( {
		...
		if ( ! fp.updates.isValidResponse( response, 'delete' ) ) {
			return;
		}

		if ( fp.updates.maybeHandleCredentialError( response, 'delete-plugin' ) ) {
			return;
		}
		...
		$document.trigger( 'fp-plugin-delete-error', response );
	};
	 ...
	 * @param {updateThemeSuccess=} args.success Optional. Success callback. Default: fp.updates.updateThemeSuccess
	 * @param {updateThemeError=}   args.error   Optional. Error callback. Default: fp.updates.updateThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	fp.updates.updateTheme = function( args ) {
		var $notice;

		args = _.extend( {
			success: fp.updates.updateThemeSuccess,
			error: fp.updates.updateThemeError
		}, args );
		...
		fp.a11y.speak( __( 'Updating... please wait.' ) );
		$notice.text( __( 'Updating...' ) );

		$document.trigger( 'fp-theme-updating', args );

		return fp.updates.ajax( 'update-theme', args );
	};
	 ...
	 */
	fp.updates.updateThemeSuccess = function( response ) {
		...
		if ( 'customize' === pagenow ) {
			...
			$notice = $( '.theme-info .notice' ).add( fp.customize.control( 'installed_theme_' + response.slug ).container.find( '.theme' ).find( '.update-message' ) );
		...
		fp.updates.addAdminNotice( _.extend( { selector: $notice }, updatedMessage ) );
		fp.a11y.speak( __( 'Update completed successfully.' ) );

		fp.updates.decrementCount( 'theme' );

		$document.trigger( 'fp-theme-update-success', response );

		// Show updated message after modal re-rendered.
		if ( isModalOpen && 'customize' !== pagenow ) {
			$( '.theme-info .theme-author' ).after( fp.updates.adminNotice( updatedMessage ) );
		}
	};
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	fp.updates.updateThemeError = function( response ) {
		...
		if ( fp.updates.maybeHandleCredentialError( response, 'update-theme' ) ) {
			return;
		}

		if ( 'customize' === pagenow ) {
			$theme = fp.customize.control( 'installed_theme_' + response.slug ).container.find( '.theme' );
		}
		...
		fp.a11y.speak( errorMessage );

		$document.trigger( 'fp-theme-update-error', response );
	};
	 ...
	 * @param {Object}               args
	 * @param {string}               args.slug    Theme stylesheet.
	 * @param {installThemeSuccess=} args.success Optional. Success callback. Default: fp.updates.installThemeSuccess
	 * @param {installThemeError=}   args.error   Optional. Error callback. Default: fp.updates.installThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	fp.updates.installTheme = function( args ) {
		var $message = $( '.theme-install[data-slug="' + args.slug + '"]' );

		args = _.extend( {
			success: fp.updates.installThemeSuccess,
			error: fp.updates.installThemeError
		}, args );
		...
		$document.trigger( 'fp-theme-installing', args );

		return fp.updates.ajax( 'install-theme', args );
	};
	 ...
	 */
	fp.updates.installThemeSuccess = function( response ) {
		var $card = $( '.fp-full-overlay-header, [data-slug=' + response.slug + ']' ),
			$message;

		$document.trigger( 'fp-theme-install-success', response );
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	fp.updates.installThemeError = function( response ) {
		var $card, $button,
			errorMessage = sprintf(
				/* translators: %s: Error string for a failed installation. */
				__( 'Installation failed: %s' ),
			...
			$message     = fp.updates.adminNotice( {
				className: 'update-message notice-error notice-alt',
				message:   errorMessage
			} );
		...
		if ( fp.updates.maybeHandleCredentialError( response, 'install-theme' ) ) {
			return;
		}
		...
		if ( 'customize' === pagenow ) {
			...
			fp.customize.notifications.remove( 'theme_installing' );
		} else {
		...
		$document.trigger( 'fp-theme-install-error', response );
	};
	 ...
	 * @param {Object}              args
	 * @param {string}              args.slug    Theme stylesheet.
	 * @param {deleteThemeSuccess=} args.success Optional. Success callback. Default: fp.updates.deleteThemeSuccess
	 * @param {deleteThemeError=}   args.error   Optional. Error callback. Default: fp.updates.deleteThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	fp.updates.deleteTheme = function( args ) {
		var $button;
		...
		args = _.extend( {
			success: fp.updates.deleteThemeSuccess,
			error: fp.updates.deleteThemeError
		}, args );
		...
		$document.trigger( 'fp-theme-deleting', args );

		return fp.updates.ajax( 'delete-theme', args );
	};
	 ...
	 */
	fp.updates.deleteThemeSuccess = function( response ) {
		...
		if ( 'themes-network' === pagenow ) {
			...
			$themeRows.css( { backgroundColor: '#faafaa' } ).fadeOut( 350, function() {
				var $views     = $( '.subsubsub' ),
					...
					deletedRow = fp.template( 'item-deleted-row' );
		...
		fp.a11y.speak( _x( 'Deleted!', 'theme' ) );

		$document.trigger( 'fp-theme-delete-success', response );
	};
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	fp.updates.deleteThemeError = function( response ) {
		var $themeRow    = $( 'tr.inactive[data-slug="' + response.slug + '"]' ),
			$button      = $( '.theme-actions .delete-theme' ),
			updateRow    = fp.template( 'item-update-row' ),
		...
		if ( fp.updates.maybeHandleCredentialError( response, 'delete-theme' ) ) {
			return;
		}
		...
		$document.trigger( 'fp-theme-delete-error', response );
	};
	 ...
	 */
	fp.updates._addCallbacks = function( data, action ) {
		if ( 'import' === pagenow && 'install-plugin' === action ) {
			data.success = fp.updates.installImporterSuccess;
			data.error   = fp.updates.installImporterError;
		}
	 ...
	 */
	fp.updates.queueChecker = function() {
		var job;

		if ( fp.updates.ajaxLocked || ! fp.updates.queue.length ) {
			return;
		}

		job = fp.updates.queue.shift();

		// Handle a queue job.
		switch ( job.action ) {
			case 'install-plugin':
				fp.updates.installPlugin( job.data );
				break;

			case 'update-plugin':
				fp.updates.updatePlugin( job.data );
				break;

			case 'delete-plugin':
				fp.updates.deletePlugin( job.data );
				break;

			case 'install-theme':
				fp.updates.installTheme( job.data );
				break;

			case 'update-theme':
				fp.updates.updateTheme( job.data );
				break;

			case 'delete-theme':
				fp.updates.deleteTheme( job.data );
				break;
	 ...
	 */
	fp.updates.requestFilesystemCredentials = function( event ) {
		if ( false === fp.updates.filesystemCredentials.available ) {
			/*
			 * After exiting the credentials request modal,
			 * return the focus to the element triggering the request.
			 */
			if ( event && ! fp.updates.$elToReturnFocusToFromCredentialsModal ) {
				fp.updates.$elToReturnFocusToFromCredentialsModal = $( event.target );
			}

			fp.updates.ajaxLocked = true;
			fp.updates.requestForCredentialsModalOpen();
		}
	};
	 ...
	 * @param {Event=} event Optional. Event interface.
	 */
	fp.updates.maybeRequestFilesystemCredentials = function( event ) {
		if ( fp.updates.shouldRequestFilesystemCredentials && ! fp.updates.ajaxLocked ) {
			fp.updates.requestFilesystemCredentials( event );
		}
	};
	 ...
	 */
	fp.updates.keydown = function( event ) {
		if ( 27 === event.keyCode ) {
			fp.updates.requestForCredentialsModalCancel();
	 ...
	 * @since 4.2.0
	 */
	fp.updates.requestForCredentialsModalOpen = function() {
		var $modal = $( '#request-filesystem-credentials-dialog' );

		$( 'body' ).addClass( 'modal-open' );
		$modal.show();
		$modal.find( 'input:enabled:first' ).trigger( 'focus' );
		$modal.on( 'keydown', fp.updates.keydown );
	};
	 ...
	 * @since 4.2.0
	 */
	fp.updates.requestForCredentialsModalClose = function() {
		$( '#request-filesystem-credentials-dialog' ).hide();
		$( 'body' ).removeClass( 'modal-open' );

		if ( fp.updates.$elToReturnFocusToFromCredentialsModal ) {
			fp.updates.$elToReturnFocusToFromCredentialsModal.trigger( 'focus' );
		}
	};
	 ...
	 */
	fp.updates.requestForCredentialsModalCancel = function() {
		...
		_.each( fp.updates.queue, function( job ) {
		...
		fp.updates.requestForCredentialsModalClose();
	};
	 ...
	 */
	fp.updates.showErrorInCredentialsForm = function( message ) {
	 ...
	 * @param {string} action   The type of request to perform.
	 */
	fp.updates.credentialError = function( response, action ) {

		// Restore callbacks.
		response = fp.updates._addCallbacks( response, action );

		fp.updates.queue.unshift( {
			action: action,
		...
		fp.updates.filesystemCredentials.available = false;
		fp.updates.showErrorInCredentialsForm( response.errorMessage );
		fp.updates.requestFilesystemCredentials();
	};
	 ...
	 */
	fp.updates.maybeHandleCredentialError = function( response, action ) {
		if ( fp.updates.shouldRequestFilesystemCredentials && response.errorCode && 'unable_to_connect_to_filesystem' === response.errorCode ) {
			fp.updates.credentialError( response, action );
			return true;
		}
	 ...
	 *                                                'update' or 'install'.
	 */
	fp.updates.isValidResponse = function( response, action ) {
	 ...
	 * @since 4.2.0
	 */
	fp.updates.beforeunload = function() {
		if ( fp.updates.ajaxLocked ) {
	...
	$( function() {
		var $pluginFilter        = $( '#plugin-filter, #plugin-information-footer' ),
			...
			$pluginSearch        = $( '.plugins-php .fp-filter-search' ),
			$pluginInstallSearch = $( '.plugin-install-php .fp-filter-search' );

		settings = _.extend( settings, window._wpUpdatesItemCounts || {} );

		if ( settings.totals ) {
			fp.updates.refreshCount();
		}
		 ...
		 */
		fp.updates.shouldRequestFilesystemCredentials = $filesystemModal.length > 0;
		...
		$filesystemModal.on( 'submit', 'form', function( event ) {
			...
			fp.updates.filesystemCredentials.ftp.hostname       = $( '#hostname' ).val();
			fp.updates.filesystemCredentials.ftp.username       = $( '#username' ).val();
			fp.updates.filesystemCredentials.ftp.password       = $( '#password' ).val();
			fp.updates.filesystemCredentials.ftp.connectionType = $( 'input[name="connection_type"]:checked' ).val();
			fp.updates.filesystemCredentials.ssh.publicKey      = $( '#public_key' ).val();
			fp.updates.filesystemCredentials.ssh.privateKey     = $( '#private_key' ).val();
			fp.updates.filesystemCredentials.fsNonce            = $( '#_fs_nonce' ).val();
			fp.updates.filesystemCredentials.available          = true;
		 ...
		 * @since 4.2.0
		 */
		$filesystemModal.on( 'click', '[data-js-action="close"], .notification-dialog-background', fp.updates.requestForCredentialsModalCancel );
		...
		$document.on( 'credential-modal-cancel', function( event, job ) {
			...
			fp.a11y.speak( __( 'Update canceled.' ) );
		} );
		...
		$bulkActionForm.on( 'click', '[data-plugin] .update-link', function( event ) {
			...
			fp.updates.maybeRequestFilesystemCredentials( event );

			// Return the user to the input box of the plugin's table row after closing the modal.
			fp.updates.$elToReturnFocusToFromCredentialsModal = $pluginRow.find( '.check-column input' );
			fp.updates.updatePlugin( {
		...
		$document.on( 'click', '.importer-item .install-now', function( event ) {
			...
			fp.updates.installPlugin( {
				slug:    $button.data( 'slug' ),
				pagenow: pagenow,
				success: fp.updates.installImporterSuccess,
				error:   fp.updates.installImporterError
			} );
		} );
		...
		$document.on( 'click', '.themes-php.network-admin .update-link', function( event ) {
			...
			fp.updates.$elToReturnFocusToFromCredentialsModal = $themeRow.find( '.check-column input' );
			fp.updates.updateTheme( {
		...
		$bulkActionForm.on( 'click', '[type="submit"]:not([name="clear-recent-list"])', function( event ) {
			...
			$document.trigger( 'fp-' + type + '-bulk-' + bulkAction, itemsSelected );
			...
			$document.on( 'fp-plugin-update-success fp-plugin-update-error fp-theme-update-success fp-theme-update-error', function( event, response ) {
				var $itemRow = $( '[data-slug="' + response.slug + '"]' ),
					$bulkActionNotice, itemName;

				if ( 'fp-' + response.update + '-update-success' === event.type ) {
					success++;
				} else {
				...
				fp.updates.adminNotice = fp.template( 'fp-bulk-updates-admin-notice' );
				...
				if ( error > 0 && ! fp.updates.queue.length ) {
					$( 'html, body' ).animate( { scrollTop: 0 } );
				}
			} );

			// Reset admin notice template after #bulk-action-notice was added.
			$document.on( 'fp-updates-notice-added', function() {
				fp.updates.adminNotice = fp.template( 'fp-updates-admin-notice' );
			} );
		...
		fp.updates.shouldSearch = function( searchStringLength ) {
			var shouldSearch = searchStringLength >= fp.updates.searchMinCharacters ||
				previousSearchStringLength > fp.updates.searchMinCharacters;
		...
		$pluginInstallSearch.on( 'keyup input', _.debounce( function( event, eventtype ) {
			...
			if ( fp.updates.shouldSearch( searchStringLength ) ) {
			...
			if ( fp.updates.searchTerm === data.s && 'typechange' !== eventtype ) {
				return;
			} else {
				$pluginFilter.empty();
				fp.updates.searchTerm = data.s;
			}
			...
			if ( ! $searchTab.length ) {
				...
				$( '.fp-filter .filter-links .current' )
					.removeClass( 'current' )
			...
			if ( 'undefined' !== typeof fp.updates.searchRequest ) {
				fp.updates.searchRequest.abort();
			}
			$( 'body' ).addClass( 'loading-content' );

			fp.updates.searchRequest = fp.ajax.post( 'search-install-plugins', data ).done( function( response ) {
				$( 'body' ).removeClass( 'loading-content' );
				$pluginFilter.append( response.items );
				delete fp.updates.searchRequest;

				if ( 0 === response.count ) {
					fp.a11y.speak( __( 'You do not appear to have any plugins available at this time.' ) );
				} else {
					fp.a11y.speak(
						sprintf(
							/* translators: %s: Number of plugins. */
		...
		$pluginSearch.on( 'keyup input', _.debounce( function( event ) {
			...
			if ( fp.updates.searchTerm === data.s ) {
				return;
			} else {
				fp.updates.searchTerm = data.s;
			}
			...
			fp.updates.searchRequest = fp.ajax.post( 'search-plugins', data ).done( function( response ) {
				...
				if ( 0 === response.count ) {
					fp.a11y.speak( __( 'No plugins found. Try a different search.'  ) );
				} else {
					fp.a11y.speak(
						sprintf(
							/* translators: %s: Number of plugins. */
		...
		$document.on( 'submit', '.search-plugins', function( event ) {
			...
			$( 'input.fp-filter-search' ).trigger( 'input' );
		} );
		...
		$( window ).on( 'message', function( event ) {
			...
			switch ( message.action ) {
				...
				// Called from `fp-admin/includes/class-fp-upgrader-skins.php`.
				case 'decrementUpdateCount':
					/** @property {string} message.upgradeType */
					fp.updates.decrementCount( message.upgradeType );
					break;
				...
				case 'update-plugin':
					...
					message.data = fp.updates._addCallbacks( message.data, message.action );

					fp.updates.queue.push( message );
					fp.updates.queueChecker();
					break;
			}
		} );
		 ...
		 * @since 4.2.0
		 */
		$( window ).on( 'beforeunload', fp.updates.beforeunload );
		...
		$document.on( 'click keyup', '.column-auto-updates .toggle-auto-update, .theme-overlay .toggle-auto-update', function( event ) {
			var data, asset, type, $parent,
				$toggler = $( this ),
				action = $toggler.attr( 'data-fp-action' ),
			...
			$.post( window.ajaxurl, data )
				.done( function( response ) {
					...
					if ( ! response.success ) {
						// if FP returns 0 for response (which can happen in a few cases),
					...
					if ( 'enable' === action ) {
						...
						$toggler.attr( 'data-fp-action', 'enable' );

						$label.text( __( 'Enable auto-updates' ) );
						$parent.find( '.auto-update-time' ).addClass( 'hidden' );
						fp.a11y.speak( __( 'Auto-updates disabled' ) );
					}

					$document.trigger( 'fp-auto-update-setting-changed', { state: action, type: type, asset: asset } );
				} )
				.fail( function() {
					...
					fp.a11y.speak( __( 'The request could not be completed.' ), 'assertive' );
				} )
				.always( function() {
...
})( jQuery, window.fp, window._wpUpdatesSettings );
