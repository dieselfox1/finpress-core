 ...
 *
 * @output fp-admin/js/privacy-tools.js
 */
			...
			$.ajax(
				{
					url: window.ajaxurl,
					data: {
						action: 'fp-privacy-export-personal-data',
	...
	$( '.remove-personal-data-handle' ).on( 'click', function( event ) {
		...
		function doNextErasure( eraserIndex, pageIndex ) {
			$.ajax({
				url: window.ajaxurl,
				data: {
					action: 'fp-privacy-erase-personal-data',
					eraser: eraserIndex,
					id: requestID,
					page: pageIndex,
					security: nonce
		...
		if ( $target.is( 'button.privacy-text-copy' ) ) {
			...
			if ( $parent.length ) {
				try {
					...
					fp.a11y.speak( __( 'The suggested policy text has been copied to your clipboard.' ) );
