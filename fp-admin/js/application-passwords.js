/**
 * @output fp-admin/js/application-passwords.js
 */
...
( function( $ ) {
	var $appPassSection = $( '#application-passwords-section' ),
		...
		tmplNewAppPass = fp.template( 'new-application-password' ),
		tmplAppPassRow = fp.template( 'application-password-row' ),
		userId = $( '#user_id' ).val();
	...
	$newAppPassButton.on( 'click', function( e ) {
		 ...
		 */
		request = fp.hooks.applyFilters( 'wp_application_passwords_new_password_request', request, userId );

		fp.apiRequest( {
			path: '/fp/v2/users/' + userId + '/application-passwords?_locale=user',
			method: 'POST',
			data: request
		} ).always( function() {
			 ...
			 */
			fp.hooks.doAction( 'wp_application_passwords_created_password', response, request );
	...
	$appPassTbody.on( 'click', '.delete', function( e ) {
		...
		if ( ! window.confirm( fp.i18n.__( 'Are you sure you want to revoke this password? This action cannot be undone.' ) ) ) {
			return;
		}
		...
		fp.apiRequest( {
			path: '/fp/v2/users/' + userId + '/application-passwords/' + uuid + '?_locale=user',
			method: 'DELETE'
		} ).always( function() {
			...
			if ( response.deleted ) {
				...
				addNotice( fp.i18n.__( 'Application password revoked.' ), 'success' ).trigger( 'focus' );
			}
		} ).fail( handleErrorResponse );
	} );
	...
	$removeAllBtn.on( 'click', function( e ) {
		...
		if ( ! window.confirm( fp.i18n.__( 'Are you sure you want to revoke all passwords? This action cannot be undone.' ) ) ) {
			return;
		}
		...
		fp.apiRequest( {
			...
			if ( response.deleted ) {
				...
				addNotice( fp.i18n.__( 'All application passwords revoked.' ), 'success' ).trigger( 'focus' );
			}
		} ).fail( handleErrorResponse );
	} );
		...
		var $notice = $( '<div></div>' )
			...
			.append(
				$( '<button></button>' )
					...
					.append( $( '<span></span>' ).addClass( 'screen-reader-text' ).text( fp.i18n.__( 'Dismiss this notice.' ) ) )
			);
