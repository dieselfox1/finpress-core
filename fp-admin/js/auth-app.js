/**
 * @output fp-admin/js/auth-app.js
 */
...
( function( $, authApp ) {
	...
	$approveBtn.on( 'click', function( e ) {
		 ...
		 */
		request = fp.hooks.applyFilters( 'wp_application_passwords_approve_app_request', request, context );

		fp.apiRequest( {
			path: '/fp/v2/users/me/application-passwords?_locale=user',
			method: 'POST',
			data: request
			 ...
			 */
			fp.hooks.doAction( 'wp_application_passwords_approve_app_request_success', response, textStatus, jqXHR );
			...
			if ( raw ) {
				...
				message = fp.i18n.sprintf(
					/* translators: %s: Application name. */
					'<label for="new-application-password-value">' + fp.i18n.__( 'Your new password for %s is:' ) + '</label>',
					'<strong></strong>'
				...
				$notice = $( '<div></div>' )
					...
					.append( '<p>' + fp.i18n.__( 'Be sure to save this in a safe location. You will not be able to retrieve it.' ) + '</p>' );
			 ...
			 * @param {jqXHR}       jqXHR       The underlying jqXHR object that made the request.
			 */
			fp.hooks.doAction( 'wp_application_passwords_approve_app_request_error', error, textStatus, errorThrown, jqXHR );
		} );
	} );
	...
	$rejectBtn.on( 'click', function( e ) {
		 ...
		 * @param {string} context.rejectUrl  The URL the user will be redirected to after rejecting the request.
		 */
		fp.hooks.doAction( 'wp_application_passwords_reject_app', context );
