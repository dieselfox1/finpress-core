/**
 * @output fp-admin/js/customize-nav-menus.js
 */
...
( function( api, fp, $ ) {
	 ...
	 *
	 * @alias fp.customize.Menus.generatePlaceholderAutoIncrementId
	 *
	 * @return {number}
	 */
	...
	/**
	 * fp.customize.Menus.AvailableItemModel
	 *
	 * A single available menu item model. See PHP's FP_Customize_Nav_Menu_Item_Setting class.
	 *
	 * @class    fp.customize.Menus.AvailableItemModel
	...
	/**
	 * fp.customize.Menus.AvailableItemCollection
	 *
	 * Collection for available menu item models.
	 *
	 * @class    fp.customize.Menus.AvailableItemCollection
	 * @augments Backbone.Collection
	 */
	api.Menus.AvailableItemCollection = Backbone.Collection.extend(/** @lends fp.customize.Menus.AvailableItemCollection.prototype */{
	 ...
	 *
	 * @since 4.7.0
	 * @alias fp.customize.Menus.insertAutoDraftPost
	 *
	...
	api.Menus.insertAutoDraftPost = function insertAutoDraftPost( params ) {
		...
		request = fp.ajax.post( 'customize-nav-menus-insert-auto-draft', {
	...
	api.Menus.AvailableMenuItemsPanelView = fp.Backbone.View.extend(/** @lends fp.customize.Menus.AvailableMenuItemsPanelView.prototype */{
		...
		/**
		 * fp.customize.Menus.AvailableMenuItemsPanelView
		 *
		 * View class for the available menu items panel.
		 *
		 * @constructs fp.customize.Menus.AvailableMenuItemsPanelView
		 * @augments   fp.Backbone.View
		 */
		initialize: function() {
			var self = this;
		...
		doSearch: function( page ) {
			...
			if ( page < 0 ) {
				...
				fp.a11y.speak( api.Menus.data.l10n.itemsLoadingMore );
			...
			self.currentRequest = fp.ajax.post( 'search-available-menu-items-customizer', params );
			...
			self.currentRequest.done(function( data ) {
				...
				if ( items && page > 1 ) {
					fp.a11y.speak( api.Menus.data.l10n.itemsFoundMore.replace( '%d', items.length ) );
				} else if ( items && page === 1 ) {
					fp.a11y.speak( api.Menus.data.l10n.itemsFound.replace( '%d', items.length ) );
				}
			});
		...
		loadItems: function( itemTypes, deprecated ) {
			...
			request = fp.ajax.post( 'load-available-menu-items-customizer', params );
		...
		submitLink: function() {
			...
			if ( ! urlRegex.test( url ) || '' === itemName.val() ) {
				if ( ! urlRegex.test( url ) ) {
					...
					fp.a11y.speak( errorText, 'assertive' );
				}
				if ( '' === itemName.val() ) {
		...
		submitNew: function( container ) {
			...
			promise.done( function( data ) {
				...
				itemElement = $( fp.template( 'available-menu-item' )( availableItem.attributes ) );
	...
	/**
	 * fp.customize.Menus.MenusPanel
	 *
	 * Customizer panel for menus. This is used only for screen options management.
	 * Note that 'menus' must match the FP_Customize_Menu_Panel::$type.
	 *
	 * @class    fp.customize.Menus.MenusPanel
	 * @augments fp.customize.Panel
	 */
	api.Menus.MenusPanel = api.Panel.extend(/** @lends fp.customize.Menus.MenusPanel.prototype */{
		...
		ready: function() {
			...
			api.section( 'menu_locations', function( section ) {
				section.headContainer.prepend(
					fp.template( 'nav-menu-locations-header' )( api.Menus.data )
				);
			} );
		},
		...
		saveManageColumnsState: _.debounce( function() {
			...
			panel._updateHiddenColumnsRequest = fp.ajax.post( 'hidden-columns', {
				hidden: panel.hidden(),
	...
	/**
	 * fp.customize.Menus.MenuSection
	 *
	 * Customizer section for menus. This is used only for lazy-loading child controls.
	 * Note that 'nav_menu' must match the FP_Customize_Menu_Section::$type.
	 *
	 * @class    fp.customize.Menus.MenuSection
	 * @augments fp.customize.Section
	 */
	api.Menus.MenuSection = api.Section.extend(/** @lends fp.customize.Menus.MenuSection.prototype */{
			...
			if ( expanded ) {
				...
				args.completeCallback = function() {
					if ( 'resolved' !== section.deferred.initSortables.state() ) {
						...
						// @todo Note that fp.customize.reflowPaneContents() is debounced,
	 ...
	 * @return {fp.customize.Menus.MenuSection} Added nav menu.
	 */
	...
	/**
	 * fp.customize.Menus.NewMenuSection
	 *
	 * Customizer section for new menus.
	 *
	 * @class    fp.customize.Menus.NewMenuSection
	 * @augments fp.customize.Section
	 */
	api.Menus.NewMenuSection = api.Section.extend(/** @lends fp.customize.Menus.NewMenuSection.prototype */{
		...
		attachEvents: function() {
			...
			section.headContainer.find( '.accordion-section-title' ).replaceWith(
				fp.template( 'nav-menu-create-menu-section-title' )
			);
			 ...
			 *
			 * @since 4.9.0
			 * @param {fp.customize.Setting} setting - Added setting.
			 * @return {void}
			 */
			 ...
			 *
			 * @since 4.9.0
			 * @param {fp.customize.Setting} setting - Removed setting.
			 * @return {void}
			 */
		...
		submit: function() {
			...
			fp.a11y.speak( api.Menus.data.l10n.menuAdded );
	...
	/**
	 * fp.customize.Menus.MenuLocationControl
	 *
	 * Customizer control for menu locations (rendered as a <select>).
	 * Note that 'nav_menu_location' must match the FP_Customize_Nav_Menu_Location_Control::$type.
	 *
	 * @class    fp.customize.Menus.MenuLocationControl
	 * @augments fp.customize.Control
	 */
	api.Menus.MenuLocationControl = api.Control.extend(/** @lends fp.customize.Menus.MenuLocationControl.prototype */{
		initialize: function( id, options ) {
			var control = this,
	...
	api.Menus.MenuItemControl = api.Control.extend(/** @lends fp.customize.Menus.MenuItemControl.prototype */{

		/**
		 * fp.customize.Menus.MenuItemControl
		 *
		 * Customizer control for menu items.
		 * Note that 'menu_item' must match the FP_Customize_Menu_Item_Control::$type.
		 *
		 * @constructs fp.customize.Menus.MenuItemControl
		 * @augments   fp.customize.Control
		 *
		 * @inheritDoc
		 */
		...
		_setupReorderUI: function() {
			...
			template = fp.template( 'menu-item-reorder-nav' );
		...
		_setupRemoveUI: function() {
			...
			$removeBtn.on( 'click', function() {
				...
				control.container.slideUp( function() {
					...
					fp.a11y.speak( api.Menus.data.l10n.itemDeleted );
		...
		/**
		 * @return {fp.customize.controlConstructor.nav_menu|null}
		 */
		...
		moveUp: function() {
			...
			fp.a11y.speak( api.Menus.data.l10n.movedUp );
		},
		...
		moveDown: function() {
			this._changePosition( 1 );
			fp.a11y.speak( api.Menus.data.l10n.movedDown );
		},
		/**
		...
		moveLeft: function() {
			this._changeDepth( -1 );
			fp.a11y.speak( api.Menus.data.l10n.movedLeft );
		},
		...
		moveRight: function() {
			this._changeDepth( 1 );
			fp.a11y.speak( api.Menus.data.l10n.movedRight );
		},
	...
	/**
	 * fp.customize.Menus.MenuNameControl
	 *
	 * Customizer control for a nav menu's name.
	 *
	 * @class    fp.customize.Menus.MenuNameControl
	 * @augments fp.customize.Control
	 */
	api.Menus.MenuNameControl = api.Control.extend(/** @lends fp.customize.Menus.MenuNameControl.prototype */{
	...
	/**
	 * fp.customize.Menus.MenuLocationsControl
	 *
	 * Customizer control for a nav menu's locations.
	 *
	 * @since 4.9.0
	 * @class    fp.customize.Menus.MenuLocationsControl
	 * @augments fp.customize.Control
	 */
	api.Menus.MenuLocationsControl = api.Control.extend(/** @lends fp.customize.Menus.MenuLocationsControl.prototype */{
	...
	/**
	 * fp.customize.Menus.MenuAutoAddControl
	 *
	 * Customizer control for a nav menu's auto add.
	 *
	 * @class    fp.customize.Menus.MenuAutoAddControl
	 * @augments fp.customize.Control
	 */
	api.Menus.MenuAutoAddControl = api.Control.extend(/** @lends fp.customize.Menus.MenuAutoAddControl.prototype */{
	...
	/**
	 * fp.customize.Menus.MenuControl
	 *
	 * Customizer control for menus.
	 * Note that 'nav_menu' must match the FP_Menu_Customize_Control::$type
	 *
	 * @class    fp.customize.Menus.MenuControl
	 * @augments fp.customize.Control
	 */
	api.Menus.MenuControl = api.Control.extend(/** @lends fp.customize.Menus.MenuControl.prototype */{
		/**
		 * Set up the control.
		 */
		ready: function() {
		...
		_handleDeletion: function() {
			...
			if ( section && section.expanded() ) {
				section.collapse({
					completeCallback: function() {
						removeSection();
						fp.a11y.speak( api.Menus.data.l10n.menuDeleted );
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( this.isReordering ) {
				...
				fp.a11y.speak( api.Menus.data.l10n.reorderModeOn );
				...
				fp.a11y.speak( api.Menus.data.l10n.reorderModeOff );
		...
		/**
		 * @return {fp.customize.controlConstructor.nav_menu_item[]}
		 */
		 ...
		 * @return {fp.customize.Menus.controlConstructor.nav_menu_item} The newly-created nav_menu_item control instance.
		 */
		addItemToMenu: function( item ) {
			...
			fp.a11y.speak( api.Menus.data.l10n.itemAdded );
		 ...
		 * @since 4.9.0
		 *
		 * @param {fp.customize.controlConstructor.nav_menu_item[]} optionalMenuItemControls
		 */
	...
	/**
	 * Extends fp.customize.controlConstructor with control constructor for
	...
	/**
	 * Extends fp.customize.panelConstructor with section constructor for menus.
	 */
	...
	/**
	 * Extends fp.customize.sectionConstructor with section constructor for menu.
	 */
	 ...
	 *
	 * @alias fp.customize.Menus.applySavedData
	 *
	 * @param {Object} data
			...
			if ( 'inserted' === update.status ) {
				...
				api.each( function( setting ) {
					...
					if ( /^nav_menu_locations\[/.test( setting.id ) && setting.get() === update.previous_term_id ) {
						...
						setting._dirty = false; // Not dirty because this is has also just been done on server in FP_Customize_Nav_Menu_Setting::update().
	 ...
	 *
	 * @alias fp.customize.Menus.focusMenuItemControl
	 *
	 ...
	 *
	 * @alias fp.customize.Menus.getMenuControl
	 *
	 * @param menuId
	 * @return {fp.customize.controlConstructor.menus[]}
	 */
	 ...
	 *
	 * @alias fp.customize.Menus.getMenuItemControl
	 *
	 * @param {string} menuItemId
	 * @return {Object|null}
	 */
	...
	/**
	 * @alias fp.customize.Menus~menuItemIdToSettingId
	 *
	 * @param {string} menuItemId
	 */
	 ...
	 *
	 * @alias fp.customize.Menus~displayNavMenuName
	 *
	 * @param {string} name
	 * @return {string}
	 */
	function displayNavMenuName( name ) {
		name = name || '';
		name = fp.sanitize.stripTagsAndEncodeText( name ); // Remove any potential tags from name.
		name = name.toString().trim();
		return name || api.Menus.data.l10n.unnamed;
	}

})( fp.customize, fp, jQuery );
