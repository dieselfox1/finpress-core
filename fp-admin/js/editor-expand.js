/**
 * @output fp-admin/js/editor-expand.js
 */
...
( function( window, $, undefined ) {
	...
	$( function() {
		var $wrap = $( '#postdivrich' ),
			$contentWrap = $( '#fp-content-wrap' ),
			$tools = $( '#fp-content-editor-tools' ),
			$visualTop = $(),
			$visualEditor = $(),
			$textTop = $( '#ed_toolbar' ),
			$textEditor = $( '#content' ),
			...
			$postBody = $('#post-body'),
			fullscreen = window.fp.editor && window.fp.editor.fullscreen,
			mceEditor,
			mceBind = function(){},
			mceUnbind = function(){},
			fixedTop = false,
		...
		$document.on( 'tinymce-editor-init.editor-expand', function( event, editor ) {
			...
			function mceGetCursorOffset() {
				...
				/*
				 * If editor.fp.getView and the selection node from the editor selection
				 * are defined, use this as a view for the offset.
				 */
				if ( editor.fp && editor.fp.getView && ( view = editor.fp.getView( node ) ) ) {
			...
			mceBind = function() {
				...
				editor.on( 'fp-toolbar-toggle', toggleAdvanced );

				// Adjust when the editor resizes.
				editor.on( 'setcontent fp-autoresize fp-toolbar-toggle', adjust );
			...
			mceUnbind = function() {
				...
				editor.off( 'fp-toolbar-toggle', toggleAdvanced );
				editor.off( 'setcontent fp-autoresize fp-toolbar-toggle', adjust );
			...
			if ( $wrap.hasClass( 'fp-editor-expand' ) ) {
		...
		function adjust( event ) {
			...
			if ( ! canPin ) {
				...
				if ( ( ! fixedBottom || ( resize && advanced ) ) &&
						// Add borderWidth for the border around the .fp-editor-container.
		...
		function on() {
			...
			$wrap.addClass( 'fp-editor-expand' );
			 ...
			 */
			$document.on( 'fp-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust )
				.on( 'postbox-toggled.editor-expand postbox-moved.editor-expand', function() {
					...
					adjust();
				}).on( 'fp-window-resized.editor-expand', function() {
					if ( mceEditor && ! mceEditor.isHidden() ) {
		...
		function off() {
			...
			$wrap.removeClass( 'fp-editor-expand' );
	...
	$( function() {
		...
		$document.on( 'tinymce-editor-init.focus', function( event, editor ) {
			...
			if ( editor.id === 'content' ) {
				...
				mceBind = function() {
					...
					editor.on( 'blur', blur );
					editor.on( 'fp-autoresize', recalcEditorRect );
				};
				...
				mceUnbind = function() {
					...
					editor.off( 'fp-autoresize', recalcEditorRect );
				};
		...
		window.fp = window.fp || {};
		window.fp.editor = window.fp.editor || {};
		window.fp.editor.dfw = {
			activate: activate,
