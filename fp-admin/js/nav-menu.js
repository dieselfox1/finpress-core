 ...
 * @output fp-admin/js/nav-menu.js
 */
...
(function($) {
	...
	api = window.wpNavMenu = {
				...
				updateParentDropdown : function() {
					return this.each(function(){
						...
						$.each( parentDropdowns, function() {
							...
							$html += '<option ' + $selected + ' value="0">' + fp.i18n._x( 'No Parent', 'menu item without a parent in navigation menu' ) + '</option>';
						...
						$.each( orderDropdowns, function() {
							...
							if ( isPrimaryMenuItem ) {
								...
								for ( let i = 1; i < totalMenuItems + 1; i++ ) {
									...
									var itemString = fp.i18n.sprintf( 
										/* translators: 1: The current menu item number, 2: The total number of menu items. */
										fp.i18n._x( '%1$s of %2$s', 'part of a total number of menu items' ),
										i,
										totalMenuItems
									);
								...
								for ( let i = 1; i < totalSubMenuItems + 1; i++ ) {
									...
									var submenuString = fp.i18n.sprintf( 
		...
		changeMenuParent : function( parentDropdown ) {
			...
			api.refreshAdvancedAccessibility();
			$this.trigger( 'focus' );
			fp.a11y.speak( menus.parentUpdated, 'polite' );
		},
		...
		changeMenuOrder : function( orderDropdown ) {
			...
			api.refreshAdvancedAccessibility();
			$this.trigger( 'focus' );
			fp.a11y.speak( menus.orderUpdated, 'polite' );
		},
		...
		initPreviewing : function() {
			...
			$( '#menu-to-edit' ).on( 'change input', '.edit-menu-item-title', function(e) {
				...
				if ( title ) {
					...
					titleEl.text( fp.i18n._x( '(no label)', 'missing menu item navigation label' ) ).addClass( 'no-title' );
				}
			} );
		},
		...
		attachMenuItemDeleteButton : function() {
			...
			$( document ).on( 'click', '.menu-items-delete', function( e ) {
				...
				if ( ! $(this).hasClass( 'disabled' ) ) {
					...
					fp.a11y.speak( deletionSpeech, 'polite' );
					that.disableBulkSelection();
		...
		attachUnsavedChangesListener : function() {
			...
			if ( 0 !== $('#menu-to-edit').length || 0 !== $('.menu-location-menus select').length ) {
				window.onbeforeunload = function(){
					if ( api.menusChanged )
						return fp.i18n.__( 'The changes you made will be lost if you navigate away from this page.' );
				};
			} else {
		...
		eventOnClickMenuDelete : function() {
			// Delete warning AYS.
			if ( window.confirm( fp.i18n.__( 'You are about to permanently delete this menu.\n\'Cancel\' to stop, \'OK\' to delete.' ) ) ) {
				window.onbeforeunload = null;
				return true;
			}
			return false;
		...
		processQuickSearchQueryResponse : function(resp, req, panel) {
			...
			if( ! $items.length ) {
				$('.categorychecklist', panel).html( '<li><p>' + fp.i18n.__( 'No results found.' ) + '</p></li>' );
		...
		removeMenuItem : function(el) {
			...
			el.addClass('deleting').animate({
					...
					api.refreshAdvancedAccessibility();
					fp.a11y.speak( menus.itemRemoved );
					$( '#menu-to-edit' ).updateParentDropdown();
					$( '#menu-to-edit' ).updateOrderDropdown();
				});
		},
