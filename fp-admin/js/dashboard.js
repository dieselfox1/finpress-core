/**
 * @output fp-admin/js/dashboard.js
 */
...
jQuery( function($) {
	...
	updateWelcomePanel = function( visible ) {
		$.post(
			...
			function() {
				fp.a11y.speak( fp.i18n.__( 'Screen Options updated.' ) );
			}
		);
	};
...
jQuery( function( $ ) {
	...
	var communityEventsData = window.communityEventsData,
		dateI18n = fp.date.dateI18n,
		format = fp.date.format,
	 ...
	 *
	 * @memberOf fp
	 * @namespace fp.communityEvents
	 */
	app = window.fp.communityEvents = /** @lends fp.communityEvents */{
		initialized: false,
		model: null,

		/**
		 * Initializes the fp.communityEvents object.
		 *
		 * @since 4.8.0
		 *
		 * @return {void}
		...
		getEvents: function( requestParams ) {
			...
			fp.ajax.post( 'get-community-events', requestParams )
				.always( function() {
		 ...
		 * @since 4.8.0
		 *
		 * @param {Object} templateParams The various parameters that will get passed to fp.template.
		...
		renderEventsTemplate: function( templateParams, initiatedBy ) {
			...
			if ( templateParams.location.ip ) {
				...
				if ( templateParams.events.length ) {
					template = fp.template( 'community-events-event-list' );
					$results.html( template( templateParams ) );
				} else {
					template = fp.template( 'community-events-no-upcoming-events' );
					$results.html( template( templateParams ) );
				}
				...
				template = fp.template( 'community-events-attend-event-near' );
				...
				template = fp.template( 'community-events-could-not-locate' );
				 ...
				 */
				fp.a11y.speak( __( 'An error occurred. Please try again.' ) );
		 ...
		 * @param {string} timeFormat A time format acceptable by `fp.date.dateI18n()`.
		 *
		 * @returns {Array}
		 */
		...
		/**
		 * Returns the user's local/browser time zone, in a form suitable for `fp.date.i18n()`.
		 *
		 * @since 5.5.2
		 *
		 ...
		 * @param {string} timeZone    A time zone string or offset which is parsable by `fp.date.i18n()`.
		 *
		 * @returns {string}
		 */
...
window.communityEventsData.l10n = window.fp.deprecateL10nObject( 'communityEventsData.l10n', window.communityEventsData.l10n, '5.6.0' );
