/**
 * @output fp-admin/js/tags-box.js
 */
...
( function( $ ) {
	var tagDelimiter = fp.i18n._x( ',', 'tag delimiter' ) || ',';
	...
	window.tagBox = {
			...
			$.each( current_tags, function( key, val ) {
				...
				if ( ! disabled ) {
					...
					xbutton = $( '<button type="button" id="' + id + '-check-num-' + key + '" class="ntdelbutton">' +
						...
						'<span class="screen-reader-text">' + fp.i18n.__( 'Remove term:' ) + ' ' + listItem.html() + '</span>' +
						'</button>' );
		...
		screenReadersMessage: function() {
			...
			switch ( this.userAction ) {
				case 'remove':
					message = fp.i18n.__( 'Term removed.' );
					break;

				case 'add':
					message = fp.i18n.__( 'Term added.' );
					break;
			...
			window.fp.a11y.speak( message, 'assertive' );
		},
