 ...
 *
 * @output fp-admin/js/tags-suggest.js
 */
( function( $ ) {
	var tempID = 0;
	var separator = fp.i18n._x( ',', 'tag delimiter' ) || ',';
	var __ = fp.i18n.__,
	    _n = fp.i18n._n,
	 ...
	 *
	 * The taxonomy can be passed in a `data-fp-taxonomy` attribute on the element or
	...
	$.fn.wpTagsSuggest = function( options ) {
		...
		var taxonomy = options.taxonomy || $element.attr( 'data-fp-taxonomy' ) || 'post_tag';
		...
		options = $.extend( {
			...
			focus: function( event, ui ) {
				$element.attr( 'aria-activedescendant', 'fp-tags-autocomplete-' + ui.item.id );
			...
			select: function( event, ui ) {
				...
				if ( $.ui.keyCode.TAB === event.keyCode ) {
					...
					window.fp.a11y.speak( fp.i18n.__( 'Term selected.' ), 'assertive' );
					event.preventDefault();
		...
		$element.autocomplete( 'instance' )._renderItem = function( ul, item ) {
			return $( '<li role="option" id="fp-tags-autocomplete-' + item.id + '">' )
				.text( item.name )
				.appendTo( ul );
		};
