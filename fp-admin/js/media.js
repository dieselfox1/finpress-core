...
( function( $ ){
	window.findPosts = {
		...
		send: function() {
			...
			$.ajax( ajaxurl, {
				...
				if ( ! x.success ) {
					$( '#find-posts-response' ).text( fp.i18n.__( 'An error has occurred. Please reload the page and try again.' ) );
				}
	...
	$( function() {
		...
		if ( $mediaGridWrap.length && window.fp && window.fp.media ) {
			...
			$mediaGridWrap.trigger( 'fp-media-grid-ready', frame );
		}
		...
		copyAttachmentURLClipboard.on( 'success', function( event ) {
			...
			fp.a11y.speak( fp.i18n.__( 'The file URL has been copied to your clipboard' ) );
		} );
	});
})( jQuery );
