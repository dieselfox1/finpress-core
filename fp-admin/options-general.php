...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://finpress.org/documentation/article/settings-general-screen/">Documentation on General Settings</a>' ) . '</p>' .
...
<td><input name="siteurl" type="url" id="siteurl" value="<?php form_option( 'siteurl' ); ?>"<?php disabled( defined( 'FP_SITEURL' ) ); ?> class="regular-text code<?php echo $wp_site_url_class; ?>" /></td>
</tr>

<tr>
<th scope="row"><label for="home"><?php _e( 'Site Address (URL)' ); ?></label></th>
<td><input name="home" type="url" id="home" aria-describedby="home-description" value="<?php form_option( 'home' ); ?>"<?php disabled( defined( 'FP_HOME' ) ); ?> class="regular-text code<?php echo $wp_home_class; ?>" />
	<?php if ( ! defined( 'FP_HOME' ) ) : ?>
...
if ( ! is_multisite() && defined( 'FPLANG' ) && '' !== FPLANG && 'en_US' !== FPLANG && ! in_array( FPLANG, $languages, true ) ) {
	$languages[] = FPLANG;
}
if ( ! empty( $languages ) || ! empty( $translations ) ) {
	?>
	<tr>
		<th scope="row"><label for="FPLANG"><?php _e( 'Site Language' ); ?><span class="dashicons dashicons-translation" aria-hidden="true"></span></label></th>
		<td>
			<?php
			$locale = get_locale();
			...
			// Add note about deprecated FPLANG constant.
			if ( defined( 'FPLANG' ) && ( '' !== FPLANG ) && FPLANG !== $locale ) {
				_deprecated_argument(
					'define()',
					'4.0.0',
					/* translators: 1: FPLANG, 2: fp-config.php */
					sprintf( __( 'The %1$s constant in your %2$s file is no longer needed.' ), 'FPLANG', 'fp-config.php' )
				);
			}
			?>
		</td>
...
foreach ( $time_formats as $format ) {
	...
	echo "\t<p class='date-time-doc'>" . __( '<a href="https://finpress.org/documentation/article/customize-date-and-time-format/">Documentation on date and time formatting</a>.' ) . "</p>\n";
?>
	</fieldset>
</td>
</tr>
...
<?php require_once ABSPATH . 'fp-admin/admin-footer.php'; ?>
