...
$wp_list_table = _get_list_table( 'FP_Plugins_List_Table' );
...
if ( $action ) {
	...
	switch ( $action ) {
		...
		case 'error_scrape':
			...
			/** This action is documented in fp-admin/includes/plugin.php */
		...
		case 'delete-selected':
			...
			require ABSPATH . 'fp-admin/update.php';
			...
			if ( ! isset( $_REQUEST['verify-delete'] ) ) {
				...
				foreach ( (array) $plugins as $plugin ) {
					...
					if ( '.' === $plugin_slug ) {
						$data = get_plugin_data( FP_PLUGIN_DIR . '/' . $plugin );
						if ( $data ) {
		...
		default:
			if ( isset( $_POST['checked'] ) ) {
				...
				/** This action is documented in fp-admin/edit.php */
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				...
				'<p>' . sprintf(
					/* translators: %s: FP_PLUGIN_DIR constant value. */
...
if ( current_user_can( 'update_plugins' ) && wp_is_auto_update_enabled_for_type( 'plugin' ) ) {
	get_current_screen()->add_help_tab(
		array(
			...
			'content' =>
					'<p>' . __( 'Auto-updates can be enabled or disabled for each individual plugin. Plugins with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the FP-Cron task scheduling system.' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://finpress.org/documentation/article/manage-plugins/">Documentation on Managing Plugins</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
<?php FP_Plugin_Dependencies::display_admin_notice_for_unmet_dependencies(); ?>
<?php FP_Plugin_Dependencies::display_admin_notice_for_circular_dependencies(); ?>
<div class="wrap">
