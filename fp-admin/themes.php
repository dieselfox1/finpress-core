...
if ( current_user_can( 'install_themes' ) ) {
	if ( is_multisite() ) {
		...
		$help_install = '<p>' . sprintf(
			/* translators: %s: https://finpress.org/themes/ */
...
if ( current_user_can( 'update_themes' ) && wp_is_auto_update_enabled_for_type( 'theme' ) ) {
	$help_tab_autoupdates =
		'<p>' . __( 'Auto-updates can be enabled or disabled for each individual theme. Themes with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the FP-Cron task scheduling system.' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://finpress.org/documentation/article/work-with-themes/">Documentation on Using Themes</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://finpress.org/documentation/article/appearance-themes-screen/">Documentation on Managing Themes</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
<div class="wrap">
	<h1 class="fp-heading-inline"><?php esc_html_e( 'Themes' ); ?>
...
if ( is_array( $submenu ) && isset( $submenu['themes.php'] ) ) {
	...
	foreach ( (array) $submenu['themes.php'] as $item ) {
		...
		if ( ! empty( $submenu[ $item[2] ] ) ) {
			...
			if ( file_exists( FP_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}" ) || ! empty( $menu_hook ) ) {
			...
			if ( file_exists( ABSPATH . "fp-admin/$menu_file" ) ) {
...
<div class="theme<?php echo $active_class; ?>">
	...
	<?php if ( $theme['hasUpdate'] ) : ?>
		<?php
		if ( $theme['updateResponse']['compatibleFP'] && $theme['updateResponse']['compatiblePHP'] ) :
			if ( $theme['hasPackage'] ) {
			...
			} elseif ( ! $theme['updateResponse']['compatibleFP'] ) {
				$theme_update_error .= sprintf(
					/* translators: %s: Theme name. */
	...
	if ( ! $theme['compatibleFP'] || ! $theme['compatiblePHP'] ) {
		$message = '';
		if ( ! $theme['compatibleFP'] && ! $theme['compatiblePHP'] ) {
	...
	<div class="theme-id-container">
		...
		<?php if ( $theme['active'] ) { ?>
			...
			if ( $theme['actions']['customize'] && current_user_can( 'edit_theme_options' ) && current_user_can( 'customize' ) ) {
				...
				><?php _e( 'Customize' ); ?></a>
			<?php } ?>
		<?php } elseif ( $theme['compatibleFP'] && $theme['compatiblePHP'] ) { ?>
			<?php
			/* translators: %s: Theme name. */
		...
		<div class="theme-autoupdate">
			<# if ( data.autoupdate.supported ) { #>
				...
				<# } else if ( data.autoupdate.enabled ) { #>
					<button type="button" class="toggle-auto-update button-link" data-slug="{{ data.id }}" data-fp-action="disable">
						<span class="dashicons dashicons-update spin hidden" aria-hidden="true"></span><span class="label">' . __( 'Disable auto-updates' ) . '</span>
					</button>
				<# } else { #>
					<button type="button" class="toggle-auto-update button-link" data-slug="{{ data.id }}" data-fp-action="enable">
