...
if ( isset( $_POST['deletepost'] ) ) {
	$action = 'delete';
} elseif ( isset( $_POST['fp-preview'] ) && 'dopreview' === $_POST['fp-preview'] ) {
	$action = 'preview';
}
...
switch ( $action ) {
	case 'post-quickdraft-save':
		...
		if ( ! str_contains( $_POST['content'], '<!-- fp:paragraph -->' ) ) {
			$_POST['content'] = sprintf(
				'<!-- fp:paragraph -->%s<!-- /fp:paragraph -->',
				str_replace( array( "\r\n", "\r", "\n" ), '<br />', $_POST['content'] )
			);
		}
	...
	case 'edit':
		...
		if ( use_block_editor_for_post( $post ) ) {
			require ABSPATH . 'fp-admin/edit-form-blocks.php';
			break;
		}
		...
		require ABSPATH . 'fp-admin/edit-form-advanced.php';
	...
	case 'editpost':
		...
		if ( isset( $_COOKIE['fp-saving-post'] ) && $_COOKIE['fp-saving-post'] === $post_id . '-check' ) {
			setcookie( 'fp-saving-post', $post_id . '-saved', time() + DAY_IN_SECONDS, ADMIN_COOKIE_PATH, COOKIE_DOMAIN, is_ssl() );
		}
