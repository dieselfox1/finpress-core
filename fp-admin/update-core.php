...
function list_core_update( $update ) {
	...
	if ( 'development' === $update->response ) {
		...
		if ( $current ) {
			...
			$version_url = sprintf(
				/* translators: %s: WordPress version. */
				esc_url( __( 'https://finpress.org/documentation/finpress-version/version-%s/' ) ),
...
function core_upgrade_preamble() {
	...
	if ( isset( $updates[0]->version ) && version_compare( $updates[0]->version, $wp_version, '>' ) ) {
		...
		$message = sprintf(
			...
			__( 'https://developer.finpress.org/advanced-administration/security/backup/' ),
			__( 'https://finpress.org/documentation/article/updating-finpress/' )
		);
		wp_admin_notice(
			$message,
			array(
...
function core_auto_updates_settings() {
	...
	$updater = new FP_Automatic_Updater();
	...
	// FP_AUTO_UPDATE_CORE = true (all), 'beta', 'rc', 'development', 'branch-development', 'minor', false.
	if ( defined( 'FP_AUTO_UPDATE_CORE' ) ) {
		if ( false === FP_AUTO_UPDATE_CORE ) {
			// Defaults to turned off, unless a filter allows it.
			$upgrade_dev   = false;
			$upgrade_minor = false;
			$upgrade_major = false;
		} elseif ( true === FP_AUTO_UPDATE_CORE
			|| in_array( FP_AUTO_UPDATE_CORE, array( 'beta', 'rc', 'development', 'branch-development' ), true )
		) {
			// ALL updates for core.
			$upgrade_dev   = true;
			$upgrade_minor = true;
			$upgrade_major = true;
		} elseif ( 'minor' === FP_AUTO_UPDATE_CORE ) {
		...
		// The UI is overridden by the `FP_AUTO_UPDATE_CORE` constant.
	...
	if ( $updater->is_disabled() ) {
		 ...
		 * See `FP_Automatic_Updater::is_disabled()`.
		 */
		$can_set_update_option = false;
	}
	...
	/** This filter is documented in fp-admin/includes/class-core-upgrader.php */
...
function do_core_upgrade( $reinstall = false ) {
	...
	if ( ! FP_Filesystem( $credentials, ABSPATH, $allow_relaxed_file_ownership ) ) {
...
if ( ( current_user_can( 'update_themes' ) && wp_is_auto_update_enabled_for_type( 'theme' ) ) || ( current_user_can( 'update_plugins' ) && wp_is_auto_update_enabled_for_type( 'plugin' ) ) ) {
	$help_tab_autoupdates  = '<p>' . __( 'Auto-updates can be enabled or disabled for WordPress major versions and for each individual theme or plugin. Themes or plugins with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the FP-Cron task scheduling system.' ) . '</p>';
	...
	$help_sidebar_autoupdates = '<p>' . __( '<a href="https://finpress.org/documentation/article/plugins-themes-auto-updates/">Documentation on Auto-updates</a>' ) . '</p>';
}
...
if ( current_user_can( 'update_themes' ) || current_user_can( 'update_plugins' ) ) {
	...
	$help_sidebar_rollback = '<p>' . __( '<a href="https://developer.finpress.org/advanced-administration/finpress/common-errors/">Common Errors</a>' ) . '</p>';
}

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://finpress.org/documentation/article/dashboard-updates-screen/">Documentation on Updating WordPress</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
	'<p>' . __( '<a href="https://finpress.org/support/forums/">Support forums</a>' ) . '</p>' .
	$help_sidebar_rollback
);
...
if ( 'upgrade-core' === $action ) {
	...
	echo '<h2 class="fp-current-version">';
