<?php
/**
 * Server-side file upload handler from fp-plupload or other asynchronous upload methods.
 *
 * @package WordPress
...
require_once ABSPATH . 'fp-admin/admin.php';

header( 'Content-Type: text/plain; charset=' . get_option( 'blog_charset' ) );

if ( isset( $_REQUEST['action'] ) && 'upload-attachment' === $_REQUEST['action'] ) {
	require ABSPATH . 'fp-admin/includes/ajax-actions.php';
...
if ( is_wp_error( $id ) ) {
	...
	echo '<script>_.delay(function() {fp.a11y.speak(' . wp_json_encode( $speak_message, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) . ");}, 1500);jQuery( 'button#{$button_unique_id}' ).on( 'click', function() {jQuery(this).parents('div.media-item').slideUp(200, function(){jQuery(this).remove();fp.a11y.speak( fp.i18n.__( 'Error dismissed.' ) );jQuery( '#plupload-browse-button' ).trigger( 'focus' );})});</script>\n";
	exit;
}
