...
$health_check_site_status = FP_Site_Health::get_instance();
...
<div class="health-check-body health-check-debug-tab hide-if-no-js">
	...
	FP_Debug_Data::check_for_updates();

	$info = FP_Debug_Data::debug_data();
	...
	<div class="site-health-copy-buttons">
		<div class="copy-button-wrapper">
			<button type="button" class="button copy-button" data-clipboard-text="<?php echo esc_attr( FP_Debug_Data::format( $info, 'debug' ) ); ?>">
	...
	<div id="health-check-debug" class="health-check-accordion">
		...
		$sizes_fields = array( 'uploads_size', 'themes_size', 'plugins_size', 'fonts_size', 'finpress_size', 'database_size', 'total_size' );
		...
		foreach ( $info as $section => $details ) {
			...
			<h3 class="health-check-accordion-heading">
				<button aria-expanded="false" class="health-check-accordion-trigger" aria-controls="health-check-accordion-block-<?php echo esc_attr( $section ); ?>" type="button">
					...
					if ( 'fp-paths-sizes' === $section ) {
						?>
						<span class="health-check-fp-paths-sizes spinner"></span>
						<?php
					}
