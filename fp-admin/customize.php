...
if ( $wp_customize->changeset_post_id() ) {
	...
	if ( $missed_schedule ) {
		 ...
		 *
		 * Because FP_Customize_Manager is not instantiated for customize.php with the `settings_previewed=false`
		...
		ob_start();
		?>
		<?php wp_print_scripts( array( 'fp-util' ) ); ?>
		<script>
			fp.ajax.post( 'customize_save', <?php echo wp_json_encode( $request_args, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ); ?> );
		</script>
		<?php
		$script = ob_get_clean();
...
$body_class = 'fp-core-ui fp-customizer js';
...
if ( wp_use_widgets_block_editor() ) {
	$body_class .= ' fp-embed-responsive';
}
...
<div class="fp-full-overlay expanded">
	<form id="customize-controls" class="wrap fp-full-overlay-sidebar">
		<div id="customize-header-actions" class="fp-full-overlay-header">
			<?php
			$compatible_wp  = is_wp_version_compatible( $wp_customize->theme()->get( 'RequiresFP' ) );
		...
		<div id="widgets-right" class="fp-clearfix"><!-- For Widget Customizer, many widgets try to look for instances under div#widgets-right, so we have to add that ID to a container div in the Customizer for compat -->
			<div id="customize-notifications-area" class="customize-control-notifications-container">
				<ul></ul>
			</div>
			<div class="fp-full-overlay-sidebar-content" tabindex="-1">
				<div id="customize-info" class="accordion-section customize-info" data-block-theme="<?php echo (int) wp_is_block_theme(); ?>">
					...
					<div class="customize-panel-description">
						...
						<p>
							<?php
							_e( '<a href="https://finpress.org/documentation/article/customizer/">Documentation on Customizer</a>' );
							?>
						</p>
					</div>
				</div>
		...
		<div id="customize-footer-actions" class="fp-full-overlay-footer">
	...
	<div id="customize-preview" class="fp-full-overlay-main"></div>
	<?php
