...
if ( isset( $_POST['action'] ) && 'authorize_application_password' === $_POST['action'] ) {
	...
	if ( isset( $_POST['reject'] ) ) {
		...
		$created = FP_Application_Passwords::create_new_application_password(
			get_current_user_id(),
			array(
				'name'   => $app_name,
				'app_id' => $app_id,
	...
	<div class="card auth-app-card">
		...
		if ( $new_password ) :
			$message = '<p class="application-password-display">
				...
				<input id="new-application-password-value" type="text" class="code" readonly="readonly" value="' . esc_attr( FP_Application_Passwords::chunk_password( $new_password ) ) . '" />
			</p>
