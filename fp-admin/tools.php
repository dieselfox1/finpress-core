...
if ( isset( $_GET['page'] ) && ! empty( $_POST ) ) {
	...
	if ( 'export_personal_data' === $_GET['page'] ) {
		require_once ABSPATH . 'fp-admin/export-personal-data.php';
		return;
	} elseif ( 'remove_personal_data' === $_GET['page'] ) {
		require_once ABSPATH . 'fp-admin/erase-personal-data.php';
		return;
	}
}

// The privacy policy guide used to be outputted from here. Since FP 5.3 it is in fp-admin/privacy-policy-guide.php.
if ( isset( $_GET['fp-privacy-policy-guide'] ) ) {
	require_once dirname( __DIR__ ) . '/fp-load.php';
	wp_redirect( admin_url( 'options-privacy.php?tab=policyguide' ), 301 );
	exit;
} elseif ( isset( $_GET['page'] ) ) {
	// These were also moved to files in FP 5.3.
	if ( 'export_personal_data' === $_GET['page'] ) {
		require_once dirname( __DIR__ ) . '/fp-load.php';
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://finpress.org/documentation/article/tools-screen/">Documentation on Tools</a>' ) . '</p>' .
