...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://developer.finpress.org/themes/">Documentation on Theme Development</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://finpress.org/documentation/article/appearance-theme-file-editor-screen/">Documentation on Editing Themes</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://developer.finpress.org/advanced-administration/finpress/edit-files/">Documentation on Editing Files</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://developer.finpress.org/themes/basics/template-tags/">Documentation on Template Tags</a>' ) . '</p>' .
...
wp_enqueue_script( 'fp-theme-plugin-editor' );
wp_add_inline_script( 'fp-theme-plugin-editor', sprintf( 'jQuery( function( $ ) { fp.themePluginEditor.init( $( "#template" ), %s ); } )', wp_json_encode( $settings, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) ) );
wp_add_inline_script( 'fp-theme-plugin-editor', 'fp.themePluginEditor.themeOrPlugin = "theme";' );
	...
	<form name="template" id="template" action="theme-editor.php" method="post">
		...
		<?php if ( ! empty( $functions ) ) : ?>
			<div id="documentation" class="hide-if-no-js">
				...
				<input disabled id="docs-lookup" type="button" class="button" value="<?php esc_attr_e( 'Look Up' ); ?>" onclick="if ( '' !== jQuery('#docs-list').val() ) { window.open( 'https://api.finpress.org/core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=<?php echo urlencode( get_user_locale() ); ?>&amp;version=<?php echo urlencode( get_bloginfo( 'version' ) ); ?>&amp;redirect=true'); }" />
			</div>
		<?php endif; ?>
...
if ( ! in_array( 'theme_editor_notice', $dismissed_pointers, true ) ) {
	...
	$excluded_referer_basenames = array( 'theme-editor.php', 'fp-login.php' );
					...
					<p>
						...
						if ( ! $theme->parent() ) {
							...
							printf(
								...
								esc_url( __( 'https://developer.finpress.org/themes/advanced-topics/child-themes/' ) )
							);
							echo '</p>';
						}
						?>
