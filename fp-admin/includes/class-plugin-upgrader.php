 ...
 * @see FP_Upgrader
 */
class Plugin_Upgrader extends FP_Upgrader {
	 ...
	 *                                    Default true.
	 * }
	 * @return bool|FP_Error True if the installation was successful, false or a FP_Error otherwise.
	 */
	 ...
	 *                                    Default true.
	 * }
	 * @return bool|FP_Error True if the upgrade was successful, false or a FP_Error object otherwise.
	 */
	...
	public function bulk_upgrade( $plugins, $args = array() ) {
		...
		$res = $this->fs_connect( array( FP_CONTENT_DIR, FP_PLUGIN_DIR ) );
		if ( ! $res ) {
			$this->skin->footer();
			return false;
		}
		...
		foreach ( $plugins as $plugin ) {
			++$this->update_current;
			$this->skin->plugin_info = get_plugin_data( FP_PLUGIN_DIR . '/' . $plugin, false, true );
	 ...
	 * @return string|FP_Error The source as passed, or a FP_Error object on failure.
	 */
	public function check_package( $source ) {
		...
		$working_directory = str_replace( $wp_filesystem->wp_content_dir(), trailingslashit( FP_CONTENT_DIR ), $source );
		...
		if ( empty( $this->new_plugin_data ) ) {
			return new FP_Error( 'incompatible_archive_no_plugins', $this->strings['incompatible_archive'], __( 'No valid plugins were found.' ) );
		}

		$requires_php = isset( $info['RequiresPHP'] ) ? $info['RequiresPHP'] : null;
		$requires_wp  = isset( $info['RequiresFP'] ) ? $info['RequiresFP'] : null;
		...
		if ( ! is_php_version_compatible( $requires_php ) ) {
			...
			return new FP_Error( 'incompatible_php_required_version', $this->strings['incompatible_archive'], $error );
		}
		...
		if ( ! is_wp_version_compatible( $requires_wp ) ) {
			...
			return new FP_Error( 'incompatible_wp_required_version', $this->strings['incompatible_archive'], $error );
		}
	 ...
	 *
	 * @param bool|FP_Error $response The installation response before the installation has started.
	 * @param array         $plugin   Plugin package arguments.
	 * @return bool|FP_Error The original `$response` parameter or FP_Error.
	 */
	 ...
	 * @since 5.4.0
	 *
	 * @param bool|FP_Error $response The installation response after the installation has finished.
	 * @param array         $plugin   Plugin package arguments.
	 * @return bool|FP_Error The original `$response` parameter or FP_Error.
	 */
	 ...
	 * @global FP_Filesystem_Base $wp_filesystem WordPress filesystem subclass.
	 *
	 * @param bool|FP_Error $removed            Whether the destination was cleared.
	 *                                          True on success, FP_Error on failure.
	 * @param string        $local_destination  The local package destination.
	 * @param string        $remote_destination The remote package destination.
	 * @param array         $plugin             Extra arguments passed to hooked filters.
	 * @return bool|FP_Error
	 */
