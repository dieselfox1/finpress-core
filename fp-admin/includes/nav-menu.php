...
require_once ABSPATH . 'fp-admin/includes/class-walker-nav-menu-edit.php';

/** Walker_Nav_Menu_Checklist class */
require_once ABSPATH . 'fp-admin/includes/class-walker-nav-menu-checklist.php';
...
function _wp_ajax_menu_quick_search( $request = array() ) {
	...
	if ( 'get-post-item' === $type ) {
		...
		if ( 'posttype' === $matches[1] && get_post_type_object( $matches[2] ) ) {
			...
			$search_results_query = new FP_Query( $args );
...
function wp_nav_menu_post_type_meta_boxes() {
	...
	foreach ( $post_types as $post_type ) {
		 ...
		 * @since 3.0.0
		 *
		 * @param FP_Post_Type|false $post_type The current object to add a menu items
		 *                                      meta box for.
		 */
...
function wp_nav_menu_item_link_meta_box() {
	...
	<div class="customlinkdiv" id="customlinkdiv">
		...
		<p id="menu-item-url-wrap" class="fp-clearfix">
		...
		<p id="menu-item-name-wrap" class="fp-clearfix">
		...
		<p class="button-controls fp-clearfix">
 ...
 *     @type FP_Post_Type $args     Extra meta box arguments (the post type object for this meta box).
 * }
 */
function wp_nav_menu_item_post_type_meta_box( $data_object, $box ) {
	...
	if ( 'page' === $post_type_name ) {
		...
		// Add suppression array to arguments for FP_Query.
	...
	<div id="<?php echo esc_attr( "posttype-{$post_type_name}" ); ?>" class="posttypediv">
		...
		>
			...
			>
				 ...
				 *
				 * @param FP_Post[] $most_recent An array of post objects being listed.
				 * @param array     $args        An array of `FP_Query` arguments for the meta box.
				 * @param array     $box         Arguments passed to `wp_nav_menu_item_post_type_meta_box()`.
				 * @param array     $recent_args An array of `FP_Query` arguments for 'Most Recent' tab.
				 */
		...
		>
			...
			<ul id="<?php echo esc_attr( "{$post_type_name}-search-checklist" ); ?>"
				data-fp-lists="<?php echo esc_attr( "list:{$post_type_name}" ); ?>"
				class="categorychecklist form-no-clear"
			>
		...
		>
			...
			>
				 ...
				 * @see FP_Query::query()
				 *
				 * @param object[]     $posts     The posts for the current post type. Mostly `FP_Post` objects, but
				 *                                can also contain "fake" post objects to represent other menu items.
				 * @param array        $args      An array of `FP_Query` arguments.
				 * @param FP_Post_Type $post_type The current post type object for this menu item meta box.
				 */
				$posts = apply_filters(
		...
		<p class="button-controls fp-clearfix" data-items-type="<?php echo esc_attr( "posttype-{$post_type_name}" ); ?>">
	...
	<div id="<?php echo esc_attr( "taxonomy-{$taxonomy_name}" ); ?>" class="taxonomydiv">
		...
		>
			...
			<ul id="<?php echo esc_attr( "{$taxonomy_name}checklist" ); ?>"
				data-fp-lists="<?php echo esc_attr( "list:{$taxonomy_name}" ); ?>"
				class="categorychecklist form-no-clear"
			>
				<?php
				$args['walker'] = $walker;
		...
		<p class="button-controls fp-clearfix" data-items-type="<?php echo esc_attr( "taxonomy-{$taxonomy_name}" ); ?>">
			<span class="list-controls hide-if-no-js">
				<input type="checkbox"<?php wp_nav_menu_disabled_check( $nav_menu_selected_id ); ?>
					id="<?php echo esc_attr( $tab_name ); ?>" class="select-all"
				/>
 ...
 * @return string|FP_Error|null The menu formatted to edit or error object on failure.
...
function wp_nav_menu_update_menu_items( $nav_menu_selected_id, $nav_menu_selected_title ) {
	...
	/** This action is documented in fp-includes/nav-menu.php */
