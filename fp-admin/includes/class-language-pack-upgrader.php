 ...
 * @see FP_Upgrader
 */
class Language_Pack_Upgrader extends FP_Upgrader {
	 ...
	 * @since 3.7.0
	 *
	 * @param false|FP_Upgrader $upgrader Optional. FP_Upgrader instance or false. If `$upgrader` is
	...
	public static function async_upgrade( $upgrader = false ) {
		 ...
		 */
		$check_vcs = new FP_Automatic_Updater();
		if ( $check_vcs->is_vcs_checkout( FP_CONTENT_DIR ) ) {
			return;
		}
	 ...
	 * @return array|bool|FP_Error The result of the upgrade, or a FP_Error object instead.
	 */
	 ...
	 *                                    Default true.
	 * }
	 * @return array|bool|FP_Error Will return an array of results, or true if there are no updates,
	 *                             false or FP_Error for initial errors.
	 */
	public function bulk_upgrade( $language_updates = array(), $args = array() ) {
		...
		// Remove any existing upgrade filters from the plugin/theme upgraders #FP29425 & #FP29230.
		...
		$res = $this->fs_connect( array( FP_CONTENT_DIR, FP_LANG_DIR ) );
		if ( ! $res ) {
			$this->skin->footer();
			return false;
		}
		...
		/*
		 * The filesystem's mkdir() is not recursive. Make sure FP_LANG_DIR exists,
		 * as we then may need to create a /plugins or /themes directory inside of it.
		 */
		$remote_destination = $wp_filesystem->find_folder( FP_LANG_DIR );
		if ( ! $wp_filesystem->exists( $remote_destination ) ) {
			if ( ! $wp_filesystem->mkdir( $remote_destination, FS_CHMOD_DIR ) ) {
				return new FP_Error( 'mkdir_failed_lang_dir', $this->strings['mkdir_failed'], $remote_destination );
			}
		}
	 ...
	 * @global FP_Filesystem_Base $wp_filesystem WordPress filesystem subclass.
	 *
	 * @param string|FP_Error $source        The path to the downloaded package source.
	 ...
	 * @return bool|FP_Error True upon success, FP_Error on failure.
	 */
	public function clear_destination( $remote_destination ) {
		global $wp_filesystem;

		$language_update    = $this->skin->language_update;
		$language_directory = FP_LANG_DIR . '/'; // Local path for use with glob().
