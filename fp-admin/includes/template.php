...
require_once ABSPATH . 'fp-admin/includes/class-walker-category-checklist.php';

/** FP_Internal_Pointers class */
require_once ABSPATH . 'fp-admin/includes/class-fp-internal-pointers.php';
	...
	foreach ( (array) $terms as $term ) {
		...
		<li id="<?php echo $id; ?>" class="popular-category">
			<label class="selectit">
				...
				<?php
				/** This filter is documented in fp-includes/category-template.php */
...
<div class="hidden" id="inline_' . $post->ID . '">
	 ...
	 * @since 4.9.8
	 *
	 * @param FP_Post      $post             The current post object.
	 * @param FP_Post_Type $post_type_object The current post's post type object.
	 */
 ...
 * @since 2.7.0
 *
 * @global FP_List_Table $wp_list_table
 *
 ...
 * @param string $mode      Optional. If set to 'single', will use FP_Post_Comments_List_Table,
 *                          otherwise FP_Comments_List_Table. Default 'single'.
...
function wp_comment_reply( $position = 1, $checkbox = false, $mode = 'single', $table_row = true ) {
	...
	if ( ! $wp_list_table ) {
		if ( 'single' === $mode ) {
			$wp_list_table = _get_list_table( 'FP_Post_Comments_List_Table' );
		} else {
			$wp_list_table = _get_list_table( 'FP_Comments_List_Table' );
		}
	}
...
<table id="list-table" style="display: none;">
	...
	<tbody id="the-list" data-fp-lists="list:meta">
	<tr><td></td></tr>
	</tbody>
...
function _list_meta_row( $entry, &$count ) {
	...
	$r .= "\n\t\t<div class='submit'>";
	$r .= get_submit_button( __( 'Delete' ), 'deletemeta small', "deletemeta[{$entry['meta_id']}]", false, array( 'data-fp-lists' => "delete:the-list:meta-{$entry['meta_id']}::_ajax_nonce=$delete_nonce" ) );
	$r .= "\n\t\t";
	$r .= get_submit_button( __( 'Update' ), 'updatemeta small', "meta-{$entry['meta_id']}-submit", false, array( 'data-fp-lists' => "add:the-list:meta-{$entry['meta_id']}::_ajax_nonce-add-meta=$update_nonce" ) );
	$r .= '</div>';
 ...
 *
 * @param FP_Post $post Optional. The post being edited.
 */
...
<div class="submit add-custom-field">
	...
	submit_button(
		...
		array(
			'id'            => 'newmeta-submit',
			'data-fp-lists' => 'add:the-list:newmeta',
		)
	);
	?>
</div>
...
function wp_import_upload_form( $action ) {
	...
	if ( ! empty( $upload_dir['error'] ) ) :
		...
		?>
<form enctype="multipart/form-data" id="import-upload-form" method="post" class="fp-upload-form" action="<?php echo esc_url( wp_nonce_url( $action, 'import-upload' ) ); ?>">
<p>
		<?php
		printf(
 ...
 * @param string|array|FP_Screen $screen        Optional. The screen or screens on which to show the box
 *                                              (such as a post type, 'link', or 'comment'). Accepts a single
 *                                              screen ID, FP_Screen object, or array of screen IDs. Default
...
function do_block_editor_incompatible_meta_box( $data_object, $box ) {
	...
	if ( empty( $plugins['classic-editor/classic-editor.php'] ) ) {
		if ( current_user_can( 'install_plugins' ) ) {
			$install_url = wp_nonce_url(
				self_admin_url( 'plugin-install.php?tab=favorites&user=finpressdotorg&save=0' ),
	...
	} elseif ( $data_object instanceof FP_Post ) {
...
function _get_plugin_from_callback( $callback ) {
	...
	if ( ! $reflection->isInternal() ) {
		...
		$plugin_dir = wp_normalize_path( FP_PLUGIN_DIR );
 ...
 * @global array $wp_meta_boxes Global meta box state.
 *
 * @param string|FP_Screen $screen      The screen identifier. If you have used add_menu_page() or
 *                                      add_submenu_page() to create a new screen (and hence screen_id)
 *                                      make sure your menu slug conforms to the limits of sanitize_key()
 *                                      otherwise the 'screen' menu may not correctly render on your page.
 ...
 *                                      for example a `FP_Post` or `FP_Comment` object.
		...
		foreach ( array( 'high', 'sorted', 'core', 'default', 'low' ) as $priority ) {
			if ( isset( $wp_meta_boxes[ $page ][ $context ][ $priority ] ) ) {
				foreach ( (array) $wp_meta_boxes[ $page ][ $context ][ $priority ] as $box ) {
					...
					if ( FP_DEBUG && ! $block_compatible && 'edit' === $screen->parent_base && ! $screen->is_block_editor() && ! isset( $_GET['meta-box-loader'] ) ) {
 ...
 *
 * @param string                 $id      Meta box ID (used in the 'id' attribute for the meta box).
 * @param string|array|FP_Screen $screen  The screen or screens on which the meta box is shown (such as a
 *                                        post type, 'link', or 'comment'). Accepts a single screen ID,
 *                                        FP_Screen object, or array of screen IDs.
...
var ajaxurl = '<?php echo esc_js( admin_url( 'admin-ajax.php', 'relative' ) ); ?>',
	...
	<?php
	/** This action is documented in fp-admin/admin-header.php */
	...
	/** This filter is documented in fp-admin/admin-header.php */
	$admin_body_classes = apply_filters( 'admin_body_class', '' );
	$admin_body_classes = ltrim( $admin_body_classes . ' ' . $admin_body_class );
	?>
<body <?php echo $admin_body_id; ?>class="fp-admin fp-core-ui no-js iframe <?php echo esc_attr( $admin_body_classes ); ?>">
<script type="text/javascript">
(function(){
...
function iframe_footer() {
	...
	<div class="hidden">
	<?php
	/** This action is documented in fp-admin/admin-footer.php */
 ...
 * @see get_post_states()
 *
 * @param FP_Post $post    The post to retrieve states for.
 ...
 *
 * @param FP_Post $post    The attachment post to retrieve states for.
 * @param bool    $display Optional. Whether to display the post states as an HTML string.
 *                         Default true.
 ...
 * @since 5.6.0
 *
 * @param FP_Post $post The attachment to retrieve states for.
...
function get_media_states( $post ) {
	 ...
	 * @param FP_Post  $post         The current attachment object.
	 */
	...
	var testCompression = {
		get : function(test) {
			...
			if (x) {
				...
				x.open('GET', ajaxurl + '?action=fp-compression-test&test='+test+'&_ajax_nonce='+compressionNonce+'&'+(new Date()).getTime(), true);
				x.send('');
			}
		},
...
function _wp_admin_html_begin() {
	...
	$admin_html_class = ( is_admin_bar_showing() ) ? 'fp-toolbar' : '';
 ...
 * @return FP_Screen Screen object.
 */
function convert_to_screen( $hook_name ) {
	if ( ! class_exists( 'FP_Screen' ) ) {
		_doing_it_wrong(
			'convert_to_screen(), add_meta_box()',
			sprintf(
				/* translators: 1: fp-admin/includes/template.php, 2: add_meta_box(), 3: add_meta_boxes */
				__( 'Likely direct inclusion of %1$s in order to use %2$s. This is very wrong. Hook the %2$s call into the %3$s action instead.' ),
				'<code>fp-admin/includes/template.php</code>',
	...
	return FP_Screen::get( $hook_name );
}
...
function _wp_block_editor_posts_page_notice() {
	wp_add_inline_script(
		'fp-notices',
		sprintf(
			'fp.data.dispatch( "core/notices" ).createWarningNotice( "%s", { isDismissible: false } )',
			__( 'You are currently editing the page that shows your latest posts.' )
		),
		'after'
	);
