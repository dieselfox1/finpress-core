<?php
/**
 * List Table API: FP_Plugins_List_Table class
 *
 * @package WordPress
 ...
 * @see FP_List_Table
 */
class FP_Plugins_List_Table extends FP_List_Table {
	/**
	 ...
	 * @since 3.1.0
	 *
	 * @see FP_List_Table::__construct() for more information on default arguments.
	 *
	...
	public function prepare_items() {
		...
		if ( ! is_multisite() || ( $screen->in_admin( 'network' ) && current_user_can( 'manage_network_plugins' ) ) ) {
			...
			/** This action is documented in fp-admin/includes/class-fp-plugins-list-table.php */
	...
	public function search_box( $text, $input_id ) {
		...
		<p class="search-box">
			...
			<input type="search" id="<?php echo esc_attr( $input_id ); ?>" class="fp-filter-search" name="s" value="<?php _admin_search_query(); ?>" />
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'recently_activated' === $status ) {
			...
			echo '<p>' . sprintf(
				...
				'<code>' . str_replace( ABSPATH, '/', FPMU_PLUGIN_DIR ) . '</code>'
			) . '</p>';
	...
	public function single_row( $item ) {
		...
		$requires_wp  = isset( $plugin_data['RequiresFP'] ) ? $plugin_data['RequiresFP'] : null;

		$compatible_php = is_php_version_compatible( $requires_php );
		$compatible_wp  = is_wp_version_compatible( $requires_wp );

		$has_dependents          = FP_Plugin_Dependencies::has_dependents( $plugin_file );
		$has_active_dependents   = FP_Plugin_Dependencies::has_active_dependents( $plugin_file );
		$has_unmet_dependencies  = FP_Plugin_Dependencies::has_unmet_dependencies( $plugin_file );
		$has_circular_dependency = FP_Plugin_Dependencies::has_circular_dependency( $plugin_file );
		...
		if ( 'mustuse' === $context ) {
			...
			if ( true === ( $dropins[ $plugin_file ][1] ) ) { // Doesn't require a constant.
				...
				$description = '<p><strong>' . $dropins[ $plugin_file ][0] . ' <span class="error-message">' . __( 'Inactive:' ) . '</span></strong> ' .
					sprintf(
						/* translators: 1: Drop-in constant name, 2: fp-config.php */
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			switch ( $column_name ) {
				...
				case 'description':
					 ...
					 *     @type string   $RequiresFP       The version of WordPress which the plugin requires.
					...
					if ( FP_Plugin_Dependencies::has_dependencies( $plugin_file ) ) {
				...
				case 'auto-updates':
					...
					if ( 'unavailable' === $action ) {
						...
						$html[] = sprintf(
							'<a href="%s" class="toggle-auto-update aria-button-if-js" data-fp-action="%s">',
	...
	protected function add_dependents_to_dependency_plugin_row( $dependency ) {
		$dependent_names = FP_Plugin_Dependencies::get_dependent_names( $dependency );
	...
	protected function add_dependencies_to_dependent_plugin_row( $dependent ) {
		$dependency_names = FP_Plugin_Dependencies::get_dependency_names( $dependent );
		...
		$notice        = '';
		$error_message = '';
		if ( FP_Plugin_Dependencies::has_unmet_dependencies( $dependent ) ) {
			if ( $is_active ) {
	...
	protected function get_dependency_view_details_link( $name, $slug ) {
		$dependency_data = FP_Plugin_Dependencies::get_dependency_data( $slug );
