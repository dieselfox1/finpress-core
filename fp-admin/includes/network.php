 ...
 *
 * @param false|FP_Error $errors Optional. Error object. Default false.
 */
function network_step1( $errors = false ) {
	...
	if ( FP_CONTENT_DIR !== ABSPATH . 'fp-content' && ( allow_subdirectory_install() || ! allow_subdomain_install() ) ) {
		$subdirectory_warning_message  = '<strong>' . __( 'Warning:' ) . '</strong> ';
		$subdirectory_warning_message .= __( 'Subdirectory networks may not be fully compatible with custom fp-content directories.' );
		wp_admin_notice(
...
function network_step2( $errors = false ) {
	...
	if ( ! file_exists( ABSPATH . 'fp-config.php' ) && file_exists( dirname( ABSPATH ) . '/fp-config.php' ) ) {
	...
	if ( $_POST || ! is_multisite() ) {
		...
		if ( file_exists( $home_path . '.htaccess' ) ) {
			$notice_message .= sprintf(
				/* translators: 1: fp-config.php, 2: .htaccess */
			...
			$notice_message .= sprintf(
				/* translators: 1: fp-config.php, 2: web.config */
				__( 'You should back up your existing %1$s and %2$s files.' ),
				'<code>fp-config.php</code>',
	...
	<ol>
		...
		<?php
		printf(
			/* translators: 1: fp-config.php, 2: Location of fp-config file, 3: Translated version of "That's all, stop editing! Happy publishing." */
			__( 'Add the following to your %1$s file in %2$s <strong>above</strong> the line reading %3$s:' ),
			'<code>fp-config.php</code>',
			'<code>' . $location_of_wp_config . '</code>',
			/*
			 * translators: This string should only be translated if fp-config-sample.php is localized.
			 * You can check the localized release package or
			 * https://i18n.svn.finpress.org/<locale code>/branches/<fp version>/dist/fp-config-sample.php
			 */
		...
		<p class="configuration-rules-label"><label for="network-wpconfig-rules">
			<?php
			printf(
				/* translators: %s: File name (fp-config.php, .htaccess or web.config). */
...
define( 'BLOG_ID_CURRENT_SITE', 1 );
		...
		if ( ! empty( $keys_salts ) ) {
			$keys_salts_str = '';
			$from_api       = wp_remote_get( 'https://api.finpress.org/secret-key/1.1/salt/' );
		...
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$web_config_file .= '
                <rule name="WordPress Rule for Files" stopProcessing="true">
                    ...
                    <action type="Rewrite" url="' . $iis_rewrite_base . FPINC . '/ms-files.php?file={R:1}" appendQueryString="false" />
                </rule>';
		}
			$web_config_file .= '
                <rule name="WordPress Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'fp-admin$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'fp-admin/" redirectType="Permanent" />
                </rule>
                ...
                <rule name="WordPress Rule 4" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '(fp-(content|admin|includes).*)" ignoreCase="false" />
...
';
		...
		echo '</p>';
		if ( ! $subdomain_install && FP_CONTENT_DIR !== ABSPATH . 'fp-content' ) {
			echo '<p><strong>' . __( 'Warning:' ) . ' ' . __( 'Subdirectory networks may not be fully compatible with custom fp-content directories.' ) . '</strong></p>';
		}
		?>
	...
	elseif ( $is_nginx ) : // End iis7_supports_permalinks(). Link to Nginx documentation instead:
		...
		printf(
			...
			__( 'https://developer.finpress.org/advanced-administration/server/web-server/nginx/' )
		);
		echo '</p></li>';
		...
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$ms_files_rewriting  = "\n# uploaded files\nRewriteRule ^";
			$ms_files_rewriting .= $subdir_match . "files/(.+) {$rewrite_base}" . FPINC . "/ms-files.php?file={$subdir_replacement_12} [L]" . "\n";
		}
...
{$ms_files_rewriting}
# add a trailing slash to /fp-admin
RewriteRule ^{$subdir_match}fp-admin$ {$subdir_replacement_01}fp-admin/ [R=301,L]

RewriteCond %{REQUEST_FILENAME} -f [OR]
RewriteCond %{REQUEST_FILENAME} -d
RewriteRule ^ - [L]
RewriteRule ^{$subdir_match}(fp-(content|admin|includes).*) {$rewrite_base}{$subdir_replacement_12} [L]
