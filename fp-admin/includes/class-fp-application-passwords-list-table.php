<?php
/**
 * List Table API: FP_Application_Passwords_List_Table class
 *
 * @package WordPress
 ...
 * @see FP_List_Table
 */
class FP_Application_Passwords_List_Table extends FP_List_Table {
	...
	public function prepare_items() {
		global $user_id;
		$this->items = array_reverse( FP_Application_Passwords::get_user_application_passwords( $user_id ) );
	}
		...
		foreach ( $columns as $column_name => $display_name ) {
			...
			switch ( $column_name ) {
				...
				case 'created':
					...
					echo '<# print( fp.date.dateI18n( ' . wp_json_encode( __( 'F j, Y' ) ) . ', data.created ) ) #>';
					break;
				case 'last_used':
					echo '<# print( data.last_used !== null ? fp.date.dateI18n( ' . wp_json_encode( __( 'F j, Y' ) ) . ", data.last_used ) : '—' ) #>";
					break;
				case 'last_ip':
					echo "{{ data.last_ip || '—' }}";
					break;
