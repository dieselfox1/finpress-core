 ...
 * @return true|FP_Error Returns true if sending the email was successful, or a FP_Error object.
 */
function _wp_privacy_resend_request( $request_id ) {
	...
	if ( ! $request || 'user_request' !== $request->post_type ) {
		return new FP_Error( 'privacy_request_error', __( 'Invalid personal data request.' ) );
	}
	...
	if ( is_wp_error( $result ) ) {
		...
		return new FP_Error( 'privacy_request_error', __( 'Unable to initiate confirmation for personal data request.' ) );
	}
 ...
 *
 * @param int $request_id Request ID.
 * @return int|FP_Error Request ID on success, or a FP_Error on failure.
 */
...
function wp_privacy_generate_personal_data_export_file( $request_id ) {
	...
	$file_basename        = 'fp-personal-data-file-' . $obscura;
...
function wp_privacy_send_personal_data_export_email( $request_id ) {
	...
	if ( ! $request || 'export_personal_data' !== $request->action_name ) {
		return new FP_Error( 'invalid_request', __( 'Invalid request ID when sending personal data export email.' ) );
	}
	...
	/** This filter is documented in fp-includes/functions.php */
...
###SITEURL###'
	...
	if ( ! $mail_success ) {
		return new FP_Error( 'privacy_email_error', __( 'Unable to send personal data export email.' ) );
	}
