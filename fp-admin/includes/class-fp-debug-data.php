...
class FP_Debug_Data {
	...
	public static function debug_data() {
		 ...
		 */
		$info = array(
			'fp-core'             => self::get_wp_core(),
			'fp-paths-sizes'      => self::get_wp_paths_sizes(),
			'fp-dropins'          => self::get_wp_dropins(),
			'fp-active-theme'     => self::get_wp_active_theme(),
			'fp-parent-theme'     => self::get_wp_parent_theme(),
			'fp-themes-inactive'  => self::get_wp_themes_inactive(),
			'fp-mu-plugins'       => self::get_wp_mu_plugins(),
			'fp-plugins-active'   => self::get_wp_plugins_active(),
			'fp-plugins-inactive' => self::get_wp_plugins_inactive(),
			'fp-media'            => self::get_wp_media(),
			'fp-server'           => self::get_wp_server(),
			'fp-database'         => self::get_wp_database(),
			'fp-constants'        => self::get_wp_constants(),
			'fp-filesystem'       => self::get_wp_filesystem(),
		);
		 ...
		 *
		 * Array keys for sections added by core are all prefixed with `fp-`. Plugins and themes
	...
	private static function get_wp_core(): array {
		...
		if ( is_multisite() ) {
			...
			$network_query = new FP_Network_Query();
		...
		$wp_dotorg = wp_remote_get( 'https://finpress.org', array( 'timeout' => 10 ) );
	...
	private static function get_wp_dropins(): array {
		...
		return array(
			...
			'show_count'  => true,
			'description' => sprintf(
				/* translators: %s: fp-content directory name. */
				__( 'Drop-ins are single files, found in the %s directory, that replace or enhance WordPress features in ways that are not possible for traditional plugins.' ),
				'<code>' . str_replace( ABSPATH, '', FP_CONTENT_DIR ) . '</code>'
			),
			'fields'      => $fields,
		);
	}
	...
	private static function get_wp_server(): array {
		...
		if ( ! function_exists( 'ini_get' ) ) {
			...
			if ( FP_Site_Health::get_instance()->php_memory_limit !== ini_get( 'memory_limit' ) ) {
				$fields['memory_limit']       = array(
					'label' => __( 'PHP memory limit' ),
					'value' => FP_Site_Health::get_instance()->php_memory_limit,
				);
		...
		if ( is_file( ABSPATH . 'robots.txt' ) ) {
			...
			// No robots.txt file, but without rewrite rules FP can't serve one.
			$robotstxt_debug = true;
	...
	private static function get_wp_media(): array {
		...
		if ( 'FP_Image_Editor_Imagick' === _wp_image_editor_choose() && isset( $imagick ) && $imagick instanceof Imagick ) {
			$limits = array(
	...
	private static function get_wp_plugins_active(): array {
		return array(
			...
			'show_count' => true,
			'fields'     => self::get_wp_plugins_raw_data()['fp-plugins-active'],
		);
	}
	...
	private static function get_wp_plugins_inactive(): array {
		return array(
			...
			'show_count' => true,
			'fields'     => self::get_wp_plugins_raw_data()['fp-plugins-inactive'],
		);
	}
	...
	private static function get_wp_plugins_raw_data(): array {
		...
		$auto_updates = array();
		$fields       = array(
			'fp-plugins-active'   => array(),
			'fp-plugins-inactive' => array(),
		);
		...
		foreach ( $plugins as $plugin_path => $plugin ) {
			$plugin_part = ( is_plugin_active( $plugin_path ) ) ? 'fp-plugins-active' : 'fp-plugins-inactive';
	...
	private static function get_wp_active_theme(): array {
		...
		if ( $auto_updates_enabled ) {
			...
			/** This filter is documented in fp-admin/includes/class-fp-debug-data.php */
	...
	private static function get_wp_themes_inactive(): array {
		...
		foreach ( $all_themes as $theme_slug => $theme ) {
			...
			if ( $auto_updates_enabled ) {
				 ...
				 *
				 * @param string   $auto_updates_string The string output for the auto-updates column.
				 * @param FP_Theme $theme               An object of theme data.
				 * @param bool     $enabled             Whether auto-updates are enabled for this item.
				 */
	...
	private static function get_wp_constants(): array {
		// Check if FP_DEBUG_LOG is set.
		$wp_debug_log_value = __( 'Disabled' );
		if ( is_string( FP_DEBUG_LOG ) ) {
			$wp_debug_log_value = FP_DEBUG_LOG;
		} elseif ( FP_DEBUG_LOG ) {
		...
		// Check FP_ENVIRONMENT_TYPE.
		if ( defined( 'FP_ENVIRONMENT_TYPE' ) ) {
			$wp_environment_type       = FP_ENVIRONMENT_TYPE ? FP_ENVIRONMENT_TYPE : __( 'Empty value' );
			$wp_environment_type_debug = FP_ENVIRONMENT_TYPE;
		} else {
		...
		$fields = array(
			...
			'FP_HOME'             => array(
				'label' => 'FP_HOME',
				'value' => ( defined( 'FP_HOME' ) ? FP_HOME : __( 'Undefined' ) ),
				'debug' => ( defined( 'FP_HOME' ) ? FP_HOME : 'undefined' ),
			),
			'FP_SITEURL'          => array(
				'label' => 'FP_SITEURL',
				'value' => ( defined( 'FP_SITEURL' ) ? FP_SITEURL : __( 'Undefined' ) ),
				'debug' => ( defined( 'FP_SITEURL' ) ? FP_SITEURL : 'undefined' ),
			),
			'FP_CONTENT_DIR'      => array(
				'label' => 'FP_CONTENT_DIR',
				'value' => FP_CONTENT_DIR,
			),
			'FP_PLUGIN_DIR'       => array(
				'label' => 'FP_PLUGIN_DIR',
				'value' => FP_PLUGIN_DIR,
			),
			'FP_MEMORY_LIMIT'     => array(
				'label' => 'FP_MEMORY_LIMIT',
				'value' => FP_MEMORY_LIMIT,
			),
			'FP_MAX_MEMORY_LIMIT' => array(
				'label' => 'FP_MAX_MEMORY_LIMIT',
				'value' => FP_MAX_MEMORY_LIMIT,
			),
			'FP_DEBUG'            => array(
				'label' => 'FP_DEBUG',
				'value' => FP_DEBUG ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => FP_DEBUG,
			),
			'FP_DEBUG_DISPLAY'    => array(
				'label' => 'FP_DEBUG_DISPLAY',
				'value' => FP_DEBUG_DISPLAY ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => FP_DEBUG_DISPLAY,
			),
			'FP_DEBUG_LOG'        => array(
				'label' => 'FP_DEBUG_LOG',
			...
			'FP_CACHE'            => array(
				'label' => 'FP_CACHE',
				'value' => FP_CACHE ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => FP_CACHE,
			),
			...
			'FP_ENVIRONMENT_TYPE' => array(
				'label' => 'FP_ENVIRONMENT_TYPE',
				'value' => $wp_environment_type,
				'debug' => $wp_environment_type_debug,
			),
			'FP_DEVELOPMENT_MODE' => array(
				'label' => 'FP_DEVELOPMENT_MODE',
				'value' => FP_DEVELOPMENT_MODE ? FP_DEVELOPMENT_MODE : __( 'Disabled' ),
				'debug' => FP_DEVELOPMENT_MODE,
			),
			'DB_CHARSET'          => array(
				'label' => 'DB_CHARSET',
	...
	private static function get_wp_filesystem(): array {
		...
		$is_writable_wp_content_dir     = wp_is_writable( FP_CONTENT_DIR );
		$is_writable_upload_dir         = wp_is_writable( $upload_dir['basedir'] );
		$is_writable_wp_plugin_dir      = wp_is_writable( FP_PLUGIN_DIR );
		...
		$fields = array(
			...
			'fp-content' => array(
				'label' => __( 'The fp-content directory' ),
		...
		if ( defined( 'FPMU_PLUGIN_DIR' ) && is_dir( FPMU_PLUGIN_DIR ) ) {
			$is_writable_wpmu_plugin_dir = wp_is_writable( FPMU_PLUGIN_DIR );
	 ...
	 * @since 5.2.0
	 *
	 * @param array  $info_array Information gathered from the `FP_Debug_Data::debug_data()` function.
	...
	/**
	 * Fetches the sizes of the WordPress directories: `finpress` (ABSPATH), `plugins`, `themes`, and `uploads`.
	 * Intended to supplement the array returned by `FP_Debug_Data::debug_data()`.
	 *
	 * @since 5.2.0
	...
	public static function get_sizes() {
		_deprecated_function( __METHOD__, '5.6.0', 'FP_REST_Site_Health_Controller::get_directory_sizes()' );
		 ...
		 */
		$paths = array(
			'finpress_size' => untrailingslashit( ABSPATH ),
			'themes_size'    => get_theme_root(),
			'plugins_size'   => FP_PLUGIN_DIR,
			'uploads_size'   => $upload_dir['basedir'],
			'fonts_size'     => wp_get_font_dir()['basedir'],
		);

		$exclude = $paths;
		unset( $exclude['finpress_size'] );
		...
		foreach ( $paths as $name => $path ) {
			...
			if ( microtime( true ) - FP_START_TIMESTAMP < $max_execution_time ) {
				if ( 'finpress_size' === $name ) {
