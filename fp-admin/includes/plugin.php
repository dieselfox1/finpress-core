 ...
 *     @type string $RequiresFP      Minimum required version of WordPress.
...
function _get_plugin_data_markup_translate( $plugin_file, $plugin_data, $markup = true, $translate = true ) {
	...
	// Sanitize the plugin filename to a FP_PLUGIN_DIR relative path.
		...
		if ( $textdomain ) {
			foreach ( array( 'Name', 'PluginURI', 'Description', 'Author', 'AuthorURI', 'Version' ) as $field ) {
				if ( ! empty( $plugin_data[ $field ] ) ) {
					// phpcs:ignore WordPress.FP.I18n.LowLevelTranslationFunction,WordPress.FP.I18n.NonSingularStringLiteralText,WordPress.FP.I18n.NonSingularStringLiteralDomain
...
function get_plugin_files( $plugin ) {
	$plugin_file = FP_PLUGIN_DIR . '/' . $plugin;
	$dir         = dirname( $plugin_file );

	$plugin_files = array( plugin_basename( $plugin_file ) );

	if ( is_dir( $dir ) && FP_PLUGIN_DIR !== $dir ) {
 ...
 * (fp-content/plugins) and in one directory above the plugins directory
 * (fp-content/plugins/my-plugin). The file it looks for has the plugin data
...
function get_plugins( $plugin_folder = '' ) {
	...
	$wp_plugins  = array();
	$plugin_root = FP_PLUGIN_DIR;
	if ( ! empty( $plugin_folder ) ) {
		$plugin_root .= $plugin_folder;
	}

	// Files in fp-content/plugins directory.
 ...
 *
 * WordPress only includes mu-plugin files in the base mu-plugins directory (fp-content/mu-plugins).
 *
 * @since 3.0.0
...
function get_mu_plugins() {
	...
	// Files in fp-content/mu-plugins directory.
	$plugins_dir = @opendir( FPMU_PLUGIN_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( FPMU_PLUGIN_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( FPMU_PLUGIN_DIR . "/$plugin_file", false, false );
	...
	if ( isset( $wp_plugins['index.php'] ) && filesize( FPMU_PLUGIN_DIR . '/index.php' ) <= 30 ) {
		// Silence is golden.
...
/**
 * Checks the fp-content directory and retrieve all drop-ins with any plugin data.
 *
 * @since 3.0.0
...
function get_dropins() {
	...
	// Files in fp-content directory.
	$plugins_dir = @opendir( FP_CONTENT_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( FP_CONTENT_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( FP_CONTENT_DIR . "/$plugin_file", false, false );
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php'      => array( __( 'Advanced caching plugin.' ), 'FP_CACHE' ),  // FP_CACHE
...
function is_network_only_plugin( $plugin ) {
	$plugin_data = get_plugin_data( FP_PLUGIN_DIR . '/' . $plugin );
	if ( $plugin_data ) {
 ...
 * @return null|FP_Error Null on success, FP_Error on invalid file.
 */
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( $network_wide && ! isset( $current[ $plugin ] )
		...
		if ( ob_get_length() > 0 ) {
			$output = ob_get_clean();
			return new FP_Error( 'unexpected_output', __( 'The plugin generated unexpected output.' ), $output );
		}
 ...
 *
 * When FP_Error is returned, it does not mean that one of the plugins had
 ...
 * @return true|FP_Error True when finished or FP_Error if there were errors during a plugin activation.
 */
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( ! empty( $errors ) ) {
		return new FP_Error( 'plugins_invalid', __( 'One of the plugins is invalid.' ), $errors );
	}
 ...
 * @return bool|null|FP_Error True on success, false if `$plugins` is empty, `FP_Error` on failure.
...
function delete_plugins( $plugins, $deprecated = '' ) {
	...
	if ( false === $credentials ) {
		if ( ! empty( $data ) ) {
			require_once ABSPATH . 'fp-admin/admin-header.php';
			echo $data;
	...
	if ( is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->has_errors() ) {
		return new FP_Error( 'fs_error', __( 'Filesystem error.' ), $wp_filesystem->errors );
	}

	// Get the base plugin folder.
	$plugins_dir = $wp_filesystem->wp_plugins_dir();
	if ( empty( $plugins_dir ) ) {
		return new FP_Error( 'fs_no_plugins_dir', __( 'Unable to locate WordPress plugin directory.' ) );
	}
	...
	foreach ( $plugins as $plugin_file ) {
		...
		if ( '.' !== $plugin_slug && ! empty( $plugin_translations[ $plugin_slug ] ) ) {
			...
			foreach ( $translations as $translation => $data ) {
				$wp_filesystem->delete( FP_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.po' );
				$wp_filesystem->delete( FP_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.mo' );
				$wp_filesystem->delete( FP_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.l10n.php' );
	...
	if ( ! empty( $errors ) ) {
		...
		return new FP_Error( 'could_not_remove_plugin', sprintf( $message, implode( ', ', $errors ) ) );
	}
 ...
 *
 * @since 2.5.0
 * @return FP_Error[] Array of plugin errors keyed by plugin file name.
 */
 ...
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 * @return int|FP_Error 0 on success, FP_Error on failure.
 */
function validate_plugin( $plugin ) {
	if ( validate_file( $plugin ) ) {
		return new FP_Error( 'plugin_invalid', __( 'Invalid plugin path.' ) );
	}
	if ( ! file_exists( FP_PLUGIN_DIR . '/' . $plugin ) ) {
		return new FP_Error( 'plugin_not_found', __( 'Plugin file does not exist.' ) );
	}

	$installed_plugins = get_plugins();
	if ( ! isset( $installed_plugins[ $plugin ] ) ) {
		return new FP_Error( 'no_plugin_header', __( 'The plugin does not have a valid header.' ) );
	}
	return 0;
}
 ...
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 * @return true|FP_Error True if requirements are met, FP_Error on failure.
 */
function validate_plugin_requirements( $plugin ) {
	$plugin_headers = get_plugin_data( FP_PLUGIN_DIR . '/' . $plugin );

	$requirements = array(
		'requires'         => ! empty( $plugin_headers['RequiresFP'] ) ? $plugin_headers['RequiresFP'] : '',
	...
	FP_Plugin_Dependencies::initialize();

	if ( FP_Plugin_Dependencies::has_unmet_dependencies( $plugin ) ) {
		$dependency_names       = FP_Plugin_Dependencies::get_dependency_names( $plugin );
		$unmet_dependencies     = array();
		$unmet_dependency_names = array();

		foreach ( $dependency_names as $dependency => $dependency_name ) {
			$dependency_file = FP_Plugin_Dependencies::get_dependency_filepath( $dependency );
...
function is_uninstallable_plugin( $plugin ) {
	...
	if ( isset( $uninstallable_plugins[ $file ] ) || file_exists( FP_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		return true;
	}
...
function uninstall_plugin( $plugin ) {
	...
	if ( file_exists( FP_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		...
		define( 'FP_UNINSTALL_PLUGIN', $file );

		wp_register_plugin_realpath( FP_PLUGIN_DIR . '/' . $file );
		include_once FP_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php';
	...
	if ( isset( $uninstallable_plugins[ $file ] ) ) {
		...
		wp_register_plugin_realpath( FP_PLUGIN_DIR . '/' . $file );
		include_once FP_PLUGIN_DIR . '/' . $file;
 ...
 *                              * Pass 'none' to leave div.fp-menu-image empty so an icon can be added via CSS.
...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $callback = '', $position = null ) {
	 ...
	 * See fp-admin/admin.php for redirect from edit.php to tools.php.
	 */
 ...
 *
 * @since 3.0.0
 * @since 4.4.0 Function was moved into the `fp-admin/includes/plugin.php` file.
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 */
function plugin_sandbox_scrape( $plugin ) {
	...
	wp_register_plugin_realpath( FP_PLUGIN_DIR . '/' . $plugin );
	include_once FP_PLUGIN_DIR . '/' . $plugin;
}
 ...
 * https://developer.finpress.org/plugins/privacy/suggesting-text-for-the-site-privacy-policy/.
 *
...
function wp_add_privacy_policy_content( $plugin_name, $policy_text ) {
	if ( ! is_admin() ) {
		_doing_it_wrong(
			...
			sprintf(
				...
				__( 'The suggested privacy policy content should be added only in fp-admin by using the %s (or later) action.' ),
	...
	if ( ! class_exists( 'FP_Privacy_Policy_Content' ) ) {
		require_once ABSPATH . 'fp-admin/includes/class-fp-privacy-policy-content.php';
	}

	FP_Privacy_Policy_Content::add( $plugin_name, $policy_text );
}
 ...
 * @return true|FP_Error True on success, false if `$plugin` was not paused,
 *                       `FP_Error` on failure.
 */
...
function deactivated_plugins_notice() {
	...
	foreach ( $deactivated_plugins as $plugin ) {
		if ( ! empty( $plugin['version_compatible'] ) && ! empty( $plugin['version_deactivated'] ) ) {
			$explanation = sprintf(
				/* translators: 1: Name of deactivated plugin, 2: Plugin version deactivated, 3: Current FP version, 4: Compatible plugin version. */
			...
			$explanation = sprintf(
				/* translators: 1: Name of deactivated plugin, 2: Plugin version deactivated, 3: Current FP version. */
