<?php
/**
 * Upgrade API: FP_Upgrader class
 *
 * Requires skin classes and FP_Upgrader subclasses for backward compatibility.
 *
 * @package WordPress
 * @subpackage Upgrader
 * @since 2.8.0
...
class FP_Upgrader {
	 ...
	 *
	 * @since 2.8.0
	 * @var Automatic_Upgrader_Skin|FP_Upgrader_Skin $skin
	 */
	public $skin = null;
	 ...
	 *
	 * This is set by FP_Upgrader::install_package(), only when the package is installed
	 * successfully. It will then be an array, unless a FP_Error is returned by the
	 * {@see 'upgrader_post_install'} filter. In that case, the FP_Error will be assigned to
	 * it.
	 *
	 * @since 2.8.0
	 *
	 * @var array|FP_Error $result {
	 ...
	 * @since 2.8.0
	 *
	 * @param FP_Upgrader_Skin $skin The upgrader skin to use. Default is a FP_Upgrader_Skin
	 *                               instance.
	 */
	public function __construct( $skin = null ) {
		if ( null === $skin ) {
			$this->skin = new FP_Upgrader_Skin();
		} else {
			$this->skin = $skin;
		}
	}
	 ...
	 * and also add the generic strings to `FP_Upgrader::$strings`.
	 *
	...
	/**
	 * Adds the generic strings to FP_Upgrader::$strings.
	 *
	 * @since 2.8.0
	 */
	public function generic_strings() {
		...
		$this->strings['fs_no_content_dir'] = sprintf( __( 'Unable to locate WordPress content directory (%s).' ), 'fp-content' );
	 ...
	 *                                               not exist, a FP_Error object will be returned.
	 *                                               Default empty array.
	 * @param bool     $allow_relaxed_file_ownership Whether to allow relaxed file ownership.
	 *                                               Default false.
	 * @return bool|FP_Error True if able to connect, false or a FP_Error otherwise.
	 */
	public function fs_connect( $directories = array(), $allow_relaxed_file_ownership = false ) {
		...
		if ( ! FP_Filesystem( $credentials, $directories[0], $allow_relaxed_file_ownership ) ) {
			$error = true;
		...
		if ( ! is_object( $wp_filesystem ) ) {
			return new FP_Error( 'fs_unavailable', $this->strings['fs_unavailable'] );
		}

		if ( is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->has_errors() ) {
			return new FP_Error( 'fs_error', $this->strings['fs_error'], $wp_filesystem->errors );
		}
			...
			switch ( $dir ) {
				case ABSPATH:
					if ( ! $wp_filesystem->abspath() ) {
						return new FP_Error( 'fs_no_root_dir', $this->strings['fs_no_root_dir'] );
					}
					break;
				case FP_CONTENT_DIR:
					if ( ! $wp_filesystem->wp_content_dir() ) {
						return new FP_Error( 'fs_no_content_dir', $this->strings['fs_no_content_dir'] );
					}
					break;
				case FP_PLUGIN_DIR:
					if ( ! $wp_filesystem->wp_plugins_dir() ) {
						return new FP_Error( 'fs_no_plugins_dir', $this->strings['fs_no_plugins_dir'] );
					}
					break;
				case get_theme_root():
					if ( ! $wp_filesystem->wp_themes_dir() ) {
						return new FP_Error( 'fs_no_themes_dir', $this->strings['fs_no_themes_dir'] );
					}
					break;
				default:
					if ( ! $wp_filesystem->find_folder( $dir ) ) {
						return new FP_Error( 'fs_no_folder', sprintf( $this->strings['fs_no_folder'], esc_html( basename( $dir ) ) ) );
					}
					break;
			}
		}
	...
	public function download_package( $package, $check_signatures = false, $hook_extra = array() ) {
		...
		if ( is_wp_error( $download_file ) && ! $download_file->get_error_data( 'softfail-filename' ) ) {
			return new FP_Error( 'download_failed', $this->strings['download_failed'], $download_file->get_error_message() );
		}
	 ...
	 * @return string|FP_Error The path to the unpacked contents, or a FP_Error on failure.
	 */
	public function unpack_package( $package, $delete_package = true ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			if ( 'incompatible_archive' === $result->get_error_code() ) {
				return new FP_Error( 'incompatible_archive', $this->strings['incompatible_archive'], $result->get_error_data() );
			}
			return $result;
		}
	...
	/**
	 * Flattens the results of FP_Filesystem_Base::dirlist() for iterating over.
	 *
	 * @since 4.9.0
	 *
	 * @param array  $nested_files Array of files as returned by FP_Filesystem_Base::dirlist().
	 ...
	 * @return true|FP_Error True upon success, FP_Error on failure.
	 */
	public function clear_destination( $remote_destination ) {
		...
		if ( ! empty( $unwritable_files ) ) {
			return new FP_Error( 'files_not_writable', $this->strings['files_not_writable'], implode( ', ', $unwritable_files ) );
		}

		if ( ! $wp_filesystem->delete( $remote_destination, true ) ) {
			return new FP_Error( 'remove_old_failed', $this->strings['remove_old_failed'] );
		}
	 ...
	 *                                               FP_Upgrader::install_package(). Default empty array.
	 * }
	 *
	 * @return array|FP_Error The result (also stored in `FP_Upgrader::$result`), or a FP_Error on failure.
	 */
	public function install_package( $args = array() ) {
		 ...
		 * upacking and downloading in FP_Upgrader->run().
		 */
		...
		if (
			...
			return new FP_Error( 'bad_request', $this->strings['bad_request'] );
		}
		 ...
		 *
		 * Returning a value that could be evaluated as a `FP_Error` will effectively
		 * short-circuit the installation, returning that value instead.
		 *
		 * @since 2.8.0
		 *
		 * @param bool|FP_Error $response   Installation response.
		 * @param array         $hook_extra Extra arguments passed to hooked filters.
		 */
		...
		if ( false === $dirlist ) {
			return new FP_Error( 'source_read_failed', $this->strings['fs_error'], $this->strings['dir_not_readable'] );
		}
		...
		if ( 1 === count( $source_files ) && $wp_filesystem->is_dir( trailingslashit( $args['source'] ) . $source_files[0] . '/' ) ) {
			...
			// There are no files?
			return new FP_Error( 'incompatible_archive_empty', $this->strings['incompatible_archive'], $this->strings['no_files'] );
		} else {
			/*
		 ...
		 * @param FP_Upgrader $upgrader      FP_Upgrader instance.
		 * @param array       $hook_extra    Extra arguments passed to hooked filters.
		 */
		...
		if ( $source !== $remote_source ) {
			...
			if ( false === $dirlist ) {
				return new FP_Error( 'new_source_read_failed', $this->strings['fs_error'], $this->strings['dir_not_readable'] );
			}
		 ...
		 * destination directory (FP_PLUGIN_DIR / fp-content/themes) intending
		 * to copy the directory into the directory, whilst they pass the source
		 * as the actual files to copy.
		 */
		$protected_directories = array( ABSPATH, FP_CONTENT_DIR, FP_PLUGIN_DIR, FP_CONTENT_DIR . '/themes' );
		...
		if ( $clear_destination ) {
			 ...
			 * @since 2.8.0
			 *
			 * @param true|FP_Error $removed            Whether the destination was cleared.
			 *                                          True upon success, FP_Error on failure.
			...
			if ( ! empty( $_files ) ) {
				...
				return new FP_Error( 'folder_exists', $this->strings['folder_exists'], $remote_destination );
			}
		}
		...
		if ( $args['clear_working']
			...
			if ( ! $wp_filesystem->exists( $remote_destination ) ) {
				if ( ! $wp_filesystem->mkdir( $remote_destination, FS_CHMOD_DIR ) ) {
					return new FP_Error( 'mkdir_failed_destination', $this->strings['mkdir_failed'], $remote_destination );
				}
			}
	 ...
	 *                                               FP_Upgrader::header() and FP_Upgrader::footer()
	 *                                               aren't called. Default false.
	 *     @type array  $hook_extra                  Extra arguments to pass to the filter hooks called by
	 *                                               FP_Upgrader::run().
	 * }
	 * @return array|false|FP_Error The result from self::install_package() on success, otherwise a FP_Error,
	...
	public function run( $options ) {
		...
		$res = $this->fs_connect( array( FP_CONTENT_DIR, $options['destination'] ) );
		...
		if ( is_wp_error( $download ) && $download->get_error_data( 'softfail-filename' ) ) {
			...
			if ( 'signature_verification_no_signature' !== $download->get_error_code() || FP_DEBUG ) {
		...
		/**
		 * Filters the result of FP_Upgrader::install_package().
		 *
		 * @since 5.7.0
		 *
		 * @param array|FP_Error $result     Result from FP_Upgrader::install_package().
		 * @param array          $hook_extra Extra arguments passed to hooked filters.
		 */
		...
		if ( is_wp_error( $result ) ) {
			...
			if ( ! empty( $options['hook_extra']['temp_backup'] ) ) {
				 ...
				 * `FP_Upgrader::restore_temp_backup()` expects an array.
				 */
		...
		if ( ! $options['is_multi'] ) {
			 ...
			 *
			 * @since 3.6.0
			 * @since 3.7.0 Added to FP_Upgrader::run().
			 * @since 4.6.0 `$translations` was added as a possible argument to `$hook_extra`.
			 *
			 * @param FP_Upgrader $upgrader   FP_Upgrader instance. In other contexts this might be a
	...
	public static function create_lock( $lock_name, $release_timeout = null ) {
		...
		if ( ! $lock_result ) {
			...
			FP_Upgrader::release_lock( $lock_name );

			return FP_Upgrader::create_lock( $lock_name, $release_timeout );
		}
	 ...
	 * }
	 *
	 * @return bool|FP_Error True on success, false on early exit, otherwise FP_Error.
	 */
	public function move_to_temp_backup_dir( $args ) {
		...
		if ( ! $wp_filesystem->is_dir( $sub_dir ) ) {
			...
			if ( ! $wp_filesystem->mkdir( $sub_dir, FS_CHMOD_DIR ) ) {
				...
				return new FP_Error( 'fs_temp_backup_mkdir', $this->strings['temp_backup_mkdir_failed'] );
			}
		}
		...
		if ( is_wp_error( $result ) ) {
			return new FP_Error( 'fs_temp_backup_move', $this->strings['temp_backup_move_failed'] );
		}
	 ...
	 *         @type string $src  The directory where the original is stored. For example, `FP_PLUGIN_DIR`.
	 *     }
	 * }
	 * @return bool|FP_Error True on success, false on early exit, otherwise FP_Error.
	 */
...
require_once ABSPATH . 'fp-admin/includes/class-plugin-upgrader.php';

/** Theme_Upgrader class */
require_once ABSPATH . 'fp-admin/includes/class-theme-upgrader.php';

/** Language_Pack_Upgrader class */
require_once ABSPATH . 'fp-admin/includes/class-language-pack-upgrader.php';

/** Core_Upgrader class */
require_once ABSPATH . 'fp-admin/includes/class-core-upgrader.php';

/** File_Upload_Upgrader class */
require_once ABSPATH . 'fp-admin/includes/class-file-upload-upgrader.php';

/** FP_Automatic_Updater class */
require_once ABSPATH . 'fp-admin/includes/class-fp-automatic-updater.php';
