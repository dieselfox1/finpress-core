<?php
/**
 * FP_Privacy_Policy_Content class.
 *
 * @package WordPress
...
final class FP_Privacy_Policy_Content {
	...
	public static function text_change_check() {
		...
		if ( $new != $old ) {
			 ...
			 */
			add_action( 'admin_notices', array( 'FP_Privacy_Policy_Content', 'policy_text_changed_notice' ) );
			$state = 'changed';
		} else {
			$state = 'not-changed';
		}
	 ...
	 *
	 * @param FP_Post|null $post The currently edited post. Default null.
	 */
	public static function notice( $post = null ) {
		...
		if ( get_current_screen()->is_block_editor() ) {
			...
			wp_add_inline_script(
				'fp-notices',
				sprintf(
					'fp.data.dispatch( "core/notices" ).createWarningNotice( "%s", { actions: [ %s ], isDismissible: false } )',
					$message,
			...
			wp_admin_notice(
				...
				array(
					'type'               => 'warning',
					'additional_classes' => array( 'inline', 'fp-pp-notice' ),
				)
			);
		}
	}
	...
	public static function get_default_content( $description = false, $blocks = true ) {
		...
		if ( $description ) {
			$strings[] = '<div class="fp-suggested-text">';
		}

		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="fp-block-heading">' . __( 'Who we are' ) . '</h2>';
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="fp-block-heading">' . __( 'Comments' ) . '</h2>';
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="fp-block-heading">' . __( 'Media' ) . '</h2>';
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="fp-block-heading">' . __( 'Cookies' ) . '</h2>';
		...
		if ( ! $description ) {
			/* translators: Default privacy policy heading. */
			$strings[] = '<h2 class="fp-block-heading">' . __( 'Embedded content from other websites' ) . '</h2>';
			/* translators: Default privacy policy text. */
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="fp-block-heading">' . __( 'Who we share your data with' ) . '</h2>';
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="fp-block-heading">' . __( 'How long we retain your data' ) . '</h2>';
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="fp-block-heading">' . __( 'What rights you have over your data' ) . '</h2>';
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="fp-block-heading">' . __( 'Where your data is sent' ) . '</h2>';
		...
		if ( $blocks ) {
			foreach ( $strings as $key => $string ) {
				if ( str_starts_with( $string, '<p>' ) ) {
					$strings[ $key ] = "<!-- fp:paragraph -->\n" . $string . "\n<!-- /fp:paragraph -->\n";
				}

				if ( str_starts_with( $string, '<h2 ' ) ) {
					$strings[ $key ] = "<!-- fp:heading -->\n" . $string . "\n<!-- /fp:heading -->\n";
				}
			}
		}
