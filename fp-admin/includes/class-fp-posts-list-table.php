<?php
/**
 * List Table API: FP_Posts_List_Table class
 *
 * @package WordPress
 ...
 * @see FP_List_Table
 */
class FP_Posts_List_Table extends FP_List_Table {
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'top' === $which ) {
			 ...
			 *                          'top' or 'bottom' for FP_Posts_List_Table,
			 *                          'bar' for FP_Media_List_Table.
			 */
	 ...
	 *
	 * @param FP_Post $item The current FP_Post object.
	 */
	public function column_cb( $item ) {
		 ...
		 * @param FP_Post $post The current FP_Post object.
		 */
	...
	public function column_default( $item, $column_name ) {
		...
		if ( $taxonomy ) {
			...
			if ( is_array( $terms ) ) {
				 ...
				 * @param FP_Term[] $terms      Array of term objects appearing in the post row.
				 */
	 ...
	 * @since 5.9.0 Renamed `$post` to `$item` to match parent class for PHP 8 named parameter support.
	 *
	 * @param FP_Post $item        Post being acted upon.
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		...
		if ( 'wp_block' === $post->post_type ) {
			$actions['export'] = sprintf(
				'<button type="button" class="fp-list-reusable-blocks__export button-link" data-id="%s" aria-label="%s">%s</button>',
				$post->ID,
				/* translators: %s: Post title. */
		...
		while ( $bulk < 2 ) :
			...
			<fieldset class="inline-edit-col-left">
				...
				<?php if ( ! $bulk && $can_publish ) : ?>
					...
					<div class="inline-edit-group fp-clearfix">
					...
					<?php foreach ( $flat_taxonomies as $taxonomy ) : ?>
						...
						<?php if ( current_user_can( $taxonomy->cap->assign_terms ) ) : ?>
							...
							<label class="inline-edit-tags">
								...
								<textarea data-fp-taxonomy="<?php echo $taxonomy_name; ?>" cols="22" rows="1" name="tax_input[<?php echo esc_attr( $taxonomy->name ); ?>]" class="tax_input_<?php echo esc_attr( $taxonomy->name ); ?>" aria-describedby="inline-edit-<?php echo esc_attr( $taxonomy->name ); ?>-desc"></textarea>
							</label>
