...
function the_media_upload_tabs() {
	...
	if ( ! empty( $tabs ) ) {
		...
		if ( isset( $redir_tab ) && array_key_exists( $redir_tab, $tabs ) ) {
			...
			/** This filter is documented in fp-admin/media-upload.php */
...
function get_image_send_to_editor( $id, $caption, $title, $align, $url = '', $rel = false, $size = 'medium', $alt = '' ) {
	...
	if ( $rel ) {
		if ( is_string( $rel ) ) {
			...
			$rel = ' rel="attachment fp-att-' . (int) $id . '"';
		}
	} else {
		$rel = '';
	}
 ...
 * @return int|FP_Error ID of the attachment or a FP_Error object on failure.
 */
function media_handle_upload( $file_id, $post_id, $post_data = array(), $overrides = array( 'test_form' => false ) ) {
	...
	if ( isset( $file['error'] ) ) {
		return new FP_Error( 'upload_error', $file['error'] );
	}
 ...
 * @return int|FP_Error The ID of the attachment or a FP_Error on failure.
 */
...
function wp_iframe( $content_func, ...$args ) {
	...
	<body<?php echo $body_id_attr; ?> class="fp-core-ui no-js">
	<script type="text/javascript">
...
function media_buttons( $editor_id = 'content' ) {
	...
	$img = '<span class="fp-media-buttons-icon"></span> ';
	...
	if ( $legacy_filter ) {
		// #FP22559. Close <a> if a plugin started by closing <a> to open their own <a> tag.
	...
	if ( isset( $send_id ) ) {
		...
		if ( ! empty( $attachment['url'] ) ) {
			...
			if ( str_contains( $attachment['url'], 'attachment_id' ) || get_attachment_link( $send_id ) === $attachment['url'] ) {
				$rel = " rel='attachment fp-att-" . esc_attr( $send_id ) . "'";
			}
 ...
 * @return string|int|FP_Error Populated HTML img tag, attachment ID, or attachment source
 *                             on success, FP_Error object otherwise.
 */
function media_sideload_image( $file, $post_id = 0, $desc = null, $return_type = 'html' ) {
	if ( ! empty( $file ) ) {
		...
		if ( ! $matches ) {
			return new FP_Error( 'image_sideload_failed', __( 'Invalid image URL.' ) );
		}
	...
	if ( ! empty( $src ) ) {
		...
		return new FP_Error( 'image_sideload_failed' );
	}
}
 ...
 * @since 3.4.0
 *
 * @param FP_Post $edit_post Attachment FP_Post object.
 ...
 * @param FP_Post $post        The FP_Post attachment object.
...
function get_media_item( $attachment_id, $args = null ) {
	...
	if ( 'image' === $type && $calling_post_id
		...
		$thumbnail  = "<a class='fp-post-thumbnail' id='fp-post-thumbnail-" . $attachment_id . "' href='#' onclick='FPSetAsThumbnail(\"$attachment_id\", \"$ajax_nonce\");return false;'>" . esc_html( $calling_post_type_object->labels->use_featured_image ) . '</a>';
	}
...
function media_upload_form( $errors = null ) {
	...
	if ( ! _device_can_upload() ) {
		echo '<p>' . sprintf(
			/* translators: %s: https://apps.finpress.org/ */
			__( 'The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.' ),
			'https://apps.finpress.org/'
		) . '</p>';
		return;
	}
	...
	*/
	...
	/** This filter is documented in fp-includes/rest-api/endpoints/class-fp-rest-attachments-controller.php */
 ...
 * @since 3.5.0
 *
 * @param FP_Post $post A post object.
 */
function edit_form_image_editor( $post ) {
	...
	?>
	<div class="wp_attachment_holder fp-clearfix">
	<?php
	...
	if ( wp_attachment_is_image( $post->ID ) ) :
		...
		<div<?php echo $open_style; ?> class="wp_attachment_image fp-clearfix" id="media-head-<?php echo $attachment_id; ?>">
	...
	elseif ( isset( $thumb_url[0] ) ) :
		?>
		<div class="wp_attachment_image fp-clearfix" id="media-head-<?php echo $attachment_id; ?>">
...
function attachment_submitbox_metadata() {
	...
	$author = new FP_User( $post->post_author );
...
function wp_read_video_metadata( $file ) {
	...
	if ( ! class_exists( 'getID3', false ) ) {
		require ABSPATH . FPINC . '/ID3/getid3.php';
	}
