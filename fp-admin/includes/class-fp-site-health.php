...
class FP_Site_Health {
	...
	public function __construct() {
		...
		if ( defined( 'DISABLE_FP_CRON' ) && DISABLE_FP_CRON ) {
	...
	public function show_site_health_tab( $tab ) {
		if ( 'debug' === $tab ) {
			require_once ABSPATH . 'fp-admin/site-health-info.php';
		}
	}

	/**
	 * Returns an instance of the FP_Site_Health class, or create one if none exist yet.
	 *
	 * @since 5.4.0
	 *
	 * @return FP_Site_Health|null
	 */
	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new FP_Site_Health();
		}
	...
	public function enqueue_scripts() {
		...
		if ( 'site-health' === $screen->id && ( ! isset( $_GET['tab'] ) || empty( $_GET['tab'] ) ) ) {
			$tests = FP_Site_Health::get_tests();
	 ...
	 */
	public function get_test_finpress_version() {
		$result = array(
			'label'       => '',
			'status'      => '',
			'badge'       => array(
	...
	public function get_test_theme_version() {
		...
		// If FP_DEFAULT_THEME doesn't exist, fall back to the latest core default theme.
		$default_theme = wp_get_theme( FP_DEFAULT_THEME );
		if ( ! $default_theme->exists() ) {
			$default_theme = FP_Theme::get_core_default_theme();
		}
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						...
						sprintf(
							...
							$default_theme ? $default_theme->name : FP_DEFAULT_THEME,
							$active_theme->name,
							$active_theme->parent()->name
						)
					);
	 ...
	 * as seen at https://make.finpress.org/hosting/handbook/handbook/server-environment/#php-extensions
	 *
	 * @since 5.2.0
	 *
	 * @return array
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					esc_url( __( 'https://make.finpress.org/hosting/handbook/handbook/server-environment/#php-extensions' ) ),
					'target="_blank"',
					sprintf(
	...
	public function get_test_sql_server() {
		...
		$result = array(
			...
			'actions'     => sprintf(
				...
				esc_url( __( 'https://finpress.org/about/requirements/' ) ),
		...
		$db_dropin = file_exists( FP_CONTENT_DIR . '/db.php' );
	...
	public function get_test_dotorg_communication() {
		...
		if ( ! is_wp_error( $wp_dotorg ) ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					sprintf(
						...
						gethostbyname( 'api.finpress.org' ),
			...
			$result['actions'] = sprintf(
				...
				esc_url( __( 'https://finpress.org/support/forums/' ) ),
	 ...
	 *
	 * When FP_DEBUG is enabled, errors and information may be disclosed to site visitors,
	...
	public function get_test_is_in_debug_mode() {
		$result = array(
			...
			'actions'     => sprintf(
				...
				esc_url( __( 'https://developer.finpress.org/advanced-administration/debug/debug-finpress/' ) ),
		...
		if ( defined( 'FP_DEBUG' ) && FP_DEBUG ) {
			if ( defined( 'FP_DEBUG_LOG' ) && FP_DEBUG_LOG ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						...
						'<code>FP_DEBUG_LOG</code>'
					)
				);
			}

			if ( defined( 'FP_DEBUG_DISPLAY' ) && FP_DEBUG_DISPLAY ) {
				...
				$result['description'] .= sprintf(
					'<p>%s</p>',
					sprintf(
						/* translators: 1: FP_DEBUG_DISPLAY, 2: FP_DEBUG */
	...
	public function get_test_https_status() {
		...
		if ( ! wp_is_using_https() ) {
			...
			if ( wp_is_https_supported() ) {
				...
				if ( defined( 'FP_HOME' ) || defined( 'FP_SITEURL' ) ) {
					$result['description'] .= sprintf(
						'<p>%s</p>',
						sprintf(
							/* translators: 1: fp-config.php, 2: FP_HOME, 3: FP_SITEURL */
							...
							'<code>fp-config.php</code>',
							'<code>FP_HOME</code>',
							'<code>FP_SITEURL</code>'
						)
					);
	 ...
	 *
	 * If scheduled events are not running, this may indicate something with FP_Cron is not working
	...
	public function get_test_background_updates() {
		...
		if ( ! class_exists( 'FP_Site_Health_Auto_Updates' ) ) {
			require_once ABSPATH . 'fp-admin/includes/class-fp-site-health-auto-updates.php';
		}
		 ...
		 */
		$automatic_updates = new FP_Site_Health_Auto_Updates();
	...
	public function get_test_update_temp_backup_writable() {
		...
		if ( false === $credentials || ! FP_Filesystem( $credentials ) ) {
			$result['status']      = 'recommended';
		...
		if ( ! $wp_content ) {
			...
			$result['description'] = sprintf(
				/* translators: %s: fp-content */
				'<p>' . __( 'The %s directory cannot be located.' ) . '</p>',
				'<code>fp-content</code>'
			);
			return $result;
		}
		...
		if ( $plugins_dir_exists && ! $plugins_dir_is_writable && $themes_dir_exists && ! $themes_dir_is_writable ) {
			...
			$result['description'] = sprintf(
				/* translators: 1: fp-content/upgrade-temp-backup/plugins, 2: fp-content/upgrade-temp-backup/themes. */
				...
				'<code>fp-content/upgrade-temp-backup/plugins</code>',
				'<code>fp-content/upgrade-temp-backup/themes</code>'
			);
			return $result;
		}
		...
		if ( $plugins_dir_exists && ! $plugins_dir_is_writable ) {
			...
			$result['description'] = sprintf(
				/* translators: %s: fp-content/upgrade-temp-backup/plugins */
		...
		if ( $themes_dir_exists && ! $themes_dir_is_writable ) {
			...
			$result['description'] = sprintf(
				/* translators: %s: fp-content/upgrade-temp-backup/themes */
		...
		if ( ! $backup_dir_exists && $upgrade_dir_exists && ! $upgrade_dir_is_writable ) {
			...
			$result['description'] = sprintf(
				/* translators: %s: fp-content/upgrade */
		...
		if ( ! $upgrade_dir_exists && ! $wp_filesystem->is_writable( $wp_content ) ) {
			...
			$result['description'] = sprintf(
				/* translators: 1: fp-content/upgrade, 2: fp-content. */
	 ...
	 *
	 * A loopback is when WordPress queries itself, for example to start a new FP_Cron instance,
	...
	public function get_test_http_requests() {
		...
		if ( defined( 'FP_HTTP_BLOCK_EXTERNAL' ) && FP_HTTP_BLOCK_EXTERNAL ) {
			$blocked = true;
		}

		if ( defined( 'FP_ACCESSIBLE_HOSTS' ) ) {
			$hosts = explode( ',', FP_ACCESSIBLE_HOSTS );
		}
		...
		if ( $blocked && 0 === count( $hosts ) ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					'<code>FP_HTTP_BLOCK_EXTERNAL</code>'
				)
			);
		}
	...
	public function get_test_rest_availability() {
		...
		$headers = array(
			...
			'X-FP-Nonce'    => wp_create_nonce( 'wp_rest' ),
		);
	...
	public function get_test_authorization_header() {
		...
		if ( ! function_exists( 'got_mod_rewrite' ) ) {
			require_once ABSPATH . 'fp-admin/includes/misc.php';
		}
		...
		if ( got_mod_rewrite() ) {
			...
			$result['actions'] .= sprintf(
				'<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				__( 'https://developer.finpress.org/rest-api/frequently-asked-questions/#why-is-authentication-not-working' ),
	...
	public function get_test_page_cache() {
		...
		$result = array(
			...
			'actions'     => sprintf(
				...
				__( 'https://developer.finpress.org/advanced-administration/performance/optimization/#caching' ),
	...
	public function get_test_persistent_object_cache() {
		...
		$action_url = apply_filters(
			...
			/* translators: Localized Support reference. */
			__( 'https://developer.finpress.org/advanced-administration/performance/optimization/#persistent-object-cache' )
		);
	...
	public function get_test_autoloaded_options() {
		...
		$result['actions'] = sprintf(
			...
			esc_url( __( 'https://developer.finpress.org/advanced-administration/performance/optimization/#autoloaded-options' ) ),
	...
	public static function get_tests() {
		$tests = array(
			...
			'async'  => array(
				'dotorg_communication' => array(
					...
					'test'              => rest_url( 'fp-site-health/v1/tests/dotorg-communication' ),
					'has_rest'          => true,
					'async_direct_test' => array( FP_Site_Health::get_instance(), 'get_test_dotorg_communication' ),
				),
				'background_updates'   => array(
					'label'             => __( 'Background updates' ),
					'test'              => rest_url( 'fp-site-health/v1/tests/background-updates' ),
					'has_rest'          => true,
					'async_direct_test' => array( FP_Site_Health::get_instance(), 'get_test_background_updates' ),
				),
				'loopback_requests'    => array(
					'label'             => __( 'Loopback request' ),
					'test'              => rest_url( 'fp-site-health/v1/tests/loopback-requests' ),
					'has_rest'          => true,
					'async_direct_test' => array( FP_Site_Health::get_instance(), 'get_test_loopback_requests' ),
				),
				'https_status'         => array(
					'label'             => __( 'HTTPS status' ),
					'test'              => rest_url( 'fp-site-health/v1/tests/https-status' ),
					'has_rest'          => true,
					'async_direct_test' => array( FP_Site_Health::get_instance(), 'get_test_https_status' ),
				),
			),
		);
		...
		if ( ! wp_is_site_protected_by_basic_auth() ) {
			$tests['async']['authorization_header'] = array(
				...
				'test'      => rest_url( 'fp-site-health/v1/tests/authorization-header' ),
				'has_rest'  => true,
		...
		if ( 'production' === wp_get_environment_type() ) {
			$tests['async']['page_cache'] = array(
				'label'             => __( 'Page cache' ),
				'test'              => rest_url( 'fp-site-health/v1/tests/page-cache' ),
				'has_rest'          => true,
				'async_direct_test' => array( FP_Site_Health::get_instance(), 'get_test_page_cache' ),
			);
		 ...
		 * to complete should run asynchronously, to avoid extended loading periods within fp-admin.
		 *
		 * @since 5.2.0
	...
	/**
	 * Initiates the FP_Cron schedule test cases.
	 *
	 * @since 5.2.0
	 */
	...
	private function get_cron_tasks() {
		...
		if ( empty( $cron_tasks ) ) {
			$this->crons = new FP_Error( 'no_tasks', __( 'No scheduled events exist on this site.' ) );
			return;
		}
	 ...
	 *
	 * If the list of crons is an instance of FP_Error, returns the instance instead of a boolean value.
	 *
	 * @since 5.2.0
	 *
	 * @return bool|FP_Error True if a cron was missed, false if not. FP_Error if the cron is set to that.
	 */
	 ...
	 * @since 5.3.0
	 *
	 * @return bool|FP_Error True if a cron is late, false if not. FP_Error if the cron is set to that.
	 */
	...
	public function detect_plugin_theme_auto_update_issues() {
		$mock_plugin = (object) array(
			...
			'new_version'   => '9.9',
			'url'           => 'https://finpress.org/plugins/a-fake-plugin/',
			'package'       => 'https://downloads.finpress.org/plugin/a-fake-plugin.9.9.zip',
			'icons'         => array(
		...
		$mock_theme = (object) array(
			'theme'        => 'a-fake-theme',
			'new_version'  => '9.9',
			'url'          => 'https://finpress.org/themes/a-fake-theme/',
			'package'      => 'https://downloads.finpress.org/theme/a-fake-theme.9.9.zip',
			'requires'     => '5.0.0',
			'requires_php' => '5.6.20',
		);
	 ...
	 *
	 * Loopbacks are what WordPress uses to communicate with itself to start up FP_Cron, scheduled posts,
	...
	public function can_perform_loopback() {
		...
		$url = site_url( 'fp-cron.php' );

		/*
		 * A post request is used for the fp-cron.php loopback test to cause the file
		 ...
		 * GET request WordPress uses for fp-cron.php loopback requests but is close
		 * enough. See https://core.trac.finpress.org/ticket/52547
		 */
	...
	public function wp_cron_scheduled_check() {
		// Bootstrap fp-admin, as FP_Cron doesn't do this for us.
		require_once trailingslashit( ABSPATH ) . 'fp-admin/includes/admin.php';
	...
	private function check_for_page_caching() {
		 ...
		 * initiating the test. If a site requires basic auth, the test will fail when it runs in FP Cron as part of
		 * wp_site_health_scheduled_check. This logic is copied from FP_Site_Health::can_perform_loopback().
		 */
		if ( isset( $_SERVER['PHP_AUTH_USER'] ) && isset( $_SERVER['PHP_AUTH_PW'] ) ) {
			$headers['Authorization'] = 'Basic ' . base64_encode( wp_unslash( $_SERVER['PHP_AUTH_USER'] ) . ':' . wp_unslash( $_SERVER['PHP_AUTH_PW'] ) );
		}
		...
		return array(
			'advanced_cache_present'        => (
				file_exists( FP_CONTENT_DIR . '/advanced-cache.php' )
				&&
				( defined( 'FP_CACHE' ) && FP_CACHE )
				&&
				/** This filter is documented in fp-settings.php */
	 ...
	 *
	 * @return FP_Error|array {
	 *     Page cache detail or else a FP_Error if unable to determine.
	 *
