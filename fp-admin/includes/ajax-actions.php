...
function wp_ajax_ajax_tag_search() {
	 ...
	 * @param FP_Taxonomy $taxonomy_object The taxonomy object.
...
function wp_ajax_imgedit_preview() {
	...
	require_once ABSPATH . 'fp-admin/includes/image-edit.php';
...
function wp_ajax_autocomplete_user() {
	...
	/** This filter is documented in fp-admin/user-new.php */
...
function wp_ajax_get_community_events() {
	require_once ABSPATH . 'fp-admin/includes/class-fp-community-events.php';
	...
	$events_client  = new FP_Community_Events( $user_id, $saved_location );
...
function wp_ajax_dashboard_widgets() {
	require_once ABSPATH . 'fp-admin/includes/dashboard.php';
...
function _wp_ajax_add_hierarchical_term() {
	...
	/** This filter is documented in fp-admin/includes/meta-boxes.php */
...
function wp_ajax_add_tag() {
	...
	if ( ! $tag || is_wp_error( $tag ) ) {
		...
		$x->add(
			array(
				'what' => 'taxonomy',
				'data' => new FP_Error( $error_code, $message ),
			)
		);
		$x->send();
	}

	$wp_list_table = _get_list_table( 'FP_Terms_List_Table', array( 'screen' => $_POST['screen'] ) );
	...
	require ABSPATH . 'fp-admin/includes/edit-tag-messages.php';
...
function wp_ajax_get_comments( $action ) {
	...
	$wp_list_table = _get_list_table( 'FP_Post_Comments_List_Table', array( 'screen' => 'edit-comments' ) );
...
function wp_ajax_replyto_comment( $action ) {
	...
	if ( isset( $_REQUEST['mode'] ) && 'dashboard' === $_REQUEST['mode'] ) {
		...
		if ( isset( $_REQUEST['mode'] ) && 'single' === $_REQUEST['mode'] ) {
			...
			$wp_list_table = _get_list_table( 'FP_Comments_List_Table', array( 'screen' => 'edit-comments' ) );
		}
		$wp_list_table->single_row( $comment );
	}
	$comment_list_item = ob_get_clean();
...
function wp_ajax_edit_comment() {
	...
	$wp_list_table = _get_list_table( $checkbox ? 'FP_Comments_List_Table' : 'FP_Post_Comments_List_Table', array( 'screen' => 'edit-comments' ) );
...
function wp_ajax_add_menu_item() {
	...
	require_once ABSPATH . 'fp-admin/includes/nav-menu.php';
	...
	/** This filter is documented in fp-admin/includes/nav-menu.php */
...
function wp_ajax_add_user( $action ) {
	...
	$wp_list_table = _get_list_table( 'FP_Users_List_Table' );
...
function wp_ajax_wp_link_ajax() {
	...
	if ( ! class_exists( '_FP_Editors', false ) ) {
		require ABSPATH . FPINC . '/class-fp-editor.php';
	}

	$results = _FP_Editors::wp_link_query( $args );
...
function wp_ajax_inline_save() {
	...
	if ( ! empty( $data['tax_input'] ) ) {
		foreach ( $data['tax_input'] as $taxonomy => $terms ) {
			...
			/** This filter is documented in fp-admin/includes/class-fp-posts-list-table.php */
	...
	$wp_list_table = _get_list_table( 'FP_Posts_List_Table', array( 'screen' => $_POST['screen'] ) );
...
function wp_ajax_inline_save_tax() {
	...
	$wp_list_table = _get_list_table( 'FP_Terms_List_Table', array( 'screen' => 'edit-' . $taxonomy ) );
...
function wp_ajax_save_widget() {
	...
	/** This action is documented in fp-admin/widgets.php */
 ...
 * @since 3.9.0
 *
 * @global FP_Customize_Manager $wp_customize
 */
...
function wp_ajax_delete_inactive_widgets() {
	...
	/** This action is documented in fp-admin/includes/ajax-actions.php */
	do_action( 'load-widgets.php' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
	/** This action is documented in fp-admin/includes/ajax-actions.php */
	do_action( 'widgets.php' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
	/** This action is documented in fp-admin/widgets.php */
...
function wp_ajax_media_create_image_subsizes() {
	 ...
	 */
	if ( ! headers_sent() ) {
		header( 'X-FP-Upload-Attachment-ID: ' . $attachment_id );
	}
...
function wp_ajax_upload_attachment() {
	 ...
	 * See https://core.trac.finpress.org/ticket/31037
	 */
...
function wp_ajax_query_attachments() {
	...
	/**
	 * Filters the arguments passed to FP_Query during an Ajax
	...
	$attachments_query = new FP_Query( $query );
	...
	header( 'X-FP-Total: ' . (int) $total_posts );
	header( 'X-FP-TotalPages: ' . $max_pages );
...
function wp_ajax_save_attachment_compat() {
	...
	/** This filter is documented in fp-admin/includes/media.php */
...
function wp_ajax_send_attachment_to_editor() {
	...
	if ( str_starts_with( $post->post_mime_type, 'image' ) ) {
		...
		$rel  = $rel ? ' rel="attachment fp-att-' . $id . '"' : ''; // Hard-coded string, $id is already sanitized.
...
function wp_ajax_get_revision_diffs() {
	require ABSPATH . 'fp-admin/includes/revision.php';
...
function wp_ajax_query_themes() {
	...
	/** This filter is documented in fp-admin/includes/class-fp-theme-install-list-table.php */
...
function wp_ajax_parse_embed() {
	...
	if ( $post_id > 0 ) {
		...
		setup_postdata( $post );
	} elseif ( ! current_user_can( 'edit_posts' ) ) { // See FP_oEmbed_Controller::get_proxy_item_permissions_check().
		wp_send_json_error();
	}
	...
	if ( 0 === $post_id ) {
		 ...
		 * their post meta caches. See FP_Embed::cache_oembed().
		 */
	...
	if ( has_shortcode( $parsed, 'audio' ) || has_shortcode( $parsed, 'video' ) ) {
		...
		ob_start();
		wp_print_scripts( array( 'mediaelement-vimeo', 'fp-mediaelement' ) );
	...
	if ( str_contains( $parsed, 'class="fp-embedded-content' ) ) {
		if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) {
			$script_src = includes_url( 'js/fp-embed.js' );
		} else {
			$script_src = includes_url( 'js/fp-embed.min.js' );
		}
...
function wp_ajax_destroy_sessions() {
	...
	$sessions = FP_Session_Tokens::get_instance( $user->ID );
...
function wp_ajax_crop_image() {
	...
	switch ( $context ) {
		case 'site-icon':
			require_once ABSPATH . 'fp-admin/includes/class-fp-site-icon.php';
			$wp_site_icon = new FP_Site_Icon();
			...
			/** This filter is documented in fp-admin/includes/class-custom-image-header.php */
...
function wp_ajax_install_theme() {
	...
	require_once ABSPATH . 'fp-admin/includes/theme.php';
	...
	$skin     = new FP_Ajax_Upgrader_Skin();
	...
	if ( is_wp_error( $result ) ) {
		...
		// Pass through the error from FP_Filesystem if one was raised.
		if ( $wp_filesystem instanceof FP_Filesystem_Base && is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->has_errors() ) {
	...
	/*
	 * See FP_Theme_Install_List_Table::_get_theme_status() if we wanted to check
...
function wp_ajax_search_plugins() {
	...
	/** @var FP_Plugins_List_Table $wp_list_table */
...
function wp_ajax_search_install_plugins() {
	...
	/** @var FP_Plugin_Install_List_Table $wp_list_table */
	$wp_list_table = _get_list_table(
...
function wp_ajax_wp_privacy_export_personal_data() {
	...
	check_ajax_referer( 'fp-privacy-export-personal-data-' . $request_id, 'security' );
...
function wp_ajax_wp_privacy_erase_personal_data() {
	...
	check_ajax_referer( 'fp-privacy-erase-personal-data-' . $request_id, 'security' );
 ...
 *
 * @since 5.2.0
 * @deprecated 5.6.0 Use FP_REST_Site_Health_Controller::test_dotorg_communication()
 * @see FP_REST_Site_Health_Controller::test_dotorg_communication()
 */
function wp_ajax_health_check_dotorg_communication() {
	_doing_it_wrong(
		...
		sprintf(
			...
			'wp_ajax_health_check_dotorg_communication',
			'FP_REST_Site_Health_Controller::test_dotorg_communication'
		),
		'5.6.0'
	);
 ...
 *
 * @since 5.2.0
 * @deprecated 5.6.0 Use FP_REST_Site_Health_Controller::test_background_updates()
 * @see FP_REST_Site_Health_Controller::test_background_updates()
 */
function wp_ajax_health_check_background_updates() {
	_doing_it_wrong(
		...
		sprintf(
			...
			__( 'The Site Health check for %1$s has been replaced with %2$s.' ),
			'wp_ajax_health_check_background_updates',
			'FP_REST_Site_Health_Controller::test_background_updates'
		),
		'5.6.0'
	);
 ...
 *
 * @since 5.2.0
 * @deprecated 5.6.0 Use FP_REST_Site_Health_Controller::test_loopback_requests()
 * @see FP_REST_Site_Health_Controller::test_loopback_requests()
 */
function wp_ajax_health_check_loopback_requests() {
	_doing_it_wrong(
		...
		sprintf(
			...
			__( 'The Site Health check for %1$s has been replaced with %2$s.' ),
			'wp_ajax_health_check_loopback_requests',
			'FP_REST_Site_Health_Controller::test_loopback_requests'
		),
		'5.6.0'
	);
 ...
 *
 * @since 5.2.0
 * @deprecated 5.6.0 Use FP_REST_Site_Health_Controller::get_directory_sizes()
 * @see FP_REST_Site_Health_Controller::get_directory_sizes()
 */
function wp_ajax_health_check_get_sizes() {
	_doing_it_wrong(
		...
		sprintf(
			...
			__( 'The Site Health check for %1$s has been replaced with %2$s.' ),
			'wp_ajax_health_check_get_sizes',
			'FP_REST_Site_Health_Controller::get_directory_sizes'
		),
		'5.6.0'
	);
	...
	if ( ! class_exists( 'FP_Debug_Data' ) ) {
		require_once ABSPATH . 'fp-admin/includes/class-fp-debug-data.php';
	}

	$sizes_data = FP_Debug_Data::get_sizes();
...
function wp_ajax_toggle_auto_updates() {
	...
	switch ( $type ) {
		case 'plugin':
			...
			/** This filter is documented in fp-admin/includes/class-fp-plugins-list-table.php */
