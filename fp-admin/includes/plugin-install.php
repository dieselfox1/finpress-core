 ...
 *         @type bool $added             Whether to return the date when the plugin was added to the finpress.org
 ...
 *         {@link https://developer.finpress.org/reference/functions/plugins_api/ function reference article}
...
function plugins_api( $action, $args = array() ) {
	...
	if ( false === $res ) {
		...
		$url = 'http://api.finpress.org/plugins/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( isset( $res->error ) ) {
				$res = new FP_Error( 'plugins_api_failed', $res->error );
			}
		}
	 ...
	 * @since 2.7.0
	 *
	 * @param object|FP_Error $res    Response object or FP_Error.
	 * @param string          $action The type of information being requested from the Plugin Installation API.
	 * @param object          $args   Plugin API arguments.
	 */
...
function install_search_form( $deprecated = true ) {
	...
	<form class="search-form search-plugins" method="get">
		...
		<input type="search" name="s" id="search-plugins" value="<?php echo esc_attr( $term ); ?>" class="fp-filter-search" />
...
<div class="upload-plugin">
	...
	<form method="post" enctype="multipart/form-data" class="fp-upload-form" action="<?php echo esc_url( self_admin_url( 'update.php?action=upload-plugin' ) ); ?>">
	...
	switch ( current_filter() ) {
		case 'install_plugins_beta':
			printf(
				...
				'https://make.finpress.org/core/handbook/about/release-cycle/features-as-plugins/'
			);
			break;
...
function install_plugin_install_status( $api, $loop = false ) {
	...
	if ( 'install' === $status ) {
		if ( is_dir( FP_PLUGIN_DIR . '/' . $api->slug ) ) {
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo esc_url( __( 'https://finpress.org/plugins/' ) . $api->slug ); ?>/"><?php _e( 'WordPress.org Plugin Page &#187;' ); ?></a></li>
				...
				<div class="counter-container">
						<span class="counter-label">
							...
							printf(
								...
								"https://finpress.org/support/plugin/{$api->slug}/reviews/?filter={$key}",
								$aria_label,
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$content = links_add_base_url( $content, 'https://finpress.org/plugins/' . $api->slug . '/' );
 ...
 * @param bool         $compatible_wp    The result of a FP compatibility check.
