...
function wp_credits( $version = '', $locale = '' ) {
	...
	$results = get_site_transient( 'finpress_credits_' . $locale );

	if ( ! is_array( $results )
		|| str_contains( $version, '-' )
		|| ( isset( $results['data']['version'] ) && ! str_starts_with( $version, $results['data']['version'] ) )
	) {
		$url     = "http://api.finpress.org/core/credits/1.1/?version={$version}&locale={$locale}";
		...
		set_site_transient( 'finpress_credits_' . $locale, $results, DAY_IN_SECONDS );
	}
...
function wp_credits_section_title( $group_data = array() ) {
	...
	if ( $group_data['name'] ) {
		if ( 'Translators' === $group_data['name'] ) {
			...
			// phpcs:ignore WordPress.FP.I18n.LowLevelTranslationFunction,WordPress.FP.I18n.NonSingularStringLiteralText
		...
		echo '<h2 class="fp-people-group-title">' . esc_html( $title ) . "</h2>\n";
	}
}
...
function wp_credits_section_list( $credits = array(), $slug = '' ) {
	...
	switch ( $group_data['type'] ) {
		case 'list':
			...
			echo '<p class="fp-credits-list">' . wp_sprintf( '%l.', $group_data['data'] ) . "</p>\n\n";
			break;
		case 'libraries':
		...
		default:
			...
			$classes = 'fp-people-group ' . ( $compact ? 'compact' : '' );
			echo '<ul class="' . $classes . '" id="fp-people-group-' . $slug . '">' . "\n";
			foreach ( $group_data['data'] as $person_data ) {
				echo '<li class="fp-person" id="fp-person-' . esc_attr( $person_data[2] ) . '">' . "\n\t";
				...
				echo '<span class="fp-person-avatar"><img src="' . esc_url( $data['url'] ) . '" srcset="' . esc_url( $data2x['url'] ) . ' 2x" class="gravatar" alt="" /></span>' . "\n";
