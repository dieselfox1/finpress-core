 ...
 * @return bool|null|FP_Error True on success, false if `$stylesheet` is empty, FP_Error on failure.
...
function delete_theme( $stylesheet, $redirect = '' ) {
	...
	if ( empty( $themes_dir ) ) {
		return new FP_Error( 'fs_no_themes_dir', __( 'Unable to locate WordPress theme directory.' ) );
	}
	...
	if ( ! empty( $theme_translations[ $stylesheet ] ) ) {
		...
		foreach ( $translations as $translation => $data ) {
			$wp_filesystem->delete( FP_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.po' );
			$wp_filesystem->delete( FP_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.mo' );
			$wp_filesystem->delete( FP_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.l10n.php' );

			$json_translation_files = glob( FP_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '-*.json' );
	...
	if ( is_multisite() ) {
		FP_Theme::network_disable_theme( $stylesheet );
	}
 ...
 *
 * @param FP_Post|null $post      Optional. The post being edited, provided for context.
 ...
 *
 * @param FP_Theme $theme Theme data object.
 */
 ...
 * @since 3.8.0
 *
 * @param FP_Theme $theme FP_Theme object.
...
function get_theme_update_available( $theme ) {
	...
	if ( isset( $themes_update->response[ $stylesheet ] ) ) {
		...
		$details_url = add_query_arg(
			...
			$update['url']
		); // Theme browser inside FP? Replace this. Also, theme preview JS will override this on the available list.
...
function get_theme_feature_list( $api = true ) {
	...
	// Loop over the fp.org canonical list and apply translations.
 ...
 *     }
 * }
 * @return object|array|FP_Error Response object or array on success, FP_Error on failure. See the
 *         {@link https://developer.finpress.org/reference/functions/themes_api/ function reference article}
...
function themes_api( $action, $args = array() ) {
	...
	if ( ! $res ) {
		$url = 'http://api.finpress.org/themes/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
		...
		if ( $ssl && is_wp_error( $request ) ) {
			if ( ! wp_doing_ajax() ) {
				wp_trigger_error(
					...
					headers_sent() || FP_DEBUG ? E_USER_WARNING : E_USER_NOTICE
				);
			}
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( isset( $res->error ) ) {
				$res = new FP_Error( 'themes_api_failed', $res->error );
			}
		}
	 ...
	 * @since 2.8.0
	 *
	 * @param array|stdClass|FP_Error $res    WordPress.org Themes API response.
	 * @param string                  $action Requested action. Likely values are 'theme_information',
	 *                                        'feature_list', or 'query_themes'.
 ...
 * @since 3.8.0
 *
 * @param FP_Theme[] $themes Optional. Array of theme objects to prepare.
...
function wp_prepare_themes_for_js( $themes = null ) {
	 ...
	 * @param FP_Theme[]|null $themes          An array of theme objects to prepare, if any.
	...
	foreach ( $themes as $theme ) {
		...
		$prepared_themes[ $slug ] = array(
			...
			'compatibleFP'   => is_wp_version_compatible( $theme->get( 'RequiresFP' ) ),
			'compatiblePHP'  => is_php_version_compatible( $theme->get( 'RequiresPHP' ) ),
			'updateResponse' => array(
				'compatibleFP'  => is_wp_version_compatible( $update_requires_wp ),
...
function customize_themes_print_templates() {
	...
	<script type="text/html" id="tmpl-customize-themes-details-view">
		...
		<div class="theme-wrap fp-clearfix" role="document">
			...
			<div class="theme-about fp-clearfix">
				...
				<div class="theme-info">
					...
					<# if ( data.hasUpdate ) { #>
						<# if ( data.updateResponse.compatibleFP && data.updateResponse.compatiblePHP ) { #>
							...
							<div class="notice notice-error notice-alt notice-large" data-slug="{{ data.id }}">
								...
								<p>
									<# if ( ! data.updateResponse.compatibleFP && ! data.updateResponse.compatiblePHP ) { #>
										...
										?>
									<# } else if ( ! data.updateResponse.compatibleFP ) { #>
										<?php
										printf(
											/* translators: %s: Theme name. */
					...
					<# if ( ! data.compatibleFP || ! data.compatiblePHP ) { #>
			...
			<div class="theme-actions">
				...
				<# } else if ( 'installed' === data.type ) { #>
					...
					<# if ( data.blockTheme ) { #>
						...
						?>
						<# if ( data.compatibleFP && data.compatiblePHP && data.actions.activate ) { #>
 ...
 * @return bool|FP_Error True on success, false if `$theme` was not paused,
 *                       `FP_Error` on failure.
 */
