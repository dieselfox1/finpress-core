...
/**
 * Class FP_Community_Events.
 *
 * A client for api.finpress.org/events.
 *
 * @since 4.8.0
 */
...
class FP_Community_Events {
	...
	/**
	 * Constructor for FP_Community_Events.
	 *
	 * @since 4.8.0
	 *
	 * @param int        $user_id       FP user ID.
	 ...
	 * than having the browser make the request directly to api.finpress.org,
	 * because it allows results to be cached server-side and shared with other
	 * users and sites in the network. This makes the process more efficient,
	 * since increasing the number of visits that get cached data means users
	 * don't have to wait as often; if the user's browser made the request
	 * directly, it would also need to make a second request to FP in order to
	 * pass the data for caching. Having FP make the request also introduces
	 ...
	 *              in `fp.communityEvents.populateDynamicEventFields()` now.
	 *
	 * @param string $location_search Optional. City name to help determine the location.
	 *                                e.g., "Seattle". Default empty string.
	 * @param string $timezone        Optional. Timezone to help determine the location.
	 *                                Default empty string.
	 * @return array|FP_Error A FP_Error on failure; an array with location and events on
	 *                        success.
	 */
	public function get_events( $location_search = '', $timezone = '' ) {
		...
		$api_url                    = 'http://api.finpress.org/events/1.0/';
