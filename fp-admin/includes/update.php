...
function find_core_auto_update() {
	...
	$auto_update = false;
	$upgrader    = new FP_Automatic_Updater();
...
function get_core_checksums( $version, $locale ) {
	$http_url = 'http://api.finpress.org/core/checksums/1.0/?' . http_build_query( compact( 'version', 'locale' ), '', '&' );
	$url      = $http_url;
...
function update_right_now_message() {
	...
	if ( current_user_can( 'update_core' ) ) {
		...
		if ( isset( $cur->response ) && 'upgrade' === $cur->response ) {
			$msg .= sprintf(
				'<a href="%s" class="button" aria-describedby="fp-version">%s</a> ',
				network_admin_url( 'update-core.php' ),
	...
	$msg .= sprintf( '<span id="fp-version">' . $content . '</span>', get_bloginfo( 'version', 'display' ), $theme_name );

	echo "<p id='fp-version-message'>$msg</p>";
}
...
function wp_theme_update_row( $theme_key, $theme ) {
	...
	/** @var FP_MS_Themes_List_Table $wp_list_table */
	$wp_list_table = _get_list_table( 'FP_MS_Themes_List_Table' );
	 ...
	 * @since 3.1.0
	 *
	 * @param FP_Theme $theme    The FP_Theme object.
...
function wp_print_admin_notice_templates() {
	?>
	<script id="tmpl-fp-updates-admin-notice" type="text/html">
		<div <# if ( data.id ) { #>id="{{ data.id }}"<# } #> class="notice {{ data.className }}"><p>{{{ data.message }}}</p></div>
	</script>
	<script id="tmpl-fp-bulk-updates-admin-notice" type="text/html">
...
function wp_recovery_mode_nag() {
	...
	$url = wp_login_url();
	$url = add_query_arg( 'action', FP_Recovery_Mode::EXIT_ACTION, $url );
	$url = wp_nonce_url( $url, FP_Recovery_Mode::EXIT_ACTION );
...
function wp_is_auto_update_enabled_for_type( $type ) {
	if ( ! class_exists( 'FP_Automatic_Updater' ) ) {
...
function wp_is_auto_update_forced_for_item( $type, $update, $item ) {
	/** This filter is documented in fp-admin/includes/class-fp-automatic-updater.php */
...
function wp_get_auto_update_message() {
	...
	if ( false === $next_update_time ) {
		$message = __( 'Automatic update not scheduled. There may be a problem with FP-Cron.' );
	} else {
		...
		if ( $overdue ) {
			$message = sprintf(
				/* translators: %s: Duration that FP-Cron has been overdue. */
				__( 'Automatic update overdue by %s. There may be a problem with FP-Cron.' ),
				$time_to_next_update
			);
		} else {
			$message = sprintf(
