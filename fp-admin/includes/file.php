...
$wp_file_descriptions = array(
	...
	// Deprecated files.
	'fp-layout.css'         => __( 'Stylesheet' ),
	'fp-comments.php'       => __( 'Comments Template' ),
	'fp-comments-popup.php' => __( 'Popup Comments Template' ),
 ...
 * @since 4.9.0
 *
 * @param FP_Theme $theme Theme object.
 * @return string[] Array of editable file extensions.
 */
function wp_get_theme_file_editable_extensions( $theme ) {
	 ...
	 * @param FP_Theme $theme         The active theme object.
	 */
...
function wp_print_file_editor_templates() {
	?>
	<script type="text/html" id="tmpl-fp-file-editor-notice">
			...
			<# } else if ( 'file_not_writable' === data.code ) { #>
				<p>
					...
					printf(
						...
						__( 'https://developer.finpress.org/advanced-administration/server/file-permissions/' )
					);
					?>
				</p>
			<# } else { #>
 ...
 * }
 * @return true|FP_Error True on success or `FP_Error` on failure.
 */
function wp_edit_theme_plugin_file( $args ) {
	if ( empty( $args['file'] ) ) {
		return new FP_Error( 'missing_file' );
	}

	if ( 0 !== validate_file( $args['file'] ) ) {
		return new FP_Error( 'bad_file' );
	}

	if ( ! isset( $args['newcontent'] ) ) {
		return new FP_Error( 'missing_content' );
	}

	if ( ! isset( $args['nonce'] ) ) {
		return new FP_Error( 'missing_nonce' );
	}
	...
	if ( ! empty( $args['plugin'] ) ) {
		...
		if ( ! current_user_can( 'edit_plugins' ) ) {
			return new FP_Error( 'unauthorized', __( 'Sorry, you are not allowed to edit plugins for this site.' ) );
		}

		if ( ! wp_verify_nonce( $args['nonce'], 'edit-plugin_' . $file ) ) {
			return new FP_Error( 'nonce_failure' );
		}

		if ( ! array_key_exists( $plugin, get_plugins() ) ) {
			return new FP_Error( 'invalid_plugin' );
		}

		if ( 0 !== validate_file( $file, get_plugin_files( $plugin ) ) ) {
			return new FP_Error( 'bad_plugin_file_path', __( 'Sorry, that file cannot be edited.' ) );
		}

		$editable_extensions = wp_get_plugin_file_editable_extensions( $plugin );

		$real_file = FP_PLUGIN_DIR . '/' . $file;
		...
		if ( 0 !== validate_file( $stylesheet ) ) {
			return new FP_Error( 'bad_theme_path' );
		}

		if ( ! current_user_can( 'edit_themes' ) ) {
			return new FP_Error( 'unauthorized', __( 'Sorry, you are not allowed to edit templates for this site.' ) );
		}

		$theme = wp_get_theme( $stylesheet );
		if ( ! $theme->exists() ) {
			return new FP_Error( 'non_existent_theme', __( 'The requested theme does not exist.' ) );
		}
		...
		if ( 0 !== validate_file( $file, array_keys( $allowed_files ) ) ) {
			return new FP_Error( 'disallowed_theme_file', __( 'Sorry, that file cannot be edited.' ) );
		}
		...
		return new FP_Error( 'missing_theme_or_plugin' );
	}

	// Ensure file is real.
	if ( ! is_file( $real_file ) ) {
		return new FP_Error( 'file_does_not_exist', __( 'File does not exist! Please double check the name and try again.' ) );
	}
	...
	if ( preg_match( '/\.([^.]+)$/', $real_file, $matches ) ) {
		...
		if ( ! in_array( $extension, $editable_extensions, true ) ) {
			return new FP_Error( 'illegal_file_type', __( 'Files of this type are not editable.' ) );
		}
	}
	...
	if ( ! is_writable( $real_file ) ) {
		return new FP_Error( 'file_not_writable' );
	}
	...
	if ( false === $written ) {
		return new FP_Error( 'unable_to_write', __( 'Unable to write to file.' ) );
	}
	...
	if ( $is_active && 'php' === $extension ) {
		...
		/** This filter is documented in fp-includes/class-fp-http-streams.php */
		...
		if ( true !== $result ) {
			...
			return new FP_Error( 'php_error', $message, $result );
		}
	}

	if ( $theme instanceof FP_Theme ) {
		$theme->cache_delete();
	}
 ...
 *                                       Default false.
 * @return string|FP_Error Filename on success, FP_Error on failure.
 */
function download_url( $url, $timeout = 300, $signature_verification = false ) {
	// WARNING: The file is not automatically deleted, the script must delete or move the file.
	if ( ! $url ) {
		return new FP_Error( 'http_no_url', __( 'No URL Provided.' ) );
	}
	...
	if ( ! $tmpfname ) {
		return new FP_Error( 'http_no_file', __( 'Could not create temporary file.' ) );
	}
	...
	if ( 200 !== $response_code ) {
		...
		return new FP_Error( 'http_404', trim( wp_remote_retrieve_response_message( $response ) ), $data );
	}
	...
	if ( $signature_verification ) {
		 ...
		 */
		$signed_hostnames = apply_filters( 'wp_signature_hosts', array( 'finpress.org', 'downloads.finpress.org', 's.w.org' ) );
 ...
 * @return bool|FP_Error True on success, false when the MD5 format is unknown/unexpected,
 *                       FP_Error on failure.
 */
 ...
 * @return bool|FP_Error True on success, false if verification not attempted,
 *                       or FP_Error describing an error condition.
 */
...
function wp_zip_file_is_valid( $file ) {
	/** This filter is documented in fp-admin/includes/file.php */
	...
	require_once ABSPATH . 'fp-admin/includes/class-pclzip.php';
 ...
 *
 * Assumes that FP_Filesystem() has already been called and set up. Does not extract
...
function unzip_file( $file, $to ) {
	...
	if ( ! $wp_filesystem || ! is_object( $wp_filesystem ) ) {
		return new FP_Error( 'fs_unavailable', __( 'Could not access filesystem.' ) );
	}
 ...
 *
 * Assumes that FP_Filesystem() has already been called and set up.
 *
 * @since 3.0.0
 * @access private
 *
...
function _unzip_file_ziparchive( $file, $to, $needed_dirs = array() ) {
	...
	if ( true !== $zopen ) {
		return new FP_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), array( 'ziparchive_error' => $zopen ) );
	}
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( ! $info ) {
			$z->close();
			return new FP_Error( 'stat_failed_ziparchive', __( 'Could not retrieve file from archive.' ) );
		}
	 ...
	 */
	if ( wp_doing_cron() ) {
		$available_space = function_exists( 'disk_free_space' ) ? @disk_free_space( FP_CONTENT_DIR ) : false;
	...
	foreach ( $needed_dirs as $_dir ) {
		...
		if ( ! $wp_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $wp_filesystem->is_dir( $_dir ) ) {
			$z->close();
			return new FP_Error( 'mkdir_failed_ziparchive', __( 'Could not create directory.' ), $_dir );
		}
	}
	 ...
	 * @since 6.4.0
	 *
	 * @param null|true|FP_Error $result         The result of the override. True on success, otherwise FP Error. Default null.
	 * @param string             $file           Full path and filename of ZIP archive.
	 * @param string             $to             Full path on the filesystem to extract archive to.
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( false === $contents ) {
			$z->close();
			return new FP_Error( 'extract_failed_ziparchive', __( 'Could not extract file from archive.' ), $info['name'] );
		}

		if ( ! $wp_filesystem->put_contents( $to . $info['name'], $contents, FS_CHMOD_FILE ) ) {
			$z->close();
			return new FP_Error( 'copy_failed_ziparchive', __( 'Could not copy file.' ), $info['name'] );
		}
	}
	 ...
	 * @since 6.4.0
	 *
	 * @param true|FP_Error $result         The result of unzipping the archive. True on success, otherwise FP_Error. Default true.
	 * @param string        $file           Full path and filename of ZIP archive.
...
function _unzip_file_pclzip( $file, $to, $needed_dirs = array() ) {
	...
	if ( ! is_array( $archive_files ) ) {
		return new FP_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), $archive->errorInfo( true ) );
	}

	if ( 0 === count( $archive_files ) ) {
		return new FP_Error( 'empty_archive_pclzip', __( 'Empty archive.' ) );
	}
	...
	foreach ( $needed_dirs as $_dir ) {
		// Only check to see if the dir exists upon creation failure. Less I/O this way.
		if ( ! $wp_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $wp_filesystem->is_dir( $_dir ) ) {
			return new FP_Error( 'mkdir_failed_pclzip', __( 'Could not create directory.' ), $_dir );
		}
	}

	/** This filter is documented in src/fp-admin/includes/file.php */
	$pre = apply_filters( 'pre_unzip_file', null, $file, $to, $needed_dirs, $required_space );
	...
	foreach ( $archive_files as $file ) {
		...
		if ( ! $wp_filesystem->put_contents( $to . $file['filename'], $file['content'], FS_CHMOD_FILE ) ) {
			return new FP_Error( 'copy_failed_pclzip', __( 'Could not copy file.' ), $file['filename'] );
		}
	}

	/** This action is documented in src/fp-admin/includes/file.php */
	$result = apply_filters( 'unzip_file', true, $file, $to, $needed_dirs, $required_space );
 ...
 * Abstraction.
 *
 * Assumes that FP_Filesystem() has already been called and setup.
 *
 * @since 2.5.0
 *
 * @global FP_Filesystem_Base $wp_filesystem WordPress filesystem subclass.
...
function copy_dir( $from, $to, $skip_list = array() ) {
	...
	if ( false === $dirlist ) {
		return new FP_Error( 'dirlist_failed_copy_dir', __( 'Directory listing failed.' ), basename( $from ) );
	}
	...
	foreach ( (array) $dirlist as $filename => $fileinfo ) {
		...
		if ( 'f' === $fileinfo['type'] ) {
			if ( ! $wp_filesystem->copy( $from . $filename, $to . $filename, true, FS_CHMOD_FILE ) ) {
				...
				if ( ! $wp_filesystem->copy( $from . $filename, $to . $filename, true, FS_CHMOD_FILE ) ) {
					return new FP_Error( 'copy_failed_copy_dir', __( 'Could not copy file.' ), $to . $filename );
				}
			}
			...
			if ( ! $wp_filesystem->is_dir( $to . $filename ) ) {
				if ( ! $wp_filesystem->mkdir( $to . $filename, FS_CHMOD_DIR ) ) {
					return new FP_Error( 'mkdir_failed_copy_dir', __( 'Could not create directory.' ), $to . $filename );
				}
			}
...
function move_dir( $from, $to, $overwrite = false ) {
	...
	if ( trailingslashit( strtolower( $from ) ) === trailingslashit( strtolower( $to ) ) ) {
		return new FP_Error( 'source_destination_same_move_dir', __( 'The source and destination are the same.' ) );
	}

	if ( $wp_filesystem->exists( $to ) ) {
		if ( ! $overwrite ) {
			return new FP_Error( 'destination_already_exists_move_dir', __( 'The destination folder already exists.' ), $to );
		} elseif ( ! $wp_filesystem->delete( $to, true ) ) {
			// Can't overwrite if the destination couldn't be deleted.
			return new FP_Error( 'destination_not_deleted_move_dir', __( 'The destination directory already exists and could not be removed.' ) );
		}
	}
	...
	if ( ! $wp_filesystem->is_dir( $to ) ) {
		if ( ! $wp_filesystem->mkdir( $to, FS_CHMOD_DIR ) ) {
			return new FP_Error( 'mkdir_failed_move_dir', __( 'Could not create directory.' ), $to );
		}
	}
 ...
 *                                                   directly to the `FP_Filesystem_*()` classes.
 *                                                   Default false.
 ...
 */
function FP_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	global $wp_filesystem;

	require_once ABSPATH . 'fp-admin/includes/class-fp-filesystem-base.php';
	...
	if ( ! class_exists( "FP_Filesystem_$method" ) ) {
		 ...
		 */
		$abstraction_file = apply_filters( 'filesystem_method_file', ABSPATH . 'fp-admin/includes/class-fp-filesystem-' . $method . '.php', $method );
 ...
 *
 * The return value can be overridden by defining the `FS_METHOD` constant in `fp-config.php`,
 * or filtering via {@see 'filesystem_method'}.
 *
 * @link https://developer.finpress.org/advanced-administration/finpress/fp-config/#finpress-upgrade-constants
 *
 * Plugins may define a custom transport handler, See FP_Filesystem().
 *
 * @since 2.5.0
 *
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $context ) {
		$context = FP_CONTENT_DIR;
	}

	// If the directory doesn't exist (fp-content/languages) then use the parent directory as we'll create it.
	if ( FP_LANG_DIR === $context && ! is_dir( $context ) ) {
 ...
 *
 * Hostnames may be in the form of hostname:portnumber (eg: finpress.org:2467)
 ...
 * @param bool|FP_Error $error                        Optional. Whether the current request has failed
...
function request_filesystem_credentials( $form_post, $type = '', $error = false, $context = '', $extra_fields = null, $allow_relaxed_file_ownership = false ) {
	 ...
	 * @param bool|FP_Error $error                        Whether the current request has failed to connect,
	 *                                                    or an error object.
...
<label for="hostname">
	...
	<input name="hostname" type="text" id="hostname" aria-describedby="request-filesystem-credentials-desc" class="code" placeholder="<?php esc_attr_e( 'example: www.finpress.org' ); ?>" value="<?php echo $hostname_value; ?>"<?php disabled( defined( 'FTP_HOST' ) ); ?> />
</label>
...
<p id="auth-keys-desc"><?php _e( 'Enter the location on the server where the public and private keys are located. If a passphrase is needed, enter that in the password field above.' ); ?></p>
	 ...
	 * from FP_Site_Health_Auto_Updates::test_check_wp_filesystem_method().
	 */
	if ( ! function_exists( 'submit_button' ) ) {
		require_once ABSPATH . 'fp-admin/includes/template.php';
	}
	?>
...
function wp_opcache_invalidate( $filepath, $force = false ) {
	 ...
	 * - https://core.trac.finpress.org/ticket/36455
	 */
...
function wp_opcache_invalidate_directory( $dir ) {
	 ...
	 *
	 * FP_Filesystem_*::dirlist() returns an array of file and directory information.
	 *
	 * This does not include a path to the file or directory.
	 * To invalidate files within sub-directories, recursion is needed
	 * to prepend an absolute path containing the sub-directory's name.
	 *
	 * @param array  $dirlist Array of file/directory information from FP_Filesystem_Base::dirlist(),
