 ...
 * @return array|FP_Error Array of post data on success, FP_Error on failure.
 */
function _wp_translate_postdata( $update = false, $post_data = null ) {
	...
	if ( $update && ! current_user_can( 'edit_post', $post_data['ID'] ) ) {
		if ( 'page' === $post_data['post_type'] ) {
			return new FP_Error( 'edit_others_pages', __( 'Sorry, you are not allowed to edit pages as this user.' ) );
		} else {
			return new FP_Error( 'edit_others_posts', __( 'Sorry, you are not allowed to edit posts as this user.' ) );
		}
	} elseif ( ! $update && ! current_user_can( $ptype->cap->create_posts ) ) {
		if ( 'page' === $post_data['post_type'] ) {
			return new FP_Error( 'edit_others_pages', __( 'Sorry, you are not allowed to create pages as this user.' ) );
		} else {
			return new FP_Error( 'edit_others_posts', __( 'Sorry, you are not allowed to create posts as this user.' ) );
		}
	}
 ...
 * @since 5.0.1
 *
 * @param array|FP_Error|null $post_data The array of post data to process, or an error object.
 *                                       Defaults to the `$_POST` superglobal.
 * @return array|FP_Error Array of post data on success, FP_Error on failure.
 */
 ...
 * @return FP_Post Post object containing all the default post data as attributes
 */
 ...
 * @since 2.1.0
 *
 * @global FP_User $current_user
 *
 * @return int|FP_Error Post ID on success, FP_Error on failure.
 */
function wp_write_post() {
	...
	if ( ! current_user_can( $ptype->cap->edit_posts ) ) {
		if ( 'page' === $ptype->name ) {
			return new FP_Error( 'edit_pages', __( 'Sorry, you are not allowed to create pages on this site.' ) );
		} else {
			return new FP_Error( 'edit_posts', __( 'Sorry, you are not allowed to create posts or drafts on this site.' ) );
		}
	}
 ...
 * @return void|int|FP_Error Void if nothing fixed. 0 or FP_Error on update failure. The post ID on update success.
 */
function _fix_attachment_links( $post ) {
	...
	foreach ( $link_matches[1] as $key => $value ) {
		if ( ! strpos( $value, '?attachment_id=' ) || ! strpos( $value, 'fp-att-' )
			|| ! preg_match( '/href=(["\'])[^"\']*\?attachment_id=(\d+)[^"\']*\\1/', $value, $url_match )
			|| ! preg_match( '/rel=["\'][^"\']*fp-att-(\d+)/', $value, $rel_match ) ) {
				continue;
		}
...
/**
 * Executes a query for attachments. An array of FP_Query arguments
...
function wp_edit_attachments_query( $q = false ) {
	fp( wp_edit_attachments_query_vars( $q ) );
...
function get_sample_permalink( $post, $title = null, $name = null ) {
	...
	if ( $ptype->hierarchical ) {
		...
		/** This filter is documented in fp-admin/edit-tag-form.php */
...
function get_sample_permalink_html( $post, $new_title = null, $new_slug = null ) {
	...
	if ( current_user_can( 'read_post', $post->ID ) ) {
		if ( 'draft' === $post->post_status || empty( $post->post_name ) ) {
			...
			$preview_target = " target='fp-preview-{$post->ID}'";
		} else {
 ...
 * @param int|FP_Post|null $post         Optional. The post ID or object associated
...
function _wp_post_thumbnail_html( $thumbnail_id = null, $post = null ) {
	...
	if ( $thumbnail_id && get_post( $thumbnail_id ) ) {
		 ...
		 * @param FP_Post      $post         The post object associated with the thumbnail.
		 */
 ...
 * @since 2.5.0
 *
 * @param int|FP_Post $post ID or object of the post to check for editing.
...
function wp_check_post_lock( $post ) {
	...
	/** This filter is documented in fp-admin/includes/ajax-actions.php */
 ...
 * @since 2.5.0
 *
 * @param int|FP_Post $post ID or object of the post being edited.
 * @return array|false {
...
function _admin_notice_post_locked() {
	...
	if ( $user ) {
		 ...
		 * @param FP_Post $post    Post object.
		 * @param FP_User $user    The user with the lock for the post.
		 */
	...
	if ( $locked ) {
		...
		$tab_last = $override ? '' : ' fp-tab-last';

		?>
		<div class="post-locked-message">
		<div class="post-locked-avatar"><?php echo get_avatar( $user->ID, 64 ); ?></div>
		<p class="currently-editing fp-tab-first" tabindex="0">
		<?php
		if ( $override ) {
		...
		if ( $override ) {
			?>
	<a class="button button-primary fp-tab-last" href="<?php echo esc_url( add_query_arg( 'get-post-lock', '1', wp_nonce_url( get_edit_post_link( $post->ID, 'url' ), 'lock-post_' . $post->ID ) ) ); ?>"><?php _e( 'Take over' ); ?></a>
			<?php
		}
		...
		<div class="post-taken-over">
			...
			<p class="fp-tab-first" tabindex="0">
			...
			?>
			<p><a class="button button-primary fp-tab-last" href="<?php echo esc_url( $sendback ); ?>"><?php echo $sendback_text; ?></a></p>
		</div>
		<?php
	}
 ...
 * @return int|FP_Error The autosave revision ID. FP_Error or 0 on error.
 */
function wp_create_post_autosave( $post_data ) {
	...
	if ( ! is_wp_error( $revision ) && 0 !== $revision ) {
		...
		/** This action is documented in fp-admin/includes/post.php */
 ...
 * @return mixed The value 0 or FP_Error on failure. The saved post ID on success.
...
function wp_autosave( $post_data ) {
	...
	if ( false === wp_verify_nonce( $post_data['_wpnonce'], 'update-post_' . $post_id ) ) {
		return new FP_Error( 'invalid_nonce', __( 'Error while saving.' ) );
	}

	$post = get_post( $post_id );

	if ( ! current_user_can( 'edit_post', $post->ID ) ) {
		return new FP_Error( 'edit_posts', __( 'Sorry, you are not allowed to edit this item.' ) );
	}
...
function get_block_editor_server_block_settings() {
	$block_registry = FP_Block_Type_Registry::get_instance();
	$blocks         = array();
...
function the_block_editor_meta_boxes() {
	...
	$script = 'window._wpLoadBlockEditor.then( function() {
		fp.data.dispatch( \'core/edit-post\' ).setAvailableMetaBoxesPerLocation( ' . wp_json_encode( $meta_boxes_per_location, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) . ' );
	} );';

	wp_add_inline_script( 'fp-edit-post', $script );

	/*
	 * When `fp-edit-post` is output in the `<head>`, the inline script needs to be manually printed.
	 * Otherwise, meta boxes will not display because inline scripts for `fp-edit-post`
	 * will not be printed again after this point.
	 */
	if ( wp_script_is( 'fp-edit-post', 'done' ) ) {
	...
	if ( $enable_custom_fields ) {
		...
		wp_enqueue_script( 'fp-lists' );
		wp_add_inline_script( 'fp-lists', $script );
	}
	...
	$script = "( function( $ ) {
		...
		$( document ).on( 'heartbeat-send.fp-refresh-nonces', function( e, data ) {
			var post_id, \$authCheck = $( '#fp-auth-check-wrap' );

			if ( check || ( \$authCheck.length && ! \$authCheck.hasClass( 'hidden' ) ) ) {
				if ( ( post_id = $( '#post_ID' ).val() ) && $( '#_wpnonce' ).val() ) {
					data['fp-refresh-metabox-loader-nonces'] = {
						post_id: post_id
					};
				}
			}
		}).on( 'heartbeat-tick.fp-refresh-nonces', function( e, data ) {
			var nonces = data['fp-refresh-metabox-loader-nonces'];

			if ( nonces ) {
				if ( nonces.replace ) {
					if ( nonces.replace.metabox_loader_nonce && window._wpMetaBoxUrl && fp.url ) {
						window._wpMetaBoxUrl= fp.url.addQueryArgs( window._wpMetaBoxUrl, { 'meta-box-loader-nonce': nonces.replace.metabox_loader_nonce } );
					}
 ...
 * @since 5.0.0
 *
 * @param FP_Post $post Current post object.
 */
function the_block_editor_meta_box_post_form_hidden_fields( $post ) {
	 ...
	 */
	ob_start();
	/** This filter is documented in fp-admin/edit-form-advanced.php */
	 ...
	 * @since 5.0.0
	 *
	 * @param FP_Post $post The post that is being edited.
	 */
 ...
 * @access private
 *
 * @param FP_Post $post An instance of FP_Post class.
 */
