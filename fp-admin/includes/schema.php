...
function wp_get_db_schema( $scope = 'all', $blog_id = null ) {
	...
	$is_multisite = is_multisite() || ( defined( 'FP_INSTALLING_NETWORK' ) && FP_INSTALLING_NETWORK );
...
function populate_options( array $options = array() ) {
	...
	$stylesheet = FP_DEFAULT_THEME;
	$template   = FP_DEFAULT_THEME;
	$theme      = wp_get_theme( FP_DEFAULT_THEME );
	if ( ! $theme->exists() ) {
		$theme = FP_Theme::get_core_default_theme();
	}

	// If we can't find a core default theme, FP_DEFAULT_THEME is the best we can do.
	if ( $theme ) {
...
if ( ! function_exists( 'install_network' ) ) :
	...
	function install_network() {
		if ( ! defined( 'FP_INSTALLING_NETWORK' ) ) {
 ...
 * @return true|FP_Error True on success, or FP_Error on warning (with the installation otherwise successful,
...
function populate_network( $network_id = 1, $domain = '', $email = '', $site_name = '', $path = '/', $subdomain_install = false ) {
	...
	if ( ! is_multisite() ) {
		...
		if ( ! $vhost_ok ) {
			...
			return new FP_Error( 'no_wildcard_dns', $msg );
		}
	}
...
function populate_network_meta( $network_id, array $meta = array() ) {
	...
	if ( FP_DEFAULT_THEME !== $stylesheet && FP_DEFAULT_THEME !== $template ) {
		$allowed_themes[ FP_DEFAULT_THEME ] = true;
	}

	// If FP_DEFAULT_THEME doesn't exist, also include the latest core default theme.
	if ( ! wp_get_theme( FP_DEFAULT_THEME )->exists() ) {
		$core_default = FP_Theme::get_core_default_theme();
		if ( $core_default ) {
