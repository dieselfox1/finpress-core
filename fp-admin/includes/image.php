 ...
 * @return string|FP_Error New filepath on success, FP_Error on failure.
 */
 ...
 *
 * @param int $attachment_id The image attachment post ID.
 * @return array|FP_Error The updated image meta data array or FP_Error object
...
function wp_update_image_subsizes( $attachment_id ) {
	...
	if ( empty( $image_meta ) || ! is_array( $image_meta ) ) {
		...
		if ( ! empty( $image_file ) ) {
			...
			return new FP_Error( 'invalid_attachment', __( 'The attached file cannot be found.' ) );
		}
	} else {
 ...
 * @access private
 *
 * @param array  $saved_data    The data returned from FP_Image_Editor after successfully saving an image.
...
function wp_create_image_subsizes( $file, $attachment_id ) {
	...
	if ( $scale_down || $convert ) {
		...
		if ( ! is_wp_error( $resized ) && is_array( $exif_meta ) ) {
			...
			$rotated = $resized; // bool true or FP_Error
		}
	...
	if ( is_callable( 'iptcparse' ) ) {
		...
		if ( ! empty( $info['APP13'] ) ) {
			...
			if ( defined( 'FP_DEBUG' ) && FP_DEBUG
				...
				// Silencing notice and warning is intentional. See https://core.trac.finpress.org/ticket/42480
				$iptc = @iptcparse( $info['APP13'] );
			}
