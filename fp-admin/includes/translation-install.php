 ...
 *     On success an associative array of translations, FP_Error on failure.
 *
...
function translations_api( $type, $args = null ) {
	if ( ! in_array( $type, array( 'plugins', 'themes', 'core' ), true ) ) {
		return new FP_Error( 'invalid_type', __( 'Invalid translation type.' ) );
	}
	...
	if ( false === $res ) {
		$url      = 'http://api.finpress.org/translations/' . $type . '/1.0/';
		$http_url = $url;
...
function wp_can_install_language_pack() {
	...
	$check = $upgrader->fs_connect( array( FP_CONTENT_DIR, FP_LANG_DIR ) );
