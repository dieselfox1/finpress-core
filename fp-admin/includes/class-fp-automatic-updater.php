<?php
/**
 * Upgrade API: FP_Automatic_Updater class
 *
 * @package WordPress
 * @subpackage Upgrader
 * @since 4.6.0
 ...
 *
 * @since 3.7.0
 * @since 4.6.0 Moved to its own file from fp-admin/includes/class-fp-upgrader.php.
 */
#[AllowDynamicProperties]
class FP_Automatic_Updater {
	...
	public function is_disabled() {
		...
		// More fine grained control can be done through the FP_AUTO_UPDATE_CORE constant and filters.
	 ...
	 * fp-content directory, for example). The underlying assumption is that if you are
	...
	public function should_update( $type, $item, $context ) {
		// Used to see if FP_Filesystem is set up to allow unattended updates.
	...
	public function update( $type, $item ) {
		...
		switch ( $type ) {
			...
			case 'plugin':
				...
				$context  = FP_PLUGIN_DIR; // We don't support custom Plugin directories, or updates for FPMU_PLUGIN_DIR.
				break;
			case 'theme':
			...
			case 'translation':
				...
				$context  = FP_CONTENT_DIR; // FP_LANG_DIR;
				break;
		}
		...
		$is_debug = FP_DEBUG && FP_DEBUG_LOG;
		if ( 'plugin' === $type ) {
		...
		/*
		 * After FP_Upgrader::upgrade() completes, maintenance mode is disabled.
		 *
		...
		if ( false === $upgrade_result ) {
			$upgrade_result = new FP_Error( 'fs_unavailable', __( 'Could not access filesystem.' ) );
		}
	...
	public function run() {
		...
		if ( ! FP_Upgrader::create_lock( 'auto_updater' ) ) {
			return;
		}
		...
		FP_Upgrader::release_lock( 'auto_updater' );
	}
	...
	protected function after_core_update( $update_result ) {
		...
		/*
		 * Any of these FP_Error codes are critical failures, as in they occurred after we started to copy core files.
		...
		/*
		 * Any other FP_Error code (like download_failed or files_not_writable) occurs before
	 ...
	 * @param mixed  $result      Optional. The result for the core update. Can be FP_Error.
	 */
	protected function send_email( $type, $core_update, $result = null ) {
		 ...
		 * @param object $core_update The update offer that was attempted.
		 * @param mixed  $result      The result for the core update. Can be FP_Error.
		 */
		...
		// If the auto-update is not to the latest version, say that the current version of FP is available instead.
		...
		if ( 'critical' === $type && is_wp_error( $result ) ) {
			 ...
			 * Loop through all errors (the main FP_Error, the update result, the rollback result) for code, data, etc.
			 */
	...
	protected function send_plugin_theme_email( $type, $successful_updates, $failed_updates ) {
		...
		$body[] = __( 'https://finpress.org/support/forums/' );
		$body[] = "\n" . __( 'The WordPress Team' );
 ...
 * Open a thread in the support forums: https://finpress.org/support/forum/alphabeta
 * Or, if you're comfortable writing a bug report: https://core.trac.finpress.org/
