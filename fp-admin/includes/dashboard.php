...
function wp_dashboard_setup() {
	...
	if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['widget_id'] ) ) {
		...
		ob_start(); // Hack - but the same hack fp-admin/widgets.php uses.
...
function _wp_dashboard_control_callback( $dashboard, $meta_box ) {
	echo '<form method="post" class="dashboard-widget-control-form fp-clearfix">';
 ...
 * @since 2.7.0
 *
 * @param FP_Post[]|false $drafts Optional. Array of posts to display. Default false.
 */
 ...
 *
 * @param FP_Comment $comment   The current comment.
...
function _wp_dashboard_recent_comments_row( &$comment, $show_date = true ) {
	...
	if ( current_user_can( 'edit_comment', $comment->comment_ID ) ) {
		...
		$actions['approve'] = sprintf(
			'<a href="%s" data-fp-lists="%s" class="vim-a aria-button-if-js" aria-label="%s">%s</a>',
			esc_url( $approve_url ),
		...
		$actions['unapprove'] = sprintf(
			'<a href="%s" data-fp-lists="%s" class="vim-u aria-button-if-js" aria-label="%s">%s</a>',
		...
		$actions['spam'] = sprintf(
			'<a href="%s" data-fp-lists="%s" class="vim-s vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
			esc_url( $spam_url ),
		...
		if ( ! EMPTY_TRASH_DAYS ) {
			$actions['delete'] = sprintf(
				'<a href="%s" data-fp-lists="%s" class="delete vim-d vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
				esc_url( $delete_url ),
		...
		/** This filter is documented in fp-admin/includes/class-fp-comments-list-table.php */
...
function wp_dashboard_recent_posts( $args ) {
	 ...
	 * @since 4.2.0
	 *
	 * @param array $query_args The arguments passed to FP_Query to produce the list of posts.
	 */
	$query_args = apply_filters( 'dashboard_recent_posts_query_args', $query_args );

	$posts = new FP_Query( $query_args );
...
function wp_dashboard_recent_comments( $total_items = 5 ) {
	...
	if ( $comments ) {
		...
		echo '<ul id="the-comment-list" data-fp-lists="list:comment">';
		...
		if ( current_user_can( 'edit_posts' ) ) {
			...
			'</h3>';
			_get_list_table( 'FP_Comments_List_Table' )->views();
		}
 ...
 * which is later replaced by Ajax call (see top of /fp-admin/index.php)
 *
 * @since 2.5.0
...
function wp_dashboard_events_news() {
	...
	<div class="finpress-news hide-if-no-js">
	...
	<p class="community-events-footer">
		<?php
			printf(
				...
				'https://make.finpress.org/community/meetups-landing-page',
				__( 'Meetups' ),
				/* translators: Hidden accessibility text. */
		...
		<?php
			printf(
				'<a href="%1$s" target="_blank">%2$s <span class="screen-reader-text"> %3$s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a>',
				/* translators: If a Rosetta site exists (e.g. https://es.finpress.org/news/), then use that. Otherwise, leave untranslated. */
				esc_url( _x( 'https://finpress.org/news/', 'Events and News dashboard widget' ) ),
				__( 'News' ),
				/* translators: Hidden accessibility text. */
				__( '(opens in a new tab)' )
			);
...
function wp_print_community_events_markup() {
	 ...
	 * won't be ready until fp.communityEvents.renderEventsTemplate() has run.
	 */
	?>
...
function wp_print_community_events_templates() {
	...
	<script id="tmpl-community-events-event-list" type="text/template">
		<# _.each( data.events, function( event ) { #>
			<li class="event event-{{ event.type }} fp-clearfix">
		...
		<# if ( data.events.length <= 2 ) { #>
			<li class="event-none">
				...
				printf(
					...
					__( 'https://make.finpress.org/community/organize-event-landing-page/' )
				);
				?>
			</li>
		<# } #>
		...
		<li class="event-none">
			<# if ( data.location.description ) { #>
				...
				printf(
					...
					__( 'https://make.finpress.org/community/handbook/meetup-organizer/welcome/' )
				);
				?>
...
function wp_dashboard_primary() {
	$feeds = array(
		'news'   => array(
			 ...
			 */
			'link'         => apply_filters( 'dashboard_primary_link', __( 'https://finpress.org/news/' ) ),
			 ...
			 */
			'url'          => apply_filters( 'dashboard_primary_feed', __( 'https://finpress.org/news/feed/' ) ),
		...
		'planet' => array(
			...
			'url'          => apply_filters(
				...
				__( 'https://planet.finpress.org/feed/' )
			),
...
function wp_check_browser_version() {
	...
	if ( false === $response ) {
		$url     = 'http://api.finpress.org/core/browse-happy/1.1/';
		$options = array(
	...
	<div class="welcome-panel-column-container">
		...
		<div class="welcome-panel-column">
			...
			<?php else : ?>
				...
				<a href="<?php echo esc_url( __( 'https://finpress.org/documentation/article/block-themes/' ) ); ?>"><?php _e( 'Learn about block themes' ); ?></a>
			<?php endif; ?>
			</div>
		</div>
	</div>
