...
if ( file_exists( FP_CONTENT_DIR . '/install.php' ) ) {
	require FP_CONTENT_DIR . '/install.php';
}
...
if ( ! function_exists( 'wp_install' ) ) :
	...
	function wp_install(
		 ...
		 * @since 3.9.0
		 *
		 * @param FP_User $user The site owner.
		 */
...
if ( ! function_exists( 'wp_install_defaults' ) ) :
	...
	function wp_install_defaults( $user_id ) {
		...
		$first_comment_email  = ! empty( $first_comment_email ) ? $first_comment_email : 'wapuu@finpress.example';
		$first_comment_url    = ! empty( $first_comment_url ) ? $first_comment_url : esc_url( __( 'https://finpress.org/' ) );
...
Commenter avatars come from <a href="%s">Gravatar</a>.'
		...
		if ( is_multisite() ) {
			...
			$privacy_policy_content = FP_Privacy_Policy_Content::get_default_content();
		}
		...
		update_option(
			'widget_block',
			array(
				2              => array( 'content' => '<!-- fp:search /-->' ),
				3              => array( 'content' => '<!-- fp:group --><div class="fp-block-group"><!-- fp:heading --><h2>' . __( 'Recent Posts' ) . '</h2><!-- /fp:heading --><!-- fp:latest-posts /--></div><!-- /fp:group -->' ),
				4              => array( 'content' => '<!-- fp:group --><div class="fp-block-group"><!-- fp:heading --><h2>' . __( 'Recent Comments' ) . '</h2><!-- /fp:heading --><!-- fp:latest-comments {"displayAvatar":false,"displayDate":false,"displayExcerpt":false} /--></div><!-- /fp:group -->' ),
				5              => array( 'content' => '<!-- fp:group --><div class="fp-block-group"><!-- fp:heading --><h2>' . __( 'Archives' ) . '</h2><!-- /fp:heading --><!-- fp:archives /--></div><!-- /fp:group -->' ),
				6              => array( 'content' => '<!-- fp:group --><div class="fp-block-group"><!-- fp:heading --><h2>' . __( 'Categories' ) . '</h2><!-- /fp:heading --><!-- fp:categories /--></div><!-- /fp:group -->' ),
				'_multiwidget' => 1,
			)
		);
		update_option(
...
'
		 ...
		 * }
		 * @param FP_User $user          The site administrator user object.
 ...
 * based on database version and FP version being updated-to.
 *
 * @ignore
 * @since 1.0.1
 *
...
function upgrade_100() {
	...
	$sql = "UPDATE $wpdb->options
		SET option_value = REPLACE(option_value, 'fp-links/links-images/', 'fp-images/links/')
		WHERE option_name LIKE %s
		AND option_value LIKE %s";
	$wpdb->query( $wpdb->prepare( $sql, $wpdb->esc_like( 'links_rating_image' ) . '%', $wpdb->esc_like( 'fp-links/links-images/' ) . '%' ) );
...
function upgrade_400() {
	...
	if ( $wp_current_db_version < 29630 ) {
		if ( ! is_multisite() && false === get_option( 'FPLANG' ) ) {
			if ( defined( 'FPLANG' ) && ( '' !== FPLANG ) && in_array( FPLANG, get_available_languages(), true ) ) {
				update_option( 'FPLANG', FPLANG );
			} else {
				update_option( 'FPLANG', '' );
			}
		}
	}
}
...
function upgrade_560() {
	...
	if ( $wp_current_db_version < 49572 ) {
		 ...
		 * Its presence may conflict with `FP_Post::__get()`.
		 */
		...
		/*
		 * When upgrading from FP < 5.6.0 set the core major auto-updates option to `unset` by default.
		 * This overrides the same option from populate_options() that is intended for new installs.
		 * See https://core.trac.finpress.org/ticket/51742.
		 */
	...
	if ( $wp_current_db_version < 49632 ) {
		 ...
		 * See https://core.trac.finpress.org/ticket/51723.
		 */
	...
	if ( $wp_current_db_version < 49752 ) {
		$results = $wpdb->get_results(
			$wpdb->prepare(
				...
				FP_Application_Passwords::USERMETA_KEY_APPLICATION_PASSWORDS
			)
		);

		if ( ! empty( $results ) ) {
			$network_id = get_main_network_id();
			update_network_option( $network_id, FP_Application_Passwords::OPTION_KEY_IN_USE, 1 );
		}
	}
}
...
function __get_option( $setting ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	...
	if ( 'home' === $setting && defined( 'FP_HOME' ) ) {
		return untrailingslashit( FP_HOME );
	}

	if ( 'siteurl' === $setting && defined( 'FP_SITEURL' ) ) {
...
function make_site_theme_from_oldschool( $theme_name, $template ) {
	...
	$site_dir    = FP_CONTENT_DIR . "/themes/$template";
	$default_dir = FP_CONTENT_DIR . '/themes/' . FP_DEFAULT_THEME;
	 ...
	 * TODO: This does not copy arbitrary include dependencies. Only the standard FP files are copied.
	 */
	$files = array(
		'index.php'             => 'index.php',
		'fp-layout.css'         => 'style.css',
		'fp-comments.php'       => 'comments.php',
		'fp-comments-popup.php' => 'comments-popup.php',
	);
	...
	foreach ( $files as $oldfile => $newfile ) {
		...
		if ( 'index.php' === $oldfile ) {
			...
			if ( str_contains( $index, 'FP_USE_THEMES' ) ) {
		...
		if ( $lines ) {
			...
			foreach ( $lines as $line ) {
				if ( preg_match( '/require.*fp-blog-header/', $line ) ) {
					$line = '//' . $line;
				}

				// Update stylesheet references.
				$line = str_replace(
					"<?php echo __get_option('siteurl'); ?>/fp-layout.css",
				...
				$line = str_replace(
					"<?php include(ABSPATH . 'fp-comments.php'); ?>",
...
function make_site_theme() {
	...
	if ( ! is_writable( FP_CONTENT_DIR . '/themes' ) ) {
		return false;
	}
	...
	if ( file_exists( ABSPATH . 'fp-layout.css' ) ) {
	...
	if ( FP_DEFAULT_THEME === $current_template ) {
