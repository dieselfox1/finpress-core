...
function iis7_rewrite_rule_exists( $filename ) {
	...
	$rules = $xpath->query( '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'finpress\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'WordPress\')]' );
...
function iis7_add_rewrite_rule( $filename, $rewrite_rule ) {
	...
	$finpress_rules = $xpath->query( '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'finpress\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'WordPress\')]' );

	if ( $finpress_rules->length > 0 ) {
		return true;
	}
...
function wp_check_locked_posts( $response, $data, $screen_id ) {
	...
	if ( array_key_exists( 'fp-check-locked-posts', $data ) && is_array( $data['fp-check-locked-posts'] ) ) {
		foreach ( $data['fp-check-locked-posts'] as $key ) {
	...
	if ( ! empty( $checked ) ) {
		$response['fp-check-locked-posts'] = $checked;
	}
...
function wp_refresh_post_lock( $response, $data, $screen_id ) {
	if ( array_key_exists( 'fp-refresh-post-lock', $data ) ) {
		...
		$response['fp-refresh-post-lock'] = $send;
	}
...
function wp_refresh_post_nonces( $response, $data, $screen_id ) {
	if ( array_key_exists( 'fp-refresh-post-nonces', $data ) ) {
		$received = $data['fp-refresh-post-nonces'];

		$response['fp-refresh-post-nonces'] = array( 'check' => 1 );
		...
		$response['fp-refresh-post-nonces'] = array(
			'replace' => array(
...
function wp_refresh_metabox_loader_nonces( $response, $data ) {
	if ( empty( $data['fp-refresh-metabox-loader-nonces'] ) ) {
		return $response;
	}

	$received = $data['fp-refresh-metabox-loader-nonces'];
	...
	$response['fp-refresh-metabox-loader-nonces'] = array(
		'replace' => array(
...
function wp_admin_canonical_url() {
	...
	?>
	<link id="fp-admin-canonical" rel="canonical" href="<?php echo esc_url( $filtered_url ); ?>" />
	<script>
		if ( window.history.replaceState ) {
			window.history.replaceState( null, null, document.getElementById( 'fp-admin-canonical' ).href + window.location.hash );
		}
	</script>
	<?php
}
 ...
 *
 * @since 3.0.0
 * @since 4.9.0 This function was moved from fp-admin/includes/ms.php so it's no longer Multisite specific.
 *
 ...
 * @param FP_Post $page  Page data object.
...
function wp_check_php_version() {
	...
	if ( false === $response ) {
		$url = 'http://api.finpress.org/core/serve-happy/1.0/';
	...
	if ( isset( $response['is_acceptable'] ) && $response['is_acceptable'] ) {
		 ...
		 *
		 * This filter is only run if the finpress.org Serve Happy API considers the PHP version acceptable, ensuring
