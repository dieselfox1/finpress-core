<?php
/**
 * Retrieves and creates the fp-config.php file.
 *
 * The permissions for the base directory must allow for writing files in order
 * for the fp-config.php to be created using this page.
 *
 * @package WordPress
...
require_once ABSPATH . 'fp-admin/includes/translation-install.php';

nocache_headers();

// Support fp-config-sample.php one level up, for the develop repo.
if ( file_exists( ABSPATH . 'fp-config-sample.php' ) ) {
	$config_file = file( ABSPATH . 'fp-config-sample.php' );
} elseif ( file_exists( dirname( ABSPATH ) . '/fp-config-sample.php' ) ) {
	$config_file = file( dirname( ABSPATH ) . '/fp-config-sample.php' );
} else {
	wp_die(
		sprintf(
			/* translators: %s: fp-config-sample.php */
			__( 'Sorry, I need a %s file to work from. Please re-upload this file to your WordPress installation.' ),
			'<code>fp-config-sample.php</code>'
		)
	);
}

// Check if fp-config.php has been created.
if ( file_exists( ABSPATH . 'fp-config.php' ) ) {
	wp_die(
		'<p>' . sprintf(
			/* translators: 1: fp-config.php, 2: install.php */
...
// Check if fp-config.php exists above the root directory but is not part of another installation.
if ( @file_exists( ABSPATH . '../fp-config.php' ) && ! @file_exists( ABSPATH . '../fp-settings.php' ) ) {
	wp_die(
		'<p>' . sprintf(
			/* translators: 1: fp-config.php, 2: install.php */
...
/**
 * Display setup fp-config.php file header.
 *
 * @ignore
 * @since 2.3.0
 *
...
<p>
	...
	<strong>
		<?php
		printf(
			/* translators: 1: fp-config-sample.php, 2: fp-config.php */
		...
		<?php
		printf(
			/* translators: 1: Documentation URL, 2: fp-config.php */
			__( 'Need more help? <a href="%1$s">Read the support article on %2$s</a>.' ),
			__( 'https://developer.finpress.org/advanced-administration/finpress/fp-config/' ),
			'<code>fp-config.php</code>'
		);
		?>
</p>
...
<form method="post" action="setup-config.php?step=2">
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td><input name="dbname" id="dbname" type="text" aria-describedby="dbname-desc" size="25" placeholder="finpress"<?php echo $autofocus; ?> />
...
<input name="noapi" type="hidden" value="1" /><?php } ?>
	...
	case 2:
		...
		/*
		* The wpdb constructor bails when FP_SETUP_CONFIG is set, so we must
		...
		*/
		...
		try {
			...
			if ( ! $no_api ) {
				$secret_keys = wp_remote_get( 'https://api.finpress.org/secret-key/1.1/salt/' );
			}
...
<p>
			<?php
			/* translators: %s: fp-config.php */
			printf( __( 'Unable to write to %s file.' ), '<code>fp-config.php</code>' );
			?>
</p>
<p id="fp-config-description">
			<?php
			/* translators: %s: fp-config.php */
			printf( __( 'You can create the %s file manually and paste the following text into it.' ), '<code>fp-config.php</code>' );
...
<p class="configuration-rules-label"><label for="fp-config">
			<?php
			/* translators: %s: fp-config.php */
			printf( __( 'Configuration rules for %s:' ), '<code>fp-config.php</code>' );
			?>
	</label></p>
<textarea id="fp-config" cols="98" rows="15" class="code" readonly="readonly" aria-describedby="fp-config-description"><?php echo $config_text; ?></textarea>
<p><?php _e( 'After you&#8217;ve done that, click &#8220;Run the installation&#8221;.' ); ?></p>
<p class="step"><a href="<?php echo $install; ?>" class="button button-large"><?php _e( 'Run the installation' ); ?></a></p>
<script>
(function(){
if ( ! /iPad|iPod|iPhone/.test( navigator.userAgent ) ) {
	var el = document.getElementById('fp-config');
	el.focus();
	el.select();
}
})();
			...
			/*
			 * If this file doesn't exist, then we are using the fp-config-sample.php
			 * file one level up, which is for the develop repo.
			 */
			if ( file_exists( ABSPATH . 'fp-config-sample.php' ) ) {
				$path_to_wp_config = ABSPATH . 'fp-config.php';
			} else {
				$path_to_wp_config = dirname( ABSPATH ) . '/fp-config.php';
			}
			...
			if ( false !== $handle ) {
				...
				if ( ! empty( $wp_config_perms ) && ! is_writable( $path_to_wp_config ) ) {
					$error_message = sprintf(
						/* translators: 1: fp-config.php, 2: Documentation URL. */
