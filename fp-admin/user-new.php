...
%4$s'
	...
	if ( ! is_multisite() ) {
		...
		if ( is_wp_error( $user_details['errors'] ) && $user_details['errors']->has_errors() ) {
			...
			/** This filter is documented in fp-includes/user.php */
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://finpress.org/documentation/article/users-add-new-screen/">Documentation on Adding New Users</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://finpress.org/support/forums/">Support forums</a>' ) . '</p>'
);

wp_enqueue_script( 'fp-ajax-response' );
	...
	if ( is_multisite() ) {
		...
		switch ( $_GET['update'] ) {
			...
			case 'addexisting':
				...
				break;
			case 'could_not_add':
				$add_user_errors = new FP_Error( 'could_not_add', __( 'That user could not be added to this site.' ) );
				break;
			case 'created_could_not_add':
				$add_user_errors = new FP_Error( 'created_could_not_add', __( 'User has been created, but could not be added to this site.' ) );
				break;
			case 'does_not_exist':
				$add_user_errors = new FP_Error( 'does_not_exist', __( 'The requested user does not exist.' ) );
				break;
			case 'enter_email':
				$add_user_errors = new FP_Error( 'enter_email', __( 'Please enter a valid email address.' ) );
				break;
		}
	} else {
		if ( 'add' === $_GET['update'] ) {
...
<table class="form-table" role="presentation">
	<tr class="form-field form-required">
		...
		<td><input name="email" type="<?php echo esc_attr( $type ); ?>" id="adduser-email" class="fp-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
...
<form method="post" name="createuser" id="createuser" class="validate" novalidate="novalidate"
	<?php
	/** This action is documented in fp-admin/user-new.php */
	do_action( 'user_new_form_tag' );
	?>
>
...
<table class="form-table" role="presentation">
	...
	<tr class="form-field form-required user-pass1-wrap">
		...
		<td>
			...
			<button type="button" class="button fp-generate-pw hide-if-no-js"><?php _e( 'Generate password' ); ?></button>
			<div class="fp-pwd">
