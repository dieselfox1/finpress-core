 ...
 * @global FP_Post_Type $post_type_object Global post type object.
...
$block_editor_context = new FP_Block_Editor_Context( array( 'post' => $post ) );
...
wp_enqueue_script( 'fp-edit-post' );
...
$preload_paths = array(
	...
	'/fp/v2/taxonomies?context=view',
	add_query_arg( 'context', 'edit', $rest_path ),
	sprintf( '/fp/v2/types/%s?context=edit', $post_type ),
	'/fp/v2/users/me',
	...
	'/fp/v2/settings',
	array( '/fp/v2/settings', 'OPTIONS' ),
	'/fp/v2/global-styles/themes/' . $active_theme . '?context=view',
	'/fp/v2/global-styles/themes/' . $active_theme . '/variations?context=view',
	'/fp/v2/themes?context=edit&status=active',
	array( '/fp/v2/global-styles/' . FP_Theme_JSON_Resolver::get_user_global_styles_post_id(), 'OPTIONS' ),
	/*
	 ...
	 */
	'/fp/v2/global-styles/' . FP_Theme_JSON_Resolver::get_user_global_styles_post_id() . '?context=' . $global_styles_endpoint_context,
	// Used by getBlockPatternCategories in useBlockEditorSettings.
	'/fp/v2/block-patterns/categories',
...
wp_add_inline_script(
	'fp-blocks',
	sprintf( 'fp.blocks.setCategories( %s );', wp_json_encode( get_block_categories( $post ), JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) ),
	'after'
);
...
wp_add_inline_script(
	'fp-blocks',
	'fp.blocks.unstable__bootstrapServerSideBlockDefinitions(' . wp_json_encode( get_block_editor_server_block_settings(), JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) . ');'
);
...
if ( ! empty( $registered_sources ) ) {
	...
	$script = sprintf( 'for ( const source of %s ) { fp.blocks.registerBlockBindingsSource( source ); }', wp_json_encode( $filtered_sources, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) );
	wp_add_inline_script(
		'fp-blocks',
		$script
	);
...
if ( $user_id ) {
	...
	/** This filter is documented in fp-admin/includes/post.php */
...
$editor_settings['__experimentalAdditionalBlockPatterns']          = FP_Block_Patterns_Registry::get_instance()->get_all_registered( true );
$editor_settings['__experimentalAdditionalBlockPatternCategories'] = FP_Block_Pattern_Categories_Registry::get_instance()->get_all_registered( true );
...
require_once ABSPATH . 'fp-admin/includes/meta-boxes.php';
...
( function() {
	window._wpLoadBlockEditor = new Promise( function( resolve ) {
		fp.domReady( function() {
			resolve( fp.editPost.initializeEditor( 'editor', "%s", %d, %s, %s ) );
		} );
	} );
} )();
JS;
...
<div class="block-editor">
	...
	<div class="wrap hide-if-js block-editor-no-js">
		<h1 class="fp-heading-inline"><?php echo esc_html( $title ); ?></h1>
		<?php
		if ( file_exists( FP_PLUGIN_DIR . '/classic-editor/classic-editor.php' ) ) {
		 ...
		 * @param FP_Post $post      The post being edited.
