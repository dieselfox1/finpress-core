...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://developer.finpress.org/advanced-administration/multisite/admin/settings/">Documentation on Network Settings</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://finpress.org/support/forums/">Support forums</a>' ) . '</p>'
);

if ( $_POST ) {
	/** This action is documented in fp-admin/network/edit.php */
	...
	if ( ! empty( $_POST['FPLANG'] ) && current_user_can( 'install_languages' ) && wp_can_install_language_pack() ) {
		$language = wp_download_language_pack( $_POST['FPLANG'] );
		if ( $language ) {
			$_POST['FPLANG'] = $language;
		}
	}
			...
			<tr>
				...
				<td>
					...
					if ( is_subdomain_install() ) {
						...
						printf(
							/* translators: 1: NOBLOGREDIRECT, 2: fp-config.php */
			...
			<tr>
				...
				<td>
					...
					if ( empty( $limited_email_domains ) ) {
						...
						// Convert from an input field. Back-compat for FPMU < 1.0.
...
<?php echo esc_textarea( get_site_option( 'first_comment' ) ); ?></textarea>
		...
		if ( ! empty( $languages ) || ! empty( $translations ) ) {
			...
			<table class="form-table" role="presentation">
				<tr>
					<th><label for="FPLANG"><?php _e( 'Default Language' ); ?><span class="dashicons dashicons-translation" aria-hidden="true"></span></label></th>
					<td>
						<?php
