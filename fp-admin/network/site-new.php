...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://developer.finpress.org/advanced-administration/multisite/admin/#network-admin-sites-screen">Documentation on Site Management</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://finpress.org/support/forum/multisite/">Support forums</a>' ) . '</p>'
);
...
if ( isset( $_REQUEST['action'] ) && 'add-site' === $_REQUEST['action'] ) {
	...
	if ( isset( $_POST['FPLANG'] ) ) {
		if ( '' === $_POST['FPLANG'] ) {
			$meta['FPLANG'] = ''; // en_US
		} elseif ( in_array( $_POST['FPLANG'], get_available_languages(), true ) ) {
			$meta['FPLANG'] = $_POST['FPLANG'];
		} elseif ( current_user_can( 'install_languages' ) && wp_can_install_language_pack() ) {
			$language = wp_download_language_pack( wp_unslash( $_POST['FPLANG'] ) );
			if ( $language ) {
				$meta['FPLANG'] = $language;
			}
		}
	}
...
<?php wp_nonce_field( 'add-blog', '_wpnonce_add-blog' ); ?>
	<table class="form-table" role="presentation">
		...
		<tr class="form-field form-required">
			...
			<td><input name="blog[email]" type="email" class="regular-text fp-suggest-user" id="admin-email" data-autocomplete-type="search" data-autocomplete-field="user_email" aria-describedby="site-admin-email" required /></td>
		</tr>
		<tr class="form-field">
