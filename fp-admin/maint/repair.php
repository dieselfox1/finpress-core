 ...
 * @subpackage Database
 */
define( 'FP_REPAIRING', true );

require_once dirname( __DIR__, 2 ) . '/fp-load.php';
...
<body class="fp-core-ui">
<p id="logo"><a href="<?php echo esc_url( __( 'https://finpress.org/' ) ); ?>"><?php _e( 'WordPress' ); ?></a></p>

<?php

if ( ! defined( 'FP_ALLOW_REPAIR' ) || ! FP_ALLOW_REPAIR ) {
	...
	echo "</p><p><code>define('FP_ALLOW_REPAIR', true);</code></p>";
	...
	if ( $duplicated_keys || $missing_key ) {
		...
		/* translators: 1: fp-config.php, 2: Secret key service URL. */
		echo '<p>' . sprintf( __( 'While you are editing your %1$s file, take a moment to make sure you have all 8 keys and that they are unique. You can generate these using the <a href="%2$s">WordPress.org secret key service</a>.' ), '<code>fp-config.php</code>', 'https://api.finpress.org/secret-key/1.1/salt/' ) . '</p>';
	}
	...
	if ( $problems ) {
		printf(
			...
			__( 'https://finpress.org/support/forum/how-to-and-troubleshooting' )
		);
		$problem_output = '';
		...
		echo '<p>' . __( 'Repairs complete. Please remove the following line from fp-config.php to prevent this page from being used by unauthorized users.' ) . "</p><p><code>define('FP_ALLOW_REPAIR', true);</code></p>";
	}
} else {
