...
$activate_cookie       = 'fp-activate-' . COOKIEHASH;
...
function wpmu_activate_stylesheet() {
	?>
	<style type="text/css">
		.fp-activate-container { width: 90%; margin: 0 auto; }
		.fp-activate-container form { margin-top: 2em; }
		#submit, #key { width: 100%; font-size: 24px; box-sizing: border-box; }
		#language { margin-top: 0.5em; }
		.fp-activate-container .error { background: #f66; color: #333; }
...
get_header( 'fp-activate' );

$blog_details = get_site();
?>

<div id="signup-content" class="widecolumn">
	<div class="fp-activate-container">
	<?php if ( ! $key ) { ?>

		<h2><?php _e( 'Activation Key Required' ); ?></h2>
		<form name="activateform" id="activateform" method="post" action="<?php echo esc_url( network_site_url( $blog_details->path . 'fp-activate.php' ) ); ?>">
			<p>
		...
		if ( is_wp_error( $result ) && in_array( $result->get_error_code(), $valid_error_codes, true ) ) {
			...
			if ( '' === $signup->domain . $signup->path ) {
				printf(
					...
					esc_url( network_site_url( $blog_details->path . 'fp-login.php', 'login' ) ),
