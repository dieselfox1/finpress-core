 ...
 * @param FP_Error|null $wp_error Optional. The error to pass. Defaults to a FP_Error instance.
 */
function login_header( $title = null, $message = '', $wp_error = null ) {
	...
	$login_header_url = __( 'https://finpress.org/' );
...
$default_actions = array(
	...
	'confirmaction',
	'login',
	FP_Recovery_Mode_Link_Service::LOGIN_ACTION_ENTERED,
);
...
setcookie( TEST_COOKIE, 'FP Cookie check', 0, COOKIEPATH, COOKIE_DOMAIN, $secure, true );

if ( SITECOOKIEPATH !== COOKIEPATH ) {
	setcookie( TEST_COOKIE, 'FP Cookie check', 0, SITECOOKIEPATH, COOKIE_DOMAIN, $secure, true );
}
...
switch ( $action ) {
	...
	case 'confirm_admin_email':
		 ...
		 * @since 5.3.0
		 *
		 * @param FP_Error $errors A `FP_Error` object containing any errors generated by using invalid
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'wp-login.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			...
			<p class="admin-email__details">
				...
				$admin_email_help_url = __( 'https://finpress.org/documentation/article/settings-general-screen/#email-address' );
	...
	case 'logout':
		 ...
		 * @param FP_User $user                  The FP_User object for the user that's logging out.
		 */
	...
	case 'rp':
		 ...
		 * @since 3.5.0
		 *
		 * @param FP_Error         $errors FP Error object.
		 * @param FP_User|FP_Error $user   FP_User object if the login and reset key match. FP_Error object otherwise.
		 */
		...
		<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( network_site_url( 'wp-login.php?action=resetpass', 'login_post' ) ); ?>" method="post" autocomplete="off">
			 ...
			 * @since 3.9.0
			 *
			 * @param FP_User $user User object of the user whose password is being reset.
			 */
	...
	case 'register':
		 ...
		 * @param int|FP_Error $errors                User id if registration was successful,
		 *                                            FP_Error object otherwise.
		 */
	...
	default:
			...
			if ( headers_sent() ) {
				$user = new FP_Error(
					...
					sprintf(
						...
						__( 'https://developer.finpress.org/advanced-administration/finpress/cookies/' ),
				...
				$user = new FP_Error(
					...
					sprintf(
						...
						__( 'https://developer.finpress.org/advanced-administration/finpress/cookies/#enable-cookies-in-your-browser' )
					)
				);
			}
		}
		 ...
		 * @param string           $redirect_to           The redirect destination URL.
		 * @param string           $requested_redirect_to The requested redirect destination URL passed as a parameter.
		 * @param FP_User|FP_Error $user                  FP_User object if login was successful, FP_Error object otherwise.
		 */
		...
		if ( ! is_wp_error( $user ) && ! $reauth ) {
			...
			if ( $user instanceof FP_User && $user->exists() && $user->has_cap( 'manage_options' ) ) {
		...
		if ( $interim_login ) {
			...
			} elseif ( FP_Recovery_Mode_Link_Service::LOGIN_ACTION_ENTERED === $action ) {
