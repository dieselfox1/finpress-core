...
function wpautop( $text, $br = true ) {
	...
	if ( $br ) {
		...
		$text = str_replace( '<FPPreserveNewline />', "\n", $text );
	}
...
function force_balance_tags( $text ) {
	...
	// FP bug fix for comments - in case you REALLY meant to type '< !--'.
	$text = str_replace( '< !--', '<    !--', $text );
	// FP bug fix for LOVE <3 (and other situations with '<' before a number).
	...
	// FP fix for the bug with HTML comments.
...
function sanitize_option( $option, $value ) {
	...
	switch ( $option ) {
		...
		case 'FPLANG':
			...
			if ( ! is_multisite() && defined( 'FPLANG' ) && '' !== FPLANG && 'en_US' !== FPLANG ) {
				$allowed[] = FPLANG;
			}
		...
		case 'tag_base':
			...
			if ( 'permalink_structure' === $option && null === $error
				...
				$error = sprintf(
					...
					__( 'https://finpress.org/documentation/article/customize-permalinks/#choosing-your-permalink-structure' )
				);
			}
			break;
...
function _links_add_base( $m ) {
	...
	return $m[1] . '=' . $m[2] .
		( preg_match( '#^(\w{1,20}):#', $m[3], $protocol ) && in_array( $protocol[1], wp_allowed_protocols(), true ) ?
			$m[3] :
			FP_Http::make_absolute_url( $m[3], $_links_add_base )
		)
		. $m[2];
}
...
// phpcs:disable WordPress.FP.CapitalPDangit.MisspelledInComment,WordPress.FP.CapitalPDangit.MisspelledInText,WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid -- 8-)
/**
...
function _print_emoji_detection_script() {
	...
	wp_print_inline_script_tag(
		sprintf( 'window._wpemojiSettings = %s;', wp_json_encode( $settings ) ) . "\n" .
			file_get_contents( ABSPATH . FPINC . '/js/wp-emoji-loader' . wp_scripts_get_suffix() . '.js' )
	);
}
