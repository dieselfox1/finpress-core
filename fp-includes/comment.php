 ...
 *
 * @since 2.0.0
 * @since 4.1.0 Refactored to leverage FP_Comment_Query over a direct query.
 *
 * @param int   $post_id The ID of the post.
 * @param array $args    {
 *     Optional. See FP_Comment_Query::__construct() for information on accepted arguments.
 *
 ...
 * }
 * @return FP_Comment[]|int[]|int The approved comments, or number of comments if `$count`
 *                                argument is true.
 */
 ...
 * @since 2.0.0
 *
 * @global FP_Comment $comment Global comment object.
 *
 * @param FP_Comment|string|int $comment Comment to retrieve.
 * @param string                $output  Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                                       correspond to a FP_Comment object, an associative array, or a numeric array,
 *                                       respectively. Default OBJECT.
 * @return FP_Comment|array|null Depends on $output value.
 */
function get_comment( $comment = null, $output = OBJECT ) {
	...
	if ( $comment instanceof FP_Comment ) {
		$_comment = $comment;
	} elseif ( is_object( $comment ) ) {
		$_comment = new FP_Comment( $comment );
	} else {
		$_comment = FP_Comment::get_instance( $comment );
	}
	 ...
	 * @since 2.3.0
	 *
	 * @param FP_Comment $_comment Comment data.
	 */
 ...
 * @since 2.7.0
 *
 * @param string|array $args Optional. Array or string of arguments. See FP_Comment_Query::__construct()
 *                           for information on accepted arguments. Default empty string.
 * @return FP_Comment[]|int[]|int List of comments or number of found comments if `$count` argument is true.
 */
 ...
 * @since 2.9.0
 *
 * @link https://developer.finpress.org/reference/functions/add_comment_meta/
 *
 ...
 * @since 2.9.0
 *
 * @link https://developer.finpress.org/reference/functions/delete_comment_meta/
 *
 * @param int    $comment_id Comment ID.
 * @param string $meta_key   Metadata name.
 ...
 * @since 2.9.0
 *
 * @link https://developer.finpress.org/reference/functions/get_comment_meta/
 *
 * @param int    $comment_id Comment ID.
 ...
 * @since 2.9.0
 *
 * @link https://developer.finpress.org/reference/functions/update_comment_meta/
 *
 * @param int    $comment_id Comment ID.
 ...
 *
 * @param FP_Comment $comment         Comment object.
 * @param FP_User    $user            Comment author's user object. The user may not exist.
 ...
 *              to return a FP_Error object instead of dying.
 ...
 *                           returning a FP_Error object, rather than executing wp_die().
 *                           Default false.
 * @return int|string|FP_Error Allowed comments return the approval status (0|1|'spam'|'trash').
 *                             If `$wp_error` is true, disallowed comments return a FP_Error.
 */
function wp_allow_comment( $commentdata, $wp_error = false ) {
	 ...
	 *
	 * Return an empty value from this filter to allow what FP considers a duplicate comment.
	 *
	 * @since 4.4.0
	 *
	...
	if ( $dupe_id ) {
		...
		if ( $wp_error ) {
			return new FP_Error( 'comment_duplicate', $comment_duplicate_message, 409 );
		} else {
			if ( wp_doing_ajax() ) {
	 ...
	 * @param bool   $wp_error             Whether to return a FP_Error object instead of executing
	...
	if ( $is_flood ) {
		...
		return new FP_Error( 'comment_flood', $comment_flood_message, 429 );
	}
...
/**
 * Hooks FP's native database-based comment-flood check.
 *
 ...
 * @since 2.7.0
 *
 * @param FP_Comment[] $comments Array of comments.
 * @return array<string, FP_Comment[]> Array of comments keyed by comment type.
 */
 ...
 *
 * @param FP_Comment[] $comments Optional. Array of FP_Comment objects. Defaults to `$wp_query->comments`.
...
function get_page_of_comment( $comment_id, $args = array() ) {
	...
	if ( null === $page ) {
		 ...
		 * @since 5.5.0
		 *
		 * @see FP_Comment_Query::__construct()
		 *
		 * @param array $comment_args {
		 *     Array of FP_Comment_Query arguments.
		 *
		 *     @type string $type               Limit paginated comments to those matching a given type.
		 *                                      Accepts 'comment', 'trackback', 'pingback', 'pings'
		 *                                      (trackbacks and pingbacks), or 'all'. Default 'all'.
		 ...
		 *     @type array  $date_query         Date query clauses to limit comments by. See FP_Date_Query.
 ...
 * @return FP_Error|true FP_Error when a comment field exceeds the limit,
 *                       otherwise true.
 */
function wp_check_comment_data_max_lengths( $comment_data ) {
	...
	if ( isset( $comment_data['comment_author'] ) && mb_strlen( $comment_data['comment_author'], '8bit' ) > $max_lengths['comment_author'] ) {
		return new FP_Error( 'comment_author_column_length', __( '<strong>Error:</strong> Your name is too long.' ), 200 );
	}

	if ( isset( $comment_data['comment_author_email'] ) && strlen( $comment_data['comment_author_email'] ) > $max_lengths['comment_author_email'] ) {
		return new FP_Error( 'comment_author_email_column_length', __( '<strong>Error:</strong> Your email address is too long.' ), 200 );
	}

	if ( isset( $comment_data['comment_author_url'] ) && strlen( $comment_data['comment_author_url'] ) > $max_lengths['comment_author_url'] ) {
		return new FP_Error( 'comment_author_url_column_length', __( '<strong>Error:</strong> Your URL is too long.' ), 200 );
	}

	if ( isset( $comment_data['comment_content'] ) && mb_strlen( $comment_data['comment_content'], '8bit' ) > $max_lengths['comment_content'] ) {
		return new FP_Error( 'comment_content_column_length', __( '<strong>Error:</strong> Your comment is too long.' ), 200 );
	}
 ...
 *
 * @param array $comment_data Array of arguments for inserting a comment.
 * @return int|string|FP_Error The approval status on success (0|1|'spam'|'trash'),
 *                             FP_Error otherwise.
 */
function wp_check_comment_data( $comment_data ) {
	 ...
	 *
	 * @since 2.1.0
	 * @since 4.9.0 Returning a FP_Error value from the filter will short-circuit comment insertion
	 *              and allow skipping further processing.
	 *
	 * @param int|string|FP_Error $approved    The approval status. Accepts 1, 0, 'spam', 'trash',
	 *                                         or FP_Error.
	 * @param array               $commentdata Comment data.
	 */
 ...
 * @global wpdb $wpdb WordPress database abstraction object.
 *
 * @param int|FP_Comment $comment_id   Comment ID or FP_Comment object.
...
function wp_delete_comment( $comment_id, $force_delete = false ) {
	 ...
	 * @param FP_Comment $comment    The comment to be deleted.
	 */
	 ...
	 *
	 * @param string     $comment_id The comment ID as a numeric string.
	 * @param FP_Comment $comment    The deleted comment.
	 */
...
function wp_trash_comment( $comment_id ) {
	 ...
	 *
	 * @param string     $comment_id The comment ID as a numeric string.
	 * @param FP_Comment $comment    The comment to be trashed.
	 */
	...
	if ( wp_set_comment_status( $comment, 'trash' ) ) {
		 ...
		 *
		 * @param string     $comment_id The comment ID as a numeric string.
		 * @param FP_Comment $comment    The trashed comment.
		 */
...
function wp_untrash_comment( $comment_id ) {
	 ...
	 *
	 * @param string     $comment_id The comment ID as a numeric string.
	 * @param FP_Comment $comment    The comment to be untrashed.
	 */
	...
	if ( wp_set_comment_status( $comment, $status ) ) {
		 ...
		 *
		 * @param string     $comment_id The comment ID as a numeric string.
		 * @param FP_Comment $comment    The untrashed comment.
		 */
...
function wp_spam_comment( $comment_id ) {
	 ...
	 * @param FP_Comment $comment    The comment to be marked as spam.
	 */
	...
	if ( wp_set_comment_status( $comment, 'spam' ) ) {
		 ...
		 *
		 * @param int        $comment_id The comment ID.
		 * @param FP_Comment $comment    The comment marked as spam.
		 */
...
function wp_unspam_comment( $comment_id ) {
	 ...
	 *
	 * @param string     $comment_id The comment ID as a numeric string.
	 * @param FP_Comment $comment    The comment to be unmarked as spam.
	 */
	...
	if ( wp_set_comment_status( $comment, $status ) ) {
		 ...
		 *
		 * @param string     $comment_id The comment ID as a numeric string.
		 * @param FP_Comment $comment    The comment unmarked as spam.
		 */
 ...
 *
 * See {@link https://core.trac.finpress.org/ticket/9235}
 *
 * @since 1.5.0
 ...
 * }
 * @param bool  $wp_error Should errors be returned as FP_Error objects instead of
 *                        executing wp_die()? Default false.
 * @return int|false|FP_Error The ID of the comment on success, false or FP_Error on failure.
 */
 ...
 * @param bool           $wp_error       Whether to return a FP_Error object if there is a failure. Default false.
...
function wp_set_comment_status( $comment_id, $comment_status, $wp_error = false ) {
	...
	if ( ! $wpdb->update( $wpdb->comments, array( 'comment_approved' => $status ), array( 'comment_ID' => $comment_old->comment_ID ) ) ) {
		if ( $wp_error ) {
			return new FP_Error( 'db_update_error', __( 'Could not update comment status.' ), $wpdb->last_error );
		} else {
			return false;
		}
	}
 ...
 * @return int|false|FP_Error The value 1 if the comment was updated, 0 if not updated.
 *                            False or a FP_Error object on failure.
 */
function wp_update_comment( $commentarr, $wp_error = false ) {
	...
	if ( empty( $comment ) ) {
		if ( $wp_error ) {
			return new FP_Error( 'invalid_comment_id', __( 'Invalid comment ID.' ) );
		} else {
			return false;
		}
	}
	...
	if ( ! empty( $commentarr['comment_post_ID'] ) && ! get_post( $commentarr['comment_post_ID'] ) ) {
		if ( $wp_error ) {
			return new FP_Error( 'invalid_post_id', __( 'Invalid post ID.' ) );
		} else {
			return false;
		}
	}
	 ...
	 *
	 * @since 4.7.0
	 * @since 5.5.0 Returning a FP_Error value from the filter will short-circuit comment update
	 *              and allow skipping further processing.
	 *
	 * @param array|FP_Error $data       The new, processed comment data, or FP_Error.
	...
	if ( false === $result ) {
		if ( $wp_error ) {
			return new FP_Error( 'db_update_error', __( 'Could not update comment in the database.' ), $wpdb->last_error );
		} else {
			return false;
		}
	}
 ...
 * @global wpdb $wpdb WordPress database abstraction object.
 *
 * @param int|FP_Post $post Post ID or object to do trackbacks on.
...
function pingback( $content, $post ) {
	...
	require_once ABSPATH . FPINC . '/class-wp-http-ixr-client.php';
	...
	foreach ( (array) $post_links as $pagelinkedto ) {
		...
		if ( $pingback_server_url ) {
			...
			$client          = new FP_HTTP_IXR_Client( $pingback_server_url );
			$client->timeout = 3;
			/**
...
function weblog_ping( $server = '', $path = '' ) {
	...
	// Using a timeout of 3 seconds should be enough to cover slow servers.
	$client             = new FP_HTTP_IXR_Client( $server, ( ( ! strlen( trim( $path ) ) || ( '/' === $path ) ) ? false : $path ) );
	$client->timeout    = 3;
 ...
 *
 * @param FP_Comment[] $comments          Array of comment objects
 ...
 * @param FP_Query  $query Query object.
 * @return FP_Post[]
 */
 ...
 * }
 * @return FP_Comment|FP_Error A FP_Comment object on success, a FP_Error object on failure.
 */
function wp_handle_comment_submission( $comment_data ) {
	...
	if ( isset( $comment_data['comment_parent'] ) ) {
		...
		if (
			0 !== $comment_parent &&
			(
				! $comment_parent_object instanceof FP_Comment ||
			...
			return new FP_Error( 'comment_reply_to_unapproved_comment', __( 'Sorry, replies to unapproved comments are not allowed.' ), 403 );
		}
	}
	...
	if ( empty( $post->comment_status ) ) {
		...
		return new FP_Error( 'comment_id_not_found' );
	...
	if ( ! comments_open( $comment_post_id ) ) {
		...
		return new FP_Error( 'comment_closed', __( 'Sorry, comments are closed for this item.' ), 403 );
		...
		return new FP_Error( 'comment_on_trash' );
		...
		if ( current_user_can( 'read_post', $comment_post_id ) ) {
			return new FP_Error( 'comment_on_draft', __( 'Sorry, comments are not allowed for this item.' ), 403 );
		} else {
			return new FP_Error( 'comment_on_draft' );
		}
		...
		return new FP_Error( 'comment_on_password_protected' );
	...
	if ( $user->exists() ) {
		...
		if ( get_option( 'comment_registration' ) ) {
			return new FP_Error( 'not_logged_in', __( 'Sorry, you must be logged in to comment.' ), 403 );
		}
	}
	...
	if ( get_option( 'require_name_email' ) && ! $user->exists() ) {
		if ( '' === $comment_author_email || '' === $comment_author ) {
			return new FP_Error( 'require_name_email', __( '<strong>Error:</strong> Please fill the required fields.' ), 200 );
		} elseif ( ! is_email( $comment_author_email ) ) {
			return new FP_Error( 'require_valid_email', __( '<strong>Error:</strong> Please enter a valid email address.' ), 200 );
		}
	}
	...
	if ( '' === $comment_content && ! $allow_empty_comment ) {
		return new FP_Error( 'require_valid_comment', __( '<strong>Error:</strong> Please type your comment text.' ), 200 );
	}
	...
	if ( ! $comment_id ) {
		return new FP_Error( 'comment_save_error', __( '<strong>Error:</strong> The comment could not be saved. Please try again later.' ), 500 );
	}
...
function wp_register_comment_personal_data_exporter( $exporters ) {
	$exporters['finpress-comments'] = array(
...
function wp_register_comment_personal_data_eraser( $erasers ) {
	$erasers['finpress-comments'] = array(
...
function wp_comments_personal_data_eraser( $email_address, $page = 1 ) {
	...
	foreach ( (array) $comments as $comment ) {
		 ...
		 * @param FP_Comment  $comment            FP_Comment object.
