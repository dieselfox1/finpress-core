...
class FP_Query {
	 ...
	 *
	 * @since 3.1.0
	 * @var FP_Tax_Query|null A taxonomy query instance.
	 */
	public $tax_query;
	 ...
	 *
	 * @since 3.2.0
	 * @var FP_Meta_Query A meta query instance.
	 */
	 ...
	 *
	 * @since 3.7.0
	 * @var FP_Date_Query A date query instance.
	 */
	 ...
	 *
	 * @since 1.5.0
	 * @var FP_Term|FP_Post_Type|FP_Post|FP_User|null
	 */
	 ...
	 *     @type array           $date_query             An associative array of FP_Date_Query arguments.
	 *                                                   See FP_Date_Query::__construct().
	 *     @type int             $day                    Day of the month. Default empty. Accepts numbers 1-31.
	 *     @type bool            $exact                  Whether to search by exact keyword. Default false.
	 *     @type string          $fields                 Post fields to query for. Accepts:
	 *                                                   - '' Returns an array of complete post objects (`FP_Post[]`).
	 ...
	 *                                                   See FP_Meta_Query::__construct() for accepted values and default value.
	 ...
	 *                                                   See FP_Meta_Query::__construct() for accepted values and default value.
	 *     @type array           $meta_query             An associative array of FP_Meta_Query arguments.
	 *                                                   See FP_Meta_Query::__construct() for accepted values.
	 ...
	 *     @type array           $tax_query              An associative array of FP_Tax_Query arguments.
	 *                                                   See FP_Tax_Query::__construct().
	...
	public function parse_query( $query = '' ) {
		 ...
		 * @since 1.5.0
		 *
		 * @param FP_Query $query The FP_Query instance (passed by reference).
		 */
	...
	public function parse_tax_query( &$q ) {
		...
		$this->tax_query = new FP_Tax_Query( $tax_query );
		 ...
		 * @since 3.7.0
		 *
		 * @param FP_Query $query The FP_Query instance.
		 */
	...
	protected function parse_search( &$q ) {
		...
		/**
		 * Filters the columns to search in a FP_Query search.
		 *
		 ...
		 * @param FP_Query $query          The current FP_Query instance.
		 */
	...
	public function get_posts() {
		...
		// Parse meta query.
		$this->meta_query = new FP_Meta_Query();
		...
		switch ( $q['fields'] ) {
			...
			case '':
				 ...
				 *
				 * This is used in `FP_Query::the_post` to determine if the
		...
		if ( $date_parameters ) {
			$date_query = new FP_Date_Query( array( $date_parameters ) );
		...
		if ( ! empty( $q['date_query'] ) ) {
			$this->date_query = new FP_Date_Query( $q['date_query'] );
		...
		if ( ! $q['suppress_filters'] ) {
			/**
			 * Filters the search SQL that is used in the WHERE clause of FP_Query.
			 *
			 * @since 3.0.0
			 *
			 * @param string   $search Search SQL for WHERE clause.
			 * @param FP_Query $query  The current FP_Query object.
			 */
		...
		if ( $skip_post_status ) {
			...
			if ( ! empty( $queried_post_types ) ) {
				...
				foreach ( $queried_post_types as $queried_post_type ) {
					...
					if ( is_user_logged_in() && $queried_post_type_object instanceof FP_Post_Type ) {
		 ...
		 *
		 * See https://core.trac.finpress.org/ticket/56841.
		 * See https://github.com/WordPress/finpress-develop/pull/6393#issuecomment-2088217429
		 */
		$old_request =
		 ...
		 * the `found_posts` and `max_num_pages` properties of the FP_Query object,
		 * passed to the filter by reference. If FP_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 4.6.0
		 *
		 * @param FP_Post[]|int[]|null $posts Return an array of post data to short-circuit FP's query,
		...
		if ( null === $this->posts ) {
			...
			if ( $split_the_query ) {
				...
				// Beginning of the string is on a new line to prevent leading whitespace. See https://core.trac.finpress.org/ticket/56841.
				$this->request =
		...
		if ( ! empty( $this->posts ) && ( $this->is_single || $this->is_page ) ) {
			...
			if ( $this->is_preview && $this->posts && current_user_can( $edit_cap, $this->posts[0]->ID ) ) {
				 ...
				 * @since 2.7.0
				 *
				 * @param FP_Post  $post_preview  The Post object.
				 * @param FP_Query $query         The FP_Query instance (passed by reference).
				 */
		 ...
		 * of the type FP_Post and are filtered.
		 */
		if ( $this->posts ) {
			$this->post_count = count( $this->posts );
	...
	/**
	 * Iterates current comment index and returns FP_Comment object.
	 *
	 * @since 2.2.0
	 *
	 * @return FP_Comment Comment object.
	 */
	 ...
	 * @since 1.5.0
	 *
	 * @see FP_Query::parse_query() for all available arguments.
	 *
	 ...
	 * @since 1.5.0
	 *
	 * @return FP_Term|FP_Post_Type|FP_Post|FP_User|null The queried object.
	 */
	 ...
	 * @since 3.1.0
	 *
	 * @see FP_Query::is_category()
	 * @see FP_Query::is_tag()
	 * @see FP_Query::is_author()
	 * @see FP_Query::is_date()
	 * @see FP_Query::is_post_type_archive()
	 * @see FP_Query::is_tax()
	 *
	 ...
	 *
	 * Otherwise the same as {@see FP_Query::is_home()}.
	 *
	 * @since 3.1.0
	 *
	 ...
	 * @since 3.1.0
	 *
	 * @see FP_Query::is_single()
	 * @see FP_Query::is_singular()
	 *
	 ...
	 * @global int     $numpages
	 *
	 * @param FP_Post|object|int $post FP_Post instance or Post ID/object.
	...
	public function setup_postdata( $post ) {
		 ...
		 *
		 * @param FP_Post  $post  The Post object (passed by reference).
		 * @param FP_Query $query The current Query object (passed by reference).
		 */
