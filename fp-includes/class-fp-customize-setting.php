...
class FP_Customize_Setting {
	...
	public function preview() {
		...
		switch ( $this->type ) {
			...
			default:
				/**
				 * Fires when the FP_Customize_Setting::preview() method is called for settings
				 ...
				 * @since 3.4.0
				 *
				 * @param FP_Customize_Setting $setting FP_Customize_Setting instance.
				 */
	 ...
	 * that `FP_Customize_Setting::_multidimensional_preview_filter()`
	 * is called for this setting.
	 *
	 * @since 4.4.0
	 *
	 * @see FP_Customize_Manager::set_post_value()
	 * @see FP_Customize_Setting::_multidimensional_preview_filter()
	 */
	 ...
	 * @since 4.4.0
	 *
	 * @see FP_Customize_Setting::$aggregated_multidimensionals
	...
	final public function save() {
		...
		/**
		 * Fires when the FP_Customize_Setting::save() method is called.
		 *
	 ...
	 * @return string|array|null|FP_Error Sanitized value, or `null`/`FP_Error` if invalid.
	 */
	 ...
	 * @return true|FP_Error True if the input was validated, otherwise FP_Error.
	 */
	public function validate( $value ) {
		...
		if ( is_null( $value ) ) {
			return new FP_Error( 'invalid_value', __( 'Invalid value.' ) );
		}

		$validity = new FP_Error();

		/**
		 * Validates a Customize setting value.
		 *
		 * Plugins should amend the `$validity` object via its `FP_Error::add()` method.
		 *
		 * The dynamic portion of the hook name, `$this->ID`, refers to the setting ID.
		 *
		 * @since 4.6.0
		 *
		 * @param FP_Error             $validity Filtered from `true` to `FP_Error` when invalid.
		 * @param mixed                $value    Value of the setting.
		 * @param FP_Customize_Setting $setting  FP_Customize_Setting instance.
		 */
	...
	protected function get_root_value( $default_value = null ) {
		...
		if ( 'option' === $this->type ) {
			...
			/*
			 * Any FP_Customize_Setting subclass implementing aggregate multidimensional
	...
	protected function update( $value ) {
		...
		if ( 'option' === $this->type || 'theme_mod' === $this->type ) {
			...
			/**
			 * Fires when the FP_Customize_Setting::update() method is called for settings
			 * not handled as theme_mods or options.
			 *
	...
	public function value() {
		...
		if ( ! $is_core_type && ! $this->is_multidimensional_aggregated ) {
			 ...
			 * @param FP_Customize_Setting $setting       The setting instance.
			 */
