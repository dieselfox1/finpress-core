<?php
/**
 * Customize API: FP_Customize_Nav_Menu_Item_Setting class
 *
 * @package WordPress
 ...
 *
 * Subclass of FP_Customize_Setting to represent a nav_menu taxonomy term, and
 * the IDs for the nav_menu_items associated with the nav menu.
 *
 * @since 4.3.0
 *
 * @see FP_Customize_Setting
 */
class FP_Customize_Nav_Menu_Item_Setting extends FP_Customize_Setting {
	 ...
	 * @var int
	 *
	 * @see FP_Customize_Nav_Menu_Item_Setting::update()
	 * @see FP_Customize_Nav_Menu_Item_Setting::amend_customize_save_response()
	 */
	 ...
	 */
	public function __construct( FP_Customize_Manager $manager, $id, array $args = array() ) {
		if ( empty( $manager->nav_menus ) ) {
			throw new Exception( 'Expected FP_Customize_Manager::$nav_menus to be set.' );
		}
	 ...
	 * @since 4.3.0
	 *
	 * @see FP_Customize_Nav_Menu_Item_Setting::value()
	 */
	protected function populate_value() {
		...
		if ( ! isset( $this->value['nav_menu_term_id'] ) && $this->post_id > 0 ) {
			$menus = wp_get_post_terms(
				$this->post_id,
				FP_Customize_Nav_Menu_Setting::TAXONOMY,
				array(
					'fields' => 'ids',
				)
			);
	 ...
	 *
	 * @see FP_Customize_Manager::post_value()
	 *
	 ...
	 *
	 * @param FP_Post[] $items An array of menu item post objects.
	 * @param FP_Term   $menu  The menu object.
	 * @param array     $args  An array of arguments used to retrieve menu item objects.
	 * @return FP_Post[] Array of menu item objects.
	 */
	...
	/**
	 * Get the value emulated into a FP_Post and set up as a nav_menu_item.
	 *
	 * @since 4.3.0
	 *
	 * @return FP_Post With wp_setup_nav_menu_item() applied.
	 */
	 ...
	 * @return array|false|null|FP_Error Null or FP_Error if an input isn't valid. False if it is marked for deletion.
	...
	public function sanitize( $value ) {
		...
		if ( '' !== $menu_item_value['url'] ) {
			...
			if ( '' === $menu_item_value['url'] ) {
				return new FP_Error( 'invalid_url', __( 'Invalid URL.' ) ); // Fail sanitization if URL is invalid.
			}
		}
	 ...
	 *                           entirely. See FP_Customize_Nav_Menu_Item_Setting::$default for what the value
	 *                           should consist of.
	 * @return null|void
	 */
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				if ( false === $r ) {
					$this->update_error  = new FP_Error( 'delete_failure' );
			...
			if ( $value['nav_menu_term_id'] < 0 ) {
				...
				if ( ! $nav_menu_setting || ! ( $nav_menu_setting instanceof FP_Customize_Nav_Menu_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new FP_Error( 'unexpected_nav_menu_setting' );
					return;
				}

				if ( false === $nav_menu_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new FP_Error( 'nav_menu_setting_failure' );
					return;
				}

				if ( (int) $value['nav_menu_term_id'] !== $nav_menu_setting->previous_term_id ) {
					$this->update_status = 'error';
					$this->update_error  = new FP_Error( 'unexpected_previous_term_id' );
					return;
				}
			...
			if ( $value['menu_item_parent'] < 0 ) {
				...
				if ( ! $parent_nav_menu_item_setting || ! ( $parent_nav_menu_item_setting instanceof FP_Customize_Nav_Menu_Item_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new FP_Error( 'unexpected_nav_menu_item_setting' );
					return;
				}

				if ( false === $parent_nav_menu_item_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new FP_Error( 'nav_menu_item_setting_failure' );
					return;
				}

				if ( (int) $value['menu_item_parent'] !== $parent_nav_menu_item_setting->previous_post_id ) {
					$this->update_status = 'error';
					$this->update_error  = new FP_Error( 'unexpected_previous_post_id' );
					return;
				}
