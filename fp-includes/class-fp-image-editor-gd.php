 ...
 * @see FP_Image_Editor
 */
class FP_Image_Editor_GD extends FP_Image_Editor {
	/**
	 * GD Resource.
	 *
	 ...
	 * @since 3.5.0
	 *
	 * @return true|FP_Error True if loaded successfully; FP_Error on failure.
	 */
	...
	public function resize( $max_w, $max_h, $crop = false ) {
		...
		return new FP_Error( 'image_resize_error', __( 'Image resize failed.' ), $this->file );
	}
	 ...
	 *     @type string $1 The y crop position. Accepts 'top', 'center', or 'bottom'.
	 * }
	 * @return resource|GdImage|FP_Error
	 */
	protected function _resize( $max_w, $max_h, $crop = false ) {
		$dims = image_resize_dimensions( $this->size['width'], $this->size['height'], $max_w, $max_h, $crop );

		if ( ! $dims ) {
			return new FP_Error( 'error_getting_dimensions', __( 'Could not calculate resized image dimensions' ), $this->file );
		}
	...
	public function crop( $src_x, $src_y, $src_w, $src_h, $dst_w = null, $dst_h = null, $src_abs = false ) {
		...
		foreach ( array( $src_w, $src_h, $dst_w, $dst_h ) as $value ) {
			if ( ! is_numeric( $value ) || (int) $value <= 0 ) {
				return new FP_Error( 'image_crop_error', __( 'Image crop failed.' ), $this->file );
			}
		}
	...
	public function rotate( $angle ) {
		...
		return new FP_Error( 'image_rotate_error', __( 'Image rotate failed.' ), $this->file );
	}
	...
	public function flip( $horz, $vert ) {
		...
		return new FP_Error( 'image_flip_error', __( 'Image flip failed.' ), $this->file );
	}
	...
	protected function _save( $image, $filename = null, $mime_type = null ) {
		...
		if ( 'image/gif' === $mime_type ) {
			if ( ! $this->make_image( $filename, 'imagegif', array( $image, $filename ) ) ) {
				return new FP_Error( 'image_save_error', __( 'Image Editor Save Failed' ) );
			}
