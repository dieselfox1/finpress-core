 ...
 * manually, define `FP_PLUGIN_DIR` and `FP_PLUGIN_URL` in `wp-config.php`.
 *
 * @access private
 * @since 3.1.0
 *
...
function ms_site_check() {
	...
	if ( '1' === $blog->deleted ) {
		if ( file_exists( FP_CONTENT_DIR . '/blog-deleted.php' ) ) {
			return FP_CONTENT_DIR . '/blog-deleted.php';
		} else {
	...
	if ( '2' === $blog->deleted ) {
		if ( file_exists( FP_CONTENT_DIR . '/blog-inactive.php' ) ) {
			return FP_CONTENT_DIR . '/blog-inactive.php';
		} else {
	...
	if ( '1' === $blog->archived || '1' === $blog->spam ) {
		if ( file_exists( FP_CONTENT_DIR . '/blog-suspended.php' ) ) {
			return FP_CONTENT_DIR . '/blog-suspended.php';
		} else {
 ...
 *
 * {@internal In 4.4.0, converted to a wrapper for FP_Network::get_by_path()}
 *
 * @since 3.9.0
 *
 ...
 * @return FP_Network|false Network object if successful. False when no network is found.
 */
function get_network_by_path( $domain, $path, $segments = null ) {
	return FP_Network::get_by_path( $domain, $path, $segments );
}
 ...
 *
 * @since 3.9.0
 * @since 4.7.0 Updated to always return a `FP_Site` object.
 *
 * @param string   $domain   Domain to check.
 * @param string   $path     Path to check.
 * @param int|null $segments Path segments to use. Defaults to null, or the full path.
 * @return FP_Site|false Site object if successful. False when no site is found.
 */
function get_site_by_path( $domain, $path, $segments = null ) {
	 ...
	 * @since 3.9.0
	 *
	 * @param null|false|FP_Site $site     Site value to return by path. Default null
	...
	if ( null !== $pre ) {
		if ( false !== $pre && ! $pre instanceof FP_Site ) {
			$pre = new FP_Site( $pre );
		}
		return $pre;
	}
 ...
 * @global FP_Site    $current_blog The current site.
 *
 * @param string $domain    The requested domain.
...
function ms_load_current_site_and_network( $domain, $path, $subdomain = false ) {
	...
	if ( defined( 'DOMAIN_CURRENT_SITE' ) && defined( 'PATH_CURRENT_SITE' ) ) {
		...
		if ( empty( $current_site ) ) {
			$current_site = FP_Network::get_by_path( $domain, $path, 1 );
		}
		...
		$current_blog = get_site_by_path( $domain, $path, 1 );
		if ( $current_blog ) {
			$current_site = FP_Network::get_instance( $current_blog->site_id ? $current_blog->site_id : 1 );
		} else {
	...
	if ( $current_blog && (int) $current_blog->site_id !== $current_site->id ) {
		$current_site = FP_Network::get_instance( $current_blog->site_id );
	}
	...
	if ( empty( $current_blog ) ) {
		 ...
		 * @since 3.9.0
		 *
		 * @param FP_Network $current_site The network that had been determined.
...
function ms_not_installed( $domain, $path ) {
	...
	$msg .= sprintf(
		...
		__( 'https://developer.finpress.org/advanced-administration/debug/debug-network/' )
	);
 ...
 * @deprecated 3.9.0
 *
 * @global FP_Network $current_site
 *
 * @return FP_Network
 */
 ...
 * @return FP_Network|false Object containing network information if found, false if not.
 */
