...
function wp_get_speculation_rules_configuration(): ?array {
	...
	if (
		! isset( $config['mode'] ) ||
		'auto' === $config['mode'] ||
		! FP_Speculation_Rules::is_valid_mode( $config['mode'] )
	) {
	...
	if (
		...
		! FP_Speculation_Rules::is_valid_eagerness( $config['eagerness'] ) ||
 ...
 * {@see 'wp_load_speculation_rules'} action and amending the passed FP_Speculation_Rules object.
 *
 * @since 6.8.0
 * @access private
 *
 * @return FP_Speculation_Rules|null Object representing the speculation rules to use, or null if speculative loading
 *                                   is disabled in the current context.
 */
function wp_get_speculation_rules(): ?FP_Speculation_Rules {
	...
	$prefixer = new FP_URL_Pattern_Prefixer();
	...
	$speculation_rules = new FP_Speculation_Rules();
	 ...
	 * @since 6.8.0
	 *
	 * @param FP_Speculation_Rules $speculation_rules Object representing the speculation rules to use.
	 */
