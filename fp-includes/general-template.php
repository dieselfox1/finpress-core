...
function get_search_form( $args = array() ) {
	 ...
	 * @since 5.5.0 The `$args` parameter was added.
	 *
	 * @link https://core.trac.finpress.org/ticket/19321
	 *
 ...
 * @since 1.5.0
 *
 * @link https://core.trac.finpress.org/ticket/1458 Explanation of 'wp_meta' action.
 */
function wp_meta() {
	/**
...
function wp_title( $sep = '&raquo;', $display = true, $seplocation = '' ) {
	...
	$t_sep = '%FP_TITLE_SEP%'; // Temporary separator, for accurate flipping, if necessary.
...
function single_post_title( $prefix = '', $display = true ) {
	 ...
	 * @param FP_Post $_post       The current post.
	 */
...
function get_the_post_type_description() {
	 ...
	 * @param FP_Post_Type $post_type_obj The post type object.
	 */
...
function get_the_modified_date( $format = '', $post = null ) {
	 ...
	 * @param string           $format   PHP date format.
	 * @param FP_Post|null     $post     FP_Post object or null if no post is found.
	 */
 ...
 *                            was written. Accepts 'G', 'U', or PHP date format.
 *                            Defaults to the 'time_format' option.
 * @param int|FP_Post $post   Post ID or post object. Default is global `$post` object.
 ...
 * @since 5.3.0
 *
 * @param int|FP_Post $post   Optional. Post ID or post object. Default is global `$post` object.
 ...
 * @since 4.6.0
 *
 * @global FP_Scripts $wp_scripts The FP_Scripts object for printing scripts.
 * @global FP_Styles  $wp_styles  The FP_Styles object for printing styles.
 *
...
function wp_dependencies_unique_hosts() {
	...
	foreach ( array( $wp_scripts, $wp_styles ) as $dependencies ) {
		if ( $dependencies instanceof FP_Dependencies && ! empty( $dependencies->queue ) ) {
			foreach ( $dependencies->queue as $handle ) {
				if ( ! isset( $dependencies->registered[ $handle ] ) ) {
					continue;
				}

				/* @var _FP_Dependency $dependency */
 ...
 * _FP_Editors should not be used directly. See https://core.trac.finpress.org/ticket/17144.
 *
 * NOTE: Once initialized the TinyMCE editor cannot be safely moved in the DOM. For that reason
 * running wp_editor() inside of a meta box is not a good idea unless only Quicktags is used.
 * On the post edit screen several actions can be used to include additional editors
 * containing TinyMCE: 'edit_page_form', 'edit_form_advanced' and 'dbx_post_sidebar'.
 * See https://core.trac.finpress.org/ticket/19173 for more information.
 *
 * @see _FP_Editors::editor()
 * @see _FP_Editors::parse_settings()
 * @since 3.3.0
 *
 * @param string $content   Initial content for the editor.
 * @param string $editor_id HTML ID attribute value for the textarea and TinyMCE.
 *                          Should not contain square brackets.
 * @param array  $settings  See _FP_Editors::parse_settings() for description.
 */
function wp_editor( $content, $editor_id, $settings = array() ) {
	if ( ! class_exists( '_FP_Editors', false ) ) {
		require ABSPATH . FPINC . '/class-wp-editor.php';
	}
	_FP_Editors::editor( $content, $editor_id, $settings );
}
...
function wp_enqueue_editor() {
	...
	_FP_Editors::enqueue_default_editor();
}
 ...
 *     @type FP_Theme $theme      Theme being edited when on the theme file editor.
...
function wp_get_code_editor_settings( $args ) {
	$settings = array(
		...
		'jshint'     => array(
			// The following are copied from <https://github.com/WordPress/finpress-develop/blob/4.8.1/.jshintrc>.
			'boss'     => true,
			'curly'    => true,
			'eqeqeq'   => true,
			'eqnull'   => true,
 ...
 *     @type int    $total              The total amount of pages. Default is the value FP_Query's
 *                                      `max_num_pages` or 1.
 ...
 *
 * @see FP_Styles::_css_href() and its {@see 'style_loader_src'} filter.
 *
 * @since 2.3.0
 *
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		...
		case 'xhtml':
			...
			break;
		case 'atom':
			$gen = '<generator uri="https://finpress.org/" version="' . esc_attr( get_bloginfo_rss( 'version' ) ) . '">WordPress</generator>';
			break;
		case 'rss2':
			$gen = '<generator>' . sanitize_url( 'https://finpress.org/?v=' . get_bloginfo_rss( 'version' ) ) . '</generator>';
			break;
		case 'rdf':
			$gen = '<admin:generatorAgent rdf:resource="' . sanitize_url( 'https://finpress.org/?v=' . get_bloginfo_rss( 'version' ) ) . '" />';
			break;
		case 'comment':
