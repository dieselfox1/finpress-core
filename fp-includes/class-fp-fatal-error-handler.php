<?php
/**
 * Error Protection API: FP_Fatal_Error_Handler class
 *
 * @package WordPress
 * @since 5.2.0
 */
...
class FP_Fatal_Error_Handler {
	...
	public function handle() {
		if ( defined( 'FP_SANDBOX_SCRAPING' ) && FP_SANDBOX_SCRAPING ) {
			return;
		}
	 ...
	 *
	 * If no such drop-in is available, this will call {@see FP_Fatal_Error_Handler::display_default_error_template()}.
	 *
	 * @since 5.2.0
	 * @since 5.3.0 The `$handled` parameter was added.
	 *
	 * @param array         $error   Error information retrieved from `error_get_last()`.
	 * @param true|FP_Error $handled Whether Recovery Mode handled the fatal error.
	 */
	protected function display_error_template( $error, $handled ) {
		if ( defined( 'FP_CONTENT_DIR' ) ) {
			// Load custom PHP error template, if present.
			$php_error_pluggable = FP_CONTENT_DIR . '/php-error.php';
	...
	protected function display_default_error_template( $error, $handled ) {
		...
		if ( ! class_exists( 'FP_Error' ) ) {
			require_once ABSPATH . FPINC . '/class-wp-error.php';
		}
		...
		$message = sprintf(
			...
			__( 'https://finpress.org/documentation/article/faq-troubleshooting/' ),
