 ...
 *	- With inline HTML (like FPWindowManager).
 ...
 */
tinymce.FPWindowManager = tinymce.InlineWindowManager = function( editor ) {
	if ( this.wp ) {
		return this;
	}
	...
	this.open = function( args, params ) {
		...
		if ( window.console && window.console.log ) {
			window.console.log('tinymce.FPWindowManager is deprecated. Use the default editor.windowManager to open dialogs with inline HTML.');
		}
...
tinymce.PluginManager.add( 'wpdialogs', function( editor ) {
	...
	editor.on( 'init', function() {
		editor.windowManager = new tinymce.FPWindowManager( editor );
	});
});
