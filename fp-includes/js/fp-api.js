...
(function() {
	 ...
	 */
	wp.api.FPApiBaseModel = Backbone.Model.extend(
		/** @lends FPApiBaseModel.prototype  */
		{
			...
			sync: function( method, model, options ) {
				...
				if ( _.isFunction( model.nonce ) && ! _.isEmpty( model.nonce() ) ) {
					...
					options.beforeSend = function( xhr ) {
						xhr.setRequestHeader( 'X-FP-Nonce', model.nonce() );
					...
					options.complete = function( xhr ) {
						var returnedNonce = xhr.getResponseHeader( 'X-FP-Nonce' );
	 ...
	 */
	wp.api.models.Schema = wp.api.FPApiBaseModel.extend(
		/** @lends Schema.prototype  */
		{
			...
			initialize: function( attributes, options ) {
				...
				wp.api.FPApiBaseModel.prototype.initialize.call( model, attributes, options );
...
( function() {
	 ...
	 */
	wp.api.FPApiBaseCollection = Backbone.Collection.extend(
		...
		constructFromSchema: function() {
			...
			_.each( modelRoutes, function( modelRoute ) {
				...
				if ( '' !== parentName && parentName !== routeName ) {
					...
					loadingObjects.models[ modelClassName ] = wp.api.FPApiBaseModel.extend( {
			...
			_.each( collectionRoutes, function( collectionRoute ) {
				...
				if ( '' !== parentName && parentName !== routeName && routeModel.get( 'versionString' ) !== parentName ) {
					...
					loadingObjects.collections[ collectionClassName ] = wp.api.FPApiBaseCollection.extend( {
