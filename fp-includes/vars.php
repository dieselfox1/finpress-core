 ...
 * generate rewrite rules for it. See https://developer.finpress.org/advanced-administration/server/web-server/nginx/
 *
 * @package WordPress
 */
...
if ( is_admin() ) {
	// fp-admin pages are checked more carefully.
	if ( is_network_admin() ) {
		preg_match( '#/fp-admin/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} elseif ( is_user_admin() ) {
		preg_match( '#/fp-admin/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} else {
		preg_match( '#/fp-admin/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	}
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( ! str_ends_with( $pagenow, '.php' ) ) {
			$pagenow .= '.php'; // For `Options +Multiviews`: /fp-admin/themes/index.php (themes.php is queried).
		}
	}
} else {
