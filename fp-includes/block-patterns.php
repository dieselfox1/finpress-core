...
/**
 * Register Core's official patterns from finpress.org/patterns.
 *
 * @since 5.8.0
 ...
 *
 * @param FP_Screen $deprecated Unused. Formerly the screen that the current request was triggered from.
 */
function _load_remote_block_patterns( $deprecated = null ) {
	...
	if ( $supports_core_patterns && $should_load_remote ) {
		$request         = new FP_REST_Request( 'GET', '/wp/v2/pattern-directory/patterns' );
...
/**
 * Register `Featured` (category) patterns from finpress.org/patterns.
 *
 * @since 5.9.0
...
function _register_remote_theme_patterns() {
	...
	$patterns          = $response->get_data();
	$patterns_registry = FP_Block_Patterns_Registry::get_instance();
	foreach ( $patterns as $pattern ) {
 ...
 * @since 6.4.0 Uses the `FP_Theme::get_block_patterns` method.
 * @access private
 */
function _register_theme_block_patterns() {
	...
	foreach ( $themes as $theme ) {
		...
		foreach ( $patterns as $file => $pattern_data ) {
			...
			// phpcs:ignore WordPress.FP.I18n.NonSingularStringLiteralText,WordPress.FP.I18n.NonSingularStringLiteralDomain,WordPress.FP.I18n.LowLevelTranslationFunction
