 ...
 * @return FP_Site|void The blog object
 */
 ...
 * @return FP_Post|null FP_Post object on success, null on failure
 */
 ...
 * @return true|FP_Error True on success or a FP_Error object if the user doesn't exist
...
function add_user_to_blog( $blog_id, $user_id, $role ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new FP_Error( 'user_does_not_exist', __( 'The requested user does not exist.' ) );
	}
	 ...
	 * @since 4.9.0
	 *
	 * @param true|FP_Error $retval  True if the user should be added to the site, error
	 *                               object otherwise.
	...
	if ( true !== $can_add_user ) {
		...
		return new FP_Error( 'user_cannot_be_added', __( 'User cannot be added to this site.' ) );
	}
...
function remove_user_from_blog( $user_id, $blog_id = 0, $reassign = 0 ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new FP_Error( 'user_does_not_exist', __( 'That user does not exist.' ) );
	}
 ...
 *     @type FP_Error $errors        FP_Error object containing any errors found.
 * }
 */
 ...
 * @param FP_User|string $user       Optional. The user object to check against the new site name.
 ...
 *     @type string|FP_User $user       By default, an empty string. A user object if provided.
 *     @type FP_Error       $errors     FP_Error containing any errors found.
 * }
 */
function wpmu_validate_blog_signup( $blogname, $blog_title, $user = '' ) {
	...
	if ( username_exists( $blogname ) ) {
		if ( ! $user instanceof FP_User || $user->user_login !== $blogname ) {
 ...
 * @return array|FP_Error An array containing information about the activated user and/or blog.
 */
function wpmu_activate_signup(
	...
	if ( empty( $signup ) ) {
		return new FP_Error( 'invalid_key', __( 'Invalid activation key.' ) );
	}

	if ( $signup->active ) {
		if ( empty( $signup->domain ) ) {
			return new FP_Error( 'already_active', __( 'The user is already active.' ), $signup );
		} else {
			return new FP_Error( 'already_active', __( 'The site is already active.' ), $signup );
		}
	}
	...
	if ( ! $user_id ) {
		return new FP_Error( 'create_user', __( 'Could not create user' ), $signup );
	}
	...
	if ( empty( $signup->domain ) ) {
		...
		if ( isset( $user_already_exists ) ) {
			return new FP_Error( 'user_already_exists', __( 'That username is already activated.' ), $signup );
		}
 ...
 *                           Default 1.
 * @return int|FP_Error Returns FP_Error object on failure, the new site ID on success.
 */
function wpmu_create_blog( $domain, $path, $title, $user_id, $options = array(), $network_id = 1 ) {
	...
	if ( domain_exists( $domain, $path, $network_id ) ) {
		return new FP_Error( 'blog_taken', __( 'Sorry, that site already exists!' ) );
	}
 ...
 *
 * @param FP_Site|int $blog_id    The new site's object or ID.
...
Disable these notifications: %3$s'
	 ...
	 *
	 * @param string  $msg  Email body.
	 * @param FP_User $user FP_User instance of the new user.
	 */
...
function wpmu_new_site_admin_notification( $site_id, $user_id ) {
	 ...
	 * @param FP_Site $site Site object of the new site.
	 * @param FP_User $user User object of the administrator of the new site.
	 */
 ...
 * @since MU (3.0.0)
 *
 * @global FP_Network $current_site The current network.
 *
 * @return FP_Network The current network.
 */
...
function upload_is_file_too_big( $upload ) {
	if ( ! is_array( $upload ) || defined( 'FP_IMPORTING' ) || get_site_option( 'upload_space_check_disabled' ) ) {
		return $upload;
	}
 ...
 * added, as when a user is invited through the regular FP Add User interface.
 *
 * @since MU (3.0.0)
 */
 ...
 * }
 * @return true|FP_Error|void True on success or a FP_Error object if the user doesn't exist
...
function add_existing_user_to_blog( $details = false ) {
	if ( is_array( $details ) ) {
		 ...
		 *
		 * @param int           $user_id User ID.
		 * @param true|FP_Error $result  True on success or a FP_Error object if the user doesn't exist
		 *                               or could not be added.
		 */
 ...
 * @since MU (3.0.0)
 *
 * @param string|FP_User $user Optional. Defaults to current user. FP_User object,
