<?php
/**
 * Comment API: FP_Comment_Query class
 *
 * @package WordPress
 * @subpackage Comments
 * @since 4.4.0
 ...
 * @since 3.1.0
 *
 * @see FP_Comment_Query::__construct() for accepted arguments.
 */
#[AllowDynamicProperties]
class FP_Comment_Query {
	 ...
	 *                                                        stored in a `children` property on the `FP_Comment` object.
	...
	public function __construct( $query = '' ) {
		$this->query_var_defaults = array(
			...
			'meta_value'                => '',
			'meta_query'                => '',
			'date_query'                => null, // See FP_Date_Query.
			'hierarchical'              => false,
			'cache_domain'              => 'core',
	 ...
	 *
	 * @since 4.2.0 Extracted from FP_Comment_Query::query().
	 *
	 * @param string|array $query FP_Comment_Query arguments. See FP_Comment_Query::__construct() for accepted arguments.
	 */
	public function parse_query( $query = '' ) {
		 ...
		 * @since 4.2.0
		 *
		 * @param FP_Comment_Query $query The FP_Comment_Query instance (passed by reference).
		 */
	 ...
	 * @since 4.2.0 Moved parsing to FP_Comment_Query::parse_query().
	 *
	 ...
	 *
	 * @return int|int[]|FP_Comment[] List of comments or number of found comments if `$count` argument is true.
	 */
	public function get_comments() {
		 ...
		 * @since 3.1.0
		 *
		 * @param FP_Comment_Query $query Current instance of FP_Comment_Query (passed by reference).
		 */
		 ...
		 * - Otherwise the filter should return an array of FP_Comment objects.
		 *
		 * Note that if the filter returns an array of comment data, it will be assigned
		 * to the `comments` property of the current FP_Comment_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_comments` and `max_num_pages` properties of the FP_Comment_Query object,
		 * passed to the filter by reference. If FP_Comment_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 5.3.0
		 * @since 5.6.0 The returned array of comment data is assigned to the `comments` property
		 *              of the current FP_Comment_Query instance.
		 *
		 * @param array|int|null   $comment_data Return an array of comment data to short-circuit FP's comment query,
		 ...
		 * @since 3.1.0
		 *
		 * @param FP_Comment[]     $_comments An array of comments.
		 * @param FP_Comment_Query $query     Current instance of FP_Comment_Query (passed by reference).
		 */
		$_comments = apply_filters_ref_array( 'the_comments', array( $_comments, &$this ) );

		// Convert to FP_Comment instances.
	...
	protected function get_comment_ids() {
		...
		if ( ! empty( $this->query_vars['date_query'] ) && is_array( $this->query_vars['date_query'] ) ) {
			$this->date_query = new FP_Date_Query( $this->query_vars['date_query'], 'comment_date' );
	...
	private function set_found_comments() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param FP_Comment_Query $comment_query        The `FP_Comment_Query` instance.
			 */
	 ...
	 * @since 4.4.0
	 *
	 * @param FP_Comment[] $comments Array of top-level comments whose descendants should be filled in.
	 * @return array
	 */
