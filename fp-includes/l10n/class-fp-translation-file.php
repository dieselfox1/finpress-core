<?php
/**
 * I18N: FP_Translation_File class.
 *
 * @package WordPress
 * @subpackage I18N
 * @since 6.5.0
...
/**
 * Class FP_Translation_File.
 *
 * @since 6.5.0
 */
abstract class FP_Translation_File {
	/**
	 * List of headers.
	 *
	 * @since 6.5.0
	...
	/**
	 * Creates a new FP_Translation_File instance for a given file.
	 *
	 * @since 6.5.0
	 *
	 * @param string      $file     File name.
	 * @param string|null $filetype Optional. File type. Default inferred from file name.
	 * @return false|FP_Translation_File
	 */
	public static function create( string $file, ?string $filetype = null ) {
		...
		switch ( $filetype ) {
			case 'mo':
				return new FP_Translation_File_MO( $file );
			case 'php':
				return new FP_Translation_File_PHP( $file );
			default:
				return false;
		}
	}
	...
	public static function transform( string $file, string $filetype ) {
		...
		switch ( $filetype ) {
			case 'mo':
				$destination = new FP_Translation_File_MO( '' );
				break;
			case 'php':
				$destination = new FP_Translation_File_PHP( '' );
				break;
			default:
				return false;
		}
	 ...
	 * @since 6.5.0
	 *
	 * @param FP_Translation_File $source Source file.
	 * @return bool True on success, false otherwise.
	 */
	protected function import( FP_Translation_File $source ): bool {
