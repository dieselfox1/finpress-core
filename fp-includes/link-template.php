 ...
 *
 * @param int|FP_Post $post Optional. Post ID or post object. Default is the global `$post`.
 */
function the_permalink( $post = 0 ) {
	 ...
	 * @param int|FP_Post $post      Post ID, FP_Post object, or 0. Default 0.
	 */
 ...
 * @since 5.7.0
 *
 * @param FP_Post|int|null $post   Optional. Post ID or post object. Defaults to global $post.
...
function get_permalink( $post = 0, $leavename = false ) {
	 ...
	 * @param FP_Post $post      The post in question.
	...
	if (
		...
		if ( str_contains( $permalink, '%category%' ) ) {
			...
			if ( $cats ) {
				 ...
				 * @since 3.5.0
				 *
				 * @param FP_Term  $cat  The category to use in the permalink.
				 * @param array    $cats Array of all categories (FP_Term objects) associated with the post.
				 * @param FP_Post  $post The post in question.
				 */
 ...
 * @since 1.5.0
 *
 * @param int|FP_Post $post      Optional. Post ID or object. Default uses the global `$post`.
 ...
 * @since 2.5.0
 *
 * @param int|FP_Term|object $cat  The ID or category object whose feed link will be retrieved.
 * @param string             $feed Optional. Feed type. Possible values include 'rss2', 'atom'.
 *                                 Default is the value of get_default_feed().
 ...
 * @since 3.0.0
 *
 * @param int|FP_Term|object $term     The ID or term object whose feed link will be retrieved.
 ...
 * @since 2.3.0
 *
 * @param int|FP_Term|object $tag  The ID or term object whose feed link will be retrieved.
 * @param string             $feed Optional. Feed type. Possible values include 'rss2', 'atom'.
 *                                 Default is the value of get_default_feed().
 ...
 * @since 2.7.0
 *
 * @param int|FP_Term|object $tag      The ID or term object whose edit link will be retrieved.
 ...
 * @param FP_Term $tag    Optional. Term object. If null, the queried object will be inspected.
 *                        Default null.
 */
 ...
 *
 * @param int|FP_Term|object $term        The ID or term object whose edit link will be retrieved.
 ...
 * @param string           $before  Optional. Display before edit link. Default empty.
 * @param string           $after   Optional. Display after edit link. Default empty.
 * @param int|FP_Term|null $term    Optional. Term ID or object. If null, the queried object will be inspected. Default null.
...
function get_search_comments_feed_link( $search_query = '', $feed = '' ) {
	...
	/** This filter is documented in fp-includes/link-template.php */
 ...
 * @since 4.4.0
 *
 * @param int|FP_Post $post         Optional. Post ID or `FP_Post` object. Defaults to global `$post`.
 ...
 *
 * @param int|FP_Comment $comment_id Optional. Comment ID or FP_Comment object.
 ...
 * @return FP_Post|null|string Post object if successful. Null if global `$post` is not set.
...
function get_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param FP_Post      $post           FP_Post object.
	 */
...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param string         $format   Link anchor format.
	 * @param string         $link     Link permalink format.
	 * @param FP_Post|string $post     The adjacent post. Empty string if no corresponding post exists.
 ...
 * (e.g. fp-blog-header.php or the fp-admin/ folder) are accessible.
 *
...
function get_admin_url( $blog_id = null, $path = '', $scheme = 'admin' ) {
	$url = get_site_url( $blog_id, 'fp-admin/', $scheme );
...
function includes_url( $path = '', $scheme = null ) {
	 ...
	 * @param string      $path   Path relative to the URL to the fp-includes directory. Blank string
...
function content_url( $path = '' ) {
	$url = set_url_scheme( FP_CONTENT_URL );
...
function plugins_url( $path = '', $plugin = '' ) {
	...
	$mu_plugin_dir = wp_normalize_path( FPMU_PLUGIN_DIR );
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url( 'fp-admin/network/', $scheme );
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url( 'fp-admin/user/', $scheme );
 ...
 * @since 4.6.0
 *
 * @param int|FP_Post $post Optional. Post ID or object. Default is global `$post`.
...
function get_avatar_data( $id_or_email, $args = null ) {
	...
	} elseif ( $id_or_email instanceof FP_User ) {
		// User object.
		$user = $id_or_email;
	} elseif ( $id_or_email instanceof FP_Post ) {
		// Post object.
		$user = get_user_by( 'id', (int) $id_or_email->post_author );
	} elseif ( $id_or_email instanceof FP_Comment ) {
