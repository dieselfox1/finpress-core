<?php
/**
 * Taxonomy API: FP_Tax_Query class
 *
 * @package WordPress
 * @subpackage Taxonomy
 * @since 4.4.0
 ...
 * taxonomy terms.
 *
 * FP_Tax_Query is a helper that allows primary query classes, such as FP_Query, to filter
...
class FP_Tax_Query {
	 ...
	 *
	 * See FP_Tax_Query::__construct() for information on tax query arguments.
	 *
	 * @since 3.1.0
	 * @var array
	 */
	...
	public function sanitize_query( $queries ) {
		...
		foreach ( $queries as $key => $query ) {
			if ( 'relation' === $key ) {
				 ...
				 * $queried_terms array, for use in FP_Query.
				 */
	 ...
	 * Generates SQL clauses to be appended to a main query.
	 *
	 * Called by the public FP_Tax_Query::get_sql(), this method
	 ...
	 * join. In the case of FP_Tax_Query, this only applies to 'IN'
	...
	private function clean_query( &$query ) {
		if ( empty( $query['taxonomy'] ) ) {
			if ( 'term_taxonomy_id' !== $query['field'] ) {
				$query = new FP_Error( 'invalid_taxonomy', __( 'Invalid taxonomy.' ) );
				return;
			}
	 ...
	 * `$query` is converted to a FP_Error object.
	 *
	 * @since 3.2.0
	 *
	 * @param array  $query           The single query. Passed by reference.
	...
	public function transform_query( &$query, $resulting_field ) {
		...
		if ( 'AND' === $query['operator'] && count( $term_list ) < count( $query['terms'] ) ) {
			$query = new FP_Error( 'inexistent_terms', __( 'Inexistent terms.' ) );
			return;
		}
