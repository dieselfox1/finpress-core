 ...
 * Example:
 *
 *     $smilies = FP_Token_Map::from_array( array(
 *         '8O' => '😯',
 *         ':(' => '🙁',
 *         ':)' => '🙂',
 *         ':?' => '😕',
 ...
 *      FP_Token_Map::from_precomputed_table(
 *          array(
 ...
 *
 * Since the `FP_Token_Map` is designed for relatively static lookups, it can be
 ...
 *
 * This can be done with `FP_Token_Map::precomputed_php_source_table()`.
 *
 ...
 * Example:
 *
 *     $map = FP_Token_Map::from_array( array( 'simple_smile:' => '🙂', 'sob:' => '😭', 'soba:' => '🍜' ) );
...
class FP_Token_Map {
	 ...
	 *
	 * @return FP_Token_Map|null Token map, unless unable to create it.
	 */
	public static function from_array( array $mappings, int $key_length = 2 ): ?FP_Token_Map {
		$map             = new FP_Token_Map();
		 ...
		 */
		usort( $shorts, 'FP_Token_Map::longest_first_then_alphabetical' );
	 ...
	 * FP_Token_Map::precomputed_php_source_tag().
	 *
	 * @since 6.6.0
	 *
	 * @param array $state {
	 ...
	 * }
	 *
	 * @return FP_Token_Map Map with precomputed data loaded.
	 */
	public static function from_precomputed_table( $state ): ?FP_Token_Map {
		...
		if ( ! $has_necessary_state ) {
			_doing_it_wrong(
				__METHOD__,
				__( 'Missing required inputs to pre-computed FP_Token_Map.' ),
				'6.6.0'
			);
			return null;
		}
