...
function create_initial_post_types() {
	FP_Post_Type::reset_default_labels();
	...
	register_post_type(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'posts',
			'rest_controller_class' => 'FP_REST_Posts_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'media',
			'rest_controller_class' => 'FP_REST_Attachments_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'FP_REST_Menu_Items_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'blocks',
			'rest_controller_class' => 'FP_REST_Blocks_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rewrite'                         => false,
			'rest_base'                       => 'templates',
			'rest_controller_class'           => 'FP_REST_Templates_Controller',
			'autosave_rest_controller_class'  => 'FP_REST_Template_Autosaves_Controller',
			'revisions_rest_controller_class' => 'FP_REST_Template_Revisions_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class'           => 'FP_REST_Global_Styles_Controller',
			'revisions_rest_controller_class' => 'FP_REST_Global_Styles_Revisions_Controller',
			'late_route_registration'         => true,
			'capabilities'                    => array(
				'read'                   => 'edit_posts',
				'create_posts'           => 'edit_theme_options',
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'FP_REST_Font_Families_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'FP_REST_Font_Faces_Controller',
			'supports'              => array( 'title' ),
		)
	);
 ...
 *                       correspond to a FP_Post object, an associative array, or a numeric array,
 *                       respectively. Default OBJECT.
 * @return FP_Post[]|array[]|int[] Array of post objects, arrays, or IDs, depending on `$output`.
 */
 ...
 * @global FP_Post $post Global post object.
 *
 * @param int|FP_Post|null $post   Optional. Post ID or post object. `null`, `false`, `0` and other PHP falsey values
 ...
 * @return FP_Post|array|null Type corresponding to $output on success or null on failure.
 *                            When $output is OBJECT, a `FP_Post` instance is returned.
 */
function get_post( $post = null, $output = OBJECT, $filter = 'raw' ) {
	...
	if ( $post instanceof FP_Post ) {
		...
		if ( empty( $post->filter ) ) {
			...
			$_post = new FP_Post( $_post );
		} elseif ( 'raw' === $post->filter ) {
			$_post = new FP_Post( $post );
		} else {
			$_post = FP_Post::get_instance( $post->ID );
		}
	} else {
		$_post = FP_Post::get_instance( $post );
	}
 ...
 * @since 2.5.0
 *
 * @param int|FP_Post $post Post ID or post object.
 ...
 * @param int|FP_Post $post    Optional. Post ID or post object. Defaults to global $post.
...
function get_post_status( $post = null ) {
	...
	if ( ! $post instanceof FP_Post || ! isset( $post->filter ) || 'sample' !== $post->filter ) {
		$post = get_post( $post );
	}
...
function register_post_status( $post_status, $args = array() ) {
	...
	if ( false === $args->label_count ) {
		// phpcs:ignore WordPress.FP.I18n.NonSingularStringLiteralSingular,WordPress.FP.I18n.NonSingularStringLiteralPlural
 ...
 * @since 2.1.0
 *
 * @param int|FP_Post|null $post Optional. Post ID or post object. Default is global $post.
 ...
 *
 * @since 3.0.0
 * @since 4.6.0 Object returned is now an instance of `FP_Post_Type`.
 *
 ...
 * @return FP_Post_Type|null FP_Post_Type object if it exists, null otherwise.
 */
 ...
 * @return string[]|FP_Post_Type[] An array of post type names or objects.
 */
 ...
 * @since 4.6.0 Post type object returned is now an instance of `FP_Post_Type`.
 ...
 *     @type string       $rest_controller_class           REST API controller class name. Default is 'FP_REST_Posts_Controller'.
 *     @type string|bool  $autosave_rest_controller_class  REST API controller class name. Default is 'FP_REST_Autosaves_Controller'.
 *     @type string|bool  $revisions_rest_controller_class REST API controller class name. Default is 'FP_REST_Revisions_Controller'.
 ...
 *         @type bool   $with_front Whether the permastruct should be prepended with FP_Rewrite::$front.
 *                                  Default true.
 ...
 * }
 * @return FP_Post_Type|FP_Error The registered post type object on success,
 *                               FP_Error object on failure.
 */
function register_post_type( $post_type, $args = array() ) {
	...
	if ( empty( $post_type ) || strlen( $post_type ) > 20 ) {
		...
		return new FP_Error( 'post_type_length_invalid', __( 'Post type names must be between 1 and 20 characters in length.' ) );
	}

	$post_type_object = new FP_Post_Type( $post_type, $args );
	 ...
	 *
	 * @since 3.3.0
	 * @since 4.6.0 Converted the `$post_type` parameter to accept a `FP_Post_Type` object.
	 *
	 * @param string       $post_type        Post type.
	 * @param FP_Post_Type $post_type_object Arguments used to register the post type.
	 */
 ...
 * @return true|FP_Error True on success, FP_Error on failure or if the post type doesn't exist.
 */
function unregister_post_type( $post_type ) {
	global $wp_post_types;

	if ( ! post_type_exists( $post_type ) ) {
		return new FP_Error( 'invalid_post_type', __( 'Invalid post type.' ) );
	}
	...
	if ( $post_type_object->_builtin ) {
		return new FP_Error( 'invalid_post_type', __( 'Unregistering a built-in post type is not allowed' ) );
	}
 ...
 * @access private
 *
 * @param object|FP_Post_Type $post_type_object Post type object.
 * @return object Object with all the labels as member variables.
 */
function get_post_type_labels( $post_type_object ) {
	$nohier_vs_hier_defaults = FP_Post_Type::get_default_labels();
 ...
 *
 * @param string|FP_Post_Type $post_type Post type name or object.
...
function is_post_type_viewable( $post_type ) {
	 ...
	 * @param FP_Post_Type $post_type   Post type object.
	 */
 ...
 * @since 5.7.0
 *
 * @param int|FP_Post|null $post Optional. Post ID or post object. Defaults to global $post.
 ...
 * @since 6.8.0
 *
 * @param int|FP_Post|null $post Optional. Post ID or `FP_Post` object. Defaults to global $post.
 ...
 * {@link https://developer.finpress.org/reference/classes/wp_query/
 * FP_Query} documentation in the Developer Handbook.
 *
 ...
 *
 * @see FP_Query
 * @see FP_Query::parse_query()
 *
 * @param array $args {
 *     Optional. Arguments to retrieve posts. See FP_Query::parse_query() for all available arguments.
 *
 *     @type int        $numberposts      Total number of posts to retrieve. Is an alias of `$posts_per_page`
 *                                        in FP_Query. Accepts -1 for all. Default 5.
 *     @type int|string $category         Category ID or comma-separated list of IDs (this or any children).
 *                                        Is an alias of `$cat` in FP_Query. Default 0.
 *     @type int[]      $include          An array of post IDs to retrieve, sticky posts will be included.
 *                                        Is an alias of `$post__in` in FP_Query. Default empty array.
 *     @type int[]      $exclude          An array of post IDs not to retrieve. Default empty array.
 *     @type bool       $suppress_filters Whether to suppress filters. Default true.
 * }
 * @return FP_Post[]|int[] Array of post objects or post IDs.
 */
 ...
 * @see sanitize_post_field()
 *
 * @param object|FP_Post|array $post    The post object or array
 * @param string               $context Optional. How to sanitize post fields.
 *                                      Accepts 'raw', 'edit', 'db', 'display',
 *                                      'attribute', or 'js'. Default 'display'.
 * @return object|FP_Post|array The now sanitized post object or array (will be the
 *                              same type as `$post`).
 */
 ...
 *                           Default false.
 * @return FP_Post|false|null Post data on success, false or null on failure.
 */
function wp_delete_post( $post_id = 0, $force_delete = false ) {
	 ...
	 * @since 4.4.0
	 *
	 * @param FP_Post|false|null $delete       Whether to go forward with deletion.
	 * @param FP_Post            $post         Post object.
 ...
 *                       See FP_Term_Query::__construct() for supported arguments.
 * @return array|FP_Error List of categories. If the `$fields` argument passed via `$args` is 'all' or
 *                        'all_with_object_id', an array of FP_Term objects will be returned. If `$fields`
 *                        is 'ids', an array of category IDs. If `$fields` is 'names', an array of category names.
 *                        FP_Error object if 'category' taxonomy doesn't exist.
 */
 ...
 *                       See FP_Term_Query::__construct() for supported arguments.
 * @return array|FP_Error Array of FP_Term objects on success or empty array if no tags were found.
 *                        FP_Error object if 'post_tag' taxonomy doesn't exist.
 */
 ...
 * @param array           $args     {
 *     Optional. Term query parameters. See FP_Term_Query::__construct() for supported arguments.
 *
 *     @type string $fields Term fields to retrieve. Default 'all'.
 * }
 * @return array|FP_Error Array of FP_Term objects on success or empty array if no terms were found.
 *                        FP_Error object if `$taxonomy` doesn't exist.
 */
 ...
 *                       correspond to a FP_Post object or an associative array, respectively.
 *                       Default ARRAY_A.
 ...
 *
 * @since 1.0.0
 * @since 2.6.0 Added the `$wp_error` parameter to allow a FP_Error to be returned on failure.
 ...
 * }
 * @param bool  $wp_error         Optional. Whether to return a FP_Error on failure. Default false.
 * @param bool  $fire_after_hooks Optional. Whether to fire the after insert hooks. Default true.
 * @return int|FP_Error The post ID on success. The value 0 or FP_Error on failure.
 */
function wp_insert_post( $postarr, $wp_error = false, $fire_after_hooks = true ) {
	...
	if ( ! empty( $postarr['ID'] ) ) {
		...
		if ( is_null( $post_before ) ) {
			if ( $wp_error ) {
				return new FP_Error( 'invalid_post', __( 'Invalid post ID.' ) );
			}
			return 0;
		}
	 ...
	 * the new post being inserted and return 0. If $wp_error is true, a FP_Error
	...
	if ( apply_filters( 'wp_insert_post_empty_content', $maybe_empty, $postarr ) ) {
		if ( $wp_error ) {
			return new FP_Error( 'empty_content', __( 'Content, title, and excerpt are empty.' ) );
		} else {
			return 0;
		}
	}
	...
	if ( ! $post_date ) {
		if ( $wp_error ) {
			return new FP_Error( 'invalid_date', __( 'Invalid date.' ) );
		} else {
			return 0;
		}
	}
	...
	if ( $update ) {
		...
		if ( false === $wpdb->update( $wpdb->posts, $data, $where ) ) {
			if ( $wp_error ) {
				...
				return new FP_Error( 'db_update_error', $message, $wpdb->last_error );
			} else {
				return 0;
			}
		}
		...
		if ( false === $wpdb->insert( $wpdb->posts, $data ) ) {
			if ( $wp_error ) {
				...
				return new FP_Error( 'db_insert_error', $message, $wpdb->last_error );
			} else {
				return 0;
			}
		}
	...
	if ( ! empty( $postarr['page_template'] ) ) {
		...
		if ( 'default' !== $postarr['page_template'] && ! isset( $page_templates[ $postarr['page_template'] ] ) ) {
			if ( $wp_error ) {
				return new FP_Error( 'invalid_page_template', __( 'Invalid page template.' ) );
			}
	...
	if ( 'attachment' !== $postarr['post_type'] ) {
		...
		if ( $update ) {
			 ...
			 *
			 * @param int     $post_id      Post ID.
			 * @param FP_Post $post_after   Post object following the update.
			 * @param FP_Post $post_before  Post object before the update.
			 */
 ...
 *
 * @since 1.0.0
 * @since 3.5.0 Added the `$wp_error` parameter to allow a FP_Error to be returned on failure.
 * @since 5.6.0 Added the `$fire_after_hooks` parameter.
 *
 ...
 * @return array|false|FP_Error Array of affected term IDs. FP_Error or false on failure.
 */
 ...
 * @return array|false|FP_Error Array of term taxonomy IDs of affected terms. FP_Error or false on failure.
 */
 ...
 * @return array|false|FP_Error Array of term taxonomy IDs of affected categories. FP_Error or false on failure.
 */
 ...
 * @since 5.6.0
 *
 * @param int|FP_Post  $post        The post ID or object that has been saved.
 * @param bool         $update      Whether this is an existing post being updated.
 * @param null|FP_Post $post_before Null for new posts, the FP_Post object prior
 ...
 *
 * @since 1.5.0
 * @since 4.7.0 `$post` can be a FP_Post object.
 ...
 * @since 4.7.0 `$post` can be a FP_Post object.
 *
 * @param int|FP_Post $post Post ID or object.
 ...
 *
 * @param int|FP_Post $page   Page object or page ID. Passed by reference.
 * @param string      $output Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                            correspond to a FP_Post object, an associative array, or a numeric array,
 *                            respectively. Default OBJECT.
 * @param string      $filter Optional. How the return value should be filtered. Accepts 'raw',
 *                            'edit', 'db', 'display'. Default 'raw'.
 * @return FP_Post|array|null FP_Post or array on success, null on failure.
 */
 ...
 * @return FP_Post|array|null FP_Post (or array) on success, or null on failure.
 */
 ...
 * @param FP_Post[] $pages   List of page objects from which descendants should be identified.
 * @return FP_Post[] List of page children.
 */
 ...
 * @since 2.0.0
 *
 * @param FP_Post[] $pages   Posts array (passed by reference).
 ...
 *
 * @param FP_Post|object|int $page Optional. Page ID or FP_Post object. Default is global $post.
 * @return string|false Page URI, false on error.
 */
function get_page_uri( $page = 0 ) {
	if ( ! $page instanceof FP_Post ) {
		$page = get_post( $page );
	}
	 ...
	 * @param FP_Post $page Page object.
	 */
 ...
 *
 * @since 1.5.0
 * @since 6.3.0 Use FP_Query internally.
 *
 * @param array|string $args {
 ...
 *                                      Default 'publish'.
 * }
 * @return FP_Post[]|false Array of pages (or hierarchical post type items). Boolean false if the
...
function get_pages( $args = array() ) {
	 ...
	 * Additionally to `FP_Query`, it has been supporting the `post_modified_gmt` field, so this logic will translate
	...
	/**
	 * Filters query arguments passed to FP_Query in get_pages.
	 *
	 * @since 6.3.0
	 *
	 * @param array $query_args  Array of arguments passed to FP_Query.
	 ...
	 * @since 2.1.0
	 *
	 * @param FP_Post[] $pages       Array of page objects.
	 * @param array     $parsed_args Array of get_pages() arguments.
	 */
 ...
 *
 * @since 2.0.0
 * @since 4.7.0 Added the `$wp_error` parameter to allow a FP_Error to be returned on failure.
 * @since 5.6.0 Added the `$fire_after_hooks` parameter.
 *
 * @see wp_insert_post()
 *
 ...
 * @param bool         $wp_error         Optional. Whether to return a FP_Error on failure. Default false.
 * @param bool         $fire_after_hooks Optional. Whether to fire the after insert hooks. Default true.
 * @return int|FP_Error The attachment ID on success. The value 0 or FP_Error on failure.
 */
 ...
 * @param int|FP_Post $post Optional. Attachment ID or object. Default is global $post.
 ...
 *              allowed FP_Post object to be passed.
 *
 * @param int|FP_Post $post Optional. Attachment ID or object. Default is global $post.
...
function wp_mime_type_icon( $mime = 0, $preferred_ext = '.png' ) {
	...
	if ( empty( $icon ) ) {
		...
		if ( ! is_array( $icon_files ) ) {
			 ...
			 */
			$icon_dir = apply_filters( 'icon_dir', ABSPATH . FPINC . '/images/media' );
 ...
 * @param int     $post_id     Post ID.
 * @param FP_Post $post        The post object.
 * @param FP_Post $post_before The previous post object.
 */
 ...
 * @since 1.5.1
 *
 * @param FP_Post[] $posts Array of post objects (passed by reference).
 */
 ...
 *
 * @param int|FP_Post $post Post ID or post object to remove from the cache.
 */
 ...
 * @since 6.1.0
 *
 * @param FP_Post[] $posts Array of post objects.
 */
 ...
 *
 * Uses XMLRPC_REQUEST and FP_IMPORTING constants.
 *
 * @since 2.3.0
 * @access private
 *
 ...
 * @since 3.1.0
 *
 * @param int|FP_Post $post         Post ID or post object where thumbnail should be attached.
 ...
 * @since 3.3.0
 *
 * @param int|FP_Post $post Post ID or post object from which the thumbnail should be removed.
 * @return bool True on success, false on failure.
 */
 ...
 * @since 4.5.0
 *
 * @param FP_Post[] $posts Array of FP_Post objects.
 */
 ...
 *
 * @param int|FP_Post $post Post ID or FP_Post object.
...
function use_block_editor_for_post( $post ) {
	 ...
	 * @param FP_Post $post             The post being checked.
	 */
