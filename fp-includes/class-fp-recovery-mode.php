<?php
/**
 * Error Protection API: FP_Recovery_Mode class
 *
 * @package WordPress
 * @since 5.2.0
 */
...
class FP_Recovery_Mode {
	 ...
	 *
	 * @since 5.2.0
	 * @var FP_Recovery_Mode_Cookie_Service
	 */
	 ...
	 *
	 * @since 5.2.0
	 * @var FP_Recovery_Mode_Key_Service
	 */
	private $key_service;
	 ...
	 *
	 * @since 5.2.0
	 * @var FP_Recovery_Mode_Link_Service
	 */
	 ...
	 *
	 * @since 5.2.0
	 * @var FP_Recovery_Mode_Email_Service
	 */
	...
	/**
	 * FP_Recovery_Mode constructor.
	 *
	 * @since 5.2.0
	 */
	public function __construct() {
		$this->cookie_service = new FP_Recovery_Mode_Cookie_Service();
		$this->key_service    = new FP_Recovery_Mode_Key_Service();
		$this->link_service   = new FP_Recovery_Mode_Link_Service( $this->cookie_service, $this->key_service );
		$this->email_service  = new FP_Recovery_Mode_Email_Service( $this->link_service );
	}
	...
	public function initialize() {
		...
		if ( defined( 'FP_RECOVERY_MODE_SESSION_ID' ) ) {
			$this->is_active  = true;
			$this->session_id = FP_RECOVERY_MODE_SESSION_ID;
	 ...
	 *
	 * This will not change after recovery mode has been initialized. {@see FP_Recovery_Mode::run()}.
	 *
	 * @since 5.2.0
	 *
	 ...
	 * @return true|FP_Error|void True if the error was handled and headers have already been sent.
	 *                            Or the request will exit to try and catch multiple errors at once.
	 *                            FP_Error if an error occurred preventing it from being handled.
	 */
	public function handle_error( array $error ) {
		...
		if ( ! $extension || $this->is_network_plugin( $extension ) ) {
			return new FP_Error( 'invalid_source', __( 'Error not caused by a plugin or theme.' ) );
		}

		if ( ! $this->is_active() ) {
			if ( ! is_protected_endpoint() ) {
				return new FP_Error( 'non_protected_endpoint', __( 'Error occurred on a non-protected endpoint.' ) );
			}

			if ( ! function_exists( 'wp_generate_password' ) ) {
				require_once ABSPATH . FPINC . '/pluggable.php';
			}
		...
		if ( ! $this->store_error( $error ) ) {
			return new FP_Error( 'storage_error', __( 'Failed to store the error.' ) );
		}
	...
	protected function get_extension_for_error( $error ) {
		...
		$wp_plugin_dir = wp_normalize_path( FP_PLUGIN_DIR );
