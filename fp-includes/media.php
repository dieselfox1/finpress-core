...
/**
 * Retrieves calculated resize dimensions for use in FP_Image_Editor.
 *
...
function wp_get_attachment_image( $attachment_id, $size = 'thumbnail', $icon = false, $attr = '' ) {
	...
	if ( $image ) {
		 ...
		 * @param FP_Post      $attachment Image attachment post.
		 * @param string|int[] $size       Requested image size. Can be any registered image size name, or
		 *                                 an array of width and height values in pixels (in that order).
		 */
...
function wp_img_tag_add_auto_sizes( string $image ): string {
	...
	$processor = new FP_HTML_Tag_Processor( $image );
 ...
 * @see https://core.trac.finpress.org/ticket/62413
 */
...
function wp_iframe_tag_add_loading_attr( $iframe, $context ) {
	...
	$optimization_attrs = wp_get_loading_optimization_attributes(
		...
		array(
			 ...
			 * TODO: Use FP_HTML_Tag_Processor to extract actual values once support is
			 * added.
			 */
 ...
 * @since 5.1.0 Code cleanup for FPCS 1.0.0 coding standards.
 ...
 * @since 3.9.0
 *
 * @param FP_Post $attachment The current attachment, provided for context.
...
function wp_get_attachment_id3_keys( $attachment, $context = 'display' ) {
	 ...
	 * @param FP_Post $attachment Attachment object.
 ...
 *                                     Default is 'names'.
 * @return string[]|FP_Taxonomy[] List of taxonomies or taxonomy names. Empty array on failure.
 */
 ...
 *                       Default 'names'.
 * @return string[]|FP_Taxonomy[] Array of names or objects of registered taxonomies for attachments.
 */
...
/**
 * Returns a FP_Image_Editor instance and loads file into it.
 *
 * @since 3.5.0
 *
 ...
 *                     Default empty array.
 * @return FP_Image_Editor|FP_Error The FP_Image_Editor object on success,
 *                                  a FP_Error object otherwise.
 */
function wp_get_image_editor( $path, $args = array() ) {
	...
	return new FP_Error( 'image_no_editor', __( 'No editor could be selected.' ) );
}
...
function _wp_image_editor_choose( $args = array() ) {
	require_once ABSPATH . FPINC . '/class-wp-image-editor.php';
	require_once ABSPATH . FPINC . '/class-wp-image-editor-gd.php';
	require_once ABSPATH . FPINC . '/class-wp-image-editor-imagick.php';
	require_once ABSPATH . FPINC . '/class-avif-info.php';
	/**
	 ...
	 *                                'FP_Image_Editor_Imagick', 'FP_Image_Editor_GD'.
	 */
	$implementations = apply_filters( 'wp_image_editors', array( 'FP_Image_Editor_Imagick', 'FP_Image_Editor_GD' ) );
 ...
 * @since 3.5.0
 *
 * @param int|FP_Post $attachment Attachment ID or object.
 * @return array|void {
...
function wp_prepare_attachment_for_js( $attachment ) {
	...
	$author = new FP_User( $attachment->post_author );
 ...
 *
 *     @type int|FP_Post $post Post ID or post object.
 * }
 */
function wp_enqueue_media( $args = array() ) {
	 ...
	 *
	 * @link https://core.trac.finpress.org/ticket/31071
	 *
 ...
 * @return FP_Post[] Array of media attached to the given post.
 */
function get_attached_media( $type, $post = 0 ) {
	 ...
	 * @since 3.6.0
	 *
	 * @param FP_Post[] $children Array of media attached to the given post.
...
function wp_register_media_personal_data_exporter( $exporters ) {
	$exporters['finpress-media'] = array(
...
function wp_getimagesize( $filename, ?array &$image_info = null ) {
	...
	if ( defined( 'FP_DEBUG' ) && FP_DEBUG && ! defined( 'FP_RUN_CORE_TESTS' ) ) {
		 ...
		 *
		 * See https://core.trac.finpress.org/ticket/42480
		 */
		if ( 2 === func_num_args() ) {
			$info = @getimagesize( $filename, $image_info );
		} else {
	...
	if ( wp_is_heic_image_mime_type( $image_mime_type ) ) {
		...
		if ( $editor instanceof FP_Image_Editor_Imagick ) {
...
function wp_get_loading_optimization_attributes( $tag_name, $attr, $context ) {
	...
	if ( null === $maybe_in_viewport ) {
		$header_enforced_contexts = array(
			'template_part_' . FP_TEMPLATE_PART_AREA_HEADER => true,
...
function wp_get_image_editor_output_format( $filename, $mime_type ) {
	 ...
	 *
	 * @see FP_Image_Editor::get_output_format()
	 *
	 * @since 5.8.0
