 ...
 * @since 3.0.0
 *
 * @param int|string|FP_Term $menu Menu ID, slug, name, or object.
 * @return FP_Term|false Menu object on success, false if $menu param isn't supplied or term does not exist.
 */
function wp_get_nav_menu_object( $menu ) {
	 ...
	 * @since 4.3.0
	 *
	 * @param FP_Term|false      $menu_obj Term from nav_menu taxonomy, or false if nothing had been found.
	 * @param int|string|FP_Term $menu     The menu ID, slug, name, or object passed to wp_get_nav_menu_object().
	 */
 ...
 * @since 3.0.0
 *
 * @param int|string|FP_Term $menu Menu ID, slug, name, or object of menu to check.
 ...
 *
 * @param string $menu_name Menu name.
 * @return int|FP_Error Menu ID on success, FP_Error object on failure.
 */
 ...
 *
 * @param int|string|FP_Term $menu Menu ID, slug, name, or object.
 * @return bool|FP_Error True on success, false or FP_Error object on failure.
 */
 ...
 * @return int|FP_Error The menu item's database ID or FP_Error object on failure.
 */
function wp_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array(), $fire_after_hooks = true ) {
	...
	if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) ) {
		return new FP_Error( 'update_nav_menu_item_failed', __( 'The given object ID is not that of a menu item.' ) );
	}

	$menu = wp_get_nav_menu_object( $menu_id );

	if ( ! $menu && 0 !== $menu_id ) {
		return new FP_Error( 'invalid_menu_id', __( 'Invalid menu ID.' ) );
	}
	...
	if ( 'custom' === $args['menu-item-type'] ) {
		...
		if ( 'taxonomy' === $args['menu-item-type'] ) {
			...
			if ( $original_object instanceof FP_Term ) {
			...
			if ( $original_object instanceof FP_Post ) {
			...
			if ( $original_object instanceof FP_Post_Type ) {
		...
		// Hack to get fp to create a post object when too many properties are empty.
 ...
 *                    Default empty array.
 * @return FP_Term[] An array of menu objects.
 */
function wp_get_nav_menus( $args = array() ) {
	 ...
	 * @see get_terms()
	 *
	 * @param FP_Term[] $menus An array of menu objects.
 ...
 *
 * @link https://core.trac.finpress.org/ticket/13958
 *
 * @since 3.2.0
 * @access private
 *
 ...
 * @since 6.1.0
 *
 * @param FP_Post[] $menu_items Array of menu item post objects.
 */
...
function wp_setup_nav_menu_item( $menu_item ) {
	...
	if ( isset( $menu_item->post_type ) ) {
		if ( 'nav_menu_item' === $menu_item->post_type ) {
			...
			/** This filter is documented in fp-includes/nav-menu.php */
 ...
 * @param FP_Post $post       The post object being transitioned from one status to another.
 */
