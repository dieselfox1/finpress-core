 ...
 * @since 4.6.0
 *
 * @see FP_Site_Query::__construct() for accepted arguments.
 */
...
class FP_Site_Query {
	 ...
	 *                                                   Default false.
	 *     @type array           $date_query             Date query clauses to limit sites by. See FP_Date_Query.
	 *                                                   Default null.
	 ...
	 * @since 4.6.0
	 *
	 * @see FP_Site_Query::__construct()
	 *
	 * @param string|array $query Array or string of FP_Site_Query arguments. See FP_Site_Query::__construct().
	 */
	public function parse_query( $query = '' ) {
		 ...
		 * @since 4.6.0
		 *
		 * @param FP_Site_Query $query The FP_Site_Query instance (passed by reference).
		 */
	...
	public function get_sites() {
		 ...
		 * @since 4.6.0
		 *
		 * @param FP_Site_Query $query Current instance of FP_Site_Query (passed by reference).
		 */
		 ...
		 * - Otherwise the filter should return an array of FP_Site objects.
		 *
		 * Note that if the filter returns an array of site data, it will be assigned
		 * to the `sites` property of the current FP_Site_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_sites` and `max_num_pages` properties of the FP_Site_Query object,
		 * passed to the filter by reference. If FP_Site_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 5.2.0
		 * @since 5.6.0 The returned array of site data is assigned to the `sites` property
		 *              of the current FP_Site_Query instance.
		 *
		 * @param FP_Site[]|int[]|int|null $site_data Return an array of site data to short-circuit FP's site query,
		 ...
		 * @since 4.6.0
		 *
		 * @param FP_Site[]     $_sites An array of FP_Site objects.
		 * @param FP_Site_Query $query  Current instance of FP_Site_Query (passed by reference).
		 */
		$_sites = apply_filters_ref_array( 'the_sites', array( $_sites, &$this ) );

		// Convert to FP_Site instances.
	...
	protected function get_site_ids() {
		...
		if ( strlen( $this->query_vars['search'] ) ) {
			...
			/**
			 * Filters the columns to search in a FP_Site_Query search.
			 *
			 ...
			 * @param FP_Site_Query $query          The current FP_Site_Query instance.
			 */
		...
		if ( ! empty( $date_query ) && is_array( $date_query ) ) {
			$this->date_query = new FP_Date_Query( $date_query, 'registered' );
	...
	private function set_found_sites() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param FP_Site_Query $site_query        The `FP_Site_Query` instance.
			 */
