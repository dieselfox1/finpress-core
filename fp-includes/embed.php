...
/**
 * Returns the initialized FP_oEmbed object.
 *
 * @since 2.9.0
 * @access private
 *
 * @return FP_oEmbed object.
 */
function _wp_oembed_get_object() {
	static $wp_oembed = null;

	if ( is_null( $wp_oembed ) ) {
		$wp_oembed = new FP_oEmbed();
	}
	return $wp_oembed;
}
...
function wp_oembed_add_provider( $format, $provider, $regex = false ) {
	if ( did_action( 'plugins_loaded' ) ) {
		...
		FP_oEmbed::_add_provider_early( $format, $provider, $regex );
	}
}
...
function wp_oembed_remove_provider( $format ) {
	if ( did_action( 'plugins_loaded' ) ) {
		...
		FP_oEmbed::_remove_provider_early( $format );
	}
...
function wp_oembed_register_route() {
	$controller = new FP_oEmbed_Controller();
 ...
 * @since 4.4.0
 *
 * @param int|FP_Post $post Optional. Post ID or object. Defaults to the current post.
...
function get_post_embed_url( $post = null ) {
	 ...
	 * @param FP_Post $post      The corresponding post object.
	 */
...
function get_post_embed_html( $width, $height, $post = null ) {
	...
	$output .= wp_get_inline_script_tag(
		file_get_contents( ABSPATH . FPINC . '/js/wp-embed' . wp_scripts_get_suffix() . '.js' )
	);
 ...
 *
 * @param FP_Post|int $post  Post ID or post object.
...
function wp_enqueue_embed_styles() {
	...
	wp_add_inline_style( $handle, file_get_contents( ABSPATH . FPINC . "/css/wp-embed-template$suffix.css" ) );
...
function print_embed_scripts() {
	wp_print_inline_script_tag(
		file_get_contents( ABSPATH . FPINC . '/js/wp-embed-template' . wp_scripts_get_suffix() . '.js' )
	);
}
...
function print_embed_sharing_dialog() {
	...
	$share_tab_finpress_id   = 'wp-embed-share-tab-finpress-' . $unique_suffix;
	$share_tab_html_id        = 'wp-embed-share-tab-html-' . $unique_suffix;
	$description_finpress_id = 'wp-embed-share-description-finpress-' . $unique_suffix;
		...
		<div class="wp-embed-share-dialog-content">
			<div class="wp-embed-share-dialog-text">
				<ul class="wp-embed-share-tabs" role="tablist">
					<li class="wp-embed-share-tab-button wp-embed-share-tab-button-finpress" role="presentation">
						<button type="button" role="tab" aria-controls="<?php echo $share_tab_finpress_id; ?>" aria-selected="true" tabindex="0"><?php esc_html_e( 'WordPress Embed' ); ?></button>
					</li>
				...
				<div id="<?php echo $share_tab_finpress_id; ?>" class="wp-embed-share-tab" role="tabpanel" aria-hidden="false">
					<input type="text" value="<?php the_permalink(); ?>" class="wp-embed-share-input" aria-label="<?php esc_attr_e( 'URL' ); ?>" aria-describedby="<?php echo $description_finpress_id; ?>" tabindex="0" readonly/>

					<p class="wp-embed-share-description" id="<?php echo $description_finpress_id; ?>">
