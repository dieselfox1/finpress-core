...
class FP_Block_Parser {
		...
		switch ( $token_type ) {
			...
			case 'void-block':
				...
				if ( 0 === $stack_depth ) {
					...
					$this->output[] = (array) new FP_Block_Parser_Block( $block_name, $attrs, array(), '', array() );
				...
				$this->add_inner_block(
					new FP_Block_Parser_Block( $block_name, $attrs, array(), '', array() ),
					$start_offset,
					$token_length
				);
				$this->offset = $start_offset + $token_length;
	 ...
	 * @return FP_Block_Parser_Block freeform block object.
	 */
	public function freeform( $inner_html ) {
		return new FP_Block_Parser_Block( null, array(), array(), $inner_html, array( $inner_html ) );
	}
	 ...
	 * @internal
	 * @since 5.0.0
	 * @param FP_Block_Parser_Block $block        The block to add to the output.
	 * @param int                   $token_start  Byte offset into the document where the first token for the block starts.
	 * @param int                   $token_length Byte length of entire block from start of opening token to end of closing token.
	 * @param int|null              $last_offset  Last byte offset into document if continuing form earlier output.
	 */
	public function add_inner_block( FP_Block_Parser_Block $block, $token_start, $token_length, $last_offset = null ) {
