 ...
 * @since 4.3.0
 *
 * @see FP_Customize_Manager
 */
#[AllowDynamicProperties]
final class FP_Customize_Nav_Menus {

	/**
	 * FP_Customize_Manager instance.
	 *
	 * @since 4.3.0
	 ...
	 * @return array|FP_Error An array of menu items on success, a FP_Error object on failure.
	 */
	public function load_available_items_query( $object_type = 'post_type', $object_name = 'page', $page = 0 ) {
		...
		if ( 'post_type' === $object_type ) {
			...
			if ( ! $post_type ) {
				return new FP_Error( 'nav_menus_invalid_post_type' );
			}
			...
			if ( 0 === $page && 'page' === $object_name ) {
				...
				if ( ! empty( $privacy_policy_page_id ) ) {
					...
					if ( $privacy_policy_page instanceof FP_Post && 'publish' === $privacy_policy_page->post_status ) {
	 ...
	 *
	 * Based on FP_Editor::wp_link_query().
	 *
	 * @since 4.3.0
	 *
	...
	public function enqueue_scripts() {
		...
		$temp_nav_menu_setting      = new FP_Customize_Nav_Menu_Setting( $this->manager, 'nav_menu[-1]' );
		$temp_nav_menu_item_setting = new FP_Customize_Nav_Menu_Item_Setting( $this->manager, 'nav_menu_item[-1]' );
	...
	public function filter_dynamic_setting_args( $setting_args, $setting_id ) {
		if ( preg_match( FP_Customize_Nav_Menu_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => FP_Customize_Nav_Menu_Setting::TYPE,
				'transport' => 'postMessage',
			);
		} elseif ( preg_match( FP_Customize_Nav_Menu_Item_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => FP_Customize_Nav_Menu_Item_Setting::TYPE,
				'transport' => 'postMessage',
			);
		}
		return $setting_args;
	...
	/**
	 * Allows non-statically created settings to be constructed with custom FP_Customize_Setting subclass.
	 *
	 * @since 4.3.0
	 *
	...
	public function filter_dynamic_setting_class( $setting_class, $setting_id, $setting_args ) {
		...
		if ( ! empty( $setting_args['type'] ) && FP_Customize_Nav_Menu_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'FP_Customize_Nav_Menu_Setting';
		} elseif ( ! empty( $setting_args['type'] ) && FP_Customize_Nav_Menu_Item_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'FP_Customize_Nav_Menu_Item_Setting';
		}
	...
	public function customize_register() {
		...
		$this->manager->register_panel_type( 'FP_Customize_Nav_Menus_Panel' );
		$this->manager->register_control_type( 'FP_Customize_Nav_Menu_Control' );
		$this->manager->register_control_type( 'FP_Customize_Nav_Menu_Name_Control' );
		$this->manager->register_control_type( 'FP_Customize_Nav_Menu_Locations_Control' );
		$this->manager->register_control_type( 'FP_Customize_Nav_Menu_Auto_Add_Control' );
		$this->manager->register_control_type( 'FP_Customize_Nav_Menu_Item_Control' );
		...
		/*
		 * Once multiple theme supports are allowed in FP_Customize_Panel,
		 * this panel can be restricted to themes that support menus or widgets.
		 */
		$this->manager->add_panel(
			new FP_Customize_Nav_Menus_Panel(
				$this->manager,
				'nav_menus',
				array(
					'title'       => __( 'Menus' ),
		...
		foreach ( $locations as $location => $description ) {
			...
			$this->manager->add_control(
				new FP_Customize_Nav_Menu_Location_Control(
					$this->manager,
					$setting_id,
					array(
						'label'       => $description,
		...
		foreach ( $menus as $menu ) {
			...
			$this->manager->add_section(
				new FP_Customize_Nav_Menu_Section(
					$this->manager,
					$section_id,
					array(
			...
			$this->manager->add_setting(
				new FP_Customize_Nav_Menu_Setting(
					$this->manager,
					$nav_menu_setting_id,
					array(
						'transport' => 'postMessage',
			...
			foreach ( array_values( $menu_items ) as $i => $item ) {
				...
				$this->manager->add_setting(
					new FP_Customize_Nav_Menu_Item_Setting(
						$this->manager,
				...
				$this->manager->add_control(
					new FP_Customize_Nav_Menu_Item_Control(
						$this->manager,
						$menu_item_setting_id,
						array(
							'label'    => $item->title,
	 ...
	 * }
	 * @return FP_Post|FP_Error Inserted auto-draft post object or error.
	 */
	public function insert_auto_draft_post( $postarr ) {
		if ( ! isset( $postarr['post_type'] ) ) {
			return new FP_Error( 'unknown_post_type', __( 'Invalid post type.' ) );
		}
		if ( empty( $postarr['post_title'] ) ) {
			return new FP_Error( 'empty_title', __( 'Empty title.' ) );
		}
		if ( ! empty( $postarr['post_status'] ) ) {
			return new FP_Error( 'status_forbidden', __( 'Status is forbidden.' ) );
		}
	 ...
	 * `FP_Customize_Nav_Menu_Items::sanitize_nav_menus_created_posts()` to
	 * remove any post IDs for which the user cannot publish or for which the
	 * post is not an auto-draft.
	 *
	 * @since 4.7.0
	 *
	 * @param FP_Customize_Setting $setting Customizer setting object.
	 */
	 ...
	 *
	 * @see wp_nav_menu()
	 * @see FP_Customize_Widgets::filter_dynamic_sidebar_params()
	 *
	 ...
	 * @see wp_nav_menu()
	 *
	 * @param FP_Customize_Partial $partial       Partial.
