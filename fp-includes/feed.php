 ...
 * @since 2.5.0
 *
 * @param int|FP_Comment $comment_id Optional comment object or ID. Defaults to global comment object.
 */
 ...
 *
 * @param int|FP_Comment $comment Optional. Comment object or ID. Defaults to global comment object.
 */
...
/**
 * Gets the UTC time of the most recently modified post from FP_Query.
 *
 ...
 * @return SimplePie\SimplePie|FP_Error SimplePie object on success or FP_Error object on failure.
 */
function fetch_feed( $url ) {
	...
	$feed->set_sanitize_class( 'FP_SimplePie_Sanitize_KSES' );
	/*
	 * We must manually overwrite $feed->sanitize because SimplePie's constructor
	 * sets it before we have a chance to set the sanitization class.
	 */
	$feed->sanitize = new FP_SimplePie_Sanitize_KSES();

	// Register the cache handler using the recommended method for SimplePie 1.3 or later.
	if ( method_exists( 'SimplePie_Cache', 'register' ) ) {
		SimplePie_Cache::register( 'wp_transient', 'FP_Feed_Cache_Transient' );
		$feed->set_cache_location( 'wp_transient' );
	} else {
		// Back-compat for SimplePie 1.2.x.
		require_once ABSPATH . FPINC . '/class-wp-feed-cache.php';
		$feed->set_cache_class( 'FP_Feed_Cache' );
	}

	$feed->set_file_class( 'FP_SimplePie_File' );
	...
	if ( $feed->error() ) {
		return new FP_Error( 'simplepie-error', $feed->error() );
	}
