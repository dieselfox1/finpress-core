 ...
 * @since 3.1.0
 *
 * @see FP_User_Query::prepare_query() for information on accepted arguments.
 */
...
class FP_User_Query {
	 ...
	 * @since 4.4.0
	 *
	 * @param string|array $args Query vars, as passed to `FP_User_Query`.
	...
	public function prepare_query( $query = array() ) {
		...
		/**
		 * Fires before the FP_User_Query has been parsed.
		 *
		 * The passed FP_User_Query object contains the query variables,
		 * not yet passed into SQL.
		 *
		 * @since 4.0.0
		 *
		 * @param FP_User_Query $query Current instance of FP_User_Query (passed by reference).
		 */
		...
		if ( $search ) {
			...
			/**
			 * Filters the columns to search in a FP_User_Query search.
			 *
			 ...
			 * @param FP_User_Query $query          The current FP_User_Query instance.
			 */
		...
		if ( ! empty( $qv['date_query'] ) && is_array( $qv['date_query'] ) ) {
			$date_query         = new FP_Date_Query( $qv['date_query'], 'user_registered' );
			$this->query_where .= $date_query->get_sql();
		}

		/**
		 * Fires after the FP_User_Query has been parsed, and before
		 * the query is executed.
		 *
		 * The passed FP_User_Query object contains SQL parts formed
	...
	public function query() {
		 ...
		 * the `total_users` property of the FP_User_Query object, passed to the filter
		 * by reference. If FP_User_Query does not perform a database query, it will not
		 * have enough information to generate these values itself.
		 *
		 * @since 5.1.0
		 *
		 * @param array|null    $results Return an array of user data to short-circuit FP's user query
		 *                               or null to allow FP to run its normal queries.
		 * @param FP_User_Query $query   The FP_User_Query instance (passed by reference).
		 */
		...
		if ( null === $this->results ) {
			...
			if ( false !== $cache_value ) {
				...
				if ( isset( $qv['count_total'] ) && $qv['count_total'] ) {
					/**
					 * Filters SELECT FOUND_ROWS() query for the current FP_User_Query instance.
					 *
					 * @since 3.2.0
					 ...
					 * @global wpdb $wpdb WordPress database abstraction object.
					 *
					 * @param string        $sql   The SELECT FOUND_ROWS() query for the current FP_User_Query.
					 * @param FP_User_Query $query The current FP_User_Query instance.
					 */
		...
		if (
			...
			foreach ( $this->results as $userid ) {
				if ( 'all_with_meta' === $qv['fields'] ) {
					$r[ $userid ] = new FP_User( $userid, '', $qv['blog_id'] );
				} else {
					$r[] = new FP_User( $userid, '', $qv['blog_id'] );
				}
			}
