...
function get_block_asset_url( $path ) {
	...
	if ( ! $wpinc_path_norm ) {
		$wpinc_path_norm = wp_normalize_path( realpath( ABSPATH . FPINC ) );
	}
...
function register_block_script_handle( $metadata, $field_name, $index = 0 ) {
	...
	// Asset file for blocks is optional. See https://core.trac.finpress.org/ticket/60460.
 ...
 *
 * @param string $path     The absolute base path for the collection ( e.g., FP_PLUGIN_DIR . '/my-plugin/blocks/' ).
...
function wp_register_block_types_from_metadata_collection( $path, $manifest = '' ) {
	...
	$block_metadata_files = FP_Block_Metadata_Registry::get_collection_block_metadata_files( $path );
...
function wp_register_block_metadata_collection( $path, $manifest ) {
	FP_Block_Metadata_Registry::register_collection( $path, $manifest );
}
 ...
 *                               of `FP_Block_Type`. See FP_Block_Type::__construct() for information
 *                               on accepted arguments. Default empty array.
 * @return FP_Block_Type|false The registered block type on success, or false on failure.
 */
function register_block_type_from_metadata( $file_or_folder, $args = array() ) {
	...
	$is_core_block        = str_starts_with( $file_or_folder, wp_normalize_path( ABSPATH . FPINC ) );
	$metadata_file_exists = $is_core_block || file_exists( $metadata_file );
	$registry_metadata    = FP_Block_Metadata_Registry::get_metadata( $file_or_folder );
	...
	return FP_Block_Type_Registry::get_instance()->register(
		$metadata['name'],
		$settings
	);
}
 ...
 *
 * @param string|FP_Block_Type $block_type Block type name including namespace, or alternatively
 *                                         a path to the JSON file with metadata definition for the block,
 *                                         or a path to the folder where the `block.json` file is located,
 *                                         or a complete FP_Block_Type instance.
 *                                         In case a FP_Block_Type is provided, the $args parameter will be ignored.
 * @param array                $args       Optional. Array of block type arguments. Accepts any public property
 *                                         of `FP_Block_Type`. See FP_Block_Type::__construct() for information
 *                                         on accepted arguments. Default empty array.
 *
...
function register_block_type( $block_type, $args = array() ) {
	...
	return FP_Block_Type_Registry::get_instance()->register( $block_type, $args );
}
 ...
 * @since 5.0.0
 *
 * @param string|FP_Block_Type $name Block type name including namespace, or alternatively
 *                                   a complete FP_Block_Type instance.
 * @return FP_Block_Type|false The unregistered block type on success, or false on failure.
 */
function unregister_block_type( $name ) {
	return FP_Block_Type_Registry::get_instance()->unregister( $name );
}
 ...
 * @see parse_blocks()
 *
 * @param int|string|FP_Post|null $post Optional. Post content, post ID, or post object.
...
function get_hooked_blocks() {
	...
	$hooked_blocks = array();
	foreach ( $block_types as $block_type ) {
		if ( ! ( $block_type instanceof FP_Block_Type ) || ! is_array( $block_type->block_hooks ) ) {
			continue;
		}
 ...
 * @param FP_Block_Template|FP_Post|array $context             The block template, template part, or pattern that the anchor block belongs to.
 * @return string
 */
function insert_hooked_blocks( &$parsed_anchor_block, $relative_position, $hooked_blocks, $context ) {
	 ...
	 * @param FP_Block_Template|FP_Post|array $context            The block template, template part, post object,
 ...
 * @param FP_Block_Template|FP_Post|array|null $context  A block template, template part, post object, or pattern
		...
		foreach ( $relative_positions as $relative_position => $hooked_block_types ) {
			foreach ( $hooked_block_types as $index => $hooked_block_type ) {
				$hooked_block_type_definition =
					FP_Block_Type_Registry::get_instance()->get_registered( $hooked_block_type );
...
function apply_block_hooks_to_content_from_post_object( $content, $post = null, $callback = 'insert_hooked_blocks' ) {
	 ...
	 * See https://core.trac.finpress.org/ticket/63287 for more details.
	 */
 ...
 * @param FP_Block_Template|FP_Post|array $context       A block template, template part, post object,
 ...
 *
 * @param array $block {
 *     An associative array of a single parsed block object. See FP_Block_Parser_Block.
 *
 ...
 * @see serialize_block()
 *
 * @param array    $block         An associative array of a single parsed block object. See FP_Block_Parser_Block.
 ...
 *
 * @param array[]  $blocks        An array of parsed blocks. See FP_Block_Parser_Block.
 * @param callable $pre_callback  Callback to run on each block in the tree before it is traversed and serialized.
 *                                It is called with the following arguments: &$block, $parent_block, $previous_block.
 *                                Its string return value will be prepended to the serialized block markup.
 * @param callable $post_callback Callback to run on each block in the tree after it is traversed and serialized.
 ...
 * @since 5.3.1
 *
 * @param FP_Block_Parser_Block $block             The parsed block object.
 ...
 * @since 5.0.0
 *
 * @global FP_Post $post The post to edit.
 *
 * @param array $parsed_block {
 *     An associative array of the block being rendered. See FP_Block_Parser_Block.
 *
 *     @type string|null $blockName    Name of block.
 *     @type array       $attrs        Attributes from block comment delimiters.
 *     @type array[]     $innerBlocks  List of inner blocks. An array of arrays that
...
function render_block( $parsed_block ) {
	 ...
	 *                                     inner blocks were found.
	 * }
	 * @param FP_Block|null $parent_block If this is a nested block, a reference to the parent block.
	 */
...
function parse_blocks( $content ) {
	 ...
	 */
	$parser_class = apply_filters( 'block_parser_class', 'FP_Block_Parser' );
 ...
 *
 * @link https://developer.finpress.org/block-editor/reference-guides/block-api/block-styles/
 *
 ...
 *                                          See FP_Block_Styles_Registry::register().
 * @return bool True if the block style was registered with success and false otherwise.
 */
function register_block_style( $block_name, $style_properties ) {
	return FP_Block_Styles_Registry::get_instance()->register( $block_name, $style_properties );
}
...
function unregister_block_style( $block_name, $block_style_name ) {
	return FP_Block_Styles_Registry::get_instance()->unregister( $block_name, $block_style_name );
}
 ...
 *
 * @param FP_Block_Type $block_type    Block type to check for support.
...
/**
 * Helper function that constructs a FP_Query args array from
 ...
 *
 * @return array Returns the constructed FP_Query arguments.
 */
function build_query_vars_from_query_block( $block, $page ) {
	...
	if ( isset( $block->context['query'] ) ) {
		...
		if ( isset( $block->context['query']['sticky'] ) && ! empty( $block->context['query']['sticky'] ) ) {
			...
			if ( 'only' === $block->context['query']['sticky'] ) {
				 ...
				 * Logic should be used before hand to determine if FP_Query should be used in the event that the array
				 * being passed to post__in is empty.
				 *
				 * @see https://core.trac.finpress.org/ticket/28099
				 */
	...
	/**
	 * Filters the arguments which will be passed to `FP_Query` for the Query Loop Block.
	 *
	 * Anything to this filter should be compatible with the `FP_Query` API to form
	 ...
	 * @since 6.1.0
	 *
	 * @param array    $query Array containing parameters for `FP_Query` as parsed by the block context.
	 * @param FP_Block $block Block instance.
	 * @param int      $page  Current query's page.
	 */
	...
	if ( get_option( 'page_comments' ) === '1' || get_option( 'page_comments' ) === true ) {
		...
		if ( $per_page > 0 ) {
			...
			if ( $page ) {
				...
				$max_num_pages = (int) ( new FP_Comment_Query( $comment_args ) )->max_num_pages;
				if ( 0 !== $max_num_pages ) {
