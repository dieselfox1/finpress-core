...
class FP_Theme_JSON_Resolver {
	 ...
	 * @since 5.8.0
	 *
	 * @return FP_Theme_JSON Entity that holds core data.
	 */
	public static function get_core_data() {
		 ...
		 * @since 6.1.0
		 *
		 * @param FP_Theme_JSON_Data $theme_json Class to access and update the underlying data.
		 */
		$theme_json = apply_filters( 'wp_theme_json_data_default', new FP_Theme_JSON_Data( $config, 'default' ) );

		/*
		 * Backward compatibility for extenders returning a FP_Theme_JSON_Data
		 * compatible class that is not a FP_Theme_JSON_Data object.
		 */
		if ( $theme_json instanceof FP_Theme_JSON_Data ) {
			static::$core = $theme_json->get_theme_json();
		} else {
			$config       = $theme_json->get_data();
			static::$core = new FP_Theme_JSON( $config, 'default' );
		}
	 ...
	 * }
	 * @return FP_Theme_JSON Entity that holds theme data.
	 */
	public static function get_theme_data( $deprecated = array(), $options = array() ) {
		...
		if ( null === static::$theme || ! static::has_same_registered_blocks( 'theme' ) ) {
			...
			if ( is_readable( $theme_json_file ) ) {
				...
				$theme_json_data = array( 'version' => FP_Theme_JSON::LATEST_SCHEMA );
			}
			 ...
			 * @param FP_Theme_JSON_Data $theme_json Class to access and update the underlying data.
			 */
			$theme_json = apply_filters( 'wp_theme_json_data_theme', new FP_Theme_JSON_Data( $theme_json_data, 'theme' ) );
			...
			if ( $theme_json instanceof FP_Theme_JSON_Data ) {
				...
				$config        = $theme_json->get_data();
				static::$theme = new FP_Theme_JSON( $config );
			}
			...
			if ( $wp_theme->parent() ) {
				...
				if ( $theme_json_file !== $parent_theme_json_file && is_readable( $parent_theme_json_file ) ) {
					...
					$parent_theme           = new FP_Theme_JSON( $parent_theme_json_data );
		 ...
		 */
		$theme_support_data = FP_Theme_JSON::get_from_editor_settings( get_classic_theme_supports_block_editor_settings() );
		...
		$with_theme_supports = new FP_Theme_JSON( $theme_support_data );
	...
	public static function get_block_data() {
		...
		$config = array( 'version' => FP_Theme_JSON::LATEST_SCHEMA );
		foreach ( $blocks as $block_name => $block_type ) {
		 ...
		 * @param FP_Theme_JSON_Data $theme_json Class to access and update the underlying data.
		 */
		$theme_json = apply_filters( 'wp_theme_json_data_blocks', new FP_Theme_JSON_Data( $config, 'blocks' ) );
		...
		if ( $theme_json instanceof FP_Theme_JSON_Data ) {
			...
			$config         = $theme_json->get_data();
			static::$blocks = new FP_Theme_JSON( $config, 'blocks' );
		}
	 ...
	 * @since 5.9.0
	 *
	 * @param FP_Theme $theme              The theme object. If empty, it
	...
	public static function get_user_data_from_wp_global_styles( $theme, $create_post = false, $post_status_filter = array( 'publish' ) ) {
		...
		$global_style_query = new FP_Query();
		...
		if ( count( $recent_posts ) === 1 ) {
			...
			$cpt_post_id = wp_insert_post(
				array(
					'post_content' => '{"version": ' . FP_Theme_JSON::LATEST_SCHEMA . ', "isGlobalStylesUserThemeJSON": true }',
					'post_status'  => 'publish',
					'post_title'   => 'Custom Styles', // Do not make string translatable, see https://core.trac.finpress.org/ticket/54518.
					'post_type'    => $post_type_filter,
					'post_name'    => sprintf( 'fp-global-styles-%s', urlencode( $stylesheet ) ),
					'tax_input'    => array(
	 ...
	 *
	 * @return FP_Theme_JSON Entity that holds styles for user data.
	 */
	public static function get_user_data() {
		...
		if ( array_key_exists( 'post_content', $user_cpt ) ) {
			...
			if ( JSON_ERROR_NONE !== $json_decoding_error ) {
				 ...
				 * @param FP_Theme_JSON_Data $theme_json Class to access and update the underlying data.
				 */
				$theme_json = apply_filters( 'wp_theme_json_data_user', new FP_Theme_JSON_Data( $config, 'custom' ) );
				...
				if ( $theme_json instanceof FP_Theme_JSON_Data ) {
					...
					$config = $theme_json->get_data();
					return new FP_Theme_JSON( $config, 'custom' );
				}
			}
		...
		/** This filter is documented in fp-includes/class-fp-theme-json-resolver.php */
		$theme_json = apply_filters( 'wp_theme_json_data_user', new FP_Theme_JSON_Data( $config, 'custom' ) );
		...
		if ( $theme_json instanceof FP_Theme_JSON_Data ) {
			...
			$config       = $theme_json->get_data();
			static::$user = new FP_Theme_JSON( $config, 'custom' );
		}
	 ...
	 *
	 * Unlike the getters
	 * {@link https://developer.finpress.org/reference/classes/wp_theme_json_resolver/get_core_data/ get_core_data},
	 * {@link https://developer.finpress.org/reference/classes/wp_theme_json_resolver/get_theme_data/ get_theme_data},
	 * and {@link https://developer.finpress.org/reference/classes/wp_theme_json_resolver/get_user_data/ get_user_data},
	...
	public static function get_style_variations( $scope = 'theme' ) {
		...
		foreach ( $variation_files as $path => $file ) {
			...
			if ( is_array( $decoded_file ) && static::style_variation_has_scope( $decoded_file, $scope ) ) {
				...
				$variation  = ( new FP_Theme_JSON( $translated ) )->get_raw_data();
	 ...
	 *
	 * @param FP_Theme_JSON $theme_json A theme json instance.
	 * @return array An array of resolved paths.
	 */
	public static function get_resolved_theme_uris( $theme_json ) {
		$resolved_theme_uris = array();

		if ( ! $theme_json instanceof FP_Theme_JSON ) {
		 ...
		 * See: FP_Font_Face_Resolver::to_theme_file_uri.
		 */
		$placeholder = 'file:./';
	 ...
	 *
	 * @param FP_Theme_JSON $theme_json A theme json instance.
	 * @return FP_Theme_JSON Theme merged with resolved paths, if any found.
	 */
	public static function resolve_theme_file_uris( $theme_json ) {
		...
		return new FP_Theme_JSON( $resolved_theme_json_data );
	}
