...
/**
 * Returns an array of FP_Theme objects based on the arguments.
 *
 ...
 * }
 * @return FP_Theme[] Array of FP_Theme objects.
 */
function wp_get_themes( $args = array() ) {
	...
	if ( is_array( $wp_theme_directories ) && count( $wp_theme_directories ) > 1 ) {
		...
		if ( isset( $theme_directories[ $current_theme ] ) ) {
			...
			if ( ! in_array( $root_of_current_theme, $wp_theme_directories, true ) ) {
				$root_of_current_theme = FP_CONTENT_DIR . $root_of_current_theme;
			}
	...
	if ( is_multisite() && null !== $args['allowed'] ) {
		...
		if ( 'network' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, FP_Theme::get_allowed_on_network() );
		} elseif ( 'site' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, FP_Theme::get_allowed_on_site( $args['blog_id'] ) );
		} elseif ( $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, FP_Theme::get_allowed( $args['blog_id'] ) );
		} else {
			$theme_directories = array_diff_key( $theme_directories, FP_Theme::get_allowed( $args['blog_id'] ) );
		}
	}
	...
	foreach ( $theme_directories as $theme => $theme_root ) {
		if ( isset( $_themes[ $theme_root['theme_root'] . '/' . $theme ] ) ) {
			...
			$themes[ $theme ] = new FP_Theme( $theme, $theme_root['theme_root'] );
...
/**
 * Gets a FP_Theme object for a theme.
 *
 * @since 3.4.0
 *
 * @global string[] $wp_theme_directories
 ...
 * @return FP_Theme Theme object. Be sure to check the object's exists() method
...
function wp_get_theme( $stylesheet = '', $theme_root = '' ) {
	...
	if ( empty( $theme_root ) ) {
		...
		if ( false === $theme_root ) {
			$theme_root = FP_CONTENT_DIR . '/themes';
		} elseif ( ! in_array( $theme_root, (array) $wp_theme_directories, true ) ) {
			$theme_root = FP_CONTENT_DIR . $theme_root;
		}
	}

	return new FP_Theme( $stylesheet, $theme_root );
}

/**
 * Clears the cache held by get_theme_roots() and FP_Theme.
 *
 * @since 3.5.0
 ...
 *                          or a folder within FP_CONTENT_DIR.
...
function register_theme_directory( $directory ) {
	...
	if ( ! file_exists( $directory ) ) {
		...
		$directory = FP_CONTENT_DIR . '/' . $directory;
...
function search_theme_directories( $force = false ) {
	...
	foreach ( $wp_theme_directories as $theme_root ) {
		if ( str_starts_with( $theme_root, FP_CONTENT_DIR ) ) {
			$relative_theme_roots[ str_replace( FP_CONTENT_DIR, '', $theme_root ) ] = $theme_root;
		} else {
	...
	foreach ( $wp_theme_directories as $theme_root ) {
		...
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				 ...
				 * Return it; FP_Theme will catch the error.
				 */
				if ( ! $found_theme ) {
					$found_themes[ $dir ] = array(
						'theme_file' => $dir . '/style.css',
...
function get_theme_root( $stylesheet_or_template = '' ) {
	...
	if ( $stylesheet_or_template ) {
		...
		if ( $theme_root ) {
			/*
			 * Always prepend FP_CONTENT_DIR unless the root currently registered as a theme directory.
...
function get_theme_root_uri( $stylesheet_or_template = '', $theme_root = '' ) {
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $wp_theme_directories, true ) ) {
			...
			if ( str_starts_with( $theme_root, FP_CONTENT_DIR ) ) {
				$theme_root_uri = content_url( str_replace( FP_CONTENT_DIR, '', $theme_root ) );
			} elseif ( str_starts_with( $theme_root, ABSPATH ) ) {
				$theme_root_uri = site_url( str_replace( ABSPATH, '', $theme_root ) );
			} elseif ( str_starts_with( $theme_root, FP_PLUGIN_DIR ) || str_starts_with( $theme_root, FPMU_PLUGIN_DIR ) ) {
...
function switch_theme( $stylesheet ) {
	 ...
	 * @param FP_Theme $new_theme FP_Theme instance of the new theme.
	 * @param FP_Theme $old_theme FP_Theme instance of the old theme.
	 */
...
function validate_current_theme() {
	...
	$default = wp_get_theme( FP_DEFAULT_THEME );
	if ( $default->exists() ) {
		switch_theme( FP_DEFAULT_THEME );
		return false;
	}

	/**
	 * If we're in an invalid state but FP_DEFAULT_THEME doesn't exist,
	 * switch to the latest core default theme that's installed.
	 *
	 * If it turns out that this latest core default theme is our current
	 * theme, then there's nothing we can do about that, so we have to bail,
	 * rather than going into an infinite loop. (This is why there are
	 * checks against FP_DEFAULT_THEME above, also.) We also can't do anything
	 * if it turns out there is no default theme installed. (That's `false`.)
	 */
	$default = FP_Theme::get_core_default_theme();
 ...
 * @return FP_Post|null The custom_css post or null if none exists.
 */
function wp_get_custom_css_post( $stylesheet = '' ) {
	...
	if ( get_stylesheet() === $stylesheet ) {
		...
		if ( ! $post && -1 !== $post_id ) {
			$query = new FP_Query( $custom_css_query_vars );
			$post  = $query->post;
			/*
 ...
 * }
 * @return FP_Post|FP_Error Post on success, error on failure.
 */
	...
	$core_content = array(
		...
		'nav_menus' => array(
			...
			'link_email'      => array(
				...
				'url'   => 'mailto:finpress@example.com',
			),
			'link_facebook'   => array(
 ...
 *              see `FP_Theme_JSON::APPEARANCE_TOOLS_OPT_INS` for a complete list.
 ...
 *      }
 * }
 * @return true|FP_Error True if the theme feature was successfully registered, a FP_Error object if not.
 */
...
/**
 * Checks if a theme has been changed and runs 'after_switch_theme' hook on the next FP load.
 *
 * See {@see 'after_switch_theme'}.
 *
 * @since 3.3.0
...
function check_theme_switched() {
	...
	if ( $stylesheet ) {
		...
		if ( $old_theme->exists() ) {
			/**
			 * Fires on the next FP load after the theme has been switched.
			 *
...
/**
 * Includes and instantiates the FP_Customize_Manager class.
 *
...
function _wp_customize_include() {
	...
	// Note that theme will be sanitized via FP_Theme.
	 ...
	 * here in the case of a customize_save action because this will cause FP to think
	...
	require_once ABSPATH . FPINC . '/class-wp-customize-manager.php';
	$GLOBALS['wp_customize'] = new FP_Customize_Manager(
		compact(
			'changeset_uuid',
			'theme',
			'messenger_channel',
 ...
 * @access private
 *
 * @global FP_Customize_Manager $wp_customize Customizer instance.
 *
 * @param string  $new_status     New post status.
 * @param string  $old_status     Old post status.
 * @param FP_Post $changeset_post Changeset post object.
 */
function _wp_customize_publish_changeset( $new_status, $old_status, $changeset_post ) {
	...
	if ( empty( $wp_customize ) ) {
		require_once ABSPATH . FPINC . '/class-wp-customize-manager.php';
		$wp_customize = new FP_Customize_Manager(
			array(
...
function is_customize_preview() {
	...
	return ( $wp_customize instanceof FP_Customize_Manager ) && $wp_customize->is_preview();
}
 ...
 *
 * @link https://core.trac.finpress.org/ticket/39752
 *
 * @since 4.8.0
 * @access private
...
function _wp_keep_alive_customize_changeset_dependent_auto_drafts( $new_status, $old_status, $post ) {
	 ...
	 * are then visible in the FP Admin. We cannot wait for the deletion of the changeset in which
 ...
 *
 * Alias of FP_Theme_JSON::get_element_class_name.
 *
 * @since 6.1.0
 *
...
function wp_theme_get_element_class_name( $element ) {
	return FP_Theme_JSON::get_element_class_name( $element );
}
...
function _add_default_theme_supports() {
	...
	add_filter(
		...
		static function ( $active, FP_Customize_Panel $panel ) {
			if (
				'nav_menus' === $panel->id &&
