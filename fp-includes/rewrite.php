 ...
 * @since 4.5.0
 *
 * @see FP_Rewrite::remove_permastruct()
 * @global FP_Rewrite $wp_rewrite WordPress rewrite component.
 *
 * @param string $name Name for permalink structure.
 */
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to FP_Query::parse_query()
 ...
 *                          FP::parse_request(). Default empty array.
...
function url_to_postid( $url ) {
	...
	if ( trim( $url, '/' ) === home_url() && 'page' === get_option( 'show_on_front' ) ) {
		...
		if ( $page_on_front && get_post( $page_on_front ) instanceof FP_Post ) {
	...
	foreach ( (array) $rewrite as $match => $query ) {
		...
		if ( preg_match( "#^$match#", $request_match, $matches ) ) {
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( (string) $key, $fp->public_query_vars, true ) ) {
					$query[ $key ] = $value;
