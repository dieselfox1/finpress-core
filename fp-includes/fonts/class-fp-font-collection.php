 ...
 */
final class FP_Font_Collection {
	/**
	...
	/**
	 * FP_Font_Collection constructor.
	 *
	 * @since 6.5.0
	 *
	 ...
	 * @since 6.5.0
	 *
	 * @return array|FP_Error An array containing the font collection data, or a FP_Error on failure.
	 */
	 ...
	 * @return array|FP_Error An array containing the font collection data on success,
	 *                        else an instance of FP_Error on failure.
	 */
	private function load_from_json( $file_or_url ) {
		...
		if ( ! $url && ! $file ) {
			...
			return new FP_Error( 'font_collection_json_missing', $message );
		}
	...
	private function load_from_file( $file ) {
		...
		if ( empty( $data ) ) {
			return new FP_Error( 'font_collection_decode_error', __( 'Error decoding the font collection JSON file contents.' ) );
		}
	...
	private function load_from_url( $url ) {
		...
		if ( false === $data ) {
			...
			if ( empty( $data ) ) {
				return new FP_Error( 'font_collection_decode_error', __( 'Error decoding the font collection data from the HTTP response JSON.' ) );
			}
	 ...
	 * @return array|FP_Error Sanitized data if valid, otherwise a FP_Error instance.
	 */
	private function sanitize_and_validate_data( $data, $required_properties = array() ) {
		$schema = self::get_sanitization_schema();
		$data   = FP_Font_Utils::sanitize_from_schema( $data, $schema );
		...
		foreach ( $required_properties as $property ) {
			if ( empty( $data[ $property ] ) ) {
				...
				_doing_it_wrong( __METHOD__, $message, '6.5.0' );
				return new FP_Error( 'font_collection_missing_property', $message );
			}
		}
			...
			'font_families' => array(
				array(
					'font_family_settings' => array(
						...
						'fontFamily' => 'FP_Font_Utils::sanitize_font_family',
