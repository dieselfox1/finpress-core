...
function wp_print_font_faces( $fonts = array() ) {
	...
	if ( empty( $fonts ) ) {
		$fonts = FP_Font_Face_Resolver::get_fonts_from_theme_json();
	}
...
function wp_print_font_faces_from_style_variations() {
	$fonts = FP_Font_Face_Resolver::get_fonts_from_style_variations();
 ...
 * }
 * @return FP_Font_Collection|FP_Error A font collection if it was registered
 *                                     successfully, or FP_Error object on failure.
 */
function wp_register_font_collection( string $slug, array $args ) {
	return FP_Font_Library::get_instance()->register_font_collection( $slug, $args );
}
...
function wp_unregister_font_collection( string $slug ) {
	return FP_Font_Library::get_instance()->unregister_font_collection( $slug );
}
