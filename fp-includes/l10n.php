 ...
 *
 * If the locale is not set already, then the FPLANG constant is used if it is
...
function get_locale() {
	...
	// FPLANG was defined in wp-config.
	if ( defined( 'FPLANG' ) ) {
		$locale = FPLANG;
	}
	...
	if ( is_multisite() ) {
		...
		if ( wp_installing() ) {
			$ms_locale = get_site_option( 'FPLANG' );
		} else {
			$ms_locale = get_option( 'FPLANG' );
			if ( false === $ms_locale ) {
				$ms_locale = get_site_option( 'FPLANG' );
			}
		}
 ...
 * @since 4.7.0
 *
 * @param int|FP_User $user User's ID or a FP_User object. Defaults to current user.
...
function get_user_locale( $user = 0 ) {
	...
	} elseif ( $user instanceof FP_User ) {
		$user_object = $user;
...
function load_textdomain( $domain, $mofile, $locale = null ) {
	/** @var FP_Textdomain_Registry $wp_textdomain_registry */
	...
	$i18n_controller = FP_Translation_Controller::get_instance();
	...
	foreach ( $translation_files as $file ) {
		...
		if ( $success ) {
			...
			$l10n[ $domain ] = new FP_Translations( $i18n_controller, $domain );
...
function unload_textdomain( $domain, $reloadable = false ) {
	...
	if ( ! $reloadable ) {
		FP_Translation_Controller::get_instance()->unload_textdomain( $domain );
	}
 ...
 *
 * Loads the .mo file in FP_LANG_DIR constant path from WordPress root.
...
function load_default_textdomain( $locale = null ) {
	...
	$return = load_textdomain( 'default', FP_LANG_DIR . "/$locale.mo", $locale );

	if ( ( is_multisite() || ( defined( 'FP_INSTALLING_NETWORK' ) && FP_INSTALLING_NETWORK ) ) && ! file_exists( FP_LANG_DIR . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default', FP_LANG_DIR . "/ms-$locale.mo", $locale );
		return $return;
	}

	if ( is_admin() || wp_installing() || ( defined( 'FP_REPAIRING' ) && FP_REPAIRING ) || doing_action( 'wp_maybe_auto_update' ) ) {
		load_textdomain( 'default', FP_LANG_DIR . "/admin-$locale.mo", $locale );
	}

	if ( is_network_admin() || ( defined( 'FP_INSTALLING_NETWORK' ) && FP_INSTALLING_NETWORK ) ) {
		load_textdomain( 'default', FP_LANG_DIR . "/admin-network-$locale.mo", $locale );
	}
 ...
 *
 * @global FP_Textdomain_Registry $wp_textdomain_registry WordPress Textdomain Registry.
 * @global array<string, FP_Translations|NOOP_Translations> $l10n An array of all currently loaded text domains.
 *
 * @param string       $domain          Unique identifier for retrieving translated strings
 * @param string|false $deprecated      Optional. Deprecated. Use the $plugin_rel_path parameter instead.
 *                                      Default false.
 * @param string|false $plugin_rel_path Optional. Relative path to FP_PLUGIN_DIR where the .mo file resides.
 *                                      Default false.
 * @return bool True when textdomain is successfully loaded, false otherwise.
 */
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	/** @var FP_Textdomain_Registry $wp_textdomain_registry */
	/** @var array<string, FP_Translations|NOOP_Translations> $l10n */
	...
	if ( false !== $plugin_rel_path ) {
		$path = FP_PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
 ...
 *
 * @param string $domain             Text domain. Unique identifier for retrieving translated strings.
 * @param string $mu_plugin_rel_path Optional. Relative to `FPMU_PLUGIN_DIR` directory in which the .mo
...
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	...
	$path = FPMU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' );
 ...
 *
 * @see FP_Scripts::set_translations()
 *
...
function load_script_textdomain( $handle, $domain = 'default', $path = '' ) {
	...
	if (
		...
		/*
		 * Ensure correct languages path when using a custom `FP_PLUGIN_DIR` / `FP_PLUGIN_URL` configuration,
		 * a custom theme root, and/or using Multisite with subdirectories.
		 * See https://core.trac.finpress.org/ticket/60891 and https://core.trac.finpress.org/ticket/62016.
		 */

		$theme_dir = array_slice( explode( '/', $theme_root ), -1 );
		$dirname   = $theme_dir[0] === $relative[0] ? 'themes' : 'plugins';

		$languages_path = FP_LANG_DIR . '/' . $dirname;
		...
		$languages_path = FP_LANG_DIR . '/plugins';
		...
		if ( ! isset( $site_url['path'] ) ) {
			...
			// Make the src relative to the FP root.
...
function _load_textdomain_just_in_time( $domain ) {
	...
	// Themes with their language directory outside of FP_LANG_DIR have a different file name.
 ...
 *
 * The default directory is FP_LANG_DIR.
 *
 * @since 3.0.0
...
function get_available_languages( $dir = null ) {
	...
	$path       = is_null( $dir ) ? FP_LANG_DIR : $dir;
...
function wp_get_installed_translations( $type ) {
	...
	$dir = 'core' === $type ? FP_LANG_DIR : FP_LANG_DIR . "/$type";
...
function is_rtl() {
	global $wp_locale;
	if ( ! ( $wp_locale instanceof FP_Locale ) ) {
		return false;
	}
...
function switch_to_locale( $locale ) {
	/* @var FP_Locale_Switcher $wp_locale_switcher */
...
function wp_get_list_item_separator() {
	...
	if ( ! ( $wp_locale instanceof FP_Locale ) ) {
		// Default value of FP_Locale::get_list_item_separator().
...
function wp_get_word_count_type() {
	...
	if ( ! ( $wp_locale instanceof FP_Locale ) ) {
		// Default value of FP_Locale::get_word_count_type().
		return 'words';
	}
...
function has_translation( string $singular, string $textdomain = 'default', ?string $locale = null ): bool {
	return FP_Translation_Controller::get_instance()->has_translation( $singular, $textdomain, $locale );
}
