<?php
/**
 * Style Engine: FP_Style_Engine_Processor class
 *
 * @package WordPress
...
class FP_Style_Engine_Processor {
	 ...
	 * @since 6.1.0
	 *
	 * @param FP_Style_Engine_CSS_Rules_Store $store The store to add.
	 * @return FP_Style_Engine_Processor Returns the object to allow chaining methods.
	 */
	public function add_store( $store ) {
		if ( ! $store instanceof FP_Style_Engine_CSS_Rules_Store ) {
			_doing_it_wrong(
				__METHOD__,
				__( '$store must be an instance of FP_Style_Engine_CSS_Rules_Store' ),
				'6.1.0'
			);
			return $this;
		}
	 ...
	 *
	 * @param FP_Style_Engine_CSS_Rule|FP_Style_Engine_CSS_Rule[] $css_rules A single, or an array of,
	 *                                                                       FP_Style_Engine_CSS_Rule objects
	...
	public function get_css( $options = array() ) {
		...
		foreach ( $this->css_rules as $rule ) {
			// See class FP_Style_Engine_CSS_Rule for the get_css method.
	...
	private function combine_rules_selectors() {
		...
		foreach ( $selectors_json as $selector => $json ) {
			...
			$this->css_rules[ $duplicate_selectors ] = new FP_Style_Engine_CSS_Rule( $duplicate_selectors, $declarations );
		}
	}
}
