 ...
 * }
 * @return int|FP_Error The new site's ID on success, or error object on failure.
 */
function wp_insert_site( array $data ) {
	...
	if ( false === $wpdb->insert( $wpdb->blogs, $prepared_data ) ) {
		return new FP_Error( 'db_insert_error', __( 'Could not insert site into the database.' ), $wpdb->last_error );
	}
	...
	if ( ! $new_site ) {
		return new FP_Error( 'get_site_error', __( 'Could not retrieve site data.' ) );
	}
	 ...
	 * @since 5.1.0
	 *
	 * @param FP_Site $new_site New site object.
	 */
	...
	if ( has_action( 'wpmu_new_blog' ) ) {
		...
		// FPLANG was passed with `$meta` to the `wpmu_new_blog` hook prior to 5.1.0.
		if ( ! array_key_exists( 'FPLANG', $meta ) ) {
			$meta['FPLANG'] = get_network_option( $new_site->network_id, 'FPLANG' );
		}
 ...
 * @return int|FP_Error The updated site's ID on success, or error object on failure.
 */
function wp_update_site( $site_id, array $data ) {
	global $wpdb;

	if ( empty( $site_id ) ) {
		return new FP_Error( 'site_empty_id', __( 'Site ID must not be empty.' ) );
	}

	$old_site = get_site( $site_id );
	if ( ! $old_site ) {
		return new FP_Error( 'site_not_exist', __( 'Site does not exist.' ) );
	}
	...
	if ( false === $wpdb->update( $wpdb->blogs, $data, array( 'blog_id' => $old_site->id ) ) ) {
		return new FP_Error( 'db_update_error', __( 'Could not update site in the database.' ), $wpdb->last_error );
	}
	 ...
	 *
	 * @param FP_Site $new_site New site object.
	 * @param FP_Site $old_site Old site object.
	 */
 ...
 * @return FP_Site|FP_Error The deleted site object on success, or error object on failure.
 */
function wp_delete_site( $site_id ) {
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `FP_Error::add()` method. If any errors
	 * are present, the site will not be deleted.
	 *
	 * @since 5.1.0
	 *
	 * @param FP_Error $errors   Error object to add validation errors to.
	 * @param FP_Site  $old_site The site object to be deleted.
	 */
	 ...
	 * @since 5.1.0
	 *
	 * @param FP_Site $old_site Deleted site object.
	 */
	...
	if ( false === $wpdb->delete( $wpdb->blogs, array( 'blog_id' => $old_site->id ) ) ) {
		return new FP_Error( 'db_delete_error', __( 'Could not delete site from the database.' ), $wpdb->last_error );
	}
 ...
 * @since 4.6.0
 *
 * @param FP_Site|int|null $site Optional. Site to retrieve. Default is the current site.
 * @return FP_Site|null The site object or null if not found.
 */
function get_site( $site = null ) {
	...
	if ( $site instanceof FP_Site ) {
		$_site = $site;
	} elseif ( is_object( $site ) ) {
		$_site = new FP_Site( $site );
	} else {
		$_site = FP_Site::get_instance( $site );
	}
 ...
 *
 * @see FP_Site_Query::parse_query()
 *
 * @param string|array $args Optional. Array or string of arguments. See FP_Site_Query::__construct()
 *                           for information on accepted arguments. Default empty array.
 * @return FP_Site[]|int[]|int List of FP_Site objects, a list of site IDs when 'fields' is set to 'ids',
 *                             or the number of sites when 'count' is passed as a query var.
 */
function get_sites( $args = array() ) {
	$query = new FP_Site_Query();
 ...
 * @param FP_Site|null $old_site Optional. Old site object if an update, or null if an insertion.
 *                               Default null.
 * @return array|FP_Error Site data ready for a database transaction, or FP_Error in case a validation
 *                        error occurred.
 */
function wp_prepare_site_data( $data, $defaults, $old_site = null ) {
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `FP_Error::add()` method.
	 *
	 * @since 5.1.0
	 *
	 * @param FP_Error     $errors   Error object to add validation errors to.
	 ...
	 * @param FP_Site|null $old_site The old site object if the data belongs to a site being updated,
 ...
 * @since 5.1.0
 *
 * @param FP_Error     $errors   Error object, passed by reference. Will contain validation errors if
 *                               any occurred.
 * @param array        $data     Associative array of complete site data. See {@see wp_insert_site()}
 *                               for the included data.
 * @param FP_Site|null $old_site The old site object if the data belongs to a site being updated,
 ...
 *
 * @param int|FP_Site $site_id Site ID or object.
 * @param array       $args    {
 ...
 *                           Default empty array.
 * }
 * @return true|FP_Error True on success, or error object on failure.
 */
function wp_initialize_site( $site_id, array $args = array() ) {
	...
	if ( ! $site ) {
		return new FP_Error( 'site_invalid_id', __( 'Site with the ID does not exist.' ) );
	}

	if ( wp_is_site_initialized( $site ) ) {
		return new FP_Error( 'site_already_initialized', __( 'The site appears to be already initialized.' ) );
	}
	 ...
	 * @param FP_Site    $site    Site that is being initialized.
	 * @param FP_Network $network Network that the site belongs to.
	 */
	...
	populate_options(
		array_merge(
			array(
				...
				'FPLANG'      => get_network_option( $network->id, 'FPLANG' ),
			),
			$args['options']
		)
	);
...
function wp_uninitialize_site( $site_id ) {
	...
	if ( ! wp_is_site_initialized( $site ) ) {
		return new FP_Error( 'site_already_uninitialized', __( 'The site appears to be already uninitialized.' ) );
	}
 ...
 *
 * @param FP_Site|int $blog The site object or ID to be cleared from cache.
 */
function clean_blog_cache( $blog ) {
	...
	if ( ! $blog ) {
		...
		// Make sure a FP_Site object exists even when the site has been deleted.
		$blog = new FP_Site(
			(object) array(
				'blog_id' => $blog_id,
				'domain'  => null,
	 ...
	 * @param FP_Site $blog            Site object.
 ...
 * @since 5.1.0
 *
 * @param FP_Site      $new_site The site object that has been inserted, updated or deleted.
 * @param FP_Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 ...
 * @since 5.1.0
 *
 * @param FP_Site      $new_site The site object after the update.
 * @param FP_Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function wp_maybe_transition_site_statuses_on_update( $new_site, $old_site = null ) {
	...
	if ( ! $old_site ) {
		$old_site = new FP_Site( new stdClass() );
	}
 ...
 *
 * @param FP_Site $new_site The site object after the update.
 * @param FP_Site $old_site The site object prior to the update.
 */
