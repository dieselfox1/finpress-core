...
require ABSPATH . FPINC . '/class-wp-dependency.php';

/** WordPress Dependencies Class */
require ABSPATH . FPINC . '/class-wp-dependencies.php';

/** WordPress Scripts Class */
require ABSPATH . FPINC . '/class-wp-scripts.php';

/** WordPress Scripts Functions */
require ABSPATH . FPINC . '/functions.wp-scripts.php';

/** WordPress Styles Class */
require ABSPATH . FPINC . '/class-wp-styles.php';

/** WordPress Styles Functions */
require ABSPATH . FPINC . '/functions.wp-styles.php';
 ...
 *
 * @param FP_Scripts $scripts            FP_Scripts object.
...
/**
 * Registers development scripts that integrate with `@finpress/scripts`.
 *
...
function wp_register_development_scripts( $scripts ) {
	...
	foreach ( $development_scripts as $script_name ) {
		$assets = include ABSPATH . FPINC . '/assets/script-loader-' . $script_name . '.php';
		if ( ! is_array( $assets ) ) {
			return;
		}
		$scripts->add(
...
function wp_default_packages_scripts( $scripts ) {
	$suffix = defined( 'FP_RUN_CORE_TESTS' ) ? '.min' : wp_scripts_get_suffix();
	/*
	 ...
	 *     'api-fetch.js' => array(...
	 */
	$assets = include ABSPATH . FPINC . "/assets/script-loader-packages{$suffix}.php";
	...
	$scripts->add_inline_script(
		...
		implode(
			...
			array(
				...
				'	var storageKey = "FP_DATA_USER_" + userId;',
				'	wp.data',
 ...
 * from `_FP_Editors::default_settings()` for the Classic block.
 *
 * @since 5.0.0
 *
...
function wp_default_scripts( $scripts ) {
	...
	$scripts->content_url     = defined( 'FP_CONTENT_URL' ) ? FP_CONTENT_URL : '';
	...
	did_action( 'init' ) && $scripts->localize(
		...
		array(
			...
			'blockThemeNotification'  => sprintf(
				...
				__( 'https://finpress.org/documentation/article/site-editor/' ),
				sprintf(
	...
	if ( is_admin() ) {
		 ...
		 * see https://core.trac.finpress.org/ticket/42321
		 */
...
function wp_default_styles( $styles ) {
	...
	$styles->content_url     = defined( 'FP_CONTENT_URL' ) ? FP_CONTENT_URL : '';
	...
	$block_library_theme_path = FPINC . "/css/dist/block-library/theme$suffix.css";
	...
	$classic_theme_styles_path = FPINC . "/css/classic-themes$suffix.css";
...
function wp_localize_community_events() {
	...
	$current_ip_address = FP_Community_Events::get_unsafe_client_ip();
...
function print_footer_scripts() {
	...
	if ( ! ( $wp_scripts instanceof FP_Scripts ) ) {
...
function print_late_styles() {
	...
	if ( ! ( $wp_styles instanceof FP_Styles ) ) {
		return;
	}
...
/**
 * Applies a filter to the list of style nodes that comes from FP_Theme_JSON::get_style_nodes().
 *
 * This particular filter removes all of the blocks from the array.
 *
 * We want FP_Theme_JSON to be ignorant of the implementation details of how the CSS is being used.
 * This filter allows us to modify the output of FP_Theme_JSON depending on whether or not we are
...
function wp_enqueue_global_styles() {
	 ...
	 *
	 * See https://core.trac.finpress.org/ticket/53494 and https://core.trac.finpress.org/ticket/61965.
	 */
	if (
...
function wp_should_load_block_editor_scripts_and_styles() {
	...
	$is_block_editor_screen = ( $current_screen instanceof FP_Screen ) && $current_screen->is_block_editor();
...
function enqueue_block_styles_assets() {
	...
	$block_styles = FP_Block_Styles_Registry::get_instance()->get_all_registered();
 ...
 *
 * @link https://core.trac.finpress.org/ticket/53494.
 *
 ...
 *
 * @link https://developer.finpress.org/block-editor/reference-guides/packages/packages-style-engine/
 *
 * @since 6.1.0
 *
 * @param array $options {
...
function wp_enqueue_stored_styles( $options = array() ) {
	...
	$additional_stores = FP_Style_Engine_CSS_Rules_Store::get_stores();
