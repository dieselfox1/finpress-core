<?php
/**
 * Error Protection API: FP_Recovery_Mode_Email_Link class
 *
 * @package WordPress
 * @since 5.2.0
 */
...
final class FP_Recovery_Mode_Email_Service {
	...
	/**
	 * FP_Recovery_Mode_Email_Service constructor.
	 *
	 * @since 5.2.0
	 *
	 * @param FP_Recovery_Mode_Link_Service $link_service
	 */
	public function __construct( FP_Recovery_Mode_Link_Service $link_service ) {
	 ...
	 * }
	 * @return true|FP_Error True if email sent, FP_Error otherwise.
	 */
	public function maybe_send_recovery_mode_email( $rate_limit, $error, $extension ) {
		...
		if ( ! $last_sent || time() > $last_sent + $rate_limit ) {
			if ( ! update_option( self::RATE_LIMIT_OPTION, time() ) ) {
				return new FP_Error( 'storage_error', __( 'Could not update the email last sent time.' ) );
			}
		...
		return new FP_Error( 'email_sent_already', $err_message );
	}
