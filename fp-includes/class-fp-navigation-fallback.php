 ...
 * @since 6.3.0
 */
class FP_Navigation_Fallback {

	/**
	 * Updates the wp_navigation custom post type schema, in order to expose
	 * additional fields in the embeddable links of FP_REST_Navigation_Fallback_Controller.
	 *
	...
	public static function update_wp_navigation_post_schema( $schema ) {
		 ...
		 *
		 * @see FP_REST_Posts_Controller::get_item_schema()
		 */
	 ...
	 * @since 6.3.0
	 *
	 * @return FP_Post|null the fallback Navigation Post or null.
	 */
	public static function get_fallback() {
		...
		if ( $fallback && ! is_wp_error( $fallback ) ) {
			...
			return $fallback instanceof FP_Post ? $fallback : static::get_most_recently_published_navigation();
		}
	 ...
	 * @since 6.3.0
	 *
	 * @return int|FP_Error The post ID of the default fallback menu or a FP_Error object.
	 */
	private static function create_classic_menu_fallback() {
		...
		if ( ! $classic_nav_menu ) {
			return new FP_Error( 'no_classic_menus', __( 'No Classic Menus found.' ) );
		}

		// If there is a classic menu then convert it to blocks.
		$classic_nav_menu_blocks = FP_Classic_To_Block_Menu_Converter::convert( $classic_nav_menu );
		...
		if ( empty( $classic_nav_menu_blocks ) ) {
			return new FP_Error( 'cannot_convert_classic_menu', __( 'Unable to convert Classic Menu to blocks.' ) );
		}
	 ...
	 * @since 6.3.0
	 *
	 * @return FP_Term|null The most appropriate classic navigation menu to use as a fallback.
	 */
	 ...
	 * @since 6.3.0
	 *
	 * @param FP_Term[] $classic_nav_menus Array of classic nav menu term objects.
	 * @return FP_Term The most recently created classic nav menu.
	 */
	 ...
	 *
	 * @param FP_Term[] $classic_nav_menus Array of classic nav menu term objects.
	 * @return FP_Term|null The classic nav menu with the slug `primary` or null.
	 */
	 ...
	 * @since 6.3.0
	 *
	 * @return FP_Term|null The classic nav menu assigned to the `primary` location or null.
	 */
