<?php
/**
 * Error Protection API: FP_Recovery_Mode_Cookie_Service class
 *
 * @package WordPress
 * @since 5.2.0
 */
...
final class FP_Recovery_Mode_Cookie_Service {
	 ...
	 * @return true|FP_Error True on success, error object on failure.
	 */
	public function validate_cookie( $cookie = '' ) {

		if ( ! $cookie ) {
			if ( empty( $_COOKIE[ RECOVERY_MODE_COOKIE ] ) ) {
				return new FP_Error( 'no_cookie', __( 'No cookie present.' ) );
			}
		...
		if ( ! ctype_digit( $created_at ) ) {
			return new FP_Error( 'invalid_created_at', __( 'Invalid cookie format.' ) );
		}
		...
		if ( time() > $created_at + $length ) {
			return new FP_Error( 'expired', __( 'Cookie expired.' ) );
		}
		...
		if ( ! hash_equals( $signature, $hashed ) ) {
			return new FP_Error( 'signature_mismatch', __( 'Invalid cookie.' ) );
		}
	 ...
	 * @param string $cookie Optionally specify the cookie string.
	 *                       If omitted, it will be retrieved from the super global.
	 * @return string|FP_Error Session ID on success, or error object on failure.
	 */
	 ...
	 * @return array|FP_Error Cookie parts array, or error object on failure.
	 */
	private function parse_cookie( $cookie ) {
		...
		if ( 4 !== count( $parts ) ) {
			return new FP_Error( 'invalid_format', __( 'Invalid cookie format.' ) );
		}
