...
function wp_get_global_settings( $path = array(), $context = array() ) {
	 ...
	 * See `wp_cache_add_non_persistent_groups` in src/fp-includes/load.php and other places.
	 *
	 ...
	 * https://make.finpress.org/core/2022/10/10/filters-for-theme-json-data/
	 *
	...
	if ( false === $settings ) {
		$settings = FP_Theme_JSON_Resolver::get_merged_data( $origin )->get_settings();
		if ( $can_use_cached ) {
 ...
 *              to "var(--fp--preset--font-size--small)" so consumers don't have to.
...
function wp_get_global_styles( $path = array(), $context = array() ) {
	...
	$merged_data = FP_Theme_JSON_Resolver::get_merged_data( $origin );
	if ( $resolve_variables ) {
		$merged_data = FP_Theme_JSON::resolve_variables( $merged_data );
	}
 ...
 *                     See {@see 'FP_Theme_JSON::get_stylesheet'} for all valid types.
...
function wp_get_global_stylesheet( $types = array() ) {
	...
	$tree                = FP_Theme_JSON_Resolver::resolve_theme_file_uris( FP_Theme_JSON_Resolver::get_merged_data() );
...
function wp_add_global_styles_for_blocks() {
	...
	$tree        = FP_Theme_JSON_Resolver::resolve_theme_file_uris( $tree );
	...
	if ( $can_use_cached ) {
		// Hash the merged FP_Theme_JSON data to bust cache on settings or styles change.
	...
	foreach ( $block_nodes as $metadata ) {
		 ...
		 * enqueued for each block on the page in class FP_Block's render function.
		...
		if ( isset( $metadata['name'] ) ) {
			if ( str_starts_with( $metadata['name'], 'core/' ) ) {
				...
				$block_handle = 'fp-block-' . $block_name;
...
function wp_clean_theme_json_cache() {
	...
	FP_Theme_JSON_Resolver::clean_cached_data();
}
...
function wp_get_theme_directory_pattern_slugs() {
	return FP_Theme_JSON_Resolver::get_theme_data( array(), array( 'with_supports' => false ) )->get_patterns();
}
...
function wp_get_theme_data_custom_templates() {
	return FP_Theme_JSON_Resolver::get_theme_data( array(), array( 'with_supports' => false ) )->get_custom_templates();
}
...
function wp_get_theme_data_template_parts() {
	...
	$metadata = FP_Theme_JSON_Resolver::get_theme_data( array(), array( 'with_supports' => false ) )->get_template_parts();
	if ( $can_use_cached ) {
 ...
 * @since 6.3.0
 *
 * @param FP_Block_Type $block_type The block's type.
...
function wp_get_block_css_selector( $block_type, $target = 'root', $fallback = false ) {
	...
	if ( $has_selectors && isset( $block_type->selectors['root'] ) ) {
		...
		$root_selector = ".fp-block-{$block_name}";
	}
	...
	if ( 1 === count( $target ) ) {
		...
		return FP_Theme_JSON::scope_selector( $root_selector, $feature_selector );
	}
