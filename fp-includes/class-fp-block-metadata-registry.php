 ...
 *
 * The FP_Block_Metadata_Registry allows plugins to register metadata for large
 ...
 * @since 6.7.0
 */
class FP_Block_Metadata_Registry {
	...
	public static function register_collection( $path, $manifest ) {
		 ...
		 *
		 * Example:
		 * * It is allowed to register a collection with path `FP_PLUGIN_DIR . '/my-plugin'`.
		 * * It is not allowed to register a collection with path `FP_PLUGIN_DIR`.
		 * * It is not allowed to register a collection with path `dirname( FP_PLUGIN_DIR )`.
		 *
	 ...
	 *
	 * For instance, if a block metadata collection is registered with path `FP_PLUGIN_DIR . '/my-plugin/blocks/'`,
	 ...
	 * * `FP_PLUGIN_DIR . '/my-plugin/blocks/block-a/block.json'`
	 * * `FP_PLUGIN_DIR . '/my-plugin/blocks/block-b/block.json'`
	 *
	 * @since 6.8.0
	 *
	...
	private static function get_default_collection_roots() {
		...
		$collection_roots = array(
			wp_normalize_path( ABSPATH . FPINC ),
			wp_normalize_path( FP_CONTENT_DIR ),
			wp_normalize_path( FPMU_PLUGIN_DIR ),
			wp_normalize_path( FP_PLUGIN_DIR ),
		);
		...
		foreach ( $theme_roots as $theme_root ) {
			$collection_roots[] = trailingslashit( wp_normalize_path( FP_CONTENT_DIR ) ) . ltrim( wp_normalize_path( $theme_root ), '/' );
		}
