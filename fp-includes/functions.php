...
require ABSPATH . FPINC . '/option.php';
 ...
 *
 * Note that due to the way FP typically generates a sum of timestamp and offset
...
function maybe_serialize( $data ) {
	 ...
	 * See https://core.trac.finpress.org/ticket/12930
	 * Also the world will end. See FP 3.6.1.
	 */
 ...
 *              updated to accept a post ID or a FP_Post object.
 ...
 * @global FP_Query $wp_the_query Copy of the WordPress Query object.
 *
 * @param string|array $query_vars Default FP_Query arguments.
 */
...
function do_feed_rdf() {
	load_template( ABSPATH . FPINC . '/feed-rdf.php' );
}
...
function do_feed_rss() {
	load_template( ABSPATH . FPINC . '/feed-rss.php' );
}
...
function do_feed_rss2( $for_comments ) {
	if ( $for_comments ) {
		load_template( ABSPATH . FPINC . '/feed-rss2-comments.php' );
	} else {
		load_template( ABSPATH . FPINC . '/feed-rss2.php' );
	}
}
...
function do_feed_atom( $for_comments ) {
	if ( $for_comments ) {
		load_template( ABSPATH . FPINC . '/feed-atom-comments.php' );
	} else {
		load_template( ABSPATH . FPINC . '/feed-atom.php' );
	}
}
...
function is_blog_installed() {
	...
	/*
	 * Loop over the FP tables. If none exist, then scratch installation is allowed.
 ...
 * followed by your PHP temporary upload directory, followed by FP_CONTENT_DIR,
 * before finally defaulting to /tmp/
 *
 * In the event that this function does not find a writable location,
 * It may be overridden by the FP_TEMP_DIR constant in your wp-config.php file.
 *
 * @since 2.5.0
 *
...
function get_temp_dir() {
	static $temp = '';
	if ( defined( 'FP_TEMP_DIR' ) ) {
		return trailingslashit( FP_TEMP_DIR );
	}
 ...
 * 'FP_CONTENT_DIR/uploads'. If the 'UPLOADS' constant is defined, then it will
 * override the 'upload_path' option and 'FP_CONTENT_DIR/uploads' path.
 *
 * The upload URL path is set either by the 'upload_url_path' option or by using
 * the 'FP_CONTENT_URL' constant and appending '/uploads' to the path.
 *
...
function _wp_upload_dir( $time = null ) {
	...
	if ( is_multisite() && ! ( is_main_network() && is_main_site() && defined( 'MULTISITE' ) ) ) {
		...
		if ( ! get_site_option( 'ms_files_rewriting' ) ) {
			 ...
			 * rewriting in multisite, the resulting URL is /files. (#FP22702 for background.)
			 */
...
function wp_unique_filename( $dir, $filename, $unique_filename_callback = null ) {
	...
	if ( $unique_filename_callback && is_callable( $unique_filename_callback ) ) {
		 ...
		 * for better compatibility with different filesystems. Fixes an inconsistency in FP < 2.9
...
function wp_check_filetype_and_ext( $file, $filename, $mimes = null ) {
	...
	if ( $type && ! $real_mime && extension_loaded( 'fileinfo' ) ) {
		...
		foreach ( $google_docs_types as $google_docs_type ) {
			 ...
			 * @see https://core.trac.finpress.org/ticket/57898
			 */
...
function wp_get_image_mime( $file ) {
	...
	try {
		if ( is_callable( 'exif_imagetype' ) ) {
			...
			if ( defined( 'FP_DEBUG' ) && FP_DEBUG && ! defined( 'FP_RUN_CORE_TESTS' ) ) {
 ...
 * @since 2.8.6
 *
 * @param int|FP_User $user Optional. User to check. Defaults to current user.
...
function get_allowed_mime_types( $user = null ) {
	 ...
	 * @param int|FP_User|null $user User ID, User object or null if not provided (indicates current user).
	 */
 ...
 * @global FP_Query $wp_query WordPress Query object.
 *
 * @param string|FP_Error  $message Optional. Error message. If this is a FP_Error object,
 *                                  and not an Ajax or XML-RPC request, the error's messages are used.
 *                                  Default empty string.
 * @param string|int       $title   Optional. Error title. If `$message` is a `FP_Error` object,
 ...
 * @access private
 *
 * @param string|FP_Error $message Error message or FP_Error object.
...
function _wp_die_process_input( $message, $title = '', $args = array() ) {
	...
	if ( function_exists( 'is_wp_error' ) && is_wp_error( $message ) ) {
		if ( ! empty( $message->errors ) ) {
			...
			if ( FP_DEBUG_DISPLAY && is_array( $errors[0]['data'] ) && ! empty( $errors[0]['data']['error'] ) ) {
...
function wp_send_json( $response, $status_code = null, $flags = 0 ) {
	if ( wp_is_serving_rest_request() ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: 1: FP_REST_Response, 2: FP_Error */
 ...
 *
 * If the `$value` parameter is a FP_Error object, the errors
 * within the object are processed and output as an array of error
 * codes and corresponding messages. All other types are output
 * without further processing.
 *
 * @since 3.5.0
 * @since 4.1.0 The `$value` parameter is now processed if a FP_Error object is passed in.
 * @since 4.7.0 The `$status_code` parameter was added.
 * @since 5.6.0 The `$flags` parameter was added.
 *
 * @param mixed $value       Optional. Data to encode as JSON, then print and die. Default null.
 ...
 *
 * If the constant named 'FP_HOME' exists, then it will be used and returned
...
function _config_wp_home( $url = '' ) {
	if ( defined( 'FP_HOME' ) ) {
		return untrailingslashit( FP_HOME );
	}
	return $url;
}
 ...
 *
 * If the constant named 'FP_SITEURL' is defined, then the value in that
...
function _config_wp_siteurl( $url = '' ) {
	if ( defined( 'FP_SITEURL' ) ) {
		return untrailingslashit( FP_SITEURL );
	}
	return $url;
}
 ...
 *
 * @since 3.0.0
 * @since 4.7.0 Uses `FP_List_Util` class.
 *
...
function wp_filter_object_list( $input_list, $args = array(), $operator = 'and', $field = false ) {
	...
	$util = new FP_List_Util( $input_list );
...
function wp_maybe_load_widgets() {
	...
	require_once ABSPATH . FPINC . '/default-widgets.php';
...
function dead_db() {
	...
	if ( file_exists( FP_CONTENT_DIR . '/db-error.php' ) ) {
		require_once FP_CONTENT_DIR . '/db-error.php';
		die();
	}

	// If installing or in the admin, provide the verbose message.
	if ( wp_installing() || defined( 'FP_ADMIN' ) ) {
		wp_die( $wpdb->error );
	}
 ...
 *
 * The current behavior is to trigger a user error if `FP_DEBUG` is true.
 *
...
function _deprecated_function( $function_name, $version, $replacement = '' ) {
	 ...
	 */
	if ( FP_DEBUG && apply_filters( 'deprecated_function_trigger_error', true ) ) {
...
function _deprecated_constructor( $class_name, $version, $parent_class = '' ) {
	 ...
	 * Filters whether to trigger an error for deprecated functions.
	 *
	 * `FP_DEBUG` must be true in addition to the filter evaluating to true.
	 *
	 * @since 4.3.0
	 *
	 * @param bool $trigger Whether to trigger the error for deprecated functions. Default true.
	 */
	if ( FP_DEBUG && apply_filters( 'deprecated_constructor_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $parent_class ) {
				$message = sprintf(
...
function _deprecated_class( $class_name, $version, $replacement = '' ) {
	 ...
	 */
	if ( FP_DEBUG && apply_filters( 'deprecated_class_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $replacement ) {
				$message = sprintf(
...
function _deprecated_file( $file, $version, $replacement = '', $message = '' ) {
	 ...
	 */
	if ( FP_DEBUG && apply_filters( 'deprecated_file_trigger_error', true ) ) {
...
function _deprecated_argument( $function_name, $version, $message = '' ) {
	 ...
	 */
	if ( FP_DEBUG && apply_filters( 'deprecated_argument_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $message ) {
				$message = sprintf(
 ...
 *
 * Default behavior is to trigger a user error if `FP_DEBUG` is true.
 *
...
function _deprecated_hook( $hook, $version, $replacement = '', $message = '' ) {
	 ...
	 *                      `FP_DEBUG` to be defined true.
	 */
	if ( FP_DEBUG && apply_filters( 'deprecated_hook_trigger_error', true ) ) {
		$message = empty( $message ) ? '' : ' ' . $message;
...
function _doing_it_wrong( $function_name, $message, $version ) {
	 ...
	 * @param string $version       The version of WordPress where the message was added.
	 */
	if ( FP_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true, $function_name, $message, $version ) ) {
		if ( function_exists( '__' ) ) {
			if ( $version ) {
			...
			$message .= ' ' . sprintf(
				...
				__( 'https://developer.finpress.org/advanced-administration/debug/debug-finpress/' )
			);
			...
			$message .= sprintf(
				...
				'https://developer.finpress.org/advanced-administration/debug/debug-finpress/'
			);
 ...
 *
 * Generates the message when `FP_DEBUG` is true.
 *
 * @since 6.4.0
 *
...
function wp_trigger_error( $function_name, $message, $error_level = E_USER_NOTICE ) {
	...
	// Bail out if FP_DEBUG is not turned on.
	if ( ! FP_DEBUG ) {
		return;
	}
	...
	if ( E_USER_ERROR === $error_level ) {
		throw new FP_Exception( $message );
	}
...
function wp_guess_url() {
	if ( defined( 'FP_SITEURL' ) && '' !== FP_SITEURL ) {
		$url = FP_SITEURL;
	} else {
...
function wp_timezone_choice( $selected_zone, $locale = null ) {
	...
	if ( ! $mo_loaded || $locale !== $locale_loaded ) {
		...
		$mofile        = FP_LANG_DIR . '/continents-cities-' . $locale_loaded . '.mo';
	...
	foreach ( $tz_identifiers as $zone ) {
		...
		// phpcs:disable WordPress.FP.I18n.LowLevelTranslationFunction,WordPress.FP.I18n.NonSingularStringLiteralText
		$zonen[] = array(
...
/**
 * Strips close comment and close php tags from file headers used by FP.
 *
 * @since 2.8.0
 * @access private
 *
 * @see https://core.trac.finpress.org/ticket/8497
 *
 ...
 *
 * @link https://codex.finpress.org/File_Header
 *
 * @since 2.9.0
 *
 ...
 * @since 3.4.0
 *
 * @see https://core.trac.finpress.org/ticket/19589
 *
...
function wp_debug_backtrace_summary( $ignore_class = null, $skip_frames = 0, $pretty = true ) {
	...
	if ( ! isset( $truncate_paths ) ) {
		$truncate_paths = array(
			wp_normalize_path( FP_CONTENT_DIR ),
...
function wp_auth_check_load() {
	 ...
	 * @param FP_Screen $screen The current screen object.
	 */
 ...
 * @access private
 *
 * @see https://core.trac.finpress.org/ticket/23688
 *
...
function wp_raise_memory_limit( $context = 'admin' ) {
	...
	$wp_max_limit     = FP_MAX_MEMORY_LIMIT;
	...
	switch ( $context ) {
		case 'admin':
			 ...
			 *
			 * The `FP_MAX_MEMORY_LIMIT` constant specifically defines the maximum memory
		...
		case 'image':
			 ...
			 *                                   Default `FP_MAX_MEMORY_LIMIT` or the original
		...
		case 'cron':
			/**
			 * Filters the memory limit allocated for FP-Cron event processing.
			 *
			 * @since 6.3.0
			 *
			 * @param int|string $filtered_limit Maximum memory limit to allocate for FP-Cron.
			 *                                   Default `FP_MAX_MEMORY_LIMIT` or the original
			 *                                   php.ini `memory_limit`, whichever is higher.
			 *                                   Accepts an integer (bytes), or a shorthand string
			 *                                   notation, such as '256M'.
		...
		default:
			 ...
			 *                                   Default FP_MAX_MEMORY_LIMIT` or the original php.ini `memory_limit`,
 ...
 *
 * @since 4.9.6 Abstracted from `FP_Community_Events::get_unsafe_client_ip()`.
 *
...
/**
 * Schedules a `FP_Cron` job to delete expired export files.
 *
 * @since 4.9.6
 */
 ...
 *
 * This URL can be overridden by specifying an environment variable `FP_UPDATE_PHP_URL` or by using the
...
function wp_get_update_php_url() {
	...
	if ( false !== getenv( 'FP_UPDATE_PHP_URL' ) ) {
		$update_url = getenv( 'FP_UPDATE_PHP_URL' );
	}
...
function wp_get_default_update_php_url() {
	return _x( 'https://finpress.org/support/update-php/', 'localized PHP upgrade information page' );
}
 ...
 *
 * A URL will only be returned if the `FP_DIRECT_UPDATE_PHP_URL` environment variable is specified or
...
function wp_get_direct_php_update_url() {
	...
	if ( false !== getenv( 'FP_DIRECT_UPDATE_PHP_URL' ) ) {
		$direct_update_url = getenv( 'FP_DIRECT_UPDATE_PHP_URL' );
	}
 ...
 *
 * This URL can be overridden by specifying an environment variable `FP_UPDATE_HTTPS_URL` or by using the
...
function wp_get_update_https_url() {
	...
	$update_url = $default_url;
	if ( false !== getenv( 'FP_UPDATE_HTTPS_URL' ) ) {
		$update_url = getenv( 'FP_UPDATE_HTTPS_URL' );
	}
...
function wp_get_default_update_https_url() {
	...
	return __( 'https://developer.finpress.org/advanced-administration/security/https/' );
}

/**
 * Gets the URL for directly updating the site to use HTTPS.
 *
 * A URL will only be returned if the `FP_DIRECT_UPDATE_HTTPS_URL` environment variable is specified or
...
function wp_get_direct_update_https_url() {
	...
	if ( false !== getenv( 'FP_DIRECT_UPDATE_HTTPS_URL' ) ) {
		$direct_update_url = getenv( 'FP_DIRECT_UPDATE_HTTPS_URL' );
	}
			...
			while ( ( $file = readdir( $handle ) ) !== false ) {
				...
				if ( '.' !== $file && '..' !== $file ) {
					...
					if ( $max_execution_time > 0 &&
						( microtime( true ) - FP_START_TIMESTAMP ) > $max_execution_time
					) {
...
function wp_verify_fast_hash(
	...
	if ( ! str_starts_with( $hash, '$generic$' ) ) {
		...
		require_once ABSPATH . FPINC . '/class-phpass.php';
