<?php
/**
 * REST API: FP_REST_Term_Search_Handler class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.6.0
 ...
 */
class FP_REST_Term_Search_Handler extends FP_REST_Search_Handler {
	 ...
	 *     @type string|int|FP_Error $total Numeric string containing the number of terms in that
	 *                                      taxonomy, 0 if there are no results, or FP_Error if
	 *                                      the requested taxonomy does not exist.
	 * }
	 */
	public function search_items( FP_REST_Request $request ) {
		$taxonomies = $request[ FP_REST_Search_Controller::PROP_SUBTYPE ];
		if ( in_array( FP_REST_Search_Controller::TYPE_ANY, $taxonomies, true ) ) {
	...
	public function prepare_item( $id, array $fields ) {
		...
		if ( in_array( FP_REST_Search_Controller::PROP_ID, $fields, true ) ) {
			$data[ FP_REST_Search_Controller::PROP_ID ] = (int) $id;
		}
		if ( in_array( FP_REST_Search_Controller::PROP_TITLE, $fields, true ) ) {
			$data[ FP_REST_Search_Controller::PROP_TITLE ] = $term->name;
		}
		if ( in_array( FP_REST_Search_Controller::PROP_URL, $fields, true ) ) {
			$data[ FP_REST_Search_Controller::PROP_URL ] = get_term_link( $id );
		}
		if ( in_array( FP_REST_Search_Controller::PROP_TYPE, $fields, true ) ) {
			$data[ FP_REST_Search_Controller::PROP_TYPE ] = $term->taxonomy;
		}
