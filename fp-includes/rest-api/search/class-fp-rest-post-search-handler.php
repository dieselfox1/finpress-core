<?php
/**
 * REST API: FP_REST_Post_Search_Handler class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.0.0
 ...
 * @since 5.0.0
 *
 * @see FP_REST_Search_Handler
 */
class FP_REST_Post_Search_Handler extends FP_REST_Search_Handler {
	 ...
	 * @since 5.0.0
	 *
	 * @param FP_REST_Request $request Full REST request.
	 * @return array {
	 ...
	 * }
	 */
	public function search_items( FP_REST_Request $request ) {

		// Get the post types to search for the current request.
		$post_types = $request[ FP_REST_Search_Controller::PROP_SUBTYPE ];
		if ( in_array( FP_REST_Search_Controller::TYPE_ANY, $post_types, true ) ) {
		 ...
		 * @param FP_REST_Request $request    The request used.
		 */
	...
	public function prepare_item( $id, array $fields ) {
		...
		if ( in_array( FP_REST_Search_Controller::PROP_ID, $fields, true ) ) {
			$data[ FP_REST_Search_Controller::PROP_ID ] = (int) $post->ID;
		}

		if ( in_array( FP_REST_Search_Controller::PROP_TITLE, $fields, true ) ) {
			if ( post_type_supports( $post->post_type, 'title' ) ) {
				add_filter( 'protected_title_format', array( $this, 'protected_title_format' ) );
				add_filter( 'private_title_format', array( $this, 'protected_title_format' ) );
				$data[ FP_REST_Search_Controller::PROP_TITLE ] = get_the_title( $post->ID );
				remove_filter( 'protected_title_format', array( $this, 'protected_title_format' ) );
				remove_filter( 'private_title_format', array( $this, 'protected_title_format' ) );
			} else {
				$data[ FP_REST_Search_Controller::PROP_TITLE ] = '';
			}
		}

		if ( in_array( FP_REST_Search_Controller::PROP_URL, $fields, true ) ) {
			$data[ FP_REST_Search_Controller::PROP_URL ] = get_permalink( $post->ID );
		}

		if ( in_array( FP_REST_Search_Controller::PROP_TYPE, $fields, true ) ) {
			$data[ FP_REST_Search_Controller::PROP_TYPE ] = $this->type;
		}

		if ( in_array( FP_REST_Search_Controller::PROP_SUBTYPE, $fields, true ) ) {
			$data[ FP_REST_Search_Controller::PROP_SUBTYPE ] = $post->post_type;
		}
