<?php
/**
 * REST API: FP_REST_URL_Details_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class FP_REST_URL_Details_Controller extends FP_REST_Controller {
	 ...
	 * @return FP_REST_Response|FP_Error The parsed details as a response object. FP_Error if there are errors.
	 */
	public function parse_url_details( $request ) {
		$url = untrailingslashit( $request['url'] );

		if ( empty( $url ) ) {
			return new FP_Error( 'rest_invalid_url', __( 'Invalid URL' ), array( 'status' => 404 ) );
		}
	 ...
	 * @since 5.9.0
	 *
	 * @return true|FP_Error True if the request has permission, else FP_Error.
	 */
	 ...
	 * @return string|FP_Error The HTTP response from the remote URL on success.
	 *                         FP_Error if no response or no content.
	 */
	private function get_remote_url( $url ) {
		 ...
		 * to that used as the default UA string by the FP HTTP API. Therefore requests from this
		 * REST endpoint are being unintentionally blocked as they are misidentified as pingback requests.
		 * By slightly modifying the UA string, but still retaining the "WordPress" identification (via "FP")
		 * we are able to work around this issue.
		 * Example UA string: `FP-URLDetails/5.9-alpha-51389 (+http://localhost:8888)`.
		*/
		$modified_user_agent = 'FP-URLDetails/' . get_bloginfo( 'version' ) . ' (+' . get_bloginfo( 'url' ) . ')';
		 ...
		 *
		 * Can be used to adjust response size limit and other FP_Http::request() args.
		 *
		 * @since 5.9.0
		 *
		...
		if ( FP_Http::OK !== wp_remote_retrieve_response_code( $response ) ) {
			// Not saving the error response to cache since the error might be temporary.
			return new FP_Error(
				'no_response',
				__( 'URL not found. Response returned a non-200 status code for this URL.' ),
				array( 'status' => FP_Http::NOT_FOUND )
			);
		}
	...
	private function get_icon( $html, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			...
			$icon     = FP_Http::make_absolute_url( $icon, $root_url );
		}
	...
	private function get_image( $meta_elements, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			$root_url = $parsed_url['scheme'] . '://' . $parsed_url['host'] . '/';
			$image    = FP_Http::make_absolute_url( $image, $root_url );
		}
