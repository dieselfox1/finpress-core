<?php
/**
 * REST API: FP_REST_Search_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.0.0
 ...
 */
class FP_REST_Search_Controller extends FP_REST_Controller {
	...
	public function __construct( array $search_handlers ) {
		...
		foreach ( $search_handlers as $search_handler ) {
			if ( ! $search_handler instanceof FP_REST_Search_Handler ) {
				_doing_it_wrong(
					__METHOD__,
					/* translators: %s: PHP class name. */
					sprintf( __( 'REST search handlers must extend the %s class.' ), 'FP_REST_Search_Handler' ),
					'5.0.0'
				);
				continue;
			}
	 ...
	 * @return true|FP_Error True if the request has search access, FP_Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		if ( ! isset( $result[ FP_REST_Search_Handler::RESULT_IDS ] ) || ! is_array( $result[ FP_REST_Search_Handler::RESULT_IDS ] ) || ! isset( $result[ FP_REST_Search_Handler::RESULT_TOTAL ] ) ) {
			return new FP_Error(
		...
		$ids = $result[ FP_REST_Search_Handler::RESULT_IDS ];
		...
		$total     = (int) $result[ FP_REST_Search_Handler::RESULT_TOTAL ];
		...
		$response = $is_head_request ? new FP_REST_Response( array() ) : rest_ensure_response( $results );
		$response->header( 'X-FP-Total', $total );
	 ...
	 * @return string[]|FP_Error List of valid subtypes, or FP_Error object on failure.
	 */
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return FP_REST_Search_Handler|FP_Error Search handler for the request type, or FP_Error object on failure.
	 */
