<?php
/**
 * REST API: FP_REST_Menu_Items_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 * @since 5.9.0
 *
 * @see FP_REST_Posts_Controller
 */
class FP_REST_Menu_Items_Controller extends FP_REST_Posts_Controller {
	 ...
	 * @return object|FP_Error Post object if ID is valid, FP_Error otherwise.
	 */
	...
	protected function check_has_read_only_access( $request ) {
		 ...
		 *                                             via the REST API.
		 * @param FP_REST_Request    $request          Full details about the request.
		 * @param FP_REST_Controller $controller       The current instance of the controller.
		 */
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new FP_Error( 'rest_post_exists', __( 'Cannot create existing post.' ), array( 'status' => 400 ) );
		}
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return FP_REST_Response|FP_Error True on success, or FP_Error object on failure.
	 */
	public function delete_item( $request ) {
		...
		if ( ! $request['force'] ) {
			...
			return new FP_Error( 'rest_trash_not_supported', sprintf( __( "Menu items do not support trashing. Set '%s' to delete." ), 'force=true' ), array( 'status' => 501 ) );
		}
		...
		if ( ! $result ) {
			return new FP_Error( 'rest_cannot_delete', __( 'The post cannot be deleted.' ), array( 'status' => 500 ) );
		}
