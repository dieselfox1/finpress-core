<?php
/**
 * REST API: FP_REST_Revisions_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class FP_REST_Revisions_Controller extends FP_REST_Controller {
	...
	public function __construct( $parent_post_type ) {
		...
		if ( ! $parent_controller ) {
			$parent_controller = new FP_REST_Posts_Controller( $parent_post_type );
		}
		...
		$this->meta              = new FP_REST_Post_Meta_Fields( $parent_post_type );
	}
	 ...
	 * @return FP_Post|FP_Error Revision post object if ID is valid, FP_Error otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		if ( ! $is_head_request ) {
			...
			$response = new FP_REST_Response( array() );
		}

		$response->header( 'X-FP-Total', (int) $total_revisions );
	...
	public function delete_item( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param FP_Post|false|null $result The revision object (if it was deleted or moved to the Trash successfully)
	 ...
	 *
	 * @param FP_Post         $item    Post revision object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return apply_filters( 'rest_prepare_revision', new FP_REST_Response( array() ), $post, $request );
		}
		 ...
		 * @param FP_Post          $post     The original revision object.
	 ...
	 * @param FP_Post $post    Post revision object.
