<?php
/**
 * REST API: FP_REST_Widget_Types_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class FP_REST_Widget_Types_Controller extends FP_REST_Controller {
	 ...
	 * @since 5.8.0
	 *
	 * @return true|FP_Error True if the widget type is visible, FP_Error otherwise.
	 */
	 ...
	 * @return array|FP_Error The array of widget data if the name is valid, FP_Error otherwise.
	 */
	public function get_widget( $id ) {
		...
		return new FP_Error( 'rest_widget_type_invalid', __( 'Invalid widget type.' ), array( 'status' => 404 ) );
	}
	 ...
	 * @param FP_REST_Request $request Full details about the request.
	 * @return FP_REST_Response Widget type data.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return apply_filters( 'rest_prepare_widget_type', new FP_REST_Response( array() ), $widget_type, $request );
		}
	...
	public function encode_form_data( $request ) {
		...
		if ( ! $widget_object ) {
			return new FP_Error(
				'rest_invalid_widget',
				__( 'Cannot preview a widget that does not extend FP_Widget.' ),
				array( 'status' => 400 )
			);
		}
	...
	/**
	 * Returns the output of FP_Widget::widget() when called with the provided
	...
	/**
	 * Returns the output of FP_Widget::form() when called with the provided
	 * instance. Used by encode_form_data() to preview a widget's form.
	 *
	 * @since 5.8.0
	 *
	 * @param FP_Widget $widget_object Widget object to call widget() on.
	 * @param array     $instance Widget instance settings.
	 * @return string
	 */
