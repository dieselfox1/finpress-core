<?php
/**
 * REST API: FP_REST_Application_Passwords_Controller class
 *
 * @package    WordPress
 * @subpackage REST_API
 * @since      5.6.0
 ...
 */
class FP_REST_Application_Passwords_Controller extends FP_REST_Controller {
	...
	public function get_items( $request ) {
		...
		$passwords = FP_Application_Passwords::get_user_application_passwords( $user->ID );
		$response  = array();
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has read access for the item, FP_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		$created = FP_Application_Passwords::create_new_application_password( $user->ID, wp_slash( (array) $prepared ) );
		...
		$password = $created[0];
		$item     = FP_Application_Passwords::get_user_application_password( $user->ID, $created[1]['uuid'] );

		$item['new_password'] = FP_Application_Passwords::chunk_password( $password );
	...
	public function update_item( $request ) {
		...
		$saved = FP_Application_Passwords::update_application_password( $user->ID, $item['uuid'], wp_slash( (array) $prepared ) );
		...
		$item = FP_Application_Passwords::get_user_application_password( $user->ID, $item['uuid'] );
	...
	public function delete_items( $request ) {
		...
		$deleted = FP_Application_Passwords::delete_all_application_passwords( $user->ID );
	...
	public function delete_item( $request ) {
		...
		$deleted  = FP_Application_Passwords::delete_application_password( $user->ID, $password['uuid'] );
	...
	public function get_current_item( $request ) {
		...
		$password = FP_Application_Passwords::get_user_application_password( $user->ID, $uuid );
	 ...
	 *
	 * @param FP_REST_Request $request Request object.
	 * @return object|FP_Error The prepared item, or FP_Error object on failure.
	 */
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$response = new FP_REST_Response( $prepared );
	 ...
	 * @since 5.6.0
	 *
	 * @param FP_User $user The requested user.
	 * @param array   $item The application password.
	 * @return array The list of links.
	 */
	protected function prepare_links( FP_User $user, $item ) {
		return array(
			'self' => array(
				'href' => rest_url(
					sprintf(
	 ...
	 *
	 * @param FP_REST_Request $request The request object.
	 * @return FP_User|FP_Error The WordPress user associated with the request, or a FP_Error if none found.
	 */
	 ...
	 *
	 * @param FP_REST_Request $request The request object.
	 * @return array|FP_Error The application password details if found, a FP_Error otherwise.
	 */
	protected function get_application_password( $request ) {
		...
		$password = FP_Application_Passwords::get_user_application_password( $user->ID, $request['uuid'] );
