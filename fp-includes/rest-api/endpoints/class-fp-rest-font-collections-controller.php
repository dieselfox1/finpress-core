 ...
 * @since 6.5.0
 */
class FP_REST_Font_Collections_Controller extends FP_REST_Controller {
	...
	public function get_items( $request ) {
		$collections_all = FP_Font_Library::get_instance()->get_font_collections();
		...
		$response = $is_head_request ? new FP_REST_Response( array() ) : rest_ensure_response( $items );

		$response->header( 'X-FP-Total', (int) $total_items );
	...
	public function get_item( $request ) {
		...
		$collection = FP_Font_Library::get_instance()->get_font_collection( $slug );

		if ( ! $collection ) {
			return new FP_Error( 'rest_font_collection_not_found', __( 'Font collection not found.' ), array( 'status' => 404 ) );
		}
	...
	* @since 6.5.0
	*
	* @param FP_Font_Collection $item    Font collection object.
	...
	*/
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( ! empty( array_intersect( $fields, $data_fields ) ) ) {
			 ...
			 * Can't exit at the beginning of the method due to the potential need to return a FP_Error object.
			 */
			if ( $request->is_method( 'HEAD' ) ) {
				/** This filter is documented in wp-includes/rest-api/endpoints/class-wp-rest-font-collections-controller.php */
				return apply_filters( 'rest_prepare_font_collection', new FP_REST_Response( array() ), $item, $request );
			}
		 ...
		 * @param FP_Font_Collection $item     The font collection object.
	 ...
	 * @since 6.5.0
	 *
	 * @param FP_Font_Collection $collection Font collection data
