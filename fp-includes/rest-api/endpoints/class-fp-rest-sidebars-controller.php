<?php
/**
 * REST API: FP_REST_Sidebars_Controller class
 *
 ...
 */
class FP_REST_Sidebars_Controller extends FP_REST_Controller {
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return FP_REST_Response Response object on success.
	 */
	...
	public function get_item( $request ) {
		...
		$sidebar = $this->get_sidebar( $request['id'] );
		if ( ! $sidebar ) {
			return new FP_Error( 'rest_sidebar_not_found', __( 'No sidebar exists with that id.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return FP_REST_Response Response object on success, or FP_Error object on failure.
	 */
	 ...
	 * @param FP_REST_Request $request Full details about the request.
	 * @return FP_REST_Response Prepared response object.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return apply_filters( 'rest_prepare_sidebar', new FP_REST_Response( array() ), $raw_sidebar, $request );
		}
