<?php
/**
 * REST API: FP_REST_Terms_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class FP_REST_Terms_Controller extends FP_REST_Controller {
	...
	public function __construct( $taxonomy ) {
		...
		$this->meta = new FP_REST_Term_Meta_Fields( $taxonomy );
	}
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return bool|FP_Error True if the request has read access, otherwise false or FP_Error object.
	 */
	...
	public function get_items( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @link https://developer.finpress.org/reference/functions/get_terms/
		 *
		...
		$response = $is_head_request ? new FP_REST_Response( array() ) : rest_ensure_response( $response );

		// Store pagination values for headers.
		$per_page = (int) $prepared_args['number'];
		$page     = (int) ceil( ( ( (int) $prepared_args['offset'] ) / $per_page ) + 1 );

		$response->header( 'X-FP-Total', (int) $total_terms );
	 ...
	 * @return FP_Term|FP_Error Term object if ID is valid, FP_Error otherwise.
	 */
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return bool|FP_Error True if the request has access to create items, otherwise false or FP_Error object.
	 */
	...
	public function create_item( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param FP_Term         $term     Inserted or updated term object.
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has access to update the item, false or FP_Error object otherwise.
	 */
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has access to delete the item, otherwise false or FP_Error object.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param FP_Term          $term     The deleted term.
	...
	public function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['parent'] ) && ! empty( $schema['properties']['parent'] ) ) {
			...
			if ( $requested_parent ) {
				...
				if ( $parent_term instanceof FP_Term ) {
	 ...
	 * @since 4.7.0
	 *
	 * @param FP_Term         $item    Term object.
	 * @param FP_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return apply_filters( "rest_prepare_{$this->taxonomy}", new FP_REST_Response( array() ), $item, $request );
		}
		 ...
		 * @param FP_Term           $item      The original term object.
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal FP_Term_Query parameter.  Use the
		 * `rest_{$this->taxonomy}_query` filter to set FP_Term_Query parameters.
		 *
		 * @since 4.7.0
		 *
		 * @param array       $query_params JSON Schema-formatted collection parameters.
		 * @param FP_Taxonomy $taxonomy     Taxonomy object.
		 */
