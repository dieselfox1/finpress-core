<?php
/**
 * REST API: FP_REST_Posts_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class FP_REST_Posts_Controller extends FP_REST_Controller {
	/**
	 * Post type.
	 *
	 * @since 4.7.0
	...
	public function __construct( $post_type ) {
		...
		$this->meta = new FP_REST_Post_Meta_Fields( $this->post_type );
	}
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				...
				array(
					...
					'args'                => $this->get_endpoint_args_for_item_schema( FP_REST_Server::CREATABLE ),
				),
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => FP_REST_Server::EDITABLE,
					'callback'            => array( $this, 'update_item' ),
					'permission_callback' => array( $this, 'update_item_permissions_check' ),
					'args'                => $this->get_endpoint_args_for_item_schema( FP_REST_Server::EDITABLE ),
				),
				array(
					'methods'             => FP_REST_Server::DELETABLE,
	 ...
	 * @return true|FP_Error True if the request has read access, FP_Error object otherwise.
	 */
	 ...
	 * @param FP_Post $post     The post been password checked.
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return FP_REST_Response|FP_Error Response object on success, or FP_Error object on failure.
	 */
	public function get_items( $request ) {
		 ...
		 * values are accepted as-passed, and their internal FP_Query parameter
		...
		if ( isset( $registered['sticky'], $request['sticky'] ) ) {
			...
			if ( $request['sticky'] ) {
				 ...
				 * FP_Query won't return "no posts" for post__in = array()
		...
		/**
		 * Filters FP_Query arguments when querying posts via the REST API.
		 *
		 ...
		 *
		 * @param array           $args    Array of arguments for FP_Query.
		 * @param FP_REST_Request $request The REST API request.
		 */
		$args       = apply_filters( "rest_{$this->post_type}_query", $args, $request );
		$query_args = $this->prepare_items_query( $args, $request );

		$posts_query  = new FP_Query();
		...
		$response = $is_head_request ? new FP_REST_Response( array() ) : rest_ensure_response( $posts );

		$response->header( 'X-FP-Total', (int) $total_posts );
		$response->header( 'X-FP-TotalPages', (int) $max_pages );
	 ...
	 * @return FP_Post|FP_Error Post object if ID is valid, FP_Error otherwise.
	 */
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return bool|FP_Error True if the request has read access for the item, FP_Error object or false otherwise.
	 */
	 ...
	 * @since 4.7.0
	 *
	 * @param FP_Post         $post    Post to check against.
	 * @param FP_REST_Request $request Request data to check.
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has access to create items, FP_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param FP_Post         $post     Inserted or updated post object.
	...
	public function update_item( $request ) {
		...
		// Filter is fired in FP_REST_Attachments_Controller subclass.
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has access to delete the item, FP_Error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @param FP_Post $post           The Post object being considered for trashing support.
		 */
		 ...
		 * @since 4.7.0
		 *
		 * @param FP_Post          $post     The deleted or trashed post.
		 * @param FP_REST_Response $response The response data.
		 * @param FP_REST_Request  $request  The request sent to the API.
		 */
	 ...
	 * @since 4.7.0
	 *
	 * @param array           $prepared_args Optional. Prepared FP_Query arguments. Default empty array.
	 * @param FP_REST_Request $request       Optional. Full details about the request.
	...
	protected function prepare_items_query( $prepared_args = array(), $request = null ) {
		...
		// Map to proper FP_Query orderby param.
	 ...
	 *
	 * @param FP_REST_Request $request Request object.
	 * @return stdClass|FP_Error Post object or FP_Error.
	 */
	 ...
	 * @return true|FP_Error True if the status is valid, or FP_Error if not.
	 */
	 ...
	 * @param FP_Post_Type $post_type   Post type.
	 * @return string|FP_Error Post status or FP_Error if lacking the proper permission.
	 */
	 ...
	 * @return bool|FP_Error Whether the post thumbnail was successfully deleted, otherwise FP_Error.
	 */
	 ...
	 * @param FP_REST_Request $request  Request.
	 * @return true|FP_Error True if template is still valid or if the same as existing value, or a FP_Error if template not supported.
	 */
	 ...
	 * @param FP_REST_Request $request The request object with post and terms data.
	 * @return null|FP_Error FP_Error on an error assigning any of the terms, otherwise null.
	 */
	 ...
	 * @since 4.7.0
	 *
	 * @param FP_Post_Type|string $post_type Post type name or object.
	 ...
	 *
	 * @param FP_Post         $item    Post object.
	 * @param FP_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return apply_filters( "rest_prepare_{$this->post_type}", new FP_REST_Response( array() ), $post, $request );
		}
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal FP_Query parameter. Use the
		 * `rest_{$this->post_type}_query` filter to set FP_Query parameters.
		 *
		 * @since 4.7.0
		 *
	 ...
	 * @return array|FP_Error A list of valid statuses, otherwise FP_Error object.
	 */
	public function sanitize_post_statuses( $statuses, $request, $parameter ) {
		$statuses = wp_parse_slug_list( $statuses );

		// The default status is different in FP_REST_Attachments_Controller.
	 ...
	 * @since 5.7.0
	 *
	 * @param array           $args    FP_Query arguments.
	 * @param FP_REST_Request $request Full details about the request.
	 * @return array Updated query arguments.
	 */
	private function prepare_tax_query( array $args, FP_REST_Request $request ) {
