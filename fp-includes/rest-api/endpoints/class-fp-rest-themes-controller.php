<?php
/**
 * REST API: FP_REST_Themes_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.0.0
 ...
 */
class FP_REST_Themes_Controller extends FP_REST_Controller {
	 ...
	 * @since 5.7.0
	 *
	 * @return true|FP_Error True if the theme can be read, FP_Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		$response->header( 'X-FP-Total', count( $themes ) );
		$response->header( 'X-FP-TotalPages', 1 );
	 ...
	 *
	 * @param FP_Theme        $item    Theme object.
	 ...
	 * @since 5.7.0
	 *
	 * @param FP_Theme $theme Theme data.
	...
	protected function prepare_links( $theme ) {
		...
		if ( $this->is_same_theme( $theme, wp_get_theme() ) ) {
			...
			$id = FP_Theme_JSON_Resolver::get_user_global_styles_post_id();
		} else {
			$user_cpt = FP_Theme_JSON_Resolver::get_user_data_from_wp_global_styles( $theme );
	 ...
	 * @since 5.7.0
	 *
	 * @param FP_Theme $theme_a First theme to compare.
	 * @param FP_Theme $theme_b Second theme to compare.
	 * @return bool
	 */
