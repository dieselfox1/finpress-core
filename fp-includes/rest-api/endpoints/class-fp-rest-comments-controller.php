<?php
/**
 * REST API: FP_REST_Comments_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class FP_REST_Comments_Controller extends FP_REST_Controller {

	/**
	 * Instance of a comment meta fields object.
	 *
	 * @since 4.7.0
	 * @var FP_REST_Comment_Meta_Fields
	 */
	protected $meta;
	...
	public function __construct() {
		...
		$this->meta = new FP_REST_Comment_Meta_Fields();
	}
	 ...
	 * @return true|FP_Error True if the request has read access, error object otherwise.
	 */
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return FP_REST_Response|FP_Error Response object on success, or error object on failure.
	 */
	public function get_items( $request ) {
		...
		/**
		 * Filters FP_Comment_Query arguments when querying comments via the REST API.
		 *
		 * @since 4.7.0
		 *
		 * @link https://developer.finpress.org/reference/classes/wp_comment_query/
		 *
		 * @param array           $prepared_args Array of arguments for FP_Comment_Query.
		...
		$response = $is_head_request ? new FP_REST_Response( array() ) : rest_ensure_response( $comments );
		$response->header( 'X-FP-Total', $total_comments );
		$response->header( 'X-FP-TotalPages', $max_pages );
	 ...
	 * @return FP_Comment|FP_Error Comment object if ID is valid, FP_Error otherwise.
	 */
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has read access for the item, error object otherwise.
	 */
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has access to create items, error object otherwise.
	 */
	public function create_item_permissions_check( $request ) {
		if ( ! is_user_logged_in() ) {
			 ...
			 * @param FP_REST_Request $request Request used to generate the
			 *                                 response.
			 */
	...
	public function create_item( $request ) {
		 ...
		 * Returning a FP_Error value from the filter will short-circuit insertion and allow
		 * skipping further processing.
		 *
		 * @since 4.7.0
		 * @since 4.8.0 `$prepared_comment` can now be a FP_Error to short-circuit insertion.
		 *
		 * @param array|FP_Error  $prepared_comment The prepared comment data for wp_insert_comment().
		 * @param FP_REST_Request $request          Request used to insert the comment.
		 */
		 ...
		 * @since 4.7.0
		 *
		 * @param FP_Comment      $comment  Inserted or updated comment object.
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has access to update the item, error object otherwise.
	 */
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has access to delete the item, error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @param FP_Comment $comment        The comment object being considered for trashing support.
		 */
		 ...
		 * @since 4.7.0
		 *
		 * @param FP_Comment       $comment  The deleted comment data.
		 * @param FP_REST_Response $response The response returned from the API.
	 ...
	 *
	 * @param FP_Comment      $item    Comment object.
	 * @param FP_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return apply_filters( 'rest_prepare_comment', new FP_REST_Response( array() ), $comment, $request );
		}
		 ...
		 * @param FP_Comment        $comment  The original comment object.
	 ...
	 *
	 * @param FP_REST_Request $request Request object.
	 * @return array|FP_Error Prepared comment, otherwise FP_Error object.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['author'] ) ) {
			$user = new FP_User( $request['author'] );
		 ...
		 * @param FP_REST_Request $request          The current request.
		 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal FP_Comment_Query parameter. Use the
		 * `rest_comment_query` filter to set FP_Comment_Query parameters.
		 *
		 * @since 4.7.0
		 *
	...
	protected function check_read_post_permission( $post, $request ) {
		...
		/*
		 * Ensure the posts controller is specifically a FP_REST_Posts_Controller instance
		 * before using methods specific to that controller.
		 */
		if ( ! $posts_controller instanceof FP_REST_Posts_Controller ) {
			$posts_controller = new FP_REST_Posts_Controller( $post->post_type );
		}
	 ...
	 * @return string|FP_Error The sanitized email address, if valid,
	 *                         otherwise an error.
	 */
