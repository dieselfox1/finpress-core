<?php
/**
 * REST API: FP_REST_Global_Styles_Controller class
 *
 * @package    WordPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class FP_REST_Global_Styles_Controller extends FP_REST_Posts_Controller {
	/**
	 ...
	 * @return stdClass|FP_Error Prepared item on success. FP_Error on when the custom CSS is not valid.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['styles'] ) || isset( $request['settings'] ) ) {
			...
			$variations = FP_Theme_JSON_Resolver::get_style_variations( 'block' );
			...
			$config['version']                     = FP_Theme_JSON::LATEST_SCHEMA;
	 ...
	 *
	 * @param FP_Post         $post    Global Styles post object.
	 * @param FP_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( $is_global_styles_user_theme_json ) {
			$theme_json = new FP_Theme_JSON( $raw_config, 'custom' );
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			if ( $theme_json ) {
				$resolved_theme_uris = FP_Theme_JSON_Resolver::get_resolved_theme_uris( $theme_json );
	...
	public function get_theme_item( $request ) {
		...
		$theme  = FP_Theme_JSON_Resolver::get_merged_data( 'theme' );
		$fields = $this->get_fields_for_response( $request );
		$data   = array();
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			$resolved_theme_uris = FP_Theme_JSON_Resolver::get_resolved_theme_uris( $theme );
			if ( ! empty( $resolved_theme_uris ) ) {
				$links['https://api.w.org/theme-file'] = $resolved_theme_uris;
			}
			$response->add_links( $links );
	...
	public function get_theme_items( $request ) {
		...
		// Register theme-defined variations e.g. from block style variation partials under `/styles`.
		$partials = FP_Theme_JSON_Resolver::get_style_variations( 'block' );
		...
		foreach ( $variations as $variation ) {
			$variation_theme_json = new FP_Theme_JSON( $variation );
			$resolved_theme_uris  = FP_Theme_JSON_Resolver::get_resolved_theme_uris( $variation_theme_json );
