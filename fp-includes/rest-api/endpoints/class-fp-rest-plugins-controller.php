<?php
/**
 * REST API: FP_REST_Plugins_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class FP_REST_Plugins_Controller extends FP_REST_Controller {
	...
	public function get_items( $request ) {
		...
		return new FP_REST_Response( $plugins );
	}
	 ...
	 * @return true|FP_Error True if can read, a FP_Error instance otherwise.
	 */
	protected function check_read_permission( $plugin ) {
		require_once ABSPATH . 'wp-admin/includes/plugin.php';

		if ( ! $this->is_plugin_installed( $plugin ) ) {
			return new FP_Error( 'rest_plugin_not_found', __( 'Plugin not found.' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( is_null( $result ) ) {
			...
			if ( $wp_filesystem instanceof FP_Filesystem_Base
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$response = new FP_REST_Response( $data );
	 ...
	 * @return array|FP_Error The plugin data, or a FP_Error if the plugin is not installed.
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @param FP_REST_Request $request The request to require the plugin matches against.
	 * @param array           $item    The plugin item.
	 * @return bool
	 */
	...
	protected function is_plugin_installed( $plugin ) {
		return file_exists( FP_PLUGIN_DIR . '/' . $plugin );
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|FP_Error True if filesystem is available, FP_Error otherwise.
	 */
	protected function is_filesystem_available() {
		...
		return new FP_Error( 'fs_unavailable', __( 'The filesystem is currently unavailable for managing plugins.' ), array( 'status' => 500 ) );
	}
