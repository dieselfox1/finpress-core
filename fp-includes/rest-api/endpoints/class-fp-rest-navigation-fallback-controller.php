<?php
/**
 * FP_REST_Navigation_Fallback_Controller class
 *
 ...
 * @since 6.3.0
 */
class FP_REST_Navigation_Fallback_Controller extends FP_REST_Controller {
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => $this->get_endpoint_args_for_item_schema( FP_REST_Server::READABLE ),
				),
	...
	public function get_item( $request ) {
		$post = FP_Navigation_Fallback::get_fallback();

		if ( empty( $post ) ) {
			return rest_ensure_response( new FP_Error( 'no_fallback_menu', __( 'No fallback menu found.' ), array( 'status' => 404 ) ) );
		}
	 ...
	 * @since 6.3.0
	 *
	 * @param FP_Post         $item    The wp_navigation Post object whose response is being prepared.
	 * @param FP_REST_Request $request Request object.
	 * @return FP_REST_Response $response The response data.
	 */
	 ...
	 * @since 6.3.0
	 *
	 * @param FP_Post $post the Navigation Menu post object.
