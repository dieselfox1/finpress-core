<?php
/**
 * REST API: FP_REST_Autosaves_Controller class.
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.0.0
 ...
 * @since 5.0.0
 *
 * @see FP_REST_Revisions_Controller
 * @see FP_REST_Controller
 */
class FP_REST_Autosaves_Controller extends FP_REST_Revisions_Controller {
	 ...
	 *
	 * @since 5.0.0
	 * @var FP_REST_Revisions_Controller
	 */
	...
	public function __construct( $parent_post_type ) {
		...
		if ( ! $revisions_controller ) {
			$revisions_controller = new FP_REST_Revisions_Controller( $parent_post_type );
		}
		...
		register_rest_route(
			...
			array(
				...
				array(
					...
					'args'                => $this->parent_controller->get_endpoint_args_for_item_schema( FP_REST_Server::EDITABLE ),
				),
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has access to create the item, FP_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ! defined( 'FP_RUN_CORE_TESTS' ) && ! defined( 'DOING_AUTOSAVE' ) ) {
	 ...
	 * @return mixed The autosave revision ID or FP_Error.
	 */
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return apply_filters( 'rest_prepare_autosave', new FP_REST_Response( array() ), $post, $request );
		}
