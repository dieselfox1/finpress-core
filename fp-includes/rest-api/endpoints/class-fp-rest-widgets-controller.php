<?php
/**
 * REST API: FP_REST_Widgets_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class FP_REST_Widgets_Controller extends FP_REST_Controller {
	...
	public function get_items( $request ) {
		...
		return new FP_REST_Response( $prepared );
	}
	...
	public function update_item( $request ) {
		 ...
		 * sidebar.
		 *
		 * See https://core.trac.finpress.org/ticket/53657.
		 */
		...
		// Allow sidebar to be unset or missing when widget is not a FP_Widget.
	...
	public function delete_item( $request ) {
		...
		if ( $request['force'] ) {
			...
			if ( $widget_object ) {
				/*
				 * FP_Widget sets `updated = true` after an update to prevent more than one widget
		 ...
		 * @param FP_REST_Response|FP_Error $response   The response data, or FP_Error object on failure.
	 ...
	 * @return string|FP_Error The saved widget ID.
	 */
	protected function save_widget( $request, $sidebar_id ) {
		...
		if ( isset( $request['instance'] ) ) {
			if ( ! $widget_object ) {
				return new FP_Error(
					'rest_invalid_widget',
					__( 'Cannot set instance on a widget that does not extend FP_Widget.' ),
					array( 'status' => 400 )
				);
			}
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return apply_filters( 'rest_prepare_widget', new FP_REST_Response( array() ), $widget, $request );
		}
		 ...
		 * @since 5.8.0
		 *
		 * @param FP_REST_Response|FP_Error $response The response object, or FP_Error object on failure.
