<?php
/**
 * REST API: FP_REST_Menus_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class FP_REST_Menus_Controller extends FP_REST_Terms_Controller {
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the current user has permission, FP_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $this->taxonomy ) ) {
				return new FP_Error( 'rest_taxonomy_not_hierarchical', __( 'Cannot set parent term, taxonomy is not hierarchical.' ), array( 'status' => 400 ) );
			}

			$parent = wp_get_nav_menu_object( (int) $request['parent'] );

			if ( ! $parent ) {
				return new FP_Error( 'rest_term_invalid', __( 'Parent term does not exist.' ), array( 'status' => 400 ) );
			}
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $request['force'] ) {
			...
			return new FP_Error( 'rest_trash_not_supported', sprintf( __( "Menus do not support trashing. Set '%s' to delete." ), 'force=true' ), array( 'status' => 501 ) );
		}
		...
		if ( ! $result || is_wp_error( $result ) ) {
			return new FP_Error( 'rest_cannot_delete', __( 'The menu cannot be deleted.' ), array( 'status' => 500 ) );
		}
	 ...
	 * @param int             $menu_id The menu id to update.
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True on success, a FP_Error on an error updating any of the locations.
	 */
