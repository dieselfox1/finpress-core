<?php
/**
 * Block Pattern Directory REST API: FP_REST_Pattern_Directory_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 *
 * This simply proxies the endpoint at http://api.finpress.org/patterns/1.0/. That isn't necessary for
 * functionality, but is desired for privacy. It prevents api.finpress.org from knowing the user's IP address.
 *
 * @since 5.8.0
 *
 * @see FP_REST_Controller
 */
class FP_REST_Pattern_Directory_Controller extends FP_REST_Controller {
	 ...
	 * @return true|FP_Error True if the request has permission, FP_Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		if ( ! $raw_patterns ) {
			$api_url = 'http://api.finpress.org/patterns/1.0/?' . build_query( $query_args );
	 ...
	 *
	 * @param object          $item    Raw pattern from api.finpress.org, before any changes.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$response = new FP_REST_Response( $prepared_pattern );
