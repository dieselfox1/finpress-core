<?php
/**
 * REST API: FP_REST_Block_Types_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class FP_REST_Block_Types_Controller extends FP_REST_Controller {

	const NAME_PATTERN = '^[a-z][a-z0-9-]*/[a-z][a-z0-9-]*$';

	/**
	 * Instance of FP_Block_Type_Registry.
	 *
	 * @since 5.5.0
	 * @var FP_Block_Type_Registry
	 */
	protected $block_registry;

	/**
	 * Instance of FP_Block_Styles_Registry.
	 *
	 * @since 5.5.0
	 * @var FP_Block_Styles_Registry
	 */
	...
	public function __construct() {
		...
		$this->block_registry = FP_Block_Type_Registry::get_instance();
		$this->style_registry = FP_Block_Styles_Registry::get_instance();
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|FP_Error True if the block type is visible, FP_Error otherwise.
	 */
	protected function check_read_permission() {
		...
		return new FP_Error( 'rest_block_type_cannot_view', __( 'Sorry, you are not allowed to manage block types.' ), array( 'status' => rest_authorization_required_code() ) );
	}
	 ...
	 * @return FP_Block_Type|FP_Error Block type object if name is valid, FP_Error otherwise.
	 */
	protected function get_block( $name ) {
		$block_type = $this->block_registry->get_registered( $name );
		if ( empty( $block_type ) ) {
			return new FP_Error( 'rest_block_type_invalid', __( 'Invalid block type.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param FP_Block_Type   $item    Block type data.
	 * @param FP_REST_Request $request Full details about the request.
	 * @return FP_REST_Response Block type data.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return apply_filters( 'rest_prepare_block_type', new FP_REST_Response( array() ), $block_type, $request );
		}
		 ...
		 * @param FP_Block_Type    $block_type The original block type object.
	 ...
	 * @since 5.5.0
	 *
	 * @param FP_Block_Type $block_type Block type data.
