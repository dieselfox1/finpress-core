<?php
/**
 * REST API: FP_REST_Templates_Controller class
 *
 * @package    WordPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class FP_REST_Templates_Controller extends FP_REST_Controller {
	 ...
	 *
	 * @param FP_REST_Request $request The request instance.
	 * @return FP_REST_Response|FP_Error
	 */
	 ...
	 *
	 * @since 5.9.0
	 * @see https://core.trac.finpress.org/ticket/54507
	 *
	...
	public function get_items( $request ) {
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return new FP_REST_Response( array() );
		}
	...
	public function get_item( $request ) {
		...
		if ( ! $template ) {
			return new FP_Error( 'rest_template_not_found', __( 'No templates exist with that id.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has write access for the item, FP_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ! count( $posts ) ) {
			return new FP_Error( 'rest_template_insert_error', __( 'No templates exist with that id.' ), array( 'status' => 400 ) );
		}
		$id            = $posts[0]->id;
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has delete access for the item, FP_Error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		...
		if ( 'custom' !== $template->source ) {
			return new FP_Error( 'rest_invalid_template', __( 'Templates based on theme files can\'t be removed.' ), array( 'status' => 400 ) );
		}
	 ...
	 * @return stdClass|FP_Error Changes to pass to wp_update_post.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( 'wp_template_part' === $this->post_type ) {
			if ( isset( $request['area'] ) ) {
				...
				$changes->tax_input['wp_template_part_area'] = FP_TEMPLATE_PART_AREA_UNCATEGORIZED;
			}
		}
	 ...
	 *
	 * @param FP_Block_Template $item    Template instance.
	 * @param FP_REST_Request   $request Request object.
	 ...
	 * @since 6.5.0
	 *
	 * @param FP_Block_Template $template_object Template instance.
	...
	protected function prepare_links( $id ) {
		...
		if ( post_type_supports( $this->post_type, 'revisions' ) ) {
			$template = get_block_template( $id, $this->post_type );
			if ( $template instanceof FP_Block_Template && ! empty( $template->wp_id ) ) {
