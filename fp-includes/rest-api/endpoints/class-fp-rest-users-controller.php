<?php
/**
 * REST API: FP_REST_Users_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class FP_REST_Users_Controller extends FP_REST_Controller {

	/**
	 * Instance of a user meta fields object.
	 *
	 * @since 4.7.0
	 * @var FP_REST_User_Meta_Fields
	 */
	protected $meta;
	...
	public function __construct() {
		...
		$this->meta = new FP_REST_User_Meta_Fields();
	}
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has read access, otherwise FP_Error object.
	 */
	...
	public function get_items( $request ) {
		...
		/**
		 * Filters FP_User_Query arguments when querying users via the REST API.
		 *
		 * @link https://developer.finpress.org/reference/classes/wp_user_query/
		 *
		 * @since 4.7.0
		 *
		...
		$query = new FP_User_Query( $prepared_args );
		...
		$response = $is_head_request ? new FP_REST_Response( array() ) : rest_ensure_response( $users );
		...
		if ( $total_users < 1 ) {
			...
			$count_query = new FP_User_Query( $prepared_args );
			$total_users = $count_query->get_total();
		}

		$response->header( 'X-FP-Total', (int) $total_users );
	 ...
	 * @return FP_User|FP_Error True if ID is valid, FP_Error otherwise.
	 */
	 ...
	 *
	 * @param FP_REST_Request $request Full details about the request.
	 * @return true|FP_Error True if the request has read access for the item, otherwise FP_Error object.
	 */
	...
	public function create_item( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param FP_User         $user     Inserted or updated user object.
	...
	public function delete_item( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param FP_User          $user     The user data.
	 ...
	 *
	 * @param FP_User         $item    User object.
	 * @param FP_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return apply_filters( 'rest_prepare_user', new FP_REST_Response( array() ), $user, $request );
		}
		 ...
		 * @param FP_User          $user     User object used to create response.
		 * @param FP_REST_Request  $request  Request object.
		 */
	 ...
	 * @return true|FP_Error True if the current user is allowed to make the role change,
	 *                       otherwise a FP_Error object.
	 */
	 ...
	 * @return string|FP_Error The sanitized username, if valid, otherwise an error.
	 */
	 ...
	 * @param FP_REST_Request $request Full details about the request.
	 * @param string          $param   The parameter name.
	 * @return string|FP_Error The sanitized password, if valid, otherwise an error.
	 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal FP_User_Query parameter.  Use the
		 * `rest_user_query` filter to set FP_User_Query arguments.
		 *
		 * @since 4.7.0
		 *
