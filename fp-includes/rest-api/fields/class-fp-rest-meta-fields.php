<?php
/**
 * REST API: FP_REST_Meta_Fields class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 4.7.0
...
abstract class FP_REST_Meta_Fields {
	 ...
	 * @param int   $object_id Object ID to fetch meta for.
	 * @return null|FP_Error Null on success, FP_Error object on failure.
	 */
	 ...
	 * @return true|FP_Error True if meta field is deleted, FP_Error otherwise.
	 */
	protected function delete_meta_value( $object_id, $meta_key, $name ) {
		...
		if ( ! delete_metadata( $meta_type, $object_id, wp_slash( $meta_key ) ) ) {
			return new FP_Error(
				...
				array(
					'key'    => $name,
					'status' => FP_Http::INTERNAL_SERVER_ERROR,
				)
			);
		}
	 ...
	 * @return true|FP_Error True if meta fields are updated, FP_Error otherwise.
	 */
	 ...
	 * @return true|FP_Error True if the meta field was updated, FP_Error otherwise.
	 */
