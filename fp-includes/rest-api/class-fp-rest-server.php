...
class FP_REST_Server {
	 ...
	 * @since 4.4.0
	 *
	 * @return FP_Error|null|true FP_Error indicates unsuccessful login, null indicates successful
	...
	public function check_authentication() {
		 ...
		 *
		 * This is used to pass a FP_Error from an authentication method back to
		 * the API.
		 *
		 ...
		 * errors.
		 *
		 * A FP_Error instance can be returned if an error occurs, and this should
		 ...
		 * @since 4.4.0
		 *
		 * @param FP_Error|null|true $errors FP_Error if authentication error, null if authentication
	 ...
	 *
	 * Note: This should only be used in FP_REST_Server::serve_request(), as it
	 * cannot handle FP_Error internally. All callbacks and other internal methods
	 * should instead return a FP_Error with the data set to an array that includes
	 * a 'status' key, with the value being the HTTP status to send.
	 *
	 * @since 4.4.0
	 *
	 * @param string $code    FP_Error-style code.
	 ...
	 * @since 6.1.0
	 *
	 * @param \FP_REST_Request $request The current request object.
	 *
	 * @return int The JSON encode options.
	 */
	protected function get_json_encode_options( FP_REST_Request $request ) {
		$options = 0;
		 ...
		 * @param FP_REST_Request $request Current request object.
		 */
	 ...
	 *
	 * @global FP_User $current_user The currently authenticated user.
	 *
	...
	public function serve_request( $path = null ) {
		/* @var FP_User|null $current_user */
		global $current_user;

		if ( $current_user instanceof FP_User && ! $current_user->exists() ) {
			/*
		...
		$request = new FP_REST_Request( $_SERVER['REQUEST_METHOD'], $path );
		...
		$expose_headers = array( 'X-FP-Total', 'X-FP-TotalPages', 'Link' );
		 ...
		 * @param FP_REST_Request $request        The request in context.
		 */
		...
		// Normalize to either FP_Error or FP_REST_Response...
		$result = rest_ensure_response( $result );

		// ...then convert FP_Error across.
		if ( is_wp_error( $result ) ) {
		 ...
		 *
		 * @param FP_HTTP_Response $result  Result to send to the client. Usually a `FP_REST_Response`.
		 * @param FP_REST_Server   $server  Server instance.
		 * @param FP_REST_Request  $request Request used to generate the response.
		 */
	 ...
	 *
	 * @param FP_REST_Response $response Response object.
	 ...
	 * @since 4.4.0
	 *
	 * @param FP_REST_Response $response Response to extract links from.
	...
	protected static function get_target_hints_for_link( $link ) {
		...
		$request = FP_REST_Request::from_url( $link['href'] );
		if ( ! $request ) {
			return null;
		}
		...
		foreach ( $headers as $name => $value ) {
			$name = FP_REST_Request::canonicalize_header_name( $name );
		...
		foreach ( $data['_links'] as $rel => $links ) {
			...
			foreach ( $links as $item ) {
				...
				if ( ! array_key_exists( $item['href'], $this->embed_cache ) ) {
					...
					$request = FP_REST_Request::from_url( $item['href'] );
					if ( ! $request ) {
						$embeds[] = array();
						continue;
					}
	 ...
	 * @param FP_REST_Response $response Response object.
	 * @param bool|string[]    $embed    Whether to embed all links, a filtered list of link relations, or no links.
	 * @return FP_REST_Response New response with wrapped data
	 */
	public function envelope_response( $response, $embed ) {
		 ...
		 * }
		 * @param FP_REST_Response $response Original response data.
		 */
	 ...
	 * @since 4.4.0
	 *
	 * @param FP_REST_Request $request Request to attempt dispatching.
	 * @return FP_REST_Response Response returned by the callback.
	 */
	 ...
	 * @since 5.6.0
	 *
	 * @param FP_REST_Request $request The request object.
	 * @return array|FP_Error The route and request handler on success or a FP_Error instance if no handler was found.
	 */
	 ...
	 * @param FP_Error|null   $response The current error object if any.
	...
	protected function respond_to_request( $request, $route, $handler, $response ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param FP_REST_Response|FP_HTTP_Response|FP_Error|mixed $response Result to send to the client.
		 *                                                                   Usually a FP_REST_Response or FP_Error.
	 ...
	 * @since 4.4.0
	 *
	 * @param FP_REST_Request $request Request data.
	 * @return FP_REST_Response The API root index data.
	 */
	public function get_index( $request ) {
		...
		$response = new FP_REST_Response( $available );
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			$response->add_link( 'help', 'https://developer.finpress.org/rest-api/' );
	 ...
	 * @since 5.7.0
	 *
	 * @param FP_REST_Response $response REST API response.
	 */
	protected function add_active_theme_link_to_index( FP_REST_Response $response ) {
	 ...
	 * @param FP_REST_Response $response REST API response.
	 */
	protected function add_site_logo_to_index( FP_REST_Response $response ) {
	 ...
	 * @param FP_REST_Response $response REST API response.
	 */
	protected function add_site_icon_to_index( FP_REST_Response $response ) {
	 ...
	 */
	protected function add_image_to_index( FP_REST_Response $response, $image_id, $type ) {
	 ...
	 * @since 4.4.0
	 *
	 * @param FP_REST_Request $request REST request instance.
	 * @return FP_REST_Response|FP_Error FP_REST_Response instance if the index was found,
	 *                                   FP_Error if the namespace isn't set.
	 */
	public function get_namespace_index( $request ) {
		 ...
		 *
		 * @param FP_REST_Response $response Response data.
		 * @param FP_REST_Request  $request  Request data. The namespace is passed as the 'namespace' parameter.
		 */
	 ...
	 * @since 5.6.0
	 *
	 * @param FP_REST_Request $batch_request The batch request object.
	 * @return FP_REST_Response The generated response object.
	 */
	public function serve_batch_request_v1( FP_REST_Request $batch_request ) {
		$requests = array();
		...
		foreach ( $batch_request['requests'] as $args ) {
			...
			if ( false === $parsed_url ) {
				$requests[] = new FP_Error( 'parse_path_failed', __( 'Could not parse the path.' ), array( 'status' => 400 ) );

				continue;
			}

			$single_request = new FP_REST_Request( isset( $args['method'] ) ? $args['method'] : 'POST', $parsed_url['path'] );
		...
		return new FP_REST_Response( array( 'responses' => $responses ), FP_Http::MULTI_STATUS );
	}
