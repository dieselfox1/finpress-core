<?php
/**
 * REST API: FP_REST_Request class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 4.4.0
 ...
 * FP_REST_Request::get_body_params(), FP_REST_Request::get_url_params(), etc.
 *
 * @since 4.4.0
 *
...
class FP_REST_Request implements ArrayAccess {
	 ...
	 *
	 * Used when checking parameters in FP_REST_Request::get_param().
	 *
	 * @since 4.4.0
	 *
	...
	protected function get_parameter_order() {
		 ...
		 *
		 * The order affects which parameters are checked when using FP_REST_Request::get_param()
	 ...
	 * @return true|FP_Error True if the JSON data was passed or no JSON data was provided, FP_Error if invalid JSON was passed.
	 */
	protected function parse_json_params() {
		...
		if ( null === $params && JSON_ERROR_NONE !== json_last_error() ) {
			...
			$error_data = array(
				'status'             => FP_Http::BAD_REQUEST,
				'json_error_code'    => json_last_error(),
				'json_error_message' => json_last_error_msg(),
			);

			return new FP_Error( 'rest_invalid_json', __( 'Invalid JSON body passed.' ), $error_data );
		}
	 ...
	 * @since 4.4.0
	 *
	 * @return true|FP_Error True if parameters were sanitized, FP_Error if an error occurred during sanitization.
	 */
	public function sanitize_params() {
		...
		foreach ( $order as $type ) {
			...
			foreach ( $this->params[ $type ] as $key => $value ) {
				...
				/** @var mixed|FP_Error $sanitized_value */
	 ...
	 * @since 4.4.0
	 *
	 * @return true|FP_Error True if there are no parameters to validate or if all pass validation,
	 *                       FP_Error if required parameters are missing.
	 */
	public function has_valid_params() {
		...
		if ( isset( $attributes['validate_callback'] ) ) {
			...
			if ( false === $valid_check ) {
				// A FP_Error instance is preferred, but false is supported for parity with the per-arg validate_callback.
				return new FP_Error( 'rest_invalid_params', __( 'Invalid parameters.' ), array( 'status' => 400 ) );
			}
		}
	...
	/**
	 * Retrieves a FP_REST_Request object from a full URL.
	 *
	 * @since 4.5.0
	 *
	 * @param string $url URL with protocol, domain, path and query args.
	 * @return FP_REST_Request|false FP_REST_Request object on success, false on failure.
	 */
	public static function from_url( $url ) {
		...
		$request = false;
		if ( ! empty( $route ) ) {
			$request = new FP_REST_Request( 'GET', $route );
		 ...
		 * @since 4.5.0
		 *
		 * @param FP_REST_Request|false $request Generated request object, or false if URL
		 *                                       could not be parsed.
