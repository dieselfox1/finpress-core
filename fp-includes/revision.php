 ...
 *
 * @since 2.6.0
 * @since 4.5.0 A `FP_Post` object can now be passed to the `$post` parameter.
 * @since 4.5.0 The optional `$autosave` parameter was deprecated and renamed to `$deprecated`.
 * @access private
 *
 * @param array|FP_Post $post       Optional. A post array or a FP_Post object being processed
...
function _wp_post_revision_fields( $post = array(), $deprecated = false ) {
	...
	// FP uses these internally either in versioning or elsewhere - they cannot be versioned.
 ...
 * @access private
 *
 * @param array|FP_Post $post     Optional. A post array or a FP_Post object to be processed
 *                                for insertion as a post revision. Default empty array.
 ...
 * @param FP_Post $post    The post object that was inserted.
 ...
 * @return int|FP_Error|void Void or 0 if error, new revision ID, if success.
 */
function wp_save_post_revision( $post_id ) {
	 ...
	 * @since 6.2.0
	 *
	 * @param FP_Post[] $revisions Array of revisions, or an empty array if none.
	 * @param int       $post_id   The ID of the post to save as a revision.
	 */
 ...
 * @return FP_Post|false The autosaved data or false on failure or when no autosave exists.
 */
 ...
 * @access private
 *
 * @param int|FP_Post|array|null $post     Post ID, post object OR post array.
 * @param bool                   $autosave Optional. Whether the revision is an autosave or not.
 *                                         Default false.
 * @return int|FP_Error FP_Error or 0 if error, new revision ID if success.
 */
function _wp_put_post_revision( $post = null, $autosave = false ) {
	...
	if ( isset( $post['post_type'] ) && 'revision' === $post['post_type'] ) {
		return new FP_Error( 'post_type', __( 'Cannot create a revision of a revision' ) );
	}
 ...
 * @since 2.6.0
 *
 * @param int|FP_Post $revision Revision ID or revision object.
 ...
 *
 * @param bool    $post_has_changed Whether the post has changed.
 * @param FP_Post $last_revision    The last revision post object.
 * @param FP_Post $post             The post object.
 * @return bool Whether the post has changed.
 */
function wp_check_revisioned_meta_fields_have_changed( $post_has_changed, FP_Post $last_revision, FP_Post $post ) {
 ...
 *
 * @param int|FP_Post $revision Revision ID or revision object.
 * @return FP_Post|false|null Null or false if error, deleted post object if success.
 */
function wp_delete_post_revision( $revision ) {
	...
	if ( $delete ) {
		 ...
		 *
		 * @param int     $revision_id Post revision ID.
		 * @param FP_Post $revision    Post revision object.
		 */
 ...
 * @return FP_Post[]|int[] Array of revision objects or IDs, or an empty array if none.
 */
 ...
 *     or a FP_Error if the post does not exist or revisions are not enabled.
 *
...
function wp_get_latest_revision_id_and_total_count( $post = 0 ) {
	...
	if ( ! $post ) {
		return new FP_Error( 'invalid_post', __( 'Invalid post.' ) );
	}

	if ( ! wp_revisions_enabled( $post ) ) {
		return new FP_Error( 'revisions_not_enabled', __( 'Revisions not enabled.' ) );
	}
 ...
 *
 * The constant FP_POST_REVISIONS can be set in wp-config to specify the limit
...
function wp_revisions_to_keep( $post ) {
	 ...
	 *
	 * Overrides the value of FP_POST_REVISIONS.
	 *
	 * @since 3.6.0
	 *
	 ...
	 *
	 * Overrides both the value of FP_POST_REVISIONS and the {@see 'wp_revisions_to_keep'} filter.
	 *
