...
function wp_check_php_mysql_versions() {
	...
	// This runs before default constants are defined, so we can't assume FP_CONTENT_DIR is set yet.
	$wp_content_dir = defined( 'FP_CONTENT_DIR' ) ? FP_CONTENT_DIR : ABSPATH . 'wp-content';

	if ( ! function_exists( 'mysqli_connect' )
		&& ! file_exists( $wp_content_dir . '/db.php' )
	) {
		require_once ABSPATH . FPINC . '/functions.php';
		...
		$message .= '<p>' . sprintf(
			...
			__( 'https://finpress.org/support/forums/' )
		) . "</p>\n";
 ...
 *
 * The type can be set via the `FP_ENVIRONMENT_TYPE` global system variable,
...
function wp_get_environment_type() {
	...
	if ( ! defined( 'FP_RUN_CORE_TESTS' ) && $current_env ) {
		return $current_env;
	}
	...
	// Add a note about the deprecated FP_ENVIRONMENT_TYPES constant.
	if ( defined( 'FP_ENVIRONMENT_TYPES' ) && function_exists( '_deprecated_argument' ) ) {
		if ( function_exists( '__' ) ) {
			/* translators: %s: FP_ENVIRONMENT_TYPES */
			$message = sprintf( __( 'The %s constant is no longer supported.' ), 'FP_ENVIRONMENT_TYPES' );
		} else {
			$message = sprintf( 'The %s constant is no longer supported.', 'FP_ENVIRONMENT_TYPES' );
		}
	...
	if ( function_exists( 'getenv' ) ) {
		$has_env = getenv( 'FP_ENVIRONMENT_TYPE' );
		if ( false !== $has_env ) {
			$current_env = $has_env;
		}
	}
	...
	if ( defined( 'FP_ENVIRONMENT_TYPE' ) && FP_ENVIRONMENT_TYPE ) {
		$current_env = FP_ENVIRONMENT_TYPE;
	}
 ...
 *
 * Development mode can be set via the `FP_DEVELOPMENT_MODE` constant in `wp-config.php`.
 ...
 *
 * Development mode is considered separately from `FP_DEBUG` and wp_get_environment_type().
...
function wp_get_development_mode() {
	...
	if ( ! defined( 'FP_RUN_CORE_TESTS' ) && null !== $current_mode ) {
		return $current_mode;
	}

	$development_mode = FP_DEVELOPMENT_MODE;

	// Exclusively for core tests, rely on the `$_wp_tests_development_mode` global.
	if ( defined( 'FP_RUN_CORE_TESTS' ) && isset( $GLOBALS['_wp_tests_development_mode'] ) ) {
...
function wp_maintenance() {
	...
	if ( file_exists( FP_CONTENT_DIR . '/maintenance.php' ) ) {
		require_once FP_CONTENT_DIR . '/maintenance.php';
		die();
	}
 ...
 *
 * Uses three constants: `FP_DEBUG`, `FP_DEBUG_DISPLAY`, and `FP_DEBUG_LOG`.
 * All three can be defined in wp-config.php. By default, `FP_DEBUG` and
 * `FP_DEBUG_LOG` are set to false, and `FP_DEBUG_DISPLAY` is set to true.
 *
 * When `FP_DEBUG` is true, all PHP notices are reported. WordPress will also
 ...
 *
 * `FP_DEBUG_DISPLAY` and `FP_DEBUG_LOG` perform no function unless `FP_DEBUG`
 * is true.
 *
 * When `FP_DEBUG_DISPLAY` is true, WordPress will force errors to be displayed.
 * `FP_DEBUG_DISPLAY` defaults to true. Defining it as null prevents WordPress
 * from changing the global configuration setting. Defining `FP_DEBUG_DISPLAY`
 * as false will force errors to be hidden.
 *
 * When `FP_DEBUG_LOG` is true, errors will be logged to `wp-content/debug.log`.
 * When `FP_DEBUG_LOG` is a valid path, errors will be logged to the specified file.
 *
 * Errors are never displayed for XML-RPC, REST, `ms-files.php`, and Ajax requests.
 *
 * @since 3.0.0
 * @since 5.1.0 `FP_DEBUG_LOG` can be a file path.
 * @access private
 */
function wp_debug_mode() {
	 ...
	 *
	 * This filter runs before it can be used by plugins. It is designed for
	 * non-web runtimes. Returning false causes the `FP_DEBUG` and related
	...
	if ( FP_DEBUG ) {
		...
		if ( in_array( strtolower( (string) FP_DEBUG_LOG ), array( 'true', '1' ), true ) ) {
			$log_path = FP_CONTENT_DIR . '/debug.log';
		} elseif ( is_string( FP_DEBUG_LOG ) ) {
			$log_path = FP_DEBUG_LOG;
		} else {
			$log_path = false;
		}
	...
	if ( defined( 'XMLRPC_REQUEST' ) || defined( 'REST_REQUEST' ) || defined( 'MS_FILES_REQUEST' )
		|| ( defined( 'FP_INSTALLING' ) && FP_INSTALLING )
 ...
 *
 * To set directory manually, define the `FP_LANG_DIR` constant
 * in wp-config.php.
 *
 * If the language directory exists within `FP_CONTENT_DIR`, it
...
function wp_set_lang_dir() {
	if ( ! defined( 'FP_LANG_DIR' ) ) {
		if ( file_exists( FP_CONTENT_DIR . '/languages' ) && @is_dir( FP_CONTENT_DIR . '/languages' )
			|| ! @is_dir( ABSPATH . FPINC . '/languages' )
		) {
			/**
			 ...
			 * @since 2.1.0
			 */
			define( 'FP_LANG_DIR', FP_CONTENT_DIR . '/languages' );
			 ...
			 * @since 2.1.0
			 */
			define( 'FP_LANG_DIR', ABSPATH . FPINC . '/languages' );

			if ( ! defined( 'LANGDIR' ) ) {
				// Old relative path maintained for backward compatibility.
				define( 'LANGDIR', FPINC . '/languages' );
			}
		}
	}
}
...
function require_wp_db() {
	...
	require_once ABSPATH . FPINC . '/class-wpdb.php';

	if ( file_exists( FP_CONTENT_DIR . '/db.php' ) ) {
		require_once FP_CONTENT_DIR . '/db.php';
	}
...
function wp_start_object_cache() {
	...
	if ( $first_init && apply_filters( 'enable_loading_object_cache_dropin', true ) ) {
		if ( ! function_exists( 'wp_cache_init' ) ) {
			 ...
			 */
			if ( file_exists( FP_CONTENT_DIR . '/object-cache.php' ) ) {
				require_once FP_CONTENT_DIR . '/object-cache.php';
		...
		} elseif ( ! wp_using_ext_object_cache() && file_exists( FP_CONTENT_DIR . '/object-cache.php' ) ) {
			/*
	...
	if ( ! wp_using_ext_object_cache() ) {
		require_once ABSPATH . FPINC . '/cache.php';
	}

	require_once ABSPATH . FPINC . '/cache-compat.php';
 ...
 * directory manually, define `FPMU_PLUGIN_DIR` and `FPMU_PLUGIN_URL`
 * in wp-config.php.
 *
 * @since 3.0.0
 * @access private
...
function wp_get_mu_plugins() {
	...
	if ( ! is_dir( FPMU_PLUGIN_DIR ) ) {
		return $mu_plugins;
	}

	$dh = opendir( FPMU_PLUGIN_DIR );
	if ( ! $dh ) {
		return $mu_plugins;
	}
	...
	while ( ( $plugin = readdir( $dh ) ) !== false ) {
		if ( str_ends_with( $plugin, '.php' ) ) {
			$mu_plugins[] = FPMU_PLUGIN_DIR . '/' . $plugin;
		}
	}
 ...
 * directory manually, define `FP_PLUGIN_DIR` and `FP_PLUGIN_URL`
 * in `wp-config.php`.
 *
 * @since 3.0.0
 * @access private
...
function wp_get_active_and_valid_plugins() {
	...
	foreach ( $active_plugins as $plugin ) {
		if ( ! validate_file( $plugin )                     // $plugin must validate as file.
			...
			&& file_exists( FP_PLUGIN_DIR . '/' . $plugin ) // $plugin must exist.
			// Not already included as a network plugin.
			&& ( ! $network_plugins || ! in_array( FP_PLUGIN_DIR . '/' . $plugin, $network_plugins, true ) )
		) {
			$plugins[] = FP_PLUGIN_DIR . '/' . $plugin;
		}
	}
 ...
 * @since 5.2.0
 *
 * @global FP_Paused_Extensions_Storage $_paused_plugins
 *
 ...
 * @since 5.2.0
 *
 * @global FP_Paused_Extensions_Storage $_paused_themes
 *
 ...
 * @since 1.5.1
 *
 * @global FP_Screen $current_screen WordPress current screen object.
 *
...
function is_blog_admin() {
	...
	} elseif ( defined( 'FP_BLOG_ADMIN' ) ) {
		return FP_BLOG_ADMIN;
	}
...
function is_network_admin() {
	...
	} elseif ( defined( 'FP_NETWORK_ADMIN' ) ) {
		return FP_NETWORK_ADMIN;
	}
...
function is_user_admin() {
	...
	} elseif ( defined( 'FP_USER_ADMIN' ) ) {
		return FP_USER_ADMIN;
	}
...
function wp_load_translations_early() {
	...
	require_once ABSPATH . FPINC . '/pomo/mo.php';
	require_once ABSPATH . FPINC . '/l10n/class-wp-translation-controller.php';
	require_once ABSPATH . FPINC . '/l10n/class-wp-translations.php';
	require_once ABSPATH . FPINC . '/l10n/class-wp-translation-file.php';
	require_once ABSPATH . FPINC . '/l10n/class-wp-translation-file-mo.php';
	require_once ABSPATH . FPINC . '/l10n/class-wp-translation-file-php.php';
	require_once ABSPATH . FPINC . '/l10n.php';
	require_once ABSPATH . FPINC . '/class-wp-textdomain-registry.php';
	require_once ABSPATH . FPINC . '/class-wp-locale.php';
	require_once ABSPATH . FPINC . '/class-wp-locale-switcher.php';
	...
	if ( ! $wp_textdomain_registry instanceof FP_Textdomain_Registry ) {
		$wp_textdomain_registry = new FP_Textdomain_Registry();
	}
	...
	while ( true ) {
		...
		if ( defined( 'FP_LANG_DIR' ) && @is_dir( FP_LANG_DIR ) ) {
			$locations[] = FP_LANG_DIR;
		}

		if ( defined( 'FP_CONTENT_DIR' ) && @is_dir( FP_CONTENT_DIR . '/languages' ) ) {
			$locations[] = FP_CONTENT_DIR . '/languages';
		}
		...
		if ( @is_dir( ABSPATH . FPINC . '/languages' ) ) {
			$locations[] = ABSPATH . FPINC . '/languages';
		}
		...
		foreach ( $locales as $locale ) {
			foreach ( $locations as $location ) {
				if ( file_exists( $location . '/' . $locale . '.mo' ) ) {
					...
					if ( defined( 'FP_SETUP_CONFIG' ) && file_exists( $location . '/admin-' . $locale . '.mo' ) ) {
	...
	$wp_locale = new FP_Locale();
}

/**
 * Checks or sets whether WordPress is in "installation" mode.
 *
 * If the `FP_INSTALLING` constant is defined during the bootstrap, `wp_installing()` will default to `true`.
 *
 * @since 4.4.0
 *
 * @param bool $is_installing Optional. True to set FP into Installing mode, false to turn Installing mode off.
 *                            Omit this parameter if you only want to fetch the current status.
 * @return bool True if FP is installing, otherwise false. When a `$is_installing` is passed, the function will
 *              report whether FP was in installing mode prior to the change to `$is_installing`.
 */
function wp_installing( $is_installing = null ) {
	static $installing = null;

	// Support for the `FP_INSTALLING` constant, defined before FP is loaded.
	if ( is_null( $installing ) ) {
		$installing = defined( 'FP_INSTALLING' ) && FP_INSTALLING;
	}
...
function wp_using_themes() {
	 ...
	 */
	return apply_filters( 'wp_using_themes', defined( 'FP_USE_THEMES' ) && FP_USE_THEMES );
}
 ...
 *
 * Returns whether `$thing` is an instance of the `FP_Error` class.
 *
 * @since 2.1.0
 *
 * @param mixed $thing The variable to check.
 * @return bool Whether the variable is an instance of FP_Error.
 */
function is_wp_error( $thing ) {
	$is_wp_error = ( $thing instanceof FP_Error );

	if ( $is_wp_error ) {
		/**
		 * Fires when `is_wp_error()` is called and its parameter is an instance of `FP_Error`.
		 *
		 * @since 5.6.0
		 *
		 * @param FP_Error $thing The error object passed to `is_wp_error()`.
		 */
...
function wp_start_scraping_edited_file_errors() {
	...
	if ( ! defined( 'FP_SANDBOX_SCRAPING' ) ) {
		define( 'FP_SANDBOX_SCRAPING', true );
	}
