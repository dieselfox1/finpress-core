 ...
 * @return FP_User|FP_Error FP_User on success, FP_Error on failure.
 */
function wp_signon( $credentials = array(), $secure_cookie = '' ) {
	 ...
	 * @param FP_User $user       FP_User object of the logged-in user.
	 */
 ...
 * @since 2.8.0
 *
 * @param FP_User|FP_Error|null $user     FP_User or FP_Error object from a previous callback. Default null.
...
function wp_authenticate_username_password(
	 ...
	 * @since 2.5.0
	 *
	 * @param FP_User|FP_Error $user     FP_User or FP_Error object if a previous
 ...
 * @since 4.5.0
 *
 * @param FP_User|FP_Error|null $user     FP_User or FP_Error object if a previous
 *                                        callback failed authentication.
...
function wp_authenticate_cookie(
	...
	if ( empty( $username ) && empty( $password ) ) {
		...
		if ( ! empty( $_COOKIE[ $auth_cookie ] ) ) {
			return new FP_Error( 'expired_session', __( 'Please log in again.' ) );
		}
 ...
 * @since 5.6.0
 *
 * @param FP_User|FP_Error|null $input_user FP_User or FP_Error object if a previous
 *                                          callback failed authentication.
 * @param string                $username   Username for authentication.
 * @param string                $password   Password for authentication.
 * @return FP_User|FP_Error|null FP_User on success, FP_Error on failure, null if
...
function wp_authenticate_application_password(
	...
	if ( $input_user instanceof FP_User ) {
		return $input_user;
	}

	if ( ! FP_Application_Passwords::is_in_use() ) {
		return $input_user;
	}
	...
	if ( $error ) {
		 ...
		 * @since 5.6.0
		 *
		 * @param FP_Error $error The authentication error.
		 */
	...
	$hashed_passwords = FP_Application_Passwords::get_user_application_passwords( $user->ID );

	foreach ( $hashed_passwords as $key => $item ) {
		if ( ! FP_Application_Passwords::check_password( $password, $item['password'] ) ) {
			continue;
		}
		 ...
		 * @since 5.6.0
		 *
		 * @param FP_Error $error    The error object.
		 * @param FP_User  $user     The user authenticating.
		...
		FP_Application_Passwords::record_application_password_usage( $user->ID, $item['uuid'] );
		 ...
		 * @since 5.6.0
		 *
		 * @param FP_User $user The user who was authenticated.
...
function wp_validate_application_password( $input_user ) {
	...
	if ( $authenticated instanceof FP_User ) {
 ...
 *
 * @param FP_User|FP_Error|null $user FP_User or FP_Error object from a previous callback. Default null.
 * @return FP_User|FP_Error FP_User on success, FP_Error if the user is considered a spammer.
 */
function wp_authenticate_spam_check( $user ) {
	if ( $user instanceof FP_User && is_multisite() ) {
		/**
		 ...
		 * @param FP_User $user    User to check against.
		 */
		$spammed = apply_filters( 'check_is_user_spammed', is_user_spammy( $user ), $user );

		if ( $spammed ) {
			return new FP_Error( 'spammer_account', __( '<strong>Error:</strong> Your account has been marked as a spammer.' ) );
		}
	}
	return $user;
}
...
function get_user_option( $option, $user = 0, $deprecated = '' ) {
	 ...
	 * @param FP_User $user   FP_User object of the user whose option is being retrieved.
	 */
 ...
 * @param int $user_id User ID.
 *
 * @return FP_User|false FP_User object on success, false on failure.
 */
 ...
 * @see FP_User_Query
 *
 * @param array $args Optional. Arguments to retrieve users. See FP_User_Query::prepare_query()
...
function get_users( $args = array() ) {
	...
	$user_search = new FP_User_Query( $args );
 ...
 * @since 3.0.0
 *
 * @link https://developer.finpress.org/reference/functions/delete_user_meta/
 *
 * @param int    $user_id    User ID
 * @param string $meta_key   Metadata name.
 ...
 * @since 3.0.0
 *
 * @link https://developer.finpress.org/reference/functions/get_user_meta/
 *
 * @param int    $user_id User ID.
 ...
 * @since 3.0.0
 *
 * @link https://developer.finpress.org/reference/functions/update_user_meta/
 *
 * @param int    $user_id    User ID.
 ...
 * Assumes role names are unique phrases. Same assumption made by FP_User_Query::prepare_query()
 * Using $strategy = 'time' this is CPU-intensive and should handle around 10^7 users.
 * Using $strategy = 'memory' this is memory-intensive and should handle around 10^5 users, but see FP Bug #12257.
 *
 * @since 3.0.0
 ...
 * @global FP_User $userdata      User data.
 ...
 *     See FP_User_Query::prepare_query() for additional available arguments.
 *
 ...
 * @since 3.0.0
 *
 * @param object|FP_User $user User object or database row to be cached
 ...
 *
 * @param FP_User|int $user User object or ID to be cleaned from the cache
 */
function clean_user_cache( $user ) {
	if ( is_numeric( $user ) ) {
		$user = new FP_User( $user );
	}
	 ...
	 *
	 * @param int     $user_id User ID.
	 * @param FP_User $user    User object.
	 */
 ...
 * @global wpdb $wpdb WordPress database abstraction object.
 *
 * @param array|object|FP_User $userdata {
 *     An array, object, or FP_User object of user data arguments.
 *
 ...
 *                                        Default empty.
 * }
 * @return int|FP_Error The newly created user's ID or a FP_Error object if the user could not
 *                      be created.
 */
function wp_insert_user( $userdata ) {
	...
	} elseif ( $userdata instanceof FP_User ) {
	...
	if ( ! empty( $userdata['ID'] ) ) {
		...
		if ( ! $old_user_data ) {
			return new FP_Error( 'invalid_user_id', __( 'Invalid user ID.' ) );
		}
	...
	if ( empty( $user_login ) ) {
		return new FP_Error( 'empty_user_login', __( 'Cannot create a user with an empty login name.' ) );
	} elseif ( mb_strlen( $user_login ) > 60 ) {
		return new FP_Error( 'user_login_too_long', __( 'Username may not be longer than 60 characters.' ) );
	}

	if ( ! $update && username_exists( $user_login ) ) {
		return new FP_Error( 'existing_user_login', __( 'Sorry, that username already exists!' ) );
	}
	...
	if ( in_array( strtolower( $user_login ), array_map( 'strtolower', $illegal_logins ), true ) ) {
		return new FP_Error( 'invalid_username', __( 'Sorry, that username is not allowed.' ) );
	}
	...
	if ( empty( $user_nicename ) ) {
		return new FP_Error( 'empty_user_nicename', __( 'Cannot create a user with an empty nicename.' ) );
	} elseif ( mb_strlen( $user_nicename ) > 50 ) {
		return new FP_Error( 'user_nicename_too_long', __( 'Nicename may not be longer than 50 characters.' ) );
	}
	...
	if ( ( ! $update || ( ! empty( $old_user_data ) && 0 !== strcasecmp( $user_email, $old_user_data->user_email ) ) )
		&& ! defined( 'FP_IMPORTING' )
		&& email_exists( $user_email )
	) {
		return new FP_Error( 'existing_user_email', __( 'Sorry, that email address is already used!' ) );
	}
	...
	if ( mb_strlen( $user_url ) > 100 ) {
		return new FP_Error( 'user_url_too_long', __( 'User URL may not be longer than 100 characters.' ) );
	}
	...
	if ( ! empty( $userdata['spam'] ) && ! is_multisite() ) {
		return new FP_Error( 'no_spam', __( 'Sorry, marking a user as spam is only supported on Multisite.' ) );
	}
	...
	if ( empty( $data ) || ! is_array( $data ) ) {
		return new FP_Error( 'empty_data', __( 'Not enough data to create this user.' ) );
	}
	...
	if ( $update ) {
		 ...
		 *
		 * @param int     $user_id       User ID.
		 * @param FP_User $old_user_data Object containing user's data prior to update.
		 * @param array   $userdata      The raw array of data passed to wp_insert_user().
		 */
 ...
 *
 * @param array|object|FP_User $userdata An array of user data or a user object of type stdClass or FP_User.
 * @return int|FP_Error The updated user's ID or a FP_Error object if the user could not be updated.
 */
 ...
 * @access private
 *
 * @param FP_User $user FP_User instance.
 ...
 *
 * @param FP_User|null $user Optional. FP_User object.
...
function wp_get_user_contact_methods( $user = null ) {
	 ...
	 * @param FP_User|null $user    FP_User object or null if none was provided.
	 */
 ...
 * @access private
 *
 * @param FP_User|null $user Optional. FP_User object. Default null.
 * @return string[] Array of contact method labels keyed by contact method.
 */
 ...
 * @since 4.4.0
 *
 * @param FP_User $user User to retrieve password reset key for.
 * @return string|FP_Error Password reset key on success. FP_Error on error.
 */
function get_password_reset_key( $user ) {
	if ( ! ( $user instanceof FP_User ) ) {
		return new FP_Error( 'invalidcombo', __( '<strong>Error:</strong> There is no account with that username or email address.' ) );
	}
	...
	if ( ! $password_reset_allowed ) {
		return new FP_Error( 'no_password_reset', __( 'Password reset is not allowed for this user' ) );
 ...
 * but have a different FP_Error code so good user feedback can be provided.
 *
 * @since 3.1.0
 *
 * @param string $key       The password reset key.
 * @param string $login     The user login.
 * @return FP_User|FP_Error FP_User object on success, FP_Error object for invalid or expired keys.
 */
function check_password_reset_key(
	...
	if ( empty( $key ) || ! is_string( $key ) ) {
		return new FP_Error( 'invalid_key', __( 'Invalid key.' ) );
	}
	...
	if ( $hash_is_correct && $expiration_time && time() < $expiration_time ) {
		...
		return new FP_Error( 'expired_key', __( 'Invalid key.' ) );
	}
	...
	if ( hash_equals( $user->user_activation_key, $key ) || ( $hash_is_correct && ! $expiration_time ) ) {
		 ...
		 *
		 * @param FP_Error $return  A FP_Error object denoting an expired key.
		 *                          Return a FP_User object to validate the key.
 ...
 * @return true|FP_Error True when finished, FP_Error object on error.
 */
function retrieve_password( $user_login = '' ) {
	 ...
	 * @since 5.7.0
	 *
	 * @param FP_User|false $user_data FP_User object if found, false if the user does not exist.
	 * @param FP_Error      $errors    A FP_Error object containing any errors generated
	 ...
	 *
	 * The filtered FP_Error object may, for example, contain errors for an invalid
	 * username or email address. A FP_Error object should always be returned,
	 ...
	 * @param bool    $send       Whether to send the email.
	 * @param string  $user_login The username for the user.
	 * @param FP_User $user_data  FP_User object.
	 */
	...
	if ( ! wp_mail( $to, $subject, $message, $headers ) ) {
		$errors->add(
			...
			sprintf(
				...
				esc_url( __( 'https://finpress.org/documentation/article/reset-your-password/' ) )
			)
		);
		return $errors;
	}
 ...
 * @return int|FP_Error Either user's ID or error on failure.
 */
function register_new_user( $user_login, $user_email ) {
	 ...
	 * @param FP_Error $errors               Contains any errors with submitted username and email,
	 ...
	 *
	 * The filtered FP_Error object may, for example, contain errors for an invalid
	 * or existing username or email address. A FP_Error object should always be returned,
	 * but may or may not contain errors.
	 *
	 ...
	 * @since 2.1.0
	 *
	 * @param FP_Error $errors               A FP_Error object containing any errors encountered
	 *                                       during registration.
...
function wp_get_all_sessions() {
	$manager = FP_Session_Tokens::get_instance( get_current_user_id() );
 ...
 * @global FP_User $current_user Checks if the current user is set.
 *
 * @return FP_User Current FP_User instance.
 */
function _wp_get_current_user() {
	global $current_user;

	if ( ! empty( $current_user ) ) {
		if ( $current_user instanceof FP_User ) {
			return $current_user;
		}

		// Upgrade stdClass to FP_User.
		...
		// $current_user has a junk value. Force to FP_User with ID 0.
		$current_user = null;
 ...
 *
 * @global FP_Error $errors FP_Error object.
 */
...
function wp_user_personal_data_exporter( $email_address ) {
	 ...
	 * }
	 * @param FP_User  $user           The user whose data is being exported.
...
function _wp_privacy_send_request_confirmation_notification( $request_id ) {
	...
	if ( ! ( $request instanceof FP_User_Request ) || 'request-confirmed' !== $request->status ) {
		return;
	}
	 ...
	 * @param FP_User_Request $request     The request that is initiating the notification.
	 */
	 ...
	 *
	 *     @type FP_User_Request $request     User request object.
...
function _wp_privacy_send_erasure_fulfillment_notification( $request_id ) {
	...
	if ( ! ( $request instanceof FP_User_Request ) || 'request-completed' !== $request->status ) {
		return;
	}
 ...
 * @return int|FP_Error                   Returns the request ID if successful, or a FP_Error object on failure.
 */
function wp_create_user_request( $email_address = '', $action_name = '', $request_data = array(), $status = 'pending' ) {
	...
	if ( ! is_email( $email_address ) ) {
		return new FP_Error( 'invalid_email', __( 'Invalid email address.' ) );
	}

	if ( ! in_array( $action_name, _wp_privacy_action_request_types(), true ) ) {
		return new FP_Error( 'invalid_action', __( 'Invalid action name.' ) );
	}

	if ( ! in_array( $status, array( 'pending', 'confirmed' ), true ) ) {
		return new FP_Error( 'invalid_status', __( 'Invalid request status.' ) );
	}
	...
	if ( $requests_query->found_posts ) {
		return new FP_Error( 'duplicate_request', __( 'An incomplete personal data request for this email address already exists.' ) );
	}
 ...
 * @return true|FP_Error True on success, `FP_Error` on failure.
 */
function wp_send_user_request( $request_id ) {
	...
	if ( ! $request ) {
		return new FP_Error( 'invalid_request', __( 'Invalid personal data request.' ) );
	}
...
###SITEURL###'
	...
	if ( ! $email_sent ) {
		return new FP_Error( 'privacy_email_error', __( 'Unable to send personal data export confirmation email.' ) );
	}
...
function wp_validate_user_request_key(
	...
	if ( ! in_array( $request->status, array( 'request-pending', 'request-failed' ), true ) ) {
		return new FP_Error( 'expired_request', __( 'This personal data request has expired.' ) );
	}

	if ( empty( $key ) ) {
		return new FP_Error( 'missing_key', __( 'The confirmation key is missing from this personal data request.' ) );
	}
	...
	if ( ! wp_verify_fast_hash( $key, $saved_key ) ) {
		return new FP_Error( 'invalid_key', __( 'The confirmation key is invalid for this personal data request.' ) );
	}

	if ( ! $expiration_time || time() > $expiration_time ) {
		return new FP_Error( 'expired_key', __( 'The confirmation key has expired for this personal data request.' ) );
	}
...
function wp_get_user_request( $request_id ) {
	...
	return new FP_User_Request( $post );
}
 ...
 * @since 5.6.0
 *
 * @param int|FP_User $user The user to check.
 * @return bool
 */
function wp_is_application_passwords_available_for_user( $user ) {
	 ...
	 *
	 * @param bool    $available True if available, false otherwise.
	 * @param FP_User $user      The user to check.
	 */
 ...
 *
 * @param int|FP_User $user The user to check.
 * @return bool|FP_Error True if allowed, false or FP_Error otherwise.
 */
