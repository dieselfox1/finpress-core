...
final class FP_Post_Type {
	 ...
	 * to use a font icon, e.g. 'dashicons-chart-pie'. Pass 'none' to leave div.fp-menu-image empty
	 ...
	 * Default empty array.
	 *
	 * @link https://developer.finpress.org/block-editor/developers/block-api/block-templates/
	 *
	 * @since 5.0.0
	 * @var array[] $template
	 */
	 ...
	 *
	 * Custom controllers must extend FP_REST_Controller.
	 *
	 * @since 4.7.4
	 ...
	 *
	 * Lazily computed. Should be accessed using {@see FP_Post_Type::get_rest_controller()}.
	 *
	 * @since 5.3.0
	 * @var FP_REST_Controller $rest_controller
	 */
	 ...
	 *
	 * Lazily computed. Should be accessed using {@see FP_Post_Type::get_revisions_rest_controller()}.
	 *
	 * @since 6.4.0
	 * @var FP_REST_Controller $revisions_rest_controller
	 */
	 ...
	 *
	 * Lazily computed. Should be accessed using {@see FP_Post_Type::get_autosave_rest_controller()}.
	 *
	 * @since 6.4.0
	 * @var FP_REST_Controller $autosave_rest_controller
	 */
	...
	public function set_props( $args ) {
		...
		// If not set, default rest_namespace to fp/v2 if show_in_rest is true.
		if ( false === $args['rest_namespace'] && ! empty( $args['show_in_rest'] ) ) {
			$args['rest_namespace'] = 'fp/v2';
		}
	...
	public function add_rewrite_rules() {
		global $wp_rewrite, $fp;

		if ( false !== $this->query_var && $fp && is_post_type_viewable( $this ) ) {
			$fp->add_query_var( $this->query_var );
		}
	...
	public function remove_rewrite_rules() {
		global $fp, $wp_rewrite, $post_type_meta_caps;

		// Remove query var.
		if ( false !== $this->query_var ) {
			$fp->remove_query_var( $this->query_var );
		}
	 ...
	 * @since 5.3.0
	 *
	 * @return FP_REST_Controller|null The controller instance, or null if the post type
	...
	public function get_rest_controller() {
		...
		$class = $this->rest_controller_class ? $this->rest_controller_class : FP_REST_Posts_Controller::class;

		if ( ! class_exists( $class ) ) {
			return null;
		}

		if ( ! is_subclass_of( $class, FP_REST_Controller::class ) ) {
			return null;
		}
	...
	public function get_revisions_rest_controller() {
		...
		$class = $this->revisions_rest_controller_class ? $this->revisions_rest_controller_class : FP_REST_Revisions_Controller::class;
		if ( ! class_exists( $class ) ) {
			return null;
		}
	...
	public function get_autosave_rest_controller() {
		...
		$class = $this->autosave_rest_controller_class ? $this->autosave_rest_controller_class : FP_REST_Autosaves_Controller::class;
