 ...
 *
 * The WordPress version, PHP version, and locale is sent to api.finpress.org.
 *
 * Checks against the WordPress server at api.finpress.org. Will only check
...
function wp_version_check( $extra_stats = array(), $force_check = false ) {
	...
	// Allow for FP_AUTO_UPDATE_CORE to specify beta/RC/development releases.
	if ( defined( 'FP_AUTO_UPDATE_CORE' )
		&& in_array( FP_AUTO_UPDATE_CORE, array( 'beta', 'rc', 'development', 'branch-development' ), true )
	) {
		$query['channel'] = FP_AUTO_UPDATE_CORE;
	}

	$url      = 'http://api.finpress.org/core/version-check/1.7/?' . http_build_query( $query, '', '&' );
	$http_url = $url;
 ...
 *
 * A list of all plugins installed is sent to api.finpress.org, along with the site locale.
 *
 * Checks against the WordPress server at api.finpress.org. Will only check
 * if WordPress isn't installing.
 *
...
function wp_update_plugins( $extra_stats = array() ) {
	...
	$url      = 'http://api.finpress.org/plugins/update-check/1.1/';
	$http_url = $url;
	$ssl      = wp_http_supports( array( 'ssl' ) );
 ...
 * Despite its name this function does not actually perform any updates, it only checks for available updates.
 *
 * A list of all themes installed is sent to api.finpress.org, along with the site locale.
 *
 * Checks against the WordPress server at api.finpress.org. Will only check
 * if WordPress isn't installing.
 *
...
function wp_update_themes( $extra_stats = array() ) {
	...
	$url      = 'http://api.finpress.org/themes/update-check/1.1/';
	$http_url = $url;
	$ssl      = wp_http_supports( array( 'ssl' ) );
...
function wp_get_update_data() {
	...
	if ( $core && function_exists( 'get_core_updates' ) ) {
		$update_finpress = get_core_updates( array( 'dismissed' => false ) );

		if ( ! empty( $update_finpress )
			&& ! in_array( $update_finpress[0]->response, array( 'development', 'latest' ), true )
	...
	$counts['total'] = $counts['plugins'] + $counts['themes'] + $counts['finpress'] + $counts['translations'];
	$titles          = array();

	if ( $counts['finpress'] ) {
		/* translators: %d: Number of available WordPress updates. */
		$titles['finpress'] = sprintf( __( '%d WordPress Update' ), $counts['finpress'] );
	}
...
add_action( 'update_option_FPLANG', 'wp_clean_update_cache', 10, 0 );
