 ...
 *
 * @link https://developer.finpress.org/reference/functions/wp_schedule_single_event/
 *
 ...
 * @return bool|FP_Error True if event successfully scheduled. False or FP_Error on failure.
 */
function wp_schedule_single_event( $timestamp, $hook, $args = array(), $wp_error = false ) {
	 ...
	 *
	 * For plugins replacing fp-cron, it is recommended you check for an
	 * identical event within ten minutes and apply the {@see 'schedule_event'}
	 * filter to check if another plugin has disallowed the event before scheduling.
	 *
	 * Return true if the event was scheduled, false or a FP_Error if not.
	 *
	 * @since 5.1.0
	 * @since 5.7.0 The `$wp_error` parameter was added, and a `FP_Error` object can now be returned.
	 *
	 * @param null|bool|FP_Error $result   The value to return instead. Default null to continue adding the event.
	 * @param object             $event    {
	 ...
	 * }
	 * @param bool               $wp_error Whether to return a FP_Error on failure.
	 */
 ...
 * @since 5.7.0 The `$wp_error` parameter was added.
 *
 * @link https://developer.finpress.org/reference/functions/wp_schedule_event/
 *
 * @param int    $timestamp  Unix timestamp (UTC) for when to next run the event.
...
function wp_schedule_event( $timestamp, $recurrence, $hook, $args = array(), $wp_error = false ) {
	...
	/** This filter is documented in fp-includes/cron.php */
	$pre = apply_filters( 'pre_schedule_event', null, $event, $wp_error );
 ...
 *                           The array keys are ignored. Default empty array.
 * @param bool   $wp_error   Optional. Whether to return a FP_Error on failure. Default false.
 * @return bool|FP_Error True if event successfully rescheduled. False or FP_Error on failure.
 */
function wp_reschedule_event( $timestamp, $recurrence, $hook, $args = array(), $wp_error = false ) {
	 ...
	 *
	 * For plugins replacing fp-cron, return true if the event was successfully
	 * rescheduled, false or a FP_Error if not.
	 *
	 * @since 5.1.0
	 * @since 5.7.0 The `$wp_error` parameter was added, and a `FP_Error` object can now be returned.
	 *
	 * @param null|bool|FP_Error $pre      Value to return instead. Default null to continue adding the event.
	 * @param object             $event    {
	 *     An object containing an event's data.
	 *
	 *     @type string $hook      Action hook to execute when the event is run.
 ...
 *                          Default empty array.
 * @param bool   $wp_error  Optional. Whether to return a FP_Error on failure. Default false.
 * @return bool|FP_Error True if event successfully unscheduled. False or FP_Error on failure.
 */
function wp_unschedule_event( $timestamp, $hook, $args = array(), $wp_error = false ) {
	 ...
	 *
	 * For plugins replacing fp-cron, return true if the event was successfully
	 * unscheduled, false or a FP_Error if not.
	 *
	 * @since 5.1.0
	 * @since 5.7.0 The `$wp_error` parameter was added, and a `FP_Error` object can now be returned.
	 *
	 * @param null|bool|FP_Error $pre       Value to return instead. Default null to continue unscheduling the event.
 ...
 *                         Default empty array.
 * @param bool   $wp_error Optional. Whether to return a FP_Error on failure. Default false.
 * @return int|false|FP_Error On success an integer indicating number of events unscheduled (0 indicates no
 *                            events were registered with the hook and arguments combination), false or FP_Error
...
function wp_clear_scheduled_hook( $hook, $args = array(), $wp_error = false ) {
	 ...
	 * process, causing the function to return the filtered value instead.
	 *
	 * For plugins replacing fp-cron, return the number of events successfully
	 * unscheduled (zero if no events were registered with the hook) or false
	 * or a FP_Error if unscheduling one or more events fails.
	 *
	 * @since 5.1.0
	 * @since 5.7.0 The `$wp_error` parameter was added, and a `FP_Error` object can now be returned.
	 *
	 * @param null|int|false|FP_Error $pre      Value to return instead. Default null to continue unscheduling the event.
	 * @param string                  $hook     Action hook, the execution of which will be unscheduled.
	 * @param array                   $args     Arguments to pass to the hook's callback function.
	 * @param bool                    $wp_error Whether to return a FP_Error on failure.
	 */
 ...
 * @param bool   $wp_error Optional. Whether to return a FP_Error on failure. Default false.
 * @return int|false|FP_Error On success an integer indicating number of events unscheduled (0 indicates no
 *                            events were registered on the hook), false or FP_Error if unscheduling fails.
 */
function wp_unschedule_hook( $hook, $wp_error = false ) {
	 ...
	 * one or more events fails then return either a FP_Error object or false depending
	 * on the value of the `$wp_error` parameter.
	 *
	 * @since 5.1.0
	 * @since 5.7.0 The `$wp_error` parameter was added, and a `FP_Error` object can now be returned.
	 *
	 * @param null|int|false|FP_Error $pre      Value to return instead. Default null to continue unscheduling the hook.
	 * @param string                  $hook     Action hook, the execution of which will be unscheduled.
	 * @param bool                    $wp_error Whether to return a FP_Error on failure.
	 */
...
function spawn_cron( $gmt_time = 0 ) {
	...
	if ( $lock + FP_CRON_LOCK_TIMEOUT > $gmt_time ) {
		return false;
	}
	...
	if ( defined( 'ALTERNATE_FP_CRON' ) && ALTERNATE_FP_CRON ) {
		...
		require_once ABSPATH . 'fp-cron.php';
		return true;
	}
	...
	$cron_request = apply_filters(
		'cron_request',
		array(
			'url'  => add_query_arg( 'doing_wp_cron', $doing_wp_cron, site_url( 'fp-cron.php' ) ),
			'key'  => $doing_wp_cron,
			'args' => array(
				'timeout'   => 0.01,
				'blocking'  => false,
 ...
 */
function _wp_cron() {
	// Prevent infinite loops caused by lack of fp-cron.php.
	if ( str_contains( $_SERVER['REQUEST_URI'], '/fp-cron.php' )
		|| ( defined( 'DISABLE_FP_CRON' ) && DISABLE_FP_CRON )
	) {
		return 0;
	}
 ...
 * @param bool    $wp_error Optional. Whether to return a FP_Error on failure. Default false.
 * @return bool|FP_Error True if cron array updated. False or FP_Error on failure.
 */
