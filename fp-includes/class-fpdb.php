 ...
 *
 * @link https://developer.finpress.org/reference/classes/wpdb/
 *
 * @since 0.71
 */
...
class wpdb {
	 ...
	 *
	 * Default is to show errors if both FP_DEBUG and FP_DEBUG_DISPLAY evaluate to true.
	 *
	 * @since 0.71
	 *
	 * @var bool
	 ...
	 *
	 * Columns not listed here default to %s. Initialized during FP load.
	 ...
	 * @since 2.0.8
	 *
	 * @link https://core.trac.finpress.org/ticket/3354
	 *
	 ...
	 * @return string|FP_Error Old prefix or FP_Error on error.
	 */
	public function set_prefix( $prefix, $set_table_names = true ) {

		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) ) {
			return new FP_Error( 'invalid_db_prefix', 'Invalid database prefix' );
		}
	...
	public function prepare( $query, ...$args ) {
		...
		while ( $key < $split_query_count ) {
			...
			if ( 'f' === $type && true === $this->allow_unsafe_unquoted_parameters
				...
				/*
				 * Before FP 6.2 the "force floats to be locale-unaware" RegEx didn't
	...
	public function check_connection( $allow_bail = true ) {
		...
		for ( $tries = 1; $tries <= $this->reconnect_retries; $tries++ ) {
			 ...
			 */
			if ( $this->reconnect_retries === $tries && FP_DEBUG ) {
				error_reporting( $error_reporting );
			}
	...
	public function placeholder_escape() {
		...
		if ( ! $placeholder ) {
			// Old FP installs may not have AUTH_SALT defined.
	 ...
	 *
	 * @param string $table Table name.
	 * @return string|FP_Error Table character set, FP_Error object if it couldn't be found.
	 */
	protected function get_table_charset( $table ) {
		 ...
		 * @since 4.2.0
		 *
		 * @param string|FP_Error|null $charset The character set to use, FP_Error object
		...
		if ( ! $results ) {
			return new FP_Error( 'wpdb_get_table_charset_failure', __( 'Could not retrieve table charset.' ) );
		}
	 ...
	 * @return string|false|FP_Error Column character set as a string. False if the column has
	 *                               no character set. FP_Error object if there was an error.
	 */
	public function get_col_charset( $table, $column ) {
		 ...
		 * @since 4.2.0
		 *
		 * @param string|null|false|FP_Error $charset The character set to use. Default null.
		 * @param string                     $table   The name of the table being checked.
	 ...
	 * @param string $table  Table name.
	 * @param string $column Column name.
	 * @return array|false|FP_Error {
	 *     Array of column length information, false if the column has no length (for
	 *     example, numeric column), FP_Error object if there was an error.
	 *
	 *     @type string $type   One of 'byte' or 'char'.
	 *     @type int    $length The column length.
	 * }
	 ...
	 * @return array|FP_Error The $data parameter, with invalid characters removed from each value.
	...
	protected function strip_invalid_text( $data ) {
		...
		if ( $db_check_string ) {
			...
			if ( ! $row ) {
				return new FP_Error( 'wpdb_strip_invalid_text_failure', __( 'Could not strip invalid text.' ) );
			}
	 ...
	 * @return string|FP_Error The converted query, or a FP_Error object if the conversion fails.
	 */
	 ...
	 * @return string|FP_Error The converted string, or a FP_Error object if the conversion fails.
	 */
	...
	public function bail( $message, $error_code = '500' ) {
		if ( $this->show_errors ) {
			...
			if ( class_exists( 'FP_Error', false ) ) {
				$this->error = new FP_Error( $error_code, $message );
			} else {
				$this->error = $message;
			}
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			...
			return new FP_Error( 'database_version', sprintf( __( '<strong>Error:</strong> WordPress %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ) );
		}
	}
	...
	/**
	 * Determines whether the database or FPDB supports a particular feature.
	 *
	 * Capability sniffs for the database server and current version of FPDB.
	 *
	 * Database sniffs are based on the version of the database server in use.
	 *
	 * FPDB sniffs are added as new features are introduced to allow theme and plugin
