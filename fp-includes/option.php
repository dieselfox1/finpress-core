		...
		if ( isset( $alloptions[ $option ] ) ) {
			...
			if ( false === $value ) {
				...
				if ( is_object( $row ) ) {
					...
					/** This filter is documented in fp-includes/option.php */
					return apply_filters( "default_option_{$option}", $default_value, $option, $passed_default );
				}
			}
		}
	...
	if ( 'alloptions' === $option || 'notoptions' === $option ) {
		wp_die(
			sprintf(
				...
				__( '%s is a protected FP option and may not be modified' ),
				esc_html( $option )
			)
		);
	}
...
function wp_load_core_site_options( $network_id = null ) {
	...
	$core_options = array( 'site_name', 'siteurl', 'active_sitewide_plugins', '_site_transient_timeout_theme_roots', '_site_transient_theme_roots', 'site_admins', 'can_compress_scripts', 'global_terms_enabled', 'ms_files_rewriting', 'FPLANG' );
...
function update_option( $option, $value, $autoload = null ) {
	 ...
	 *
	 * See https://core.trac.finpress.org/ticket/38903
	 */
...
function wp_user_settings() {
	...
	if ( isset( $_COOKIE[ 'fp-settings-' . $user_id ] ) ) {
		$cookie = preg_replace( '/[^A-Za-z0-9=&_]/', '', $_COOKIE[ 'fp-settings-' . $user_id ] );
		...
		if ( isset( $_COOKIE[ 'fp-settings-time-' . $user_id ] ) ) {
			$current = (int) preg_replace( '/[^0-9]/', '', $_COOKIE[ 'fp-settings-time-' . $user_id ] );
		}
	...
	setcookie( 'fp-settings-' . $user_id, $settings, time() + YEAR_IN_SECONDS, SITECOOKIEPATH, '', $secure );
	setcookie( 'fp-settings-time-' . $user_id, time(), time() + YEAR_IN_SECONDS, SITECOOKIEPATH, '', $secure );
	$_COOKIE[ 'fp-settings-' . $user_id ] = $settings;
}
...
function delete_all_user_settings() {
	...
	setcookie( 'fp-settings-' . $user_id, ' ', time() - YEAR_IN_SECONDS, SITECOOKIEPATH );
}
...
function update_network_option( $network_id, $option, $value ) {
	 ...
	 * unnecessary database calls for otherwise identical object instances.
	 *
	 * See https://core.trac.finpress.org/ticket/44956
	 */
	if ( $value === $old_value || maybe_serialize( $value ) === maybe_serialize( $old_value ) ) {
		return false;
	}
