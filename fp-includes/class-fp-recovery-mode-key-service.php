<?php
/**
 * Error Protection API: FP_Recovery_Mode_Key_Service class
 *
 * @package WordPress
 * @since 5.2.0
 */
...
final class FP_Recovery_Mode_Key_Service {
	...
	public function validate_recovery_mode_key( $token, $key, $ttl ) {
		...
		if ( ! isset( $records[ $token ] ) ) {
			return new FP_Error( 'token_not_found', __( 'Recovery Mode not initialized.' ) );
		}
		...
		if ( ! is_array( $record ) || ! isset( $record['hashed_key'], $record['created_at'] ) ) {
			return new FP_Error( 'invalid_recovery_key_format', __( 'Invalid recovery key format.' ) );
		}

		if ( ! wp_verify_fast_hash( $key, $record['hashed_key'] ) ) {
			return new FP_Error( 'hash_mismatch', __( 'Invalid recovery key.' ) );
		}

		if ( time() > $record['created_at'] + $ttl ) {
			return new FP_Error( 'key_expired', __( 'Recovery key expired.' ) );
		}
