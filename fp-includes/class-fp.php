...
class FP {
	...
	public function parse_request( $extra_query_vars = '' ) {
		 ...
		 * @param FP           $wp               Current WordPress environment instance.
		...
		if ( ! empty( $rewrite ) ) {
			...
			if ( ! empty( $this->matched_rule ) ) {
				...
				$query = addslashes( FP_MatchesMapRegex::apply( $query, $matches ) );
		 ...
		 * @since 2.1.0
		 *
		 * @param FP $wp Current WordPress environment instance (passed by reference).
		 */
	...
	public function send_headers() {
		...
		if ( ! empty( $this->query_vars['error'] ) ) {
			...
			// We're showing a feed, so FP is indeed the only thing that last changed.
	 ...
	 * @global FP_Post|null $post         The current post, if available.
	 * @global string       $request      The SQL statement for the request.
	 * @global int          $more         Only set, if single page or post.
	 * @global int          $single       If single page or post. Only set, if single page or post.
	 * @global FP_User      $authordata   Only set, if author archive.
	 */
	...
	public function handle_404() {
		 ...
		 * @param FP_Query $wp_query WordPress Query object.
		 */
	 ...
	 *
	 * The action {@see 'wp'} has one parameter that references the FP object. It
