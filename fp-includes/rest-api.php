...
function register_rest_route( $route_namespace, $route, $args = array(), $override = false ) {
	if ( empty( $route_namespace ) ) {
		 ...
		 * non-namespaced route, call `FP_REST_Server::register_route` directly.
		 */
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
 ...
 * @global FP $wp Current WordPress environment instance.
 */
...
function create_initial_rest_routes() {
	...
	// Post types.
	$controller = new FP_REST_Post_Types_Controller();
	$controller->register_routes();

	// Post statuses.
	$controller = new FP_REST_Post_Statuses_Controller();
	$controller->register_routes();

	// Taxonomies.
	$controller = new FP_REST_Taxonomies_Controller();
	$controller->register_routes();
	...
	// Users.
	$controller = new FP_REST_Users_Controller();
	$controller->register_routes();

	// Application Passwords
	$controller = new FP_REST_Application_Passwords_Controller();
	$controller->register_routes();

	// Comments.
	$controller = new FP_REST_Comments_Controller();
	$controller->register_routes();

	$search_handlers = array(
		new FP_REST_Post_Search_Handler(),
		new FP_REST_Term_Search_Handler(),
		new FP_REST_Post_Format_Search_Handler(),
	);
	 ...
	 *                               handler instance must extend the `FP_REST_Search_Handler` class.
	 *                               Default is only a handler for posts.
	 */
	$search_handlers = apply_filters( 'wp_rest_search_handlers', $search_handlers );

	$controller = new FP_REST_Search_Controller( $search_handlers );
	$controller->register_routes();

	// Block Renderer.
	$controller = new FP_REST_Block_Renderer_Controller();
	$controller->register_routes();

	// Block Types.
	$controller = new FP_REST_Block_Types_Controller();
	$controller->register_routes();

	// Settings.
	$controller = new FP_REST_Settings_Controller();
	$controller->register_routes();

	// Themes.
	$controller = new FP_REST_Themes_Controller();
	$controller->register_routes();

	// Plugins.
	$controller = new FP_REST_Plugins_Controller();
	$controller->register_routes();

	// Sidebars.
	$controller = new FP_REST_Sidebars_Controller();
	$controller->register_routes();

	// Widget Types.
	$controller = new FP_REST_Widget_Types_Controller();
	$controller->register_routes();

	// Widgets.
	$controller = new FP_REST_Widgets_Controller();
	$controller->register_routes();

	// Block Directory.
	$controller = new FP_REST_Block_Directory_Controller();
	$controller->register_routes();

	// Pattern Directory.
	$controller = new FP_REST_Pattern_Directory_Controller();
	$controller->register_routes();

	// Block Patterns.
	$controller = new FP_REST_Block_Patterns_Controller();
	$controller->register_routes();

	// Block Pattern Categories.
	$controller = new FP_REST_Block_Pattern_Categories_Controller();
	$controller->register_routes();

	// Site Health.
	$site_health = FP_Site_Health::get_instance();
	$controller  = new FP_REST_Site_Health_Controller( $site_health );
	$controller->register_routes();

	// URL Details.
	$controller = new FP_REST_URL_Details_Controller();
	$controller->register_routes();

	// Menu Locations.
	$controller = new FP_REST_Menu_Locations_Controller();
	$controller->register_routes();

	// Site Editor Export.
	$controller = new FP_REST_Edit_Site_Export_Controller();
	$controller->register_routes();

	// Navigation Fallback.
	$controller = new FP_REST_Navigation_Fallback_Controller();
	$controller->register_routes();

	// Font Collections.
	$font_collections_controller = new FP_REST_Font_Collections_Controller();
 ...
 * Do a REST request.
 *
 * Used primarily to route internal requests through FP_REST_Server.
 *
 * @since 4.4.0
 *
 * @param FP_REST_Request|string $request Request.
 * @return FP_REST_Response REST response.
 */
 ...
 * @since 4.5.0
 *
 * @global FP_REST_Server $wp_rest_server REST server instance.
 *
 * @return FP_REST_Server REST server instance.
 */
function rest_get_server() {
	/* @var FP_REST_Server $wp_rest_server */
	...
	if ( empty( $wp_rest_server ) ) {
		 ...
		 * @since 4.4.0
		 *
		 * @param string $class_name The name of the server class. Default 'FP_REST_Server'.
		 */
		$wp_rest_server_class = apply_filters( 'wp_rest_server_class', 'FP_REST_Server' );
		 ...
		 * @since 4.4.0
		 *
		 * @param FP_REST_Server $wp_rest_server Server object.
		 */
 ...
 *
 * @param array|string|FP_REST_Request $request Request to check.
 * @return FP_REST_Request REST request instance.
 */
function rest_ensure_request( $request ) {
	if ( $request instanceof FP_REST_Request ) {
		return $request;
	}

	if ( is_string( $request ) ) {
		return new FP_REST_Request( 'GET', $request );
	}
 ...
 *
 * This implements FP_REST_Response, allowing usage of `set_status`/`header`/etc
 * without needing to double-check the object. Will also allow FP_Error to indicate error
 * responses, so users should immediately check for this value.
 *
 * @since 4.4.0
 *
 * @param FP_REST_Response|FP_Error|FP_HTTP_Response|mixed $response Response to check.
 * @return FP_REST_Response|FP_Error If response generated an error, FP_Error, if response
 *                                   is already an instance, FP_REST_Response, otherwise
 *                                   returns a new FP_REST_Response instance.
 */
function rest_ensure_response( $response ) {
	...
	if ( $response instanceof FP_REST_Response ) {
		return $response;
	}

	/*
	 * While FP_HTTP_Response is the base class of FP_REST_Response, it doesn't provide
	 * all the required methods used in FP_REST_Server::dispatch().
	 */
	if ( $response instanceof FP_HTTP_Response ) {
		return new FP_REST_Response(
			$response->get_data(),
			$response->get_status(),
	...
	return new FP_REST_Response( $response );
}
...
function rest_handle_deprecated_function( $function_name, $replacement, $version ) {
	...
	header( sprintf( 'X-FP-DeprecatedFunction: %s', $string ) );
}
...
function rest_handle_deprecated_argument( $function_name, $message, $version ) {
	...
	header( sprintf( 'X-FP-DeprecatedParam: %s', $string ) );
}
...
function rest_handle_doing_it_wrong( $function_name, $message, $version ) {
	...
	header( sprintf( 'X-FP-DoingItWrong: %s', $string ) );
}
 ...
 * @param FP_REST_Server  $handler  ResponseHandler instance (usually FP_REST_Server).
 * @param FP_REST_Request $request  The request that was used to make current response.
 * @return FP_REST_Response Modified response, either response or `null` to indicate pass-through.
 */
 ...
 * @since 4.4.0
 *
 * @param FP_REST_Response $response Current response being served.
 * @param FP_REST_Server   $server   ResponseHandler instance (usually FP_REST_Server).
 * @param FP_REST_Request  $request  The request that was used to make current response.
 * @return FP_REST_Response Response to be served, with "Allow" header if route has allowed methods.
 */
 ...
 * @param FP_REST_Server   $server   ResponseHandler instance (usually FP_REST_Server).
 * @param FP_REST_Request  $request  The request that was used to make current response.
 * @return FP_REST_Response Response to be served, trimmed down to contain a subset of fields.
 */
...
/**
 * Adds the REST API URL to the FP RSD endpoint.
 *
 * @since 4.4.0
 *
 * @see get_rest_url()
...
function rest_output_rsd() {
	...
	?>
	<api name="FP-API" blogID="1" preferred="false" apiLink="<?php echo esc_url( $api_root ); ?>" />
	<?php
}
 ...
 *
 * @param FP_Error|mixed $result Error from another authentication handler,
 *                               null if we should handle it, or another value if not.
 * @return FP_Error|mixed|bool FP_Error if the cookie is invalid, the $result, otherwise true.
 */
function rest_cookie_check_errors( $result ) {
	...
	} elseif ( isset( $_SERVER['HTTP_X_FP_NONCE'] ) ) {
		$nonce = $_SERVER['HTTP_X_FP_NONCE'];
	}
	...
	if ( ! $result ) {
		...
		return new FP_Error( 'rest_cookie_invalid_nonce', __( 'Cookie check failed' ), array( 'status' => 403 ) );
	}

	// Send a refreshed nonce in header.
	rest_get_server()->send_header( 'X-FP-Nonce', wp_create_nonce( 'wp_rest' ) );
 ...
 *
 * @global FP_User|FP_Error|null $wp_rest_application_password_status
 * @global string|null $wp_rest_application_password_uuid
 *
 * @param FP_Error $user_or_error The authenticated user or error instance.
 ...
 * @global FP_User|FP_Error|null $wp_rest_application_password_status
 *
 * @param FP_Error|null|true $result Error from another authentication handler,
 *                                   null if we should handle it, or another value if not.
 * @return FP_Error|null|true FP_Error if the application password is invalid, the $result, otherwise true.
 */
function rest_application_password_check_errors( $result ) {
	...
	if ( $wp_rest_application_password_status instanceof FP_User ) {
		return true;
	}
 ...
 * @since 5.6.0
 *
 * @param FP_REST_Response $response The index response object.
 * @return FP_REST_Response
 */
 ...
 *                           user email, FP_User object, FP_Post object, or FP_Comment object.
...
/**
 * Formats a combining operation error into a FP_Error object.
 *
 * @since 5.6.0
 *
 ...
 * @return FP_Error      The combining operation error.
 */
function rest_get_combining_operation_error( $value, $param, $errors ) {
	...
	if ( count( $schema_titles ) === count( $errors ) ) {
		...
		return new FP_Error( 'rest_no_matching_schema', wp_sprintf( __( '%1$s is not a valid %2$l.' ), $param, $schema_titles ) );
	}

	/* translators: %s: Parameter. */
	return new FP_Error( 'rest_no_matching_schema', sprintf( __( '%s does not match any of the expected formats.' ), $param ) );
}
 ...
 * @param array  $args    The schema array to use.
 * @param string $param   The parameter name, used in error messages.
 * @return array|FP_Error The matching schema or FP_Error instance if all schemas do not match.
 */
 ...
 * @return array|FP_Error                The matching schema or FP_Error instance if the number of matching schemas is not equal to one.
 */
 ...
 * @param array  $args   The schema array to use.
 * @param string $param  The parameter name, used in error messages.
 * @return true|FP_Error True if the "enum" contains the value or a FP_Error instance otherwise.
 */
function rest_validate_enum( $value, $args, $param ) {
	...
	if ( count( $encoded_enum_values ) === 1 ) {
		...
		return new FP_Error( 'rest_not_in_enum', wp_sprintf( __( '%1$s is not %2$s.' ), $param, $encoded_enum_values[0] ) );
	}

	/* translators: 1: Parameter, 2: List of valid values. */
	return new FP_Error( 'rest_not_in_enum', wp_sprintf( __( '%1$s is not one of %2$l.' ), $param, $encoded_enum_values ) );
}
		...
		switch ( $args['format'] ) {
			case 'hex-color':
				if ( ! rest_parse_hex_color( $value ) ) {
					return new FP_Error( 'rest_invalid_hex_color', __( 'Invalid hex color.' ) );
				}
				break;

			case 'date-time':
				if ( false === rest_parse_date( $value ) ) {
					return new FP_Error( 'rest_invalid_date', __( 'Invalid date.' ) );
				}
				break;

			case 'email':
				if ( ! is_email( $value ) ) {
					return new FP_Error( 'rest_invalid_email', __( 'Invalid email address.' ) );
				}
				break;
			case 'ip':
				if ( ! rest_is_ip_address( $value ) ) {
					/* translators: %s: IP address. */
					return new FP_Error( 'rest_invalid_ip', sprintf( __( '%s is not a valid IP address.' ), $param ) );
				}
				break;
			case 'uuid':
				if ( ! wp_is_uuid( $value ) ) {
					/* translators: %s: The name of a JSON field expecting a valid UUID. */
					return new FP_Error( 'rest_invalid_uuid', sprintf( __( '%s is not a valid UUID.' ), $param ) );
				}
				break;
		}
	}
...
function rest_validate_array_value_from_schema( $value, $args, $param ) {
	...
	if ( ! empty( $args['uniqueItems'] ) && ! rest_validate_array_contains_unique_items( $value ) ) {
		/* translators: %s: Parameter. */
		return new FP_Error( 'rest_duplicate_items', sprintf( __( '%s has duplicate items.' ), $param ) );
	}
 ...
 * @param string $param The parameter name, used in error messages.
 * @return mixed|FP_Error The sanitized value or a FP_Error instance if the value cannot be safely sanitized.
 */
...
function rest_preload_api_request( $memo, $path ) {
	...
	$request = new FP_REST_Request( $method, $path_parts['path'] );
...
function rest_get_route_for_post_type_items( $post_type ) {
	 ...
	 *
	 * @param string       $route      The route path.
	 * @param FP_Post_Type $post_type  The post type object.
	 */
...
function rest_get_route_for_taxonomy_items( $taxonomy ) {
	 ...
	 *
	 * @param string      $route    The route path.
	 * @param FP_Taxonomy $taxonomy The taxonomy object.
	 */
 ...
 *                       on `EDITABLE` endpoints. Default FP_REST_Server::CREATABLE.
 * @return array The endpoint arguments.
 */
function rest_get_endpoint_args_for_schema( $schema, $method = FP_REST_Server::CREATABLE ) {
	...
	foreach ( $schema_properties as $field_id => $params ) {
		...
		if ( FP_REST_Server::CREATABLE === $method && isset( $params['default'] ) ) {
			$endpoint_args[ $field_id ]['default'] = $params['default'];
		}

		if ( FP_REST_Server::CREATABLE === $method && ! empty( $params['required'] ) ) {
 ...
 * @since 5.7.0
 *
 * @param FP_Error $error FP_Error instance.
 *
 * @return FP_REST_Response List of associative arrays with code and message keys.
 */
function rest_convert_error_to_response( $error ) {
	...
	return new FP_REST_Response( $data, $status );
}
