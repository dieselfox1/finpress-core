...
class FP_Theme_JSON {
	 ...
	 *                       Example output: "--fp--preset--duotone--blue: <value>" will generate as many CSS Custom Properties as presets defined
	 ...
	 * @since 6.3.0 Replaced value_func for duotone with `null`. Custom properties are handled by class-fp-duotone.php.
	...
	const PRESETS_METADATA = array(
		array(
			...
			'value_key'         => 'ratio',
			'css_vars'          => '--fp--preset--aspect-ratio--$slug',
			'classes'           => array(),
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'gradient',
			'css_vars'          => '--fp--preset--gradient--$slug',
		...
		array(
			...
			'use_default_names' => false,
			'value_func'        => null, // CSS Custom Properties for duotone are handled by block supports in class-fp-duotone.php.
			'css_vars'          => null,
			'classes'           => array(),
		...
		array(
			...
			'css_vars'          => '--fp--preset--font-size--$slug',
		...
		array(
			...
			'css_vars'          => '--fp--preset--font-family--$slug',
		...
		array(
			...
			'use_default_names' => true,
			'value_key'         => 'size',
			'css_vars'          => '--fp--preset--spacing--$slug',
			'classes'           => array(),
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'shadow',
			'css_vars'          => '--fp--preset--shadow--$slug',
			'classes'           => array(),
	 ...
	 *              `letter-spacing`, `margin-*`, `padding-*`, `--fp--style--block-gap`,
	 ...
	 *              `--fp--style--root--padding-*`, and `box-shadow` properties,
	 *              removed the `--fp--style--block-gap` property.
	...
	const PROPERTIES_METADATA = array(
		...
		'--fp--style--root--padding'        => array( 'spacing', 'padding' ),
		'--fp--style--root--padding-top'    => array( 'spacing', 'padding', 'top' ),
		'--fp--style--root--padding-right'  => array( 'spacing', 'padding', 'right' ),
		'--fp--style--root--padding-bottom' => array( 'spacing', 'padding', 'bottom' ),
		'--fp--style--root--padding-left'   => array( 'spacing', 'padding', 'left' ),
	...
	const ELEMENTS = array(
		'link'    => 'a:where(:not(.fp-element-button))', // The `where` is needed to lower the specificity.
		...
		'h5'      => 'h5',
		'h6'      => 'h6',
		// We have the .fp-block-button__link class so that this will target older buttons that have been serialized.
		'button'  => '.fp-element-button, .fp-block-button__link',
		// The block classes are necessary to target older content that won't use the new class names.
		'caption' => '.fp-element-caption, .fp-block-audio figcaption, .fp-block-embed figcaption, .fp-block-gallery figcaption, .fp-block-image figcaption, .fp-block-table figcaption, .fp-block-video figcaption',
		'cite'    => 'cite',
	);
	...
	public function __construct( $theme_json = array( 'version' => self::LATEST_SCHEMA ), $origin = 'theme' ) {
		...
		$this->theme_json    = FP_Theme_JSON_Schema::migrate( $theme_json, $origin );
	...
	protected static function get_blocks_metadata() {
		...
		$style_registry = FP_Block_Styles_Registry::get_instance();

		// Is there metadata for all currently registered blocks?
		$blocks = array_diff_key( $blocks, static::$blocks_metadata );
		if ( empty( $blocks ) ) {
			/*
			 * New block styles may have been registered within FP_Block_Styles_Registry.
		...
		foreach ( $blocks as $block_name => $block_type ) {
			...
			// Block style variations can be registered through the FP_Block_Styles_Registry as well as block.json.
	...
	public function get_stylesheet( $types = array( 'variables', 'styles', 'presets' ), $origins = null, $options = array() ) {
		...
		if ( in_array( 'styles', $types, true ) ) {
			...
			$columns_selector       = '.fp-block-columns';
			$post_template_selector = '.fp-block-post-template';
			if ( ! empty( $options['scope'] ) ) {
	...
	protected function get_layout_styles( $block_metadata, $types = array() ) {
		...
		if ( isset( $block_metadata['name'] ) ) {
			$block_type = FP_Block_Type_Registry::get_instance()->get_registered( $block_metadata['name'] );
	 ...
	 *
	 *     block-selector {
	 *       --fp--preset--category--slug: value;
	 *       --fp--custom--variable: value;
	 *     }
	 *
	 * @since 5.8.0
	 ...
	 * @return string The CSS Custom Property. Something along the lines of `--fp--preset--color--black`.
	 */
	 ...
	 *
	 * For example, assuming the given prefix is '--fp'
	 ...
	 *
	 *     {
	 *       '--fp--some-property': 'value',
	 *       '--fp--nested-property--sub-property': 'value'
	 *     }
	 *
	 * @since 5.8.0
	 *
	...
	protected static function compute_style_properties( $styles, $settings = array(), $properties = null, $theme_json = null, $selector = null, $use_root_padding = null ) {
		...
		$root_style_length        = strlen( '--fp--style--root--' );
		...
		foreach ( $properties as $css_property => $value_path ) {
			...
			$is_root_style = str_starts_with( $css_property, '--fp--style--root--' );
			 ...
			 */
			if ( '--fp--style--root--padding' === $css_property && is_string( $value ) ) {
				continue;
			}
	 ...
	 *              to the standard form "--fp--preset--color--secondary".
	...
	public function get_styles_for_block( $block_metadata ) {
		...
		if ( ! empty( $block_metadata['variations'] ) ) {
			foreach ( $block_metadata['variations'] as $style_variation ) {
				...
				foreach ( $variation_declarations as $current_selector => $new_declarations ) {
					 ...
					 * - `.fp-block-list:not(.fp-block-list .fp-block-list)`
					 * - `.fp-block-image img, .fp-block-image.my-class img`
					 */
		 ...
		 * set the min-height to '100%'. Minus `--fp-admin--admin-bar--height` for logged-in view.
		...
		if ( $should_set_root_min_height ) {
			$block_rules .= static::to_ruleset(
				...
				array(
					array(
						'name'  => 'min-height',
						'value' => 'calc(100% - var(--fp-admin--admin-bar--height, 0px))',
					),
				)
			);
		}
		...
		if ( '.fp-block-separator' === $selector ) {
	...
	public function get_root_layout_rules( $selector, $block_metadata ) {
		...
		if ( isset( $settings['layout']['contentSize'] ) || isset( $settings['layout']['wideSize'] ) ) {
			...
			$css         .= static::ROOT_CSS_PROPERTIES_SELECTOR . ' { --fp--style--global--content-size: ' . $content_size . ';';
			$css         .= '--fp--style--global--wide-size: ' . $wide_size . '; }';
		}
		...
		if ( $use_root_padding ) {
			...
			$css .= '.fp-site-blocks { padding-top: var(--fp--style--root--padding-top); padding-bottom: var(--fp--style--root--padding-bottom); }';
			// Right and left padding are applied to the first container with `.has-global-padding` class.
			$css .= '.has-global-padding { padding-right: var(--fp--style--root--padding-right); padding-left: var(--fp--style--root--padding-left); }';
			// Alignfull children of the container with left and right padding have negative margins so they can still be full width.
			$css .= '.has-global-padding > .alignfull { margin-right: calc(var(--fp--style--root--padding-right) * -1); margin-left: calc(var(--fp--style--root--padding-left) * -1); }';
			...
			$css .= '.has-global-padding :where(:not(.alignfull.is-layout-flow) > .has-global-padding:not(.fp-block-block, .alignfull)) { padding-right: 0; padding-left: 0; }';
			// Alignfull direct children of the containers that are targeted by the rule above do not need negative margins.
			$css .= '.has-global-padding :where(:not(.alignfull.is-layout-flow) > .has-global-padding:not(.fp-block-block, .alignfull)) > .alignfull { margin-left: 0; margin-right: 0; }';
		}

		$css .= '.fp-site-blocks > .alignleft { float: left; margin-right: 2em; }';
		$css .= '.fp-site-blocks > .alignright { float: right; margin-left: 2em; }';
		$css .= '.fp-site-blocks > .aligncenter { justify-content: center; margin-left: auto; margin-right: auto; }';
		...
		if ( isset( $this->theme_json['settings']['spacing']['blockGap'] ) ) {
			...
			$css            .= ":where(.fp-site-blocks) > * { margin-block-start: $block_gap_value; margin-block-end: 0; }";
			$css            .= ':where(.fp-site-blocks) > :first-child { margin-block-start: 0; }';
			$css            .= ':where(.fp-site-blocks) > :last-child { margin-block-end: 0; }';

			// For backwards compatibility, ensure the legacy block gap CSS variable is still available.
			$css .= static::ROOT_CSS_PROPERTIES_SELECTOR . " { --fp--style--block-gap: $block_gap_value; }";
		}
	 ...
	 *
	 * @param FP_Theme_JSON $incoming Data to merge.
	 */
		...
		foreach ( $setting_nodes as $metadata ) {
			...
			foreach ( $origins as $origin ) {
				...
				foreach ( $duotone_presets[ $origin ] as $duotone_preset ) {
					$filters .= FP_Duotone::get_filter_svg_from_preset( $duotone_preset );
				}
			}
		}
	...
	public static function remove_insecure_properties( $theme_json, $origin = 'theme' ) {
		...
		$theme_json = FP_Theme_JSON_Schema::migrate( $theme_json, $origin );
	 ...
	 * For example, `var:preset|color|vivid-green-cyan` becomes `var(--fp--preset--color--vivid-green-cyan)`.
	 *
	 * @since 6.3.0
	 *
			...
			if ( 0 <= strpos( $style, 'var(' ) ) {
				...
				if ( $has_matches ) {
					...
					foreach ( $var_parts[1] as $index => $var_part ) {
						...
						$rule_to_replace = $var_parts[0][ $index ]; // the css rule to replace e.g. var(--fp--preset--color--vivid-green-cyan).
	 ...
	 * @since 6.3.0
	 *
	 * @param FP_Theme_JSON $theme_json The theme json resolver.
	 * @return FP_Theme_JSON The $theme_json with resolved variables.
	 */
