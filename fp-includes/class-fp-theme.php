...
final class FP_Theme implements ArrayAccess {
	 ...
	 * @param FP_Theme|null $_child If this theme is a parent theme, the child may be passed for validation purposes.
	 */
	public function __construct( $theme_dir, $theme_root, $_child = null ) {
		...
		if ( ! isset( self::$persistently_cache ) ) {
			...
			self::$persistently_cache = apply_filters( 'wp_cache_themes_persistently', false, 'FP_Theme' );
		...
		if ( is_array( $cache ) ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet ) ) {
				...
				$this->errors = new FP_Error( 'theme_no_stylesheet', __( 'Stylesheet is missing.' ) );
			}
			...
			$this->headers['Name']        = $this->stylesheet;
			$this->errors                 = new FP_Error( 'theme_stylesheet_not_readable', __( 'Stylesheet is not readable.' ) );
			$this->template               = $this->stylesheet;
			$this->block_theme            = false;
			$this->block_template_folders = $this->default_template_folders;
			$this->cache_add(
		...
		if ( ! $this->template ) {
			...
			if ( ! $this->is_block_theme() && ! file_exists( $theme_path . '/index.php' ) ) {
				$error_message = sprintf(
					...
					'<code>index.php</code>',
					__( 'https://developer.finpress.org/themes/advanced-topics/child-themes/' ),
					'<code>Template</code>',
					'<code>style.css</code>'
				);
				$this->errors = new FP_Error( 'theme_no_index', $error_message );
				$this->cache_add(
					'theme',
					array(
						'block_template_folders' => $this->get_block_template_folders(),
		...
		if ( ! is_array( $cache )
			...
			if ( '.' !== $parent_dir
				...
				$this->parent = new FP_Theme( $this->template, $this->theme_root, $this );
				return;
			}
		}
		...
		if ( $this->template !== $this->stylesheet ) {
			...
			if ( $_child instanceof FP_Theme && $_child->template === $this->stylesheet ) {
				$_child->parent = null;
				$_child->errors = new FP_Error(
			...
			$this->parent = new FP_Theme( $this->template, isset( $theme_root_template ) ? $theme_root_template : $this->theme_root, $this );
		}

		if ( wp_paused_themes()->get( $this->stylesheet ) && ( ! is_wp_error( $this->errors ) || ! isset( $this->errors->errors['theme_paused'] ) ) ) {
			$this->errors = new FP_Error( 'theme_paused', __( 'This theme failed to load properly and was paused within the admin backend.' ) );
		}
	 ...
	 * @since 3.4.0
	 *
	 * @return FP_Error|false FP_Error if there are errors, or false.
	 */
	 ...
	 * @since 3.4.0
	 *
	 * @return FP_Theme|false Parent theme, or false if the active theme is not a child theme.
	 */
	 ...
	 *                       'ThemeURI', 'AuthorURI', 'Status', 'Tags', 'RequiresFP', 'RequiresPHP',
	 *                       'UpdateURI'.
	 ...
	 * for all other URLs returned by FP_Theme, so we pass it to the public function
	...
	public function get_page_templates( $post = null, $post_type = 'page' ) {
		 ...
		 * @param FP_Theme     $theme          The theme object.
		 * @param FP_Post|null $post           The post being edited, provided for context, or null.
	 ...
	 * @since 4.4.0
	 *
	 * @return FP_Theme|false Object, or false if no theme is installed, which would be bad.
	 */
	 ...
	 * @since 3.4.0
	 *
	 * @param FP_Theme[] $themes Array of theme objects to sort (passed by reference).
	 */
	public static function sort_by_name( &$themes ) {
		if ( str_starts_with( get_user_locale(), 'en_' ) ) {
			uasort( $themes, array( 'FP_Theme', '_name_sort' ) );
		} else {
			foreach ( $themes as $key => $theme ) {
				$theme->translate_header( 'Name', $theme->headers['Name'] );
			}
			uasort( $themes, array( 'FP_Theme', '_name_sort_i18n' ) );
		}
	}
	 ...
	 * @since 3.4.0
	 *
	 * @param FP_Theme $a First theme.
	 * @param FP_Theme $b Second theme.
