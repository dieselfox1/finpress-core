<?php
/**
 * Locale API: FP_Textdomain_Registry class.
 *
...
class FP_Textdomain_Registry {
	 ...
	 * @since 6.5.0
	 *
	 * @param FP_Upgrader $upgrader   Unused. FP_Upgrader instance. In other contexts this might be a
	...
	public function invalidate_mo_files_cache( $upgrader, $hook_extra ) {
		...
		foreach ( $translation_types as $type ) {
			switch ( $type ) {
				case 'plugin':
					wp_cache_delete( md5( FP_LANG_DIR . '/plugins/' ), 'translation_files' );
					break;
				case 'theme':
					wp_cache_delete( md5( FP_LANG_DIR . '/themes/' ), 'translation_files' );
					break;
				default:
					wp_cache_delete( md5( FP_LANG_DIR . '/' ), 'translation_files' );
					break;
			}
		}
	}
