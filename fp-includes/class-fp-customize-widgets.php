...
final class FP_Customize_Widgets {
	 ...
	 * @since 4.2.0
	 *
	 * @param false|array $args       The arguments to the FP_Customize_Setting constructor.
	 ...
	 * @since 3.9.0
	 *
	 * @see FP_Customize_Widgets::handle_theme_switch()
	 *
		...
		foreach ( $sidebars_widgets as $sidebar_id => $sidebar_widget_ids ) {
			...
			if ( $is_registered_sidebar || $is_inactive_widgets ) {
				...
				if ( $is_active_sidebar ) {
					...
					$section = new FP_Customize_Sidebar_Section( $this->manager, $section_id, $section_args );
					$this->manager->add_section( $section );

					if ( $use_widgets_block_editor ) {
						$control = new FP_Sidebar_Block_Editor_Control(
							$this->manager,
							$setting_id,
							array(
								'section'     => $section_id,
						...
						$control = new FP_Widget_Area_Customize_Control(
							$this->manager,
							$setting_id,
							array(
								'section'    => $section_id,
			...
			if ( ! $use_widgets_block_editor ) {
				...
				foreach ( $sidebar_widget_ids as $i => $widget_id ) {
					...
					$control = new FP_Widget_Form_Customize_Control(
						$this->manager,
						$setting_id,
						array(
	 ...
	 * @since 4.4.0
	 *
	 * @see FP_Customize_Panel::$active_callback
	 *
	 * @global array $wp_registered_sidebars
	 * @return bool Active.
	 */
	 ...
	 * @return array|FP_Error Array containing a widget's id_base and number components,
	 *                        or a FP_Error object.
	 */
	public function parse_widget_setting_id( $setting_id ) {
		if ( ! preg_match( '/^(widget_(.+?))(?:\[(\d+)\])?$/', $setting_id, $matches ) ) {
			return new FP_Error( 'widget_setting_invalid_id' );
		}
	...
	public function enqueue_scripts() {
		...
		if ( wp_use_widgets_block_editor() ) {
			$block_editor_context = new FP_Block_Editor_Context(
				array(
	 ...
	 *
	 * @param string $widget_id Widget ID.
	 * @return array|FP_Error Array containing the updated widget information.
	...
	public function call_widget_update( $widget_id ) {
		...
		if ( ! empty( $_POST['sanitized_widget_setting'] ) ) {
			...
			if ( false === $sanitized_widget_setting ) {
				...
				return new FP_Error( 'widget_setting_malformed' );
			}

			$instance = $this->sanitize_widget_instance( $sanitized_widget_setting, $parsed_id['id_base'] );
			if ( is_null( $instance ) ) {
				$this->stop_capturing_option_updates();
				return new FP_Error( 'widget_setting_unsanitized' );
			}
		...
		if ( 0 !== $this->count_captured_options() ) {
			if ( $this->count_captured_options() > 1 ) {
				$this->stop_capturing_option_updates();
				return new FP_Error( 'widget_setting_too_many_options' );
			}

			$updated_option_name = key( $this->get_captured_options() );
			if ( $updated_option_name !== $option_name ) {
				$this->stop_capturing_option_updates();
				return new FP_Error( 'widget_setting_unexpected_option' );
			}
		}
		 ...
		 * in place from FP_Customize_Setting::preview() will use this value
	 ...
	 * }
	 * @see FP_Customize_Nav_Menus::filter_wp_nav_menu_args()
	 *
	 * @return array Params.
	 */
