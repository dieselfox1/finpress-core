 ...
 * @since 6.8.0
 *
 * @param FP_Block_Template $block_template The block template object.
...
function locate_block_template( $template, $type, array $templates ) {
	...
	return ABSPATH . FPINC . '/template-canvas.php';
}
 ...
 * @return FP_Block_Template|null template A template object, or null if none could be found.
 */
...
/**
 * Sets the current FP_Query to return auto-draft posts.
 *
 * The auto-draft status indicates a new post, so allow the the FP_Query instance to
 * return an auto-draft post for template resolution when editing a new post.
 *
 * @access private
 * @since 5.9.0
 *
 * @param FP_Query $wp_query Current FP_Query instance, passed by reference.
 */
 ...
 * }
 * @return FP_Block_Template|FP_Error The registered template object on success, FP_Error object on failure.
 */
function register_block_template( $template_name, $args = array() ) {
	return FP_Block_Templates_Registry::get_instance()->register( $template_name, $args );
}
 ...
 *
 * @param string $template_name Template name in the form of `plugin_uri//template_name`.
 * @return FP_Block_Template|FP_Error The unregistered template object on success, FP_Error object on failure or if the
 *                                    template doesn't exist.
 */
function unregister_block_template( $template_name ) {
	return FP_Block_Templates_Registry::get_instance()->unregister( $template_name );
}
