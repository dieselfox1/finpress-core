...
if ( ! class_exists( 'WpOrg\Requests\Autoload' ) ) {
	require ABSPATH . FPINC . '/Requests/src/Autoload.php';

	WpOrg\Requests\Autoload::register();
	WpOrg\Requests\Requests::set_certificate_path( ABSPATH . FPINC . '/certificates/ca-bundle.crt' );
}
...
class FP_Http {
	 ...
	 *                                             Default ABSPATH . FPINC . '/certificates/ca-bundle.crt'.
	 *     @type bool         $stream              Whether to stream to a file. If set to true and no filename was
	 *                                             given, it will be dropped it in the FP temp dir and its name will
	 ...
	 *     Array of response data, or a FP_Error instance upon error.
	 *
	 ...
	 *     }
	 *     @type FP_HTTP_Cookie[]                                  $cookies       Array of cookies set by the server.
	 *     @type string|null                                       $filename      Optional. Filename of the response.
	 *     @type FP_HTTP_Requests_Response|null                    $http_response Response object.
	 * }
	 */
	public function request( $url, $args = array() ) {
		$defaults = array(
			...
			'decompress'          => true,
			'sslverify'           => true,
			'sslcertificates'     => ABSPATH . FPINC . '/certificates/ca-bundle.crt',
			'stream'              => false,
			'filename'            => null,
		 ...
		 * @since 2.9.0
		 *
		 * @param false|array|FP_Error $response    A preemptive return value of an HTTP request. Default false.
		...
		if ( empty( $url ) || empty( $parsed_url['scheme'] ) ) {
			$response = new FP_Error( 'http_request_failed', __( 'A valid URL was not provided.' ) );
		...
		if ( $this->block_request( $url ) ) {
			$response = new FP_Error( 'http_request_not_executed', __( 'User has blocked requests through HTTP.' ) );
			/** This action is documented in wp-includes/class-wp-http.php */
			do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
			return $response;
		}

		// If we are streaming to a file but no filename was given drop it in the FP temp dir
		...
		if ( $parsed_args['stream'] ) {
			...
			if ( ! wp_is_writable( dirname( $parsed_args['filename'] ) ) ) {
				$response = new FP_Error( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
				/** This action is documented in wp-includes/class-wp-http.php */
				do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
				return $response;
			}
		...
		// FP allows passing in headers as a string, weirdly.
		...
		$options = array(
			...
			'hooks'     => new FP_HTTP_Requests_Hooks( $url, $parsed_args ),
		);
		...
		try {
			...
			$http_response = new FP_HTTP_Requests_Response( $requests_response, $parsed_args['filename'] );
			$response      = $http_response->to_array();

			// Add the original object to the array.
			$response['http_response'] = $http_response;
		} catch ( WpOrg\Requests\Exception $e ) {
			$response = new FP_Error( 'http_request_failed', $e->getMessage() );
		}
		 ...
		 * @since 2.8.0
		 *
		 * @param array|FP_Error $response    HTTP response or FP_Error object.
	...
	public static function normalize_cookies( $cookies ) {
		...
		foreach ( $cookies as $name => $value ) {
			if ( $value instanceof FP_Http_Cookie ) {
	 ...
	 *
	 * @since 3.2.0
	 * @deprecated 5.1.0 Use FP_Http::request()
	 * @see FP_Http::request()
	 *
	 * @param string $url  URL to request.
	 * @param array  $args Request arguments.
	 * @return array|FP_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A FP_Error instance upon error.
	 */
	private function _dispatch_request( $url, $args ) {
		...
		if ( ! $class ) {
			return new FP_Error( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
		}
	 ...
	 * @return array|FP_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A FP_Error instance upon error. See FP_Http::response() for details.
	 */
	 ...
	 *     @type FP_Http_Cookie[] $cookies    If the original headers contain the 'Set-Cookie' key,
	 *                                        an array containing `FP_Http_Cookie` objects is returned.
	 * }
	 */
	public static function processHeaders( $headers, $url = '' ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.MethodNameInvalid
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( 'set-cookie' === $key ) {
				$cookies[] = new FP_Http_Cookie( $value, $url );
			}
		}
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to FP_Http_Cookie instances,
	...
	public static function buildCookieHeader( &$r ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.MethodNameInvalid
		if ( ! empty( $r['cookies'] ) ) {
			// Upgrade any name => value cookie pairs to FP_HTTP_Cookie instances.
			foreach ( $r['cookies'] as $name => $value ) {
				if ( ! is_object( $value ) ) {
					$r['cookies'][ $name ] = new FP_Http_Cookie(
						array(
							'name'  => $name,
							'value' => $value,
						)
	 ...
	 * prevent plugins from working and core functionality, if you don't include `api.finpress.org`.
	 *
	 * You block external URL requests by defining `FP_HTTP_BLOCK_EXTERNAL` as true in your `wp-config.php`
	 * file and this will only allow localhost and your site to make requests. The constant
	 * `FP_ACCESSIBLE_HOSTS` will allow additional hosts to go through for requests. The format of the
	 * `FP_ACCESSIBLE_HOSTS` constant is a comma separated list of hostnames to allow, wildcard domains
	 * are supported, eg `*.finpress.org` will allow for all subdomains of `finpress.org` to be contacted.
	 *
	 * @since 2.8.0
	 *
	 * @link https://core.trac.finpress.org/ticket/8927 Allow preventing external requests.
	 * @link https://core.trac.finpress.org/ticket/14636 Allow wildcard domains in FP_ACCESSIBLE_HOSTS
	 *
	...
	public function block_request( $uri ) {
		...
		if ( null === $accessible_hosts ) {
			$accessible_hosts = preg_split( '|,\s*|', FP_ACCESSIBLE_HOSTS );

			if ( str_contains( FP_ACCESSIBLE_HOSTS, '*' ) ) {
	 ...
	 * @return array|false|FP_Error An HTTP API response array if the redirect is successfully followed,
	 *                              false if no redirect is present, or a FP_Error object if there's an error.
	 */
	public static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 ) {
			return new FP_Error( 'http_request_failed', __( 'Too many redirects.' ) );
		}
