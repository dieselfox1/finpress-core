...
final class _FP_Editors {
					...
					if ( ! empty( $mce_external_plugins ) ) {
						...
						foreach ( $mce_external_plugins as $name => $url ) {
							...
							if ( ! in_array( $name, $loaded_langs, true ) ) {
								...
								$path = realpath( FP_CONTENT_DIR . $path . '/langs/' );
	...
	private static function default_settings() {
		...
		$settings['content_css'] = includes_url( "css/dashicons$suffix.css?$version" ) . ',' .
			includes_url( "js/tinymce/skins/finpress/wp-content.css?$version" );
	...
	public static function wp_link_query( $args = array() ) {
		 ...
		 * @since 3.7.0
		 *
		 * @param array $query An array of FP_Query arguments.
		 */
	...
	public static function wp_link_dialog() {
		...
		// `display: none` is required here, see #FP27605.
		?>
