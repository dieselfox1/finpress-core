 ...
 * @since 6.7.0
 */
final class FP_Block_Templates_Registry {
	/**
	 * Registered templates, as `$name => $instance` pairs.
	 *
	 * @since 6.7.0
	 * @var FP_Block_Template[] $registered_block_templates Registered templates.
	 */
	 ...
	 *
	 * @since 6.7.0
	 * @var FP_Block_Templates_Registry|null
	 */
	 ...
	 * @return FP_Block_Template|FP_Error The registered template on success, or FP_Error on failure.
	 */
	public function register( $template_name, $args = array() ) {
		...
		if ( $error_message ) {
			...
			return new FP_Error( $error_code, $error_message );
		}
	 ...
	 * @since 6.7.0
	 *
	 * @return FP_Block_Template[] Associative array of `$template_name => $template` pairs.
	 */
	 ...
	 *
	 * @param string $template_name Template name including namespace.
	 * @return FP_Block_Template|null The registered template, or null if it is not registered.
	 */
	 ...
	 *
	 * @param string $template_name Template name including namespace.
	 * @return FP_Block_Template|FP_Error The unregistered template on success, or FP_Error on failure.
	 */
	public function unregister( $template_name ) {
		if ( ! $this->is_registered( $template_name ) ) {
			...
			/* translators: %s: Template name. */
			return new FP_Error( 'template_not_registered', __( 'Template "%s" is not registered.' ) );
		}
	 ...
	 * @since 6.7.0
	 *
	 * @return FP_Block_Templates_Registry The main instance.
	 */
