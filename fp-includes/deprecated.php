 ...
 *
 * @link https://developer.finpress.org/themes/basics/the-loop/
 *
 * @since 1.0.1
 * @deprecated 1.5.0
 *
 ...
 * @see get_terms()
 *
 * @link https://developer.finpress.org/reference/functions/get_all_category_ids/
 *
 ...
 * @deprecated 3.3.0
 *
 * @param FP_Admin_Bar $wp_admin_bar FP_Admin_Bar instance.
 */
...
function get_theme_data( $theme_file ) {
	_deprecated_function( __FUNCTION__, '3.4.0', 'wp_get_theme()' );
	$theme = new FP_Theme( wp_basename( dirname( $theme_file ) ), dirname( dirname( $theme_file ) ) );
 ...
 *
 * This is no longer needed as FP_Post lazy-loads the ancestors
 ...
 *
 * @param FP_Post $post Post object, passed by reference (unused).
 */
 ...
 * @return mixed FP_Error on failure. String with new destination path.
 */
 ...
 * @return FP_Post|null Post object or array holding post contents and information
 */
...
function rich_edit_exists() {
	...
	if ( ! isset( $wp_rich_edit_exists ) )
		$wp_rich_edit_exists = file_exists( ABSPATH . FPINC . '/js/tinymce/tinymce.js' );
...
function wp_get_http( $url, $file_path = false, $red = 1 ) {
	_deprecated_function( __FUNCTION__, '4.4.0', 'FP_Http' );
	...
	// FP_HTTP no longer follows redirects for HEAD requests.
...
function wp_ajax_press_this_save_post() {
	...
	if ( is_plugin_active( 'press-this/press-this-plugin.php' ) ) {
		include FP_PLUGIN_DIR . '/press-this/class-wp-press-this-plugin.php';
		$wp_press_this = new FP_Press_This_Plugin();
...
function _get_path_to_translation( $domain, $reset = false ) {
	_deprecated_function( __FUNCTION__, '6.1.0', 'FP_Textdomain_Registry' );
...
function _get_path_to_translation_from_lang_dir( $domain ) {
	...
	$path = FP_LANG_DIR . '/plugins/' . $mofile;
	if ( in_array( $path, $cached_mofiles, true ) ) {
		return $path;
	}

	$path = FP_LANG_DIR . '/themes/' . $mofile;
	if ( in_array( $path, $cached_mofiles, true ) ) {
		return $path;
	}
...
function get_page_by_title( $page_title, $output = OBJECT, $post_type = 'page' ) {
	_deprecated_function( __FUNCTION__, '6.2.0', 'FP_Query' );
	global $wpdb;
...
function wp_get_loading_attr_default( $context ) {
	 ...
	 */
	$header_area = FP_TEMPLATE_PART_AREA_HEADER;
...
function wp_get_duotone_filter_id( $preset ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );
	return FP_Duotone::get_filter_id_from_preset( $preset );
}
...
function wp_get_duotone_filter_property( $preset ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );
	return FP_Duotone::get_filter_css_property_value_from_preset( $preset );
}
 ...
 *
 * @since 5.9.1
 * @deprecated 6.3.0 Use FP_Duotone::get_filter_svg_from_preset() instead.
 *
 * @access private
 *
 * @param array $preset Duotone preset value as seen in theme.json.
...
function wp_get_duotone_filter_svg( $preset ) {
	_deprecated_function( __FUNCTION__, '6.3.0', 'FP_Duotone::get_filter_svg_from_preset()' );
	return FP_Duotone::get_filter_svg_from_preset( $preset );
}
 ...
 *
 * @since 5.8.0
 * @deprecated 6.3.0 Use FP_Duotone::register_duotone_support() instead.
 *
 * @access private
 *
 * @param FP_Block_Type $block_type Block Type.
...
function wp_register_duotone_support( $block_type ) {
	_deprecated_function( __FUNCTION__, '6.3.0', 'FP_Duotone::register_duotone_support()' );
	return FP_Duotone::register_duotone_support( $block_type );
}
 ...
 * @since 5.8.0
 * @since 6.1.0 Allow unset for preset colors.
 * @deprecated 6.3.0 Use FP_Duotone::render_duotone_support() instead.
 *
 * @access private
 *
...
function wp_render_duotone_support( $block_content, $block ) {
	_deprecated_function( __FUNCTION__, '6.3.0', 'FP_Duotone::render_duotone_support()' );
	$wp_block = new FP_Block( $block );
	return FP_Duotone::render_duotone_support( $block_content, $block, $wp_block );
}
...
function wp_get_global_styles_svg_filters() {
	...
	$tree = FP_Theme_JSON_Resolver::get_merged_data();
 ...
 *
 * Using `FP_Theme_JSON_Resolver`, it gets the fonts defined
...
function _wp_theme_json_webfonts_handler() {
	...
	$fn_get_webfonts_from_theme_json = static function() {
		...
		$settings = FP_Theme_JSON_Resolver::get_merged_data()->get_settings();

		// If in the editor, add webfonts defined in variations.
		if ( is_admin() || wp_is_rest_endpoint() ) {
			$variations = FP_Theme_JSON_Resolver::get_style_variations();
...
function print_embed_styles() {
	...
	<style<?php echo $type_attr; ?>>
		<?php echo file_get_contents( ABSPATH . FPINC . "/css/wp-embed-template$suffix.css" ); ?>
	</style>
	<?php
}
...
function wp_update_https_detection_errors() {
	 ...
	 *
	 * Returning a `FP_Error` from the filter will effectively short-circuit the default logic of trying a remote
	 * request to the site over HTTPS, storing the errors array from the returned `FP_Error` instead.
	 *
	 * @since 5.7.0
	 ...
	 *
	 * @param null|FP_Error $pre Error object to short-circuit detection,
 ...
 *
 * @since 6.2.0
 * @deprecated 6.7.0 Use {@see 'wp_get_global_stylesheet'} instead for top-level custom CSS, or {@see 'FP_Theme_JSON::get_styles_for_block'} for block-level custom CSS.
 *
...
function wp_get_global_styles_custom_css() {
	 ...
	 * @see https://make.finpress.org/core/2022/10/10/filters-for-theme-json-data/
	 *
...
function wp_add_editor_classic_theme_styles( $editor_settings ) {
	...
	$classic_theme_styles = ABSPATH . FPINC . "/css/classic-themes$suffix.css";
