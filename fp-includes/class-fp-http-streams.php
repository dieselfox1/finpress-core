<?php
/**
 * HTTP API: FP_Http_Streams class
 *
 * @package WordPress
 * @subpackage HTTP
 * @since 4.4.0
 ...
 * @deprecated 6.4.0 Use FP_Http
 * @see FP_Http
 */
#[AllowDynamicProperties]
class FP_Http_Streams {
	/**
	 * Send a HTTP request to a URI using PHP Streams.
	 *
	 * @see FP_Http::request() For default options descriptions.
	 *
	 * @since 2.7.0
	 * @since 3.7.0 Combined with the fsockopen transport and switched to stream_socket_client().
	 *
	 ...
	 * @return array|FP_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'. A FP_Error instance upon error
	 */
	public function request( $url, $args = array() ) {
		...
		FP_Http::buildCookieHeader( $parsed_args );
		...
		if ( $is_local ) {
			...
			/** This filter is documented in fp-includes/class-fp-http.php */
		...
		if ( false === $handle ) {
			...
			if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str ) {
				return new FP_Error( 'http_request_failed', __( 'The SSL certificate for the host could not be verified.' ) );
			}

			return new FP_Error( 'http_request_failed', $connection_error . ': ' . $connection_error_str );
		}
		...
		if ( $parsed_args['stream'] ) {
			...
			while ( ! feof( $handle ) && $keep_reading ) {
				...
				if ( ! $body_started ) {
					$response .= $block;
					if ( strpos( $response, "\r\n\r\n" ) ) {
						$processed_response = FP_Http::processResponse( $response );
						$body_started       = true;
				...
				if ( $bytes_written_to_file !== $this_block_size ) {
					...
					return new FP_Error( 'http_request_failed', __( 'Failed to write request to temporary file.' ) );
				}
		...
		$processed_headers = FP_Http::processHeaders( $processed_response['headers'], $url );
		...
		// Handle redirects.
		$redirect_response = FP_Http::handle_redirects( $url, $parsed_args, $response );
		...
		if ( ! empty( $processed_response['body'] )
			...
			$processed_response['body'] = FP_Http::chunkTransferDecode( $processed_response['body'] );
		}

		if ( true === $parsed_args['decompress']
			&& true === FP_Http_Encoding::should_decode( $processed_headers['headers'] )
		) {
			$processed_response['body'] = FP_Http_Encoding::decompress( $processed_response['body'] );
		}
	...
	public static function verify_ssl_certificate( $stream, $host ) {
		 ...
		 */
		$host_type = ( FP_Http::is_ip_address( $host ) ? 'ip' : 'dns' );
 ...
 * All code should make use of FP_Http directly through its API.
 *
 * @see FP_HTTP::request
 *
 * @since 2.7.0
 * @deprecated 3.7.0 Please use FP_HTTP::request() directly
 */
class FP_HTTP_Fsockopen extends FP_Http_Streams {
