...
/**
 * Taxonomy API: FP_Term_Query class.
 *
 * @package WordPress
 * @subpackage Taxonomy
 * @since 4.6.0
 ...
 * @since 4.6.0
 *
 * @see FP_Term_Query::__construct() for accepted arguments.
 */
...
class FP_Term_Query {
	 ...
	 *                                                   - 'all' Returns an array of complete term objects (`FP_Term[]`).
	 *                                                   - 'all_with_object_id' Returns an array of term objects
	 *                                                     with the 'object_id' param (`FP_Term[]`). Works only
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query FP_Term_Query arguments. See FP_Term_Query::__construct() for accepted arguments.
	 */
	public function parse_query( $query = '' ) {
		 ...
		 * @since 4.6.0
		 *
		 * @param FP_Term_Query $query Current instance of FP_Term_Query.
		 */
	 ...
	 * FP_Term_Query::get_terms() for details.
	 *
	 * @since 4.6.0
	 *
	 * @param string|array $query Array or URL query string of parameters.
	 * @return FP_Term[]|int[]|string[]|string Array of terms, or number of terms as numeric string
	 ...
	 *
	 * The following will result in an array of `FP_Term` objects being returned:
	 *
	 *   - 'all'
	 *   - 'all_with_object_id'
	 *
	...
	public function get_terms() {
		 ...
		 * @since 4.6.0
		 *
		 * @param FP_Term_Query $query Current instance of FP_Term_Query (passed by reference).
		 */
		 ...
		 * @since 5.3.0
		 *
		 * @param array|null    $terms Return an array of term data to short-circuit FP's term query,
		 *                             or null to allow FP queries to run normally.
		 * @param FP_Term_Query $query The FP_Term_Query instance, passed by reference.
		 */
				...
				if ( ! $term->count ) {
					...
					if ( is_array( $children ) ) {
						foreach ( $children as $child_id ) {
							...
							if ( $child instanceof FP_Term && $child->count ) {
								continue 2;
							}
						}
					}
	 ...
	 * @since 6.0.0
	 *
	 * @param FP_Term[] $term_objects Array of term objects.
	 * @param string    $_fields      Field to format.
	 *
	 * @return FP_Term[]|int[]|string[] Array of terms / strings / ints depending on field requested.
	 */
	 ...
	 * @since 4.6.0
	 *
	 * @param string $orderby_raw Raw 'orderby' value passed to FP_Term_Query.
	 ...
	 * @return FP_Term[] Array of `FP_Term` objects.
	 */
	 ...
	 * @global wpdb $wpdb WordPress database abstraction object.
	 *
	 * @param array  $args FP_Term_Query arguments.
