 ...
 * @return string|FP_Error A list of category parents on success, FP_Error on failure.
 */
 ...
 * @return FP_Term[] Array of FP_Term objects, one for each category assigned to the post.
 */
function get_the_category( $post_id = false ) {
	 ...
	 *
	 * @param FP_Term[] $categories An array of categories to return for the post.
 ...
 * @return string|FP_Error Category name on success, FP_Error on failure.
 */
...
function get_the_category_list( $separator = '', $parents = '', $post_id = false ) {
	 ...
	 * @since 4.4.0
	 *
	 * @param FP_Term[] $categories An array of the post's categories.
 ...
 *                                            to check against.
 * @param int|null|FP_Post          $post     Optional. Post to check. Defaults to the current post.
 ...
 *     Optional. Array or string of arguments to generate a categories drop-down element. See FP_Term_Query::__construct()
...
function wp_dropdown_categories( $args = '' ) {
	...
	if ( empty( $categories ) && ! $parsed_args['hide_if_empty'] && ! empty( $parsed_args['show_option_none'] ) ) {
		 ...
		 * @param FP_Term|null $category The category object, or null if there's no corresponding category.
		 */
 ...
 *
 * @param array|string $args {
 *     Array of optional arguments. See get_categories(), get_terms(), and FP_Term_Query::__construct()
 *     for information on additional accepted arguments.
 *
 ...
 * @since 4.8.0 Added the `show_count` argument.
 *
 * @param FP_Term[]    $tags Array of FP_Term objects to generate the tag cloud for.
...
function wp_generate_tag_cloud( $tags, $args = '' ) {
	 ...
	 * @since 2.8.0
	 *
	 * @param FP_Term[] $tags Ordered array of terms.
	...
	if ( $args['filter'] ) {
		 ...
		 *                                equals 'array'.
		 * @param FP_Term[]       $tags   An array of terms used in the tag cloud.
 ...
 * @return FP_Term[]|false|FP_Error Array of FP_Term objects on success, false if there are no terms
 *                                  or the post does not exist, FP_Error on failure.
 */
function get_the_tags( $post = 0 ) {
	 ...
	 * @see get_the_terms()
	 *
	 * @param FP_Term[]|false|FP_Error $terms Array of FP_Term objects on success, false if there are no terms
	 *                                        or the post does not exist, FP_Error on failure.
	 */
 ...
 * @return string|false|FP_Error A list of tags on success, false if there are no terms,
 *                               FP_Error on failure.
 */
...
function get_the_terms( $post, $taxonomy ) {
	 ...
	 * @since 3.1.0
	 *
	 * @param FP_Term[]|FP_Error $terms    Array of attached terms, or FP_Error on failure.
	 * @param int                $post_id  Post ID.
 ...
 * @return string|false|FP_Error A list of terms on success, false if there are no terms,
 *                               FP_Error on failure.
 */
 ...
 * }
 * @return string|FP_Error A list of term parents on success, FP_Error or empty string on failure.
 */
 ...
 * @param int|FP_Post      $post     Optional. Post to check. Defaults to the current post.
