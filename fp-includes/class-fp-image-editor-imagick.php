 ...
 * @see FP_Image_Editor
 */
class FP_Image_Editor_Imagick extends FP_Image_Editor {
	/**
	 * Imagick object.
	 *
	 * @var Imagick
	 ...
	 * @since 3.5.0
	 *
	 * @return true|FP_Error True if loaded; FP_Error on failure.
	 */
	public function load() {
		...
		if ( ! is_file( $this->file ) && ! wp_is_stream( $this->file ) ) {
			return new FP_Error( 'error_loading_image', __( 'File does not exist?' ), $this->file );
		}
		...
		try {
			...
			if ( ! $this->image->valid() ) {
				return new FP_Error( 'invalid_image', __( 'File is not an image.' ), $this->file );
			}
			...
			return new FP_Error( 'invalid_image', $e->getMessage(), $this->file );
		}
	 ...
	 * @return true|FP_Error True if set successfully; FP_Error on failure.
	 */
	public function set_quality( $quality = null, $dims = array() ) {
		...
		try {
			...
			return new FP_Error( 'image_quality_error', $e->getMessage() );
		}
		return true;
	}
	...
	protected function update_size( $width = null, $height = null ) {
		...
		if ( ! $width || ! $height ) {
			try {
				...
				return new FP_Error( 'invalid_image', __( 'Could not read image size.' ), $this->file );
			}
		}
	 ...
	 * see https://core.trac.finpress.org/ticket/58202.
	 *
	 * Note:
	...
	public function resize( $max_w, $max_h, $crop = false ) {
		...
		if ( ! $dims ) {
			return new FP_Error( 'error_getting_dimensions', __( 'Could not calculate resized image dimensions' ) );
		}
	...
	protected function thumbnail_image( $dst_w, $dst_h, $filter_name = 'FILTER_TRIANGLE', $strip_meta = true ) {
		...
		try {
			...
			return new FP_Error( 'image_resize_error', $e->getMessage() );
		}
	}
	 ...
	 * }
	 * @return array|FP_Error The image data array for inclusion in the `sizes` array in the image meta,
	 *                        FP_Error object on error.
	 */
	public function make_subsize( $size_data ) {
		if ( ! isset( $size_data['width'] ) && ! isset( $size_data['height'] ) ) {
			return new FP_Error( 'image_subsize_create_error', __( 'Cannot resize the image. Both width and height are not set.' ) );
		}
		...
		if ( ( $this->size['width'] === $size_data['width'] ) && ( $this->size['height'] === $size_data['height'] ) ) {
			return new FP_Error( 'image_subsize_create_error', __( 'The image already has the requested size.' ) );
		}
	...
	public function crop( $src_x, $src_y, $src_w, $src_h, $dst_w = null, $dst_h = null, $src_abs = false ) {
		...
		try {
			...
			return new FP_Error( 'image_crop_error', $e->getMessage() );
		}
	...
	public function rotate( $angle ) {
		...
		try {
			...
			return new FP_Error( 'image_rotate_error', $e->getMessage() );
		}
	...
	public function flip( $horz, $vert ) {
		try {
			...
			return new FP_Error( 'image_flip_error', $e->getMessage() );
		}
	 ...
	 * @since 5.3.0
	 *
	 * @return bool|FP_Error True if the image was rotated. False if no EXIF data or if the image doesn't need rotation.
	 *                       FP_Error if error while rotating.
	 */
	public function maybe_exif_rotate() {
		if ( is_callable( array( $this->image, 'setImageOrientation' ) ) && defined( 'Imagick::ORIENTATION_TOPLEFT' ) ) {
			return parent::maybe_exif_rotate();
		} else {
			return new FP_Error( 'write_exif_error', __( 'The image cannot be rotated because the embedded meta data cannot be updated.' ) );
		}
	}
	 ...
	 *     Array on success or FP_Error if the file failed to save.
	 *
	...
	public function save( $destfilename = null, $mime_type = null ) {
		...
		if ( ! is_wp_error( $saved ) ) {
			...
			try {
				...
				return new FP_Error( 'image_save_error', $e->getMessage(), $this->file );
			}
		}
	...
	protected function remove_pdf_alpha_channel() {
		...
		if ( $version['versionNumber'] >= 0x675 ) {
			try {
				...
				return new FP_Error( 'pdf_alpha_process_failed', $e->getMessage() );
			}
		}
	}
	...
	protected function _save( $image, $filename = null, $mime_type = null ) {
		...
		try {
			...
			return new FP_Error( 'image_save_error', $e->getMessage(), $filename );
		}
	 ...
	 * @return true|FP_Error True on success, FP_Error object on failure.
	 */
	public function stream( $mime_type = null ) {
		...
		try {
			...
			return new FP_Error( 'image_stream_error', $e->getMessage() );
		}
	 ...
	 * @since 4.5.0
	 *
	 * @return true|FP_Error True if stripping metadata was successful. FP_Error object on error.
	 */
	protected function strip_meta() {
		...
		try {
			...
			return new FP_Error( 'image_strip_meta_error', $e->getMessage() );
		}
	 ...
	 * @since 4.7.0
	 *
	 * @return string|FP_Error File to load or FP_Error on failure.
	 */
	protected function pdf_setup() {
		try {
			...
			return new FP_Error( 'pdf_setup_failed', $e->getMessage(), $this->file );
		}
	}
