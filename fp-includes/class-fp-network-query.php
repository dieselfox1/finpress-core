<?php
/**
 * Network API: FP_Network_Query class
 *
 * @package WordPress
 ...
 * @since 4.6.0
 *
 * @see FP_Network_Query::__construct() for accepted arguments.
 */
#[AllowDynamicProperties]
class FP_Network_Query {
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query FP_Network_Query arguments. See FP_Network_Query::__construct() for accepted arguments.
	 */
	public function parse_query( $query = '' ) {
		 ...
		 * @since 4.6.0
		 *
		 * @param FP_Network_Query $query The FP_Network_Query instance (passed by reference).
		 */
	 ...
	 * @return array|int List of FP_Network objects, a list of network IDs when 'fields' is set to 'ids',
	...
	public function get_networks() {
		 ...
		 * @since 4.6.0
		 *
		 * @param FP_Network_Query $query Current instance of FP_Network_Query (passed by reference).
		 */
		 ...
		 * - Otherwise the filter should return an array of FP_Network objects.
		 *
		 * Note that if the filter returns an array of network data, it will be assigned
		 * to the `networks` property of the current FP_Network_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_networks` and `max_num_pages` properties of the FP_Network_Query object,
		 * passed to the filter by reference. If FP_Network_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 5.2.0
		 * @since 5.6.0 The returned array of network data is assigned to the `networks` property
		 *              of the current FP_Network_Query instance.
		 *
		 * @param array|int|null   $network_data Return an array of network data to short-circuit FP's network query,
		 ...
		 * @since 4.6.0
		 *
		 * @param FP_Network[]     $_networks An array of FP_Network objects.
		 * @param FP_Network_Query $query     Current instance of FP_Network_Query (passed by reference).
		 */
		$_networks = apply_filters_ref_array( 'the_networks', array( $_networks, &$this ) );

		// Convert to FP_Network instances.
	...
	private function set_found_networks() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param FP_Network_Query $network_query        The `FP_Network_Query` instance.
			 */
