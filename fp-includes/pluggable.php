...
if ( ! function_exists( 'wp_set_current_user' ) ) :
	 ...
	 * @since 2.0.3
	 *
	 * @global FP_User $current_user The current user object which holds the user data.
	 *
	 * @param int|null $id   User ID.
	 * @param string   $name User's username.
	 * @return FP_User Current user User object.
	 */
	function wp_set_current_user( $id, $name = '' ) {
		...
		&& ( $current_user instanceof FP_User )
		&& ( $id === $current_user->ID )
		&& ( null !== $id )
		) {
			return $current_user;
		...
		$current_user = new FP_User( $id, $name );
...
if ( ! function_exists( 'get_user_by' ) ) :
	...
	function get_user_by( $field, $value ) {
		$userdata = FP_User::get_data_by( $field, $value );
...
if ( ! function_exists( 'wp_mail' ) ) :
	...
	function wp_mail( $to, $subject, $message, $headers = '', $attachments = array(), $embeds = array() ) {
		...
		if ( ! ( $phpmailer instanceof PHPMailer\PHPMailer\PHPMailer ) ) {
			require_once ABSPATH . FPINC . '/PHPMailer/PHPMailer.php';
			require_once ABSPATH . FPINC . '/PHPMailer/SMTP.php';
			require_once ABSPATH . FPINC . '/PHPMailer/Exception.php';
			require_once ABSPATH . FPINC . '/class-wp-phpmailer.php';
			$phpmailer = new FP_PHPMailer( true );
		...
		try {
			...
			do_action( 'wp_mail_failed', new FP_Error( 'wp_mail_failed', $e->getMessage(), $mail_error_data ) );
		...
		try {
			 ...
			 * @since 4.4.0
			 *
			 * @param FP_Error $error A FP_Error object with the PHPMailer\PHPMailer\Exception message, and an array
			 *                        containing the mail recipient, subject, message, headers, and attachments.
			 */
			do_action( 'wp_mail_failed', new FP_Error( 'wp_mail_failed', $e->getMessage(), $mail_data ) );
...
if ( ! function_exists( 'wp_authenticate' ) ) :
	 ...
	 * @return FP_User|FP_Error FP_User object if the credentials are valid,
	 *                          otherwise FP_Error.
	 */
	function wp_authenticate(
		 ...
		 *
		 * A FP_User object is returned if the credentials authenticate a user.
		 * FP_Error or null otherwise.
		 *
		 * @since 2.8.0
		 * @since 4.5.0 `$username` now accepts an email address.
		 *
		 * @param null|FP_User|FP_Error $user     FP_User if the user is authenticated.
		 *                                        FP_Error or null otherwise.
		...
		if ( null === $user || false === $user ) {
			 ...
			 */
			$user = new FP_Error( 'authentication_failed', __( '<strong>Error:</strong> Invalid username, email address or incorrect password.' ) );
		}
		...
		if ( is_wp_error( $user ) && ! in_array( $user->get_error_code(), $ignore_codes, true ) ) {
			 ...
			 *
			 * @param string   $username Username or email address.
			 * @param FP_Error $error    A FP_Error object with the authentication failure details.
			 */
...
if ( ! function_exists( 'wp_generate_auth_cookie' ) ) :
	...
	function wp_generate_auth_cookie( $user_id, $expiration, $scheme = 'auth', $token = '' ) {
		...
		if ( ! $token ) {
			$manager = FP_Session_Tokens::get_instance( $user_id );
...
if ( ! function_exists( 'wp_notify_postauthor' ) ) :
	...
	function wp_notify_postauthor( $comment_id, $deprecated = null ) {
		...
		if ( FP_Http::is_ip_address( $comment->comment_author_IP ) ) {
...
if ( ! function_exists( 'wp_password_change_notification' ) ) :
	...
	function wp_password_change_notification( $user ) {
		...
		if ( 0 !== strcasecmp( $user->user_email, get_option( 'admin_email' ) ) ) {
			 ...
			 * }
			 * @param FP_User $user     User object for user whose password was changed.
...
if ( ! function_exists( 'wp_new_user_notification' ) ) :
	...
	function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
		 ...
		 * @param FP_User $user User object for new user.
		 */
...
if ( ! function_exists( 'wp_verify_nonce' ) ) :
	...
	function wp_verify_nonce( $nonce, $action = -1 ) {
		 ...
		 * @param FP_User    $user   The current user object.
...
if ( ! function_exists( 'wp_rand' ) ) :
	...
	function wp_rand( $min = null, $max = null ) {
		...
		if ( strlen( $rnd_value ) < 8 ) {
			...
			if ( ! defined( 'FP_SETUP_CONFIG' ) && ! defined( 'FP_INSTALLING' ) ) {
...
if ( ! function_exists( 'wp_text_diff' ) ) :
	 ...
	 * @uses Text_Diff
	 * @uses FP_Text_Diff_Renderer_Table
	 *
	 * @param string       $left_string  "old" (left) version of string.
	 * @param string       $right_string "new" (right) version of string.
	 * @param string|array $args {
	 *     Associative array of options to pass to FP_Text_Diff_Renderer_Table().
	 *
	...
	function wp_text_diff( $left_string, $right_string, $args = null ) {
		...
		if ( ! class_exists( 'FP_Text_Diff_Renderer_Table', false ) ) {
			require ABSPATH . FPINC . '/wp-diff.php';
		}
		...
		$renderer    = new FP_Text_Diff_Renderer_Table( $args );
