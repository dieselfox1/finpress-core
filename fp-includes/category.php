 ...
 * @return FP_Term|array|FP_Error|null Category data in type defined by $output parameter.
 *                                     Returns a FP_Term object with backwards compatible property aliases filled in.
 *                                     FP_Error if $category is empty, null if it does not exist.
 */
 ...
 *
 * It is also possible that it will return a FP_Error object on failure. Check
 ...
 *                              correspond to a FP_Term object, an associative array, or a numeric array,
 *                              respectively. Default OBJECT.
 * @return FP_Term|array|FP_Error|null Type is based on $output value.
 */
 ...
 * }
 * @return FP_Term[]|int|FP_Error Array of 'post_tag' term objects, a count thereof,
...
function get_tags( $args = '' ) {
	...
	if ( empty( $tags ) ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param FP_Term[]|int|FP_Error $tags Array of 'post_tag' term objects, a count thereof,
		 *                                     or FP_Error if any of the taxonomies do not exist.
 ...
 * @since 2.3.0
 *
 * @param int|FP_Term|object $tag    A tag ID or object.
 * @param string             $output Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                                   correspond to a FP_Term object, an associative array, or a numeric array,
 *                                   respectively. Default OBJECT.
 * @param string             $filter Optional. How to sanitize tag fields. Default 'raw'.
 * @return FP_Term|array|FP_Error|null Tag data in type defined by $output parameter.
 *                                     FP_Error if $tag is empty, null if it does not exist.
 */
 ...
 *
 * @since 2.3.0
 * @since 4.4.0 The `$category` parameter now also accepts a FP_Term object.
 * @access private
 *
 * @param array|object|FP_Term $category Category row object or array.
 */
