...
if ( ! defined( 'FP_TEMPLATE_PART_AREA_HEADER' ) ) {
	define( 'FP_TEMPLATE_PART_AREA_HEADER', 'header' );
}
if ( ! defined( 'FP_TEMPLATE_PART_AREA_FOOTER' ) ) {
	define( 'FP_TEMPLATE_PART_AREA_FOOTER', 'footer' );
}
if ( ! defined( 'FP_TEMPLATE_PART_AREA_SIDEBAR' ) ) {
	define( 'FP_TEMPLATE_PART_AREA_SIDEBAR', 'sidebar' );
}
if ( ! defined( 'FP_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) {
	define( 'FP_TEMPLATE_PART_AREA_UNCATEGORIZED', 'uncategorized' );
}
...
function get_allowed_block_template_part_areas() {
	$default_area_definitions = array(
		...
		array(
			'area'        => FP_TEMPLATE_PART_AREA_HEADER,
		...
		array(
			'area'        => FP_TEMPLATE_PART_AREA_FOOTER,
...
function _filter_block_template_part_area( $type ) {
	...
	$warning_message = sprintf(
		...
		$type,
		FP_TEMPLATE_PART_AREA_UNCATEGORIZED
	);
	wp_trigger_error( __FUNCTION__, $warning_message );
	return FP_TEMPLATE_PART_AREA_UNCATEGORIZED;
}
...
function _add_block_template_part_area_info( $template_info ) {
	...
	if ( isset( $theme_data[ $template_info['slug'] ]['area'] ) ) {
		...
		$template_info['area'] = FP_TEMPLATE_PART_AREA_UNCATEGORIZED;
	}
 ...
 * @param string $template_type Template type. Either 'wp_template' or 'wp_template_part'.
 * @return FP_Block_Template Template.
 */
function _build_block_template_result_from_file( $template_file, $template_type ) {
	$default_template_types = get_default_block_template_types();
	$theme                  = get_stylesheet();

	$template                 = new FP_Block_Template();
	...
	if ( 'wp_template' === $template_type ) {
		$registered_template = FP_Block_Templates_Registry::get_instance()->get_by_slug( $template_file['slug'] );
 ...
 * @param FP_Block_Template $template  Template to mutate adding the description and title computed.
 * @return bool Returns true if the referenced post was found and false otherwise.
 */
function _wp_build_title_and_description_for_single_post_type_block_template( $post_type, $slug, FP_Block_Template $template ) {
	...
	$posts_with_same_title_query = new FP_Query( $args );
 ...
 */
function _wp_build_title_and_description_for_taxonomy_block_template( $taxonomy, $slug, FP_Block_Template $template ) {
 ...
 * @access private
 *
 * @param FP_Post $post  Template post.
 ...
 * @return FP_Block_Template|FP_Error Template or error object.
 */
function _build_block_template_object_from_post_object( $post, $terms = array(), $meta = array() ) {
	if ( empty( $terms['wp_theme'] ) ) {
		return new FP_Error( 'template_missing_theme', __( 'No theme is defined for this template.' ) );
	}
...
function _build_block_template_result_from_post( $post ) {
	...
	if ( 'wp_template' === $post->post_type ) {
		$registered_template = FP_Block_Templates_Registry::get_instance()->get_by_slug( $template->slug );
		if ( $registered_template ) {
			$template->plugin      = $registered_template->plugin;
			$template->origin      =
 ...
 * }
 * @param string $template_type Template type. Either 'wp_template' or 'wp_template_part'.
 * @return FP_Block_Template[] Array of block templates.
 */
function get_block_templates( $query = array(), $template_type = 'wp_template' ) {
	 ...
	 * @since 5.9.0
	 *
	 * @param FP_Block_Template[]|null $block_templates Return an array of block templates to short-circuit the default query,
	 *                                                  or null to allow FP to run its normal queries.
	 * @param array  $query {
	...
	$template_query = new FP_Query( $wp_query_args );
	$query_result   = array();
	...
	if ( ! isset( $query['wp_id'] ) ) {
		...
		if ( 'wp_template' === $template_type ) {
			...
			$registered_templates          = FP_Block_Templates_Registry::get_instance()->get_by_query( $query );
	 ...
	 * @since 5.9.0
	 *
	 * @param FP_Block_Template[] $query_result Array of found block templates.
	 * @param array               $query {
	 *     Arguments to retrieve templates. All arguments are optional.
	 *
	 *     @type string[] $slug__in  List of slugs to include.
 ...
 *                              Default 'wp_template'.
 * @return FP_Block_Template|null Template.
 */
function get_block_template( $id, $template_type = 'wp_template' ) {
	 ...
	 * @since 5.9.0
	 *
	 * @param FP_Block_Template|null $block_template Return block template object to short-circuit the default query,
	 *                                               or null to allow FP to run its normal queries.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'wp_template' or 'wp_template_part'.
	 */
	 ...
	 * @since 5.9.0
	 *
	 * @param FP_Block_Template|null $block_template The found block template, or null if there isn't one.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'wp_template' or 'wp_template_part'.
	 */
 ...
 * @param string $template_type Optional. Template type. Either 'wp_template' or 'wp_template_part'.
 *                              Default 'wp_template'.
 * @return FP_Block_Template|null The found block template, or null if there isn't one.
 */
function get_block_file_template( $id, $template_type = 'wp_template' ) {
	...
	$block_template = FP_Block_Templates_Registry::get_instance()->get_by_slug( $slug );
	 ...
	 * @since 5.9.0
	 *
	 * @param FP_Block_Template|null $block_template The found block template, or null if there is none.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'wp_template' or 'wp_template_part'.
	 */
	return apply_filters( 'get_block_file_template', $block_template, $id, $template_type );
...
function wp_generate_block_templates_export_file() {
	...
	if ( ! class_exists( 'ZipArchive' ) ) {
		return new FP_Error( 'missing_zip_package', __( 'Zip Export not supported.' ) );
	}
	...
	if ( true !== $zip->open( $filename, ZipArchive::CREATE | ZipArchive::OVERWRITE ) ) {
		return new FP_Error( 'unable_to_create_zip', __( 'Unable to open export file (archive) for writing.' ) );
	}
	...
	$tree = FP_Theme_JSON_Resolver::get_theme_data( array(), array( 'with_supports' => false ) );
	// Merge with user data.
	$tree->merge( FP_Theme_JSON_Resolver::get_user_data() );
 ...
 * @param FP_REST_Request $deprecated Deprecated. Not used.
 * @return stdClass|FP_Error The updated object representing a template or template part.
 */
function inject_ignored_hooked_blocks_metadata_attributes( $changes, $deprecated = null ) {
	...
	// Required for the FP_Block_Template. Update the post object with the current time.
	...
	$template = _build_block_template_object_from_post_object( new FP_Post( $post ), $terms, $meta );
