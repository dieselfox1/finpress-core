<?php
/**
 * Feed API: FP_SimplePie_File class
 *
 * @package WordPress
 * @subpackage Feed
 * @since 4.7.0
...
class FP_SimplePie_File extends SimplePie\File {
	...
	public function __construct( $url, $timeout = 10, $redirects = 5, $headers = null, $useragent = null, $force_fsockopen = false ) {
		...
		if ( preg_match( '/^http(s)?:\/\//i', $url ) ) {
			...
			if ( SimplePie\Misc::get_default_useragent() !== $this->useragent ) { // Use default FP user agent unless custom has been specified.
			...
			if ( is_wp_error( $res ) ) {
				$this->error   = 'FP HTTP Error: ' . $res->get_error_message();
				$this->success = false;
