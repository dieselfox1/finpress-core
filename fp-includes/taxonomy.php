...
function create_initial_taxonomies() {
	...
	FP_Taxonomy::reset_default_labels();
	...
	register_taxonomy(
		...
		array(
			...
			'rest_controller_class' => 'FP_REST_Terms_Controller',
		)
	);
	...
	register_taxonomy(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'menus',
			'rest_controller_class' => 'FP_REST_Menus_Controller',
		)
	);
 ...
 * @since 3.0.0
 *
 * @global FP_Taxonomy[] $wp_taxonomies The registered taxonomies.
 *
 ...
 *                         Default 'and'.
 * @return string[]|FP_Taxonomy[] An array of taxonomy names or objects.
 */
 ...
 * @global FP_Taxonomy[] $wp_taxonomies The registered taxonomies.
 *
 * @param string|string[]|FP_Post $object_type Name of the type of taxonomy object, or an object (row from posts).
 * @param string                  $output      Optional. The type of output to return in the array. Accepts either
 *                                             'names' or 'objects'. Default 'names'.
 * @return string[]|FP_Taxonomy[] The names or objects of all taxonomies of `$object_type`.
 */
 ...
 * @return FP_Taxonomy|false The taxonomy object or false if $taxonomy doesn't exist.
 */
 ...
 *
 * @global FP_Taxonomy[] $wp_taxonomies Registered taxonomies.
 *
 ...
 *     @type string        $rest_controller_class REST API Controller class name. Default is 'FP_REST_Terms_Controller'.
 ...
 *         @type bool   $with_front   Should the permastruct be prepended with FP_Rewrite::$front. Default true.
 ...
 *                                                Default false.
 * }
 * @return FP_Taxonomy|FP_Error The registered taxonomy object on success, FP_Error object on failure.
 */
function register_taxonomy( $taxonomy, $object_type, $args = array() ) {
	...
	if ( empty( $taxonomy ) || strlen( $taxonomy ) > 32 ) {
		...
		return new FP_Error( 'taxonomy_length_invalid', __( 'Taxonomy names must be between 1 and 32 characters in length.' ) );
	}

	$taxonomy_object = new FP_Taxonomy( $taxonomy, $object_type, $args );
 ...
 * @since 4.5.0
 *
 * @global FP_Taxonomy[] $wp_taxonomies List of taxonomies.
 *
 * @param string $taxonomy Taxonomy name.
 * @return true|FP_Error True on success, FP_Error on failure or if the taxonomy doesn't exist.
 */
function unregister_taxonomy( $taxonomy ) {
	global $wp_taxonomies;

	if ( ! taxonomy_exists( $taxonomy ) ) {
		return new FP_Error( 'invalid_taxonomy', __( 'Invalid taxonomy.' ) );
	}
	...
	if ( $taxonomy_object->_builtin ) {
		return new FP_Error( 'invalid_taxonomy', __( 'Unregistering a built-in taxonomy is not allowed.' ) );
	}
 ...
 *
 * @param FP_Taxonomy $tax Taxonomy object.
 * @return object {
...
function get_taxonomy_labels( $tax ) {
	...
	$nohier_vs_hier_defaults = FP_Taxonomy::get_default_labels();
 ...
 * On failure of finding a valid taxonomy, it will return a FP_Error.
 *
 ...
 * }
 * @return string[]|FP_Error An array of object IDs as numeric strings on success,
 *                           FP_Error if the taxonomy does not exist.
 */
...
function get_tax_sql( $tax_query, $primary_table, $primary_id_column ) {
	$tax_query_obj = new FP_Tax_Query( $tax_query );
 ...
 *
 * @since 2.3.0
 * @since 4.4.0 Converted to return a FP_Term object if `$output` is `OBJECT`.
 *              The `$taxonomy` parameter was made optional.
 *
 * @see sanitize_term_field() The $context param lists the available values for get_term_by() $filter param.
 *
 * @param int|FP_Term|object $term     If integer, term data will be fetched from the database,
 *                                     or from the cache if available.
 *                                     If stdClass object (as in the results of a database query),
 *                                     will apply filters and return a `FP_Term` object with the `$term` data.
 *                                     If `FP_Term`, will return `$term`.
 * @param string             $taxonomy Optional. Taxonomy name that `$term` is part of.
 * @param string             $output   Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                                     correspond to a FP_Term object, an associative array, or a numeric array,
 *                                     respectively. Default OBJECT.
 * @param string             $filter   Optional. How to sanitize term fields. Default 'raw'.
 * @return FP_Term|array|FP_Error|null FP_Term instance (or array) on success, depending on the `$output` value.
 *                                     FP_Error if `$taxonomy` does not exist. Null for miscellaneous failure.
 */
function get_term( $term, $taxonomy = '', $output = OBJECT, $filter = 'raw' ) {
	if ( empty( $term ) ) {
		return new FP_Error( 'invalid_term', __( 'Empty Term.' ) );
	}
	...
	if ( $term instanceof FP_Term ) {
		$_term = $term;
	} elseif ( is_object( $term ) ) {
		if ( empty( $term->filter ) || 'raw' === $term->filter ) {
			$_term = sanitize_term( $term, $taxonomy, 'raw' );
			$_term = new FP_Term( $_term );
		} else {
			$_term = FP_Term::get_instance( $term->term_id );
		}
	} else {
		$_term = FP_Term::get_instance( $term, $taxonomy );
	}
	 ...
	 *
	 * @since 2.3.0
	 * @since 4.4.0 `$_term` is now a `FP_Term` object.
	 *
	 * @param FP_Term $_term    Term object.
 ...
 *              a FP_Term object if `$output` is `OBJECT`.
 ...
 *                             respectively. Default OBJECT.
 * @param string     $filter   Optional. How to sanitize term fields. Default 'raw'.
 * @return FP_Term|array|false FP_Term instance (or array) on success, depending on the `$output` value.
 ...
 * @param string $taxonomy Taxonomy name.
 * @return array|FP_Error List of term IDs. FP_Error returned if `$taxonomy` does not exist.
 */
 ...
 *
 * @since 2.3.0
 * @since 4.4.0 The `$taxonomy` parameter was made optional. `$term` can also now accept a FP_Term object.
 *
 * @see sanitize_term_field()
 *
 * @param string      $field    Term field to fetch.
 * @param int|FP_Term $term     Term ID or object.
 * @param string      $taxonomy Optional. Taxonomy name. Default empty.
 * @param string      $context  Optional. How to sanitize term fields. Look at sanitize_term_field() for available options.
 *                              Default 'display'.
 * @return string|int|null|FP_Error Will return an empty string if $term is not an object or if $field is not set in $term.
 */
 ...
 * @param string     $taxonomy Taxonomy name.
 * @return string|int|null|FP_Error Will return empty string if $term is not an object.
 */
 ...
 * FP_Term_Query::get_terms() for details. In all cases, a `FP_Error` object will
 ...
 *              a list of FP_Term objects.
 * @since 4.5.0 Changed the function signature so that the `$args` array can be provided as the first parameter.
 *              Introduced 'meta_key' and 'meta_value' parameters. Introduced the ability to order results by metadata.
 * @since 4.8.0 Introduced 'suppress_filter' parameter.
 *
 * @param array|string $args       Optional. Array or string of arguments. See FP_Term_Query::__construct()
 *                                 for information on accepted arguments. Default empty array.
 * @param array|string $deprecated Optional. Argument array, when using the legacy function parameter format.
 *                                 If present, this parameter will be interpreted as `$args`, and the first
 *                                 function parameter will be parsed as a taxonomy or array of taxonomies.
 *                                 Default empty.
 * @return FP_Term[]|int[]|string[]|string|FP_Error Array of terms, a count thereof as a numeric string,
 *                                                  or FP_Error if any of the taxonomies do not exist.
 *                                                  See the function description for more information.
 */
function get_terms( $args = array(), $deprecated = '' ) {
	$term_query = new FP_Term_Query();
	...
	// Don't pass suppress_filter to FP_Term_Query.
	 ...
	 * @param FP_Term_Query $term_query The FP_Term_Query object.
	 */
 ...
 *                           Default false.
 * @return int|false|FP_Error Meta ID on success, false on failure.
 *                            FP_Error when term_id is ambiguous between taxonomies.
 */
function add_term_meta( $term_id, $meta_key, $meta_value, $unique = false ) {
	if ( wp_term_is_shared( $term_id ) ) {
		return new FP_Error( 'ambiguous_term_id', __( 'Term meta cannot be added to terms that are shared between taxonomies.' ), $term_id );
	}
 ...
 * @return int|bool|FP_Error Meta ID if the key didn't exist. true on successful update,
 ...
 *                                 function parameter will be parsed as a taxonomy or array of taxonomies.
 *                                 Default empty.
 * @return string|FP_Error Numeric string containing the number of terms in that
 *                         taxonomy or FP_Error if the taxonomy does not exist.
 */
 ...
 *                               Default false.
 * }
 * @return bool|int|FP_Error True on success, false if term does not exist. Zero on attempted
 *                           deletion of default Category. FP_Error if the taxonomy does not exist.
 */
function wp_delete_term( $term, $taxonomy, $args = array() ) {
	 ...
	 * @param int     $tt_id        Term taxonomy ID.
	 * @param string  $taxonomy     Taxonomy slug.
	 * @param FP_Term $deleted_term Copy of the already-deleted term.
 ...
 * @return bool|int|FP_Error Returns true if completes delete action; false if term doesn't exist;
 *                           Zero on attempted deletion of default Category; FP_Error object is
 *                           also a possibility.
 */
 ...
 *              'all_with_object_id', an array of `FP_Term` objects will be returned.
 * @since 4.7.0 Refactored to use FP_Term_Query, and to support any FP_Term_Query arguments.
 ...
 * @param array|string    $args       See FP_Term_Query::__construct() for supported arguments.
 * @return FP_Term[]|int[]|string[]|string|FP_Error Array of terms, a count thereof as a numeric string,
 *                                                  or FP_Error if any of the taxonomies do not exist.
 *                                                  See FP_Term_Query::get_terms() for more information.
 */
function wp_get_object_terms( $object_ids, $taxonomies, $args = array() ) {
	 ...
	 * @since 4.2.0
	 *
	 * @param FP_Term[]|int[]|string[]|string $terms      Array of terms or a count thereof as a numeric string.
 ...
 * a FP_Error object will be returned.
 *
 * If the term already exists on the same hierarchical level,
 * or the term slug and name are not unique, a FP_Error object will be returned.
 *
 * @global wpdb $wpdb WordPress database abstraction object.
 *
 * @since 2.3.0
 ...
 * }
 * @return array|FP_Error {
 *     An array of the new term data, FP_Error otherwise.
 *
...
function wp_insert_term( $term, $taxonomy, $args = array() ) {
	 ...
	 *
	 * @param string|FP_Error $term     The term name to add, or a FP_Error object if there's an error.
	 * @param string          $taxonomy Taxonomy slug.
	...
	if ( is_int( $term ) && 0 === $term ) {
		return new FP_Error( 'invalid_term_id', __( 'Invalid term ID.' ) );
	}

	if ( '' === trim( $term ) ) {
		return new FP_Error( 'empty_term_name', __( 'A name is required for this term.' ) );
	}
	...
	if ( (int) $args['parent'] > 0 && ! term_exists( (int) $args['parent'] ) ) {
		return new FP_Error( 'missing_parent', __( 'Parent term does not exist.' ) );
	}
	...
	if ( '' === $name ) {
		return new FP_Error( 'invalid_term_name', __( 'Invalid term name.' ) );
	}
	...
	if ( $name_match ) {
		...
		if ( ! $slug_provided || $name_match->slug === $slug || $slug_match ) {
			if ( is_taxonomy_hierarchical( $taxonomy ) ) {
				...
				if ( $existing_term ) {
					return new FP_Error( 'term_exists', __( 'A term with the name provided already exists with this parent.' ), $existing_term->term_id );
				}
			} else {
				return new FP_Error( 'term_exists', __( 'A term with the name provided already exists in this taxonomy.' ), $name_match->term_id );
			}
		}
	}
	...
	if ( false === $wpdb->insert( $wpdb->terms, $data ) ) {
		return new FP_Error( 'db_insert_error', __( 'Could not insert term into the database.' ), $wpdb->last_error );
	}
	...
	if ( false === $wpdb->insert( $wpdb->term_taxonomy, compact( 'term_id', 'taxonomy', 'description', 'parent' ) + array( 'count' => 0 ) ) ) {
		return new FP_Error( 'db_insert_error', __( 'Could not insert term taxonomy into the database.' ), $wpdb->last_error );
	}
 ...
 * @return array|FP_Error Term taxonomy IDs of the affected terms or FP_Error on failure.
 */
	...
	if ( ! $append && isset( $t->sort ) && $t->sort ) {
		...
		if ( $values ) {
			if ( false === $wpdb->query( "INSERT INTO $wpdb->term_relationships (object_id, term_taxonomy_id, term_order) VALUES " . implode( ',', $values ) . ' ON DUPLICATE KEY UPDATE term_order = VALUES(term_order)' ) ) {
				return new FP_Error( 'db_insert_error', __( 'Could not insert term relationship into the database.' ), $wpdb->last_error );
			}
		}
	}
 ...
 * @return array|FP_Error Term taxonomy IDs of the affected terms.
 */
 ...
 * @param string           $taxonomy  Taxonomy name.
 * @return bool|FP_Error True on success, false or FP_Error on failure.
 */
 ...
 * If you set 'slug' and it isn't unique, then a FP_Error is returned.
 ...
 *     @type string $slug        The term slug to use. Default empty string.
 * }
 * @return array|FP_Error An array containing the `term_id` and `term_taxonomy_id`,
 *                        FP_Error otherwise.
 */
function wp_update_term( $term_id, $taxonomy, $args = array() ) {
	...
	if ( $duplicate && $duplicate->term_id !== $term_id ) {
		...
		if ( $empty_slug || ( $parent !== (int) $term['parent'] ) ) {
			...
			return new FP_Error( 'duplicate_term_slug', sprintf( __( 'The slug &#8220;%s&#8221; is already in use by another term.' ), $slug ) );
		}
	}
 ...
 *
 * @since 2.3.0
 * @since 4.7.0 Returns a `FP_Error` object if there's an error with
 *              any of the matched terms.
 *
 * @param int    $id       Term object ID, for example a post, comment, or user ID.
 * @param string $taxonomy Taxonomy name.
 * @return bool|FP_Term[]|FP_Error Array of `FP_Term` objects, if cached.
 *                                 False if cache is empty for `$taxonomy` and `$id`.
 *                                 FP_Error if get_term() returns an error object for any term.
 */
 ...
 * @since 2.3.0
 *
 * @param FP_Term[] $terms    Array of term objects to change.
 ...
 * @return array|FP_Error The subset of $terms that are descendants of $term_id.
 */
 ...
 * @global wpdb $wpdb WordPress database abstraction object.
 *
 * @param object[]|FP_Term[] $terms    List of term objects (passed by reference).
 ...
 * @param FP_Taxonomy $taxonomy Current taxonomy object of terms.
 */
function _update_post_term_count( $terms, $taxonomy ) {
	 ...
	 * @param FP_Taxonomy $taxonomy      Current taxonomy object.
	 */
 ...
 *                                     Default: true.
 * @return int|FP_Error When the current term does not need to be split (or cannot be split on the current
 *                      database schema), `$term_id` is returned. When the term is successfully split, the
 *                      new term_id is returned. A FP_Error is returned for miscellaneous errors.
 */
function _split_shared_term( $term_id, $term_taxonomy_id, $record = true ) {
	...
	if ( false === $wpdb->insert( $wpdb->terms, $new_term_data ) ) {
		return new FP_Error( 'db_insert_error', __( 'Could not split shared term.' ), $wpdb->last_error );
	}
 ...
 * @global FP_Rewrite $wp_rewrite WordPress rewrite component.
 *
 * @param FP_Term|int|string $term     The term object, ID, or slug whose link will be retrieved.
 * @param string             $taxonomy Optional. Taxonomy. Default empty.
 * @return string|FP_Error URL of the taxonomy term archive on success, FP_Error if term does not exist.
 */
function get_term_link( $term, $taxonomy = '' ) {
	...
	if ( ! is_object( $term ) ) {
		$term = new FP_Error( 'invalid_term', __( 'Empty Term.' ) );
	}
	 ...
	 * @param FP_Term $term     The term object.
	 */
	 ...
	 * @param FP_Term $term     Term object.
	 * @param string  $taxonomy Taxonomy slug.
	 */
 ...
 *
 *     @type int|FP_Post $post   Post ID or object to get taxonomies of. Default current post.
 ...
 * @return bool|FP_Error FP_Error on input error.
 */
function is_object_in_term( $object_id, $taxonomy, $terms = null ) {
	$object_id = (int) $object_id;
	if ( ! $object_id ) {
		return new FP_Error( 'invalid_object', __( 'Invalid object ID.' ) );
	}
 ...
 * @since 5.1.0
 *
 * @param string|FP_Taxonomy $taxonomy Taxonomy name or object.
 ...
 * @since 6.1.0
 *
 * @param int|FP_Term $term Term ID or term object.
