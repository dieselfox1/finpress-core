<?php
/**
 * HTML API: FP_HTML_Processor class
 *
 * @package WordPress
 * @subpackage HTML-API
 * @since 6.4.0
 ...
 *
 * While the `FP_HTML_Tag_Processor` is a valuable tool for modifying
 ...
 * Example:
 *
 *     $processor = FP_HTML_Processor::create_fragment( $html );
 ...
 * if one is currently selected. Call FP_HTML_Processor::get_breadcrumbs()
 ...
 * @since 6.4.0
 *
 * @see FP_HTML_Tag_Processor
 * @see https://html.spec.whatwg.org/
 */
class FP_HTML_Processor extends FP_HTML_Tag_Processor {
	/**
	 ...
	 * internal bookmarks and are automatically released when the referring FP_HTML_Token
	 * goes out of scope and is garbage-collected.
	 *
	 * @since 6.4.0
	 *
	 * @see FP_HTML_Processor::$release_internal_bookmark_on_destruct
	 *
	 * @var int
	 */
	 ...
	 * @since 6.7.0
	 *
	 * @var FP_HTML_Unsupported_Exception|null
	 */
	private $unsupported_exception = null;

	/**
	 * Releases a bookmark when PHP garbage-collects its wrapping FP_HTML_Token instance.
	 *
	 ...
	 * @since 6.4.0
	 *
	 * @see FP_HTML_Processor::create_fragment()
	 *
	...
	public function __construct( $html, $use_the_static_create_methods_instead = null ) {
		...
		if ( self::CONSTRUCTOR_UNLOCK_CODE !== $use_the_static_create_methods_instead ) {
			_doing_it_wrong(
				__METHOD__,
				sprintf(
					/* translators: %s: FP_HTML_Processor::create_fragment(). */
					__( 'Call %s to create an HTML Processor instead of calling the constructor directly.' ),
					'<code>FP_HTML_Processor::create_fragment()</code>'
				),
				'6.4.0'
			);
		}

		$this->state = new FP_HTML_Processor_State();

		$this->state->stack_of_open_elements->set_push_handler(
			function ( FP_HTML_Token $token ): void {
				...
				$this->element_queue[] = new FP_HTML_Stack_Event( $token, FP_HTML_Stack_Event::PUSH, $provenance );
		...
		$this->state->stack_of_open_elements->set_pop_handler(
			function ( FP_HTML_Token $token ): void {
				...
				$same_node             = isset( $this->state->current_token ) && $token->node_name === $this->state->current_token->node_name;
				$provenance            = ( ! $same_node || $is_virtual ) ? 'virtual' : 'real';
				$this->element_queue[] = new FP_HTML_Stack_Event( $token, FP_HTML_Stack_Event::POP, $provenance );
		 ...
		 * a private method into FP_HTML_Token classes without
	 ...
	 * instantiated with a `BODY` context node via `FP_HTML_Processor::create_fragment( $html )`.
	 *
	...
	private function create_fragment_at_current_node( string $html ) {
		...
		$fragment_processor->bookmarks['root-node'] = new FP_HTML_Span( 0, 0 );
		$root_node                                  = new FP_HTML_Token(
			'root-node',
			'HTML',
			false
		);
		...
		$fragment_processor->bookmarks['context-node']   = new FP_HTML_Span( 0, 0 );
		...
		if ( 'TEMPLATE' === $fragment_processor->context_node->node_name ) {
			$fragment_processor->state->stack_of_template_insertion_modes[] = FP_HTML_Processor_State::INSERTION_MODE_IN_TEMPLATE;
		}
	 ...
	 *
	 * @throws FP_HTML_Unsupported_Exception Halts execution of the parser.
	 *
	 * @since 6.7.0
	 *
	...
	private function bail( string $message ) {
		...
		$this->unsupported_exception = new FP_HTML_Unsupported_Exception(
			$message,
	 ...
	 * Example
	 *
	 *     $processor = FP_HTML_Processor::create_fragment( '<template><strong><button><em><p><em>' );
	 *     false === $processor->next_tag();
	 *     FP_HTML_Processor::ERROR_UNSUPPORTED === $processor->get_last_error();
	 *
	 * @since 6.4.0
	 *
	 ...
	 *
	 * @return FP_HTML_Unsupported_Exception|null
	 */
	 ...
	 * implemented as {@see FP_HTML_Processor::next_token()}, which instead calls
	 * this method similarly to how {@see FP_HTML_Tag_Processor::next_token()}
	 * calls the {@see FP_HTML_Tag_Processor::base_class_next_token()} method.
	 *
	...
	private function next_visitable_token(): bool {
		...
		$is_pop = FP_HTML_Stack_Event::POP === $this->current_element->operation;
	 ...
	 * Example:
	 *
	 *     $p = FP_HTML_Processor::create_fragment( '<div></div>' );
	...
	public function is_tag_closer(): bool {
		return $this->is_virtual()
			? ( FP_HTML_Stack_Event::POP === $this->current_element->operation && '#tag' === $this->get_token_type() )
	 ...
	 * Example:
	 *
	 *     $processor = FP_HTML_Processor::create_fragment( '<div><span><figure><img></figure></span></div>' );
	 ...
	 * @since 6.6.0
	 *
	 * @param FP_HTML_Token|null $node Optional. Node to examine, if provided.
	 *                                 Default is to examine current node.
	 * @return bool|null Whether to expect a closer for the currently-matched node,
	 *                   or `null` if not matched on any token.
	 */
	public function expects_closer( ?FP_HTML_Token $node = null ): ?bool {
	...
	public function step( $node_to_process = self::PROCESS_NEXT_NODE ): bool {
		...
		if ( self::PROCESS_NEXT_NODE === $node_to_process ) {
			parent::next_token();
			if ( FP_HTML_Tag_Processor::STATE_TEXT_NODE === $this->parser_state ) {
		...
		if (
			FP_HTML_Tag_Processor::STATE_INCOMPLETE_INPUT === $this->parser_state ||
			FP_HTML_Tag_Processor::STATE_COMPLETE === $this->parser_state
		) {
			return false;
		}
		...
		$is_start_tag          = FP_HTML_Tag_Processor::STATE_MATCHED_TAG === $this->parser_state && ! $is_closer;
		$token_name            = $this->get_token_name();

		if ( self::REPROCESS_CURRENT_NODE !== $node_to_process ) {
			$this->state->current_token = new FP_HTML_Token(
				$this->bookmark_token(),
				$token_name,
		...
		try {
			...
			switch ( $this->state->insertion_mode ) {
				case FP_HTML_Processor_State::INSERTION_MODE_INITIAL:
					return $this->step_initial();

				case FP_HTML_Processor_State::INSERTION_MODE_BEFORE_HTML:
					return $this->step_before_html();

				case FP_HTML_Processor_State::INSERTION_MODE_BEFORE_HEAD:
					return $this->step_before_head();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_HEAD:
					return $this->step_in_head();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_HEAD_NOSCRIPT:
					return $this->step_in_head_noscript();

				case FP_HTML_Processor_State::INSERTION_MODE_AFTER_HEAD:
					return $this->step_after_head();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_BODY:
					return $this->step_in_body();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_TABLE:
					return $this->step_in_table();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_TABLE_TEXT:
					return $this->step_in_table_text();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_CAPTION:
					return $this->step_in_caption();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_COLUMN_GROUP:
					return $this->step_in_column_group();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_TABLE_BODY:
					return $this->step_in_table_body();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_ROW:
					return $this->step_in_row();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_CELL:
					return $this->step_in_cell();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_SELECT:
					return $this->step_in_select();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_SELECT_IN_TABLE:
					return $this->step_in_select_in_table();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_TEMPLATE:
					return $this->step_in_template();

				case FP_HTML_Processor_State::INSERTION_MODE_AFTER_BODY:
					return $this->step_after_body();

				case FP_HTML_Processor_State::INSERTION_MODE_IN_FRAMESET:
					return $this->step_in_frameset();

				case FP_HTML_Processor_State::INSERTION_MODE_AFTER_FRAMESET:
					return $this->step_after_frameset();

				case FP_HTML_Processor_State::INSERTION_MODE_AFTER_AFTER_BODY:
					return $this->step_after_after_body();

				case FP_HTML_Processor_State::INSERTION_MODE_AFTER_AFTER_FRAMESET:
		...
		} catch ( FP_HTML_Unsupported_Exception $e ) {
			/*
	 ...
	 * Example:
	 *
	 *     $processor = FP_HTML_Processor::create_fragment( '<p><strong><em><img></em></strong></p>' );
	 *     $processor->next_tag( 'IMG' );
	 ...
	 * Example:
	 *
	 *     $processor = FP_HTML_Processor::create_fragment( '<div><p></p></div>' );
	 ...
	 * a new processor using {@see FP_HTML_Processor::create_fragment} or
	 * {@see FP_HTML_Processor::create_full_parser} and call {@see FP_HTML_Processor::serialize}
	 ...
	 * Example:
	 *
	 *     echo FP_HTML_Processor::normalize( '<a href=#anchor v=5 href="/" enabled>One</a another v=5><!--' );
	 *     // <a href="#anchor" v="5" enabled>One</a>
	 *
	 *     echo FP_HTML_Processor::normalize( '<div></p>fun<table><td>cell</div>' );
	 *     // <div><p></p>fun<table><tbody><tr><td>cell</td></tr></tbody></table></div>
	 *
	 *     echo FP_HTML_Processor::normalize( '<![CDATA[invalid comment]]> syntax < <> "oddities"' );
	 ...
	 *
	 * This differs from {@see FP_HTML_Processor::normalize} in that it starts with
	 ...
	 * Example:
	 *
	 *     $processor = FP_HTML_Processor::create_fragment( '<a href=#anchor v=5 href="/" enabled>One</a another v=5><!--' );
	 *     echo $processor->serialize();
	 *     // <a href="#anchor" v="5" enabled>One</a>
	 *
	 *     $processor = FP_HTML_Processor::create_fragment( '<div></p>fun<table><td>cell</div>' );
	 *     echo $processor->serialize();
	 *     // <div><p></p>fun<table><tbody><tr><td>cell</td></tr></tbody></table></div>
	 *
	 *     $processor = FP_HTML_Processor::create_fragment( '<![CDATA[invalid comment]]> syntax < <> "oddities"' );
	 *     echo $processor->serialize();
	 *     // <!--[CDATA[invalid comment]]--> syntax &lt; &lt;&gt; &quot;oddities&quot;
	 *
	 * @since 6.7.0
	...
	public function serialize(): ?string {
		if ( FP_HTML_Tag_Processor::STATE_READY !== $this->parser_state ) {
			wp_trigger_error(
				__METHOD__,
	 ...
	 * logic for the generalized FP_HTML_Processor::step() function.
	 *
	 * @since 6.7.0
	 *
	 * @throws FP_HTML_Unsupported_Exception When encountering unsupported HTML input.
	 *
	 * @see https://html.spec.whatwg.org/#the-initial-insertion-mode
	 * @see FP_HTML_Processor::step
	 *
		...
		switch ( $op ) {
			...
			case 'html':
				...
				if ( null !== $doctype && 'quirks' === $doctype->indicated_compatibility_mode ) {
					$this->compat_mode = FP_HTML_Tag_Processor::QUIRKS_MODE;
				}

				/*
				 * > Then, switch the insertion mode to "before html".
				 */
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_BEFORE_HTML;
				$this->insert_html_element( $this->state->current_token );
				return true;
		}
	...
	private function step_before_html(): bool {
		...
		switch ( $op ) {
			...
			case '+HTML':
				$this->insert_html_element( $this->state->current_token );
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_BEFORE_HEAD;
				return true;
	...
	private function step_before_head(): bool {
		...
		switch ( $op ) {
			...
			case '+HEAD':
				...
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_HEAD;
				return true;
	...
	private function step_in_head(): bool {
		...
		switch ( $op ) {
			...
			case '+NOSCRIPT':
				$this->insert_html_element( $this->state->current_token );
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_HEAD_NOSCRIPT;
				return true;
			...
			case '-HEAD':
				$this->state->stack_of_open_elements->pop();
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_AFTER_HEAD;
				return true;
			...
			case '+TEMPLATE':
				...
				$this->state->insertion_mode                      = FP_HTML_Processor_State::INSERTION_MODE_IN_TEMPLATE;
				$this->state->stack_of_template_insertion_modes[] = FP_HTML_Processor_State::INSERTION_MODE_IN_TEMPLATE;
	...
	private function step_after_head(): bool {
		...
		switch ( $op ) {
			...
			case '+BODY':
				$this->insert_html_element( $this->state->current_token );
				$this->state->frameset_ok    = false;
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_BODY;
				return true;
			...
			case '+FRAMESET':
				$this->insert_html_element( $this->state->current_token );
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_FRAMESET;
				return true;
			...
			case '+TITLE':
				 ...
				 * FPCS from getting confused at the switch structure without a return,
	...
	private function step_in_body(): bool {
		...
		switch ( $op ) {
			...
			case '-BODY':
				...
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_AFTER_BODY;
				/*
			...
			case '+TABLE':
				 ...
				 */
				if (
					FP_HTML_Tag_Processor::QUIRKS_MODE !== $this->compat_mode &&
				...
				$this->insert_html_element( $this->state->current_token );
				$this->state->frameset_ok    = false;
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_TABLE;
				return true;
			...
			case '+SELECT':
				...
				switch ( $this->state->insertion_mode ) {
					...
					case FP_HTML_Processor_State::INSERTION_MODE_IN_ROW:
					case FP_HTML_Processor_State::INSERTION_MODE_IN_CELL:
						$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_SELECT_IN_TABLE;
						break;
					 ...
					 */
					default:
						$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_SELECT;
						break;
				}
				return true;
	...
	private function step_in_table(): bool {
		...
		switch ( $op ) {
			...
			case '+CAPTION':
				...
				$this->state->active_formatting_elements->insert_marker();
				$this->insert_html_element( $this->state->current_token );
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_CAPTION;
				return true;
			...
			case '+COLGROUP':
				$this->state->stack_of_open_elements->clear_to_table_context();
				$this->insert_html_element( $this->state->current_token );
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_COLUMN_GROUP;
				return true;
			...
			case '+THEAD':
				$this->state->stack_of_open_elements->clear_to_table_context();
				$this->insert_html_element( $this->state->current_token );
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_TABLE_BODY;
				return true;
	...
	private function step_in_table_text(): bool {
		$this->bail( 'No support for parsing in the ' . FP_HTML_Processor_State::INSERTION_MODE_IN_TABLE_TEXT . ' state.' );
	}
	...
	private function step_in_table_body(): bool {
		...
		switch ( $op ) {
			...
			case '+TR':
				...
				$this->insert_html_element( $this->state->current_token );
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_ROW;
				return true;
	...
	private function step_in_row(): bool {
		...
		switch ( $op ) {
			...
			case '+TD':
				...
				$this->insert_html_element( $this->state->current_token );
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_IN_CELL;
				$this->state->active_formatting_elements->insert_marker();
				return true;
	...
	private function step_in_template(): bool {
		...
		switch ( $op ) {
			...
			case '+THEAD':
				array_pop( $this->state->stack_of_template_insertion_modes );
				$this->state->stack_of_template_insertion_modes[] = FP_HTML_Processor_State::INSERTION_MODE_IN_TABLE;
				$this->state->insertion_mode                      = FP_HTML_Processor_State::INSERTION_MODE_IN_TABLE;
				return $this->step( self::REPROCESS_CURRENT_NODE );
			...
			case '+COL':
				array_pop( $this->state->stack_of_template_insertion_modes );
				$this->state->stack_of_template_insertion_modes[] = FP_HTML_Processor_State::INSERTION_MODE_IN_COLUMN_GROUP;
				$this->state->insertion_mode                      = FP_HTML_Processor_State::INSERTION_MODE_IN_COLUMN_GROUP;
				return $this->step( self::REPROCESS_CURRENT_NODE );
			...
			case '+TR':
				array_pop( $this->state->stack_of_template_insertion_modes );
				$this->state->stack_of_template_insertion_modes[] = FP_HTML_Processor_State::INSERTION_MODE_IN_TABLE_BODY;
				$this->state->insertion_mode                      = FP_HTML_Processor_State::INSERTION_MODE_IN_TABLE_BODY;
				return $this->step( self::REPROCESS_CURRENT_NODE );
			...
			case '+TH':
				array_pop( $this->state->stack_of_template_insertion_modes );
				$this->state->stack_of_template_insertion_modes[] = FP_HTML_Processor_State::INSERTION_MODE_IN_ROW;
				$this->state->insertion_mode                      = FP_HTML_Processor_State::INSERTION_MODE_IN_ROW;
				return $this->step( self::REPROCESS_CURRENT_NODE );
		}
		...
		if ( ! $is_closer ) {
			array_pop( $this->state->stack_of_template_insertion_modes );
			$this->state->stack_of_template_insertion_modes[] = FP_HTML_Processor_State::INSERTION_MODE_IN_BODY;
			$this->state->insertion_mode                      = FP_HTML_Processor_State::INSERTION_MODE_IN_BODY;
			return $this->step( self::REPROCESS_CURRENT_NODE );
		}
	...
	private function step_after_body(): bool {
		...
		switch ( $op ) {
			...
			case '-HTML':
				...
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_AFTER_AFTER_BODY;
				/*
		...
		switch ( $op ) {
			...
			case '-FRAMESET':
				...
				if ( ! isset( $this->context_node ) && ! $this->state->stack_of_open_elements->current_node_is( 'FRAMESET' ) ) {
					$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_AFTER_FRAMESET;
				}
	...
	private function step_after_frameset(): bool {
		...
		switch ( $op ) {
			 ...
			 */
			case '-HTML':
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_AFTER_AFTER_FRAMESET;
				/*
				 * The HTML element is not removed from the stack of open elements.
				 * Only internal state has changed, this does not qualify as a "step"
				 * in terms of advancing through the document to another token.
	 ...
	 * Example:
	 *
	 *     $processor = new FP_HTML_Tag_Processor( '<div class="test">Test</div>' );
	 ...
	 * Example:
	 *
	 *     $p = FP_HTML_Processor::create_fragment( '<div enabled class="test" data-test-id="14">Test</div>' );
	 *     $p->next_token() === true;
	 ...
	 * Example:
	 *
	 *     $p = new FP_HTML_Tag_Processor( '<div data-ENABLED class="test" DATA-test-id="14">Test</div>' );
	 ...
	 * Example:
	 *
	 *     $p = FP_HTML_Processor::create_fragment( "<div class='free &lt;egg&lt;\tlang-en'>" );
	 *     $p->next_tag();
	...
	public function seek( $bookmark_name ): bool {
		...
		if ( 'backward' === $direction ) {
			...
			if ( null === $this->context_node ) {
				$this->change_parsing_namespace( 'html' );
				$this->state->insertion_mode = FP_HTML_Processor_State::INSERTION_MODE_INITIAL;
				$this->breadcrumbs           = array();

				$this->bookmarks['initial'] = new FP_HTML_Span( 0, 0 );
				parent::seek( 'initial' );
	 ...
	 *
	 *     $p = new FP_HTML_Tag_Processor( $html );
	 *     $in_list = false;
	 ...
	 * @since 6.7.0 Full spec support.
	 *
	 * @see FP_HTML_Processor::generate_implied_end_tags
	 * @see https://html.spec.whatwg.org/#generate-implied-end-tags
	 */
	 ...
	 * @since 6.7.0
	 *
	 * @return FP_HTML_Token|null The adjusted current node.
	 */
	private function get_adjusted_current_node(): ?FP_HTML_Token {
			...
			switch ( $node->node_name ) {
				...
				case 'HTML':
					$this->state->insertion_mode = isset( $this->state->head_element )
						? FP_HTML_Processor_State::INSERTION_MODE_AFTER_HEAD
						: FP_HTML_Processor_State::INSERTION_MODE_BEFORE_HEAD;
					return;
			}
		}
	 ...
	 *
	 * @param FP_HTML_Token $token Name of bookmark pointing to element in original input HTML.
	 */
	private function insert_html_element( FP_HTML_Token $token ): void {
	 ...
	 * @see https://html.spec.whatwg.org/#insert-a-foreign-element
	 *
	 * @param FP_HTML_Token $token                     Insert this token. The token's namespace and
	 ...
	 */
	private function insert_foreign_element( FP_HTML_Token $token, bool $only_add_to_element_stack ): void {
		$adjusted_current_node = $this->get_adjusted_current_node();
	 ...
	 * @return FP_HTML_Token Newly-created virtual token.
	 */
	private function insert_virtual_node( $token_name, $bookmark_name = null ): FP_HTML_Token {
		$here = $this->bookmarks[ $this->state->current_token->bookmark_name ];
		$name = $bookmark_name ?? $this->bookmark_token();

		$this->bookmarks[ $name ] = new FP_HTML_Span( $here->start, 0 );

		$token = new FP_HTML_Token( $name, $token_name, false );
		$this->insert_html_element( $token );
		return $token;
	}
	 ...
	 *
	 * @param FP_HTML_Token|string $tag_name Node to check, or only its name if in the HTML namespace.
	 ...
	 *
	 * This class extends the FP_HTML_Tag_Processor, which has a public class
	 ...
	 * @access private
	 */
	const CONSTRUCTOR_UNLOCK_CODE = 'Use FP_HTML_Processor::create_fragment() instead of calling the class constructor directly.';
}
