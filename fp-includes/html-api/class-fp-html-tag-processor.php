<?php
/**
 * HTML API: FP_HTML_Tag_Processor class
 *
 ...
 * Example:
 *
 *     $tags = new FP_HTML_Tag_Processor( $html );
 ...
 * Example:
 *
 *     $processor = new FP_HTML_Tag_Processor( 'This <div is="a" partial="token' );
 ...
 * Example:
 *
 *     $processor = new FP_HTML_Tag_Processor( '<style>// there could be more styling to come' );
 *     false === $processor->next_tag();
 *
 *     $processor = new FP_HTML_Tag_Processor( '<style>// this is everything</style><div>' );
...
class FP_HTML_Tag_Processor {
	 ...
	 * @var int
	 *
	 * @see FP_HTML_Tag_Processor::set_bookmark()
	 */
	 ...
	 * @var int
	 *
	 * @see FP_HTML_Tag_Processor::seek()
	 */
	 ...
	 * @since 6.5.0
	 *
	 * @see FP_HTML_Tag_Processor::STATE_READY
	 * @see FP_HTML_Tag_Processor::STATE_COMPLETE
	 * @see FP_HTML_Tag_Processor::STATE_INCOMPLETE_INPUT
	 * @see FP_HTML_Tag_Processor::STATE_MATCHED_TAG
	 * @see FP_HTML_Tag_Processor::STATE_TEXT_NODE
	 * @see FP_HTML_Tag_Processor::STATE_CDATA_NODE
	 * @see FP_HTML_Tag_Processor::STATE_COMMENT
	 * @see FP_HTML_Tag_Processor::STATE_DOCTYPE
	 * @see FP_HTML_Tag_Processor::STATE_PRESUMPTUOUS_TAG
	 * @see FP_HTML_Tag_Processor::STATE_FUNKY_COMMENT
	 *
	 * @var string
	 */
	 ...
	 *         'id' => new FP_HTML_Attribute_Token( 'id', 9, 6, 5, 11, false )
	 *     );
	 *
	 ...
	 *     $this->attributes = array(
	 *         'id'    => new FP_HTML_Attribute_Token( 'id', 9, 6, 5, 11, false ),
	 *         'class' => new FP_HTML_Attribute_Token( 'class', 23, 7, 17, 13, false )
	 *     );
	 *
	 ...
	 *         'wp-block-group' => FP_HTML_Tag_Processor::ADD_CLASS,
	 *         'wp-group'       => FP_HTML_Tag_Processor::REMOVE_CLASS
	 *     );
	 *
	 * @since 6.2.0
	 * @var bool[]
	 ...
	 *     $modifications[] = new FP_HTML_Text_Replacement( $start, $length, $new_value );
	 *
	 *     // Correspondingly, something like this will appear in this array.
	 *     $lexical_updates = array(
	 *         FP_HTML_Text_Replacement( 14, 28, 'https://my-site.my-domain/wp-content/uploads/2014/08/kittens.jpg' )
	 *     );
	 *
	 * @since 6.2.0
	 * @var FP_HTML_Text_Replacement[]
	 */
	 ...
	 * Example:
	 *
	 *     $processor = new FP_HTML_Tag_Processor( '<input type="text" value="Th' );
	 ...
	 * Example:
	 *
	 *     $p = new FP_HTML_Tag_Processor( "<div class='free &lt;egg&lt;\tlang-en'>" );
	 *     $p->next_tag();
	...
	public function set_bookmark( $name ): bool {
		...
		$this->bookmarks[ $name ] = new FP_HTML_Span( $this->token_starts_at, $this->token_length );
	...
	private function parse_next_attribute(): bool {
		...
		if ( ! isset( $this->attributes[ $comparable_name ] ) ) {
			$this->attributes[ $comparable_name ] = new FP_HTML_Attribute_Token(
				$attribute_name,
				$value_start,
				$value_length,
				$attribute_start,
		 ...
		 */
		$duplicate_span = new FP_HTML_Span( $attribute_start, $attribute_end - $attribute_start );
	 ...
	 * @since 6.2.0
	 *
	 * @see FP_HTML_Tag_Processor::$lexical_updates
	 * @see FP_HTML_Tag_Processor::$classname_updates
	 */
	...
	/**
	 * Compare two FP_HTML_Text_Replacement objects.
	 *
	 * @since 6.2.0
	 *
	 * @param FP_HTML_Text_Replacement $a First attribute update.
	 * @param FP_HTML_Text_Replacement $b Second attribute update.
	 * @return int Comparison value for string order.
	 */
	private static function sort_start_ascending( FP_HTML_Text_Replacement $a, FP_HTML_Text_Replacement $b ): int {
	...
	private function get_enqueued_attribute_value( string $comparable_name ) {
		...
		return FP_HTML_Decoder::decode_attribute( $enqueued_value );
	}
	 ...
	 * Example:
	 *
	 *     $p = new FP_HTML_Tag_Processor( '<div enabled class="test" data-test-id="14">Test</div>' );
	...
	public function get_attribute( $name ) {
		...
		return FP_HTML_Decoder::decode_attribute( $raw_value );
	}
	 ...
	 * Example:
	 *
	 *     $p = new FP_HTML_Tag_Processor( '<div class="test">Test</div>' );
	 *     $p->next_tag() === true;
	 *     $p->get_tag() === 'DIV';
	 *
	 *     $p->next_tag() === false;
	 ...
	 * Example:
	 *
	 *     $p = new FP_HTML_Tag_Processor( '<div></div>' );
	 ...
	 * Example:
	 *
	 *     $processor = new FP_HTML_Tag_Processor( "\x00Apples & Oranges" );
	 ...
	 *
	 *     $processor = new FP_HTML_Tag_Processor( "&#x13; \r\n\tMore" );
	...
	public function subdivide_text_appropriately(): bool {
		...
		while ( $at < $end ) {
			...
			if ( $at < $end && '&' === $this->html[ $at ] ) {
				...
				$replacement         = FP_HTML_Decoder::read_character_reference( 'data', $this->html, $at, $matched_byte_length );
	...
	public function get_modifiable_text(): string {
		...
		$decoded = FP_HTML_Decoder::decode_text_node( $text );
	...
	public function set_modifiable_text( string $plaintext_content ): bool {
		if ( self::STATE_TEXT_NODE === $this->parser_state ) {
			$this->lexical_updates['modifiable text'] = new FP_HTML_Text_Replacement(
				$this->text_starts_at,
				$this->text_length,
	...
	public function set_attribute( $name, $value ): bool {
		...
		if ( isset( $this->attributes[ $comparable_name ] ) ) {
			...
			$this->lexical_updates[ $comparable_name ] = new FP_HTML_Text_Replacement(
	...
	public function remove_attribute( $name ): bool {
		 ...
		 *    Result: <div />
		 */
		$this->lexical_updates[ $name ] = new FP_HTML_Text_Replacement(
		...
		foreach ( $this->duplicate_attributes[ $name ] ?? array() as $attribute_token ) {
			$this->lexical_updates[] = new FP_HTML_Text_Replacement(
	 ...
	 * @since 6.2.0
	 *
	 * @see FP_HTML_Tag_Processor::get_updated_html()
	 *
	 ...
	 * its information.
	 *
	 * @return FP_HTML_Doctype_Info|null The DOCTYPE declaration information or `null` if not
	 *                                   currently at a DOCTYPE node.
	 */
	public function get_doctype_info(): ?FP_HTML_Doctype_Info {
		if ( self::STATE_DOCTYPE !== $this->parser_state ) {
			return null;
		}

		return FP_HTML_Doctype_Info::from_doctype_token( substr( $this->html, $this->token_starts_at, $this->token_length ) );
	}
	 ...
	 * @access private
	 */
	const STATE_FUNKY_COMMENT = 'STATE_FP_FUNKY';
