<?php
/**
 * HTML API: FP_HTML_Open_Elements class
 *
 * @package WordPress
 * @subpackage HTML-API
 * @since 6.4.0
 ...
 * @see FP_HTML_Processor
 */
class FP_HTML_Open_Elements {
	/**
	 ...
	 * @return FP_HTML_Token|null Name of the node on the stack at the given location,
	 *                            or `null` if the location isn't on the stack.
	 */
	public function at( int $nth ): ?FP_HTML_Token {
	 ...
	 * @since 6.4.0
	 *
	 * @param FP_HTML_Token $token Look for this node in the stack.
	 * @return bool Whether the referenced node is in the stack of open elements.
	 */
	public function contains_node( FP_HTML_Token $token ): bool {
		foreach ( $this->walk_up() as $item ) {
			if ( $token === $item ) {
				return true;
			}
	 ...
	 * @since 6.4.0
	 *
	 * @return FP_HTML_Token|null Last node in the stack of open elements, if one exists, otherwise null.
	 */
	public function current_node(): ?FP_HTML_Token {
	 ...
	 *
	 * @see FP_HTML_Tag_Processor::get_token_type
	 * @see FP_HTML_Tag_Processor::get_token_name
	 *
	 * @since 6.7.0
	 *
	 * @access private
	 ...
	 * @see https://html.spec.whatwg.org/#stack-of-open-elements
	 *
	 * @param FP_HTML_Token $stack_item Item to add onto stack.
	 */
	public function push( FP_HTML_Token $stack_item ): void {
	 ...
	 * @since 6.4.0
	 *
	 * @param FP_HTML_Token $token The node to remove from the stack of open elements.
	 * @return bool Whether the node was found and removed from the stack of open elements.
	 */
	public function remove_node( FP_HTML_Token $token ): bool {
	 ...
	 * see FP_HTML_Open_Elements::walk_up().
	 *
	 * @since 6.4.0
	 */
	 ...
	 * see FP_HTML_Open_Elements::walk_down().
	 *
	 * @since 6.4.0
	 * @since 6.5.0 Accepts $above_this_node to start traversal above a given node, if it exists.
	 *
	 * @param FP_HTML_Token|null $above_this_node Optional. Start traversing above this node,
	 *                                            if provided and if the node exists.
	 */
	public function walk_up( ?FP_HTML_Token $above_this_node = null ) {
	 ...
	 * @since 6.4.0
	 *
	 * @param FP_HTML_Token $item Element that was added to the stack of open elements.
	 */
	public function after_element_push( FP_HTML_Token $item ): void {
	 ...
	 * @since 6.4.0
	 *
	 * @param FP_HTML_Token $item Element that was removed from the stack of open elements.
	 */
	public function after_element_pop( FP_HTML_Token $item ): void {
		/*
		 * When adding support for new elements, expand this switch to trap
		 * cases where the precalculated value needs to change.
		 */
