...
class FP_Block {
	 ...
	 * @var array
	 *
	 * @see FP_Block::process_block_bindings()
	 */
	 ...
	 * @param FP_Block_Type_Registry $registry          Optional block type registry.
	 */
	 ...
	 *
	 * If the block has inner blocks, the method recursively processes them by creating new instances of `FP_Block`
	 ...
	 *
	 * If the block has inner blocks, this method initializes a new `FP_Block_List` for them, ensuring the
	...
	public function refresh_parsed_block_dependents() {
		if ( ! empty( $this->parsed_block['innerBlocks'] ) ) {
			...
			$this->inner_blocks = new FP_Block_List( $this->parsed_block['innerBlocks'], $child_context, $this->registry );
		}
	...
	private function replace_html( string $block_content, string $attribute_name, $source_value ) {
		...
		switch ( $block_type->attributes[ $attribute_name ]['source'] ) {
			...
			case 'rich-text':
				...
				foreach ( $selectors as $selector ) {
					...
					if ( strcasecmp( $block_reader->get_tag(), $selector ) === 0 || $block_reader->next_tag(
						...
						// TODO: Use `FP_HTML_Processor::set_inner_html` method once it's available.
			...
			case 'attribute':
				$amended_content = new FP_HTML_Tag_Processor( $block_content );
	...
	private static function get_block_bindings_processor( string $block_content ) {
		$internal_processor_class = new class('', FP_HTML_Processor::CONSTRUCTOR_UNLOCK_CODE) extends FP_HTML_Processor {
			/**
	...
	public function render( $options = array() ) {
		...
		if ( $is_dynamic ) {
			$global_post = $post;
			$parent      = FP_Block_Supports::$block_to_render;

			FP_Block_Supports::$block_to_render = $this->parsed_block;

			$block_content = (string) call_user_func( $this->block_type->render_callback, $this->attributes, $block_content, $this );

			FP_Block_Supports::$block_to_render = $parent;
		 ...
		 * @param FP_Block $instance      The block instance.
		 */
