<?php
/**
 * FP_Application_Passwords class
 *
 * @package WordPress
 * @since   5.6.0
 */
...
class FP_Application_Passwords {
	 ...
	 *
	 * @since 5.6.0
	 * @since 5.7.0 Returns FP_Error if application name already exists.
	 ...
	 *     Application password details, or a FP_Error instance if an error occurs.
	 *
	...
	public static function create_new_application_password( $user_id, $args = array() ) {
		...
		if ( empty( $args['name'] ) ) {
			return new FP_Error( 'application_password_empty_name', __( 'An application name is required to create an application password.' ), array( 'status' => 400 ) );
		}
		...
		if ( ! $saved ) {
			return new FP_Error( 'db_error', __( 'Could not save application password.' ) );
		}
	 ...
	 *     @type string|null $last_ip   The IP address the application password was last used by.
	 * }
	 * @return true|FP_Error True if successful, otherwise a FP_Error instance is returned on error.
	 */
	public static function update_application_password( $user_id, $uuid, $update = array() ) {
		...
		return new FP_Error( 'application_password_not_found', __( 'Could not find an application password with that id.' ) );
	}
	 ...
	 * @param int    $user_id User ID.
	 * @param string $uuid    The password's UUID.
	 * @return true|FP_Error True if the usage was recorded, a FP_Error if an error occurs.
	 */
	 ...
	 * @param int    $user_id User ID.
	 * @param string $uuid    The password's UUID.
	 * @return true|FP_Error Whether the password was successfully found and deleted, a FP_Error otherwise.
	 */
	public static function delete_application_password( $user_id, $uuid ) {
		...
		foreach ( $passwords as $key => $item ) {
			if ( $item['uuid'] === $uuid ) {
				...
				if ( ! $saved ) {
					return new FP_Error( 'db_error', __( 'Could not delete application password.' ) );
				}
	 ...
	 *
	 * @param int $user_id User ID.
	 * @return int|FP_Error The number of passwords that were deleted or a FP_Error on failure.
	 */
	public static function delete_all_application_passwords( $user_id ) {
		...
		if ( $passwords ) {
			...
			if ( ! $saved ) {
				return new FP_Error( 'db_error', __( 'Could not delete application passwords.' ) );
			}
