 ...
 *
 * The Rewrite along with FP class function as a front controller for WordPress.
...
class FP_Rewrite {
	 ...
	 * `FP_Rewrite::$index` with a trailing slash appended. Otherwise the root
	 * will be empty.
	 *
	 * @since 1.5.0
	 * @var string
	 *
	 * @see FP_Rewrite::init()
	 * @see FP_Rewrite::using_index_permalinks()
	 */
	public $root = '';
	 ...
	 * @var bool
	 *
	 * @see FP_Rewrite::mod_rewrite_rules()
	 */
	 ...
	 * Now it tells FP::parse_request() to check if a URL matching the page
	 ...
	 * `FP_Rewrite::$rewritereplace` and are rewritten to the query
	 * variables listed in FP_Rewrite::$queryreplace.
	 *
	 ...
	 * of rewrite tags, see FP_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var string[]
	 */
	...
	/**
	 * Query variables that rewrite tags map to, see FP_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var string[]
	 */
	 ...
	 * @since 1.5.0
	 *
	 * @see FP_Rewrite::$rewritecode
	 * @see FP_Rewrite::$rewritereplace
	 * @see FP_Rewrite::$queryreplace
	 *
	 ...
	 *
	 * The main FP_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of FP_Rewrite::generate_rewrite_rules() that allows for shorter
	 * list of parameters. See the method for longer description of what generating
	 * rewrite rules does.
	 *
	 * @since 1.5.0
	 *
	 * @see FP_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 *
	 ...
	 * reference to the current FP_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		 ...
		 * @since 1.5.0
		 *
		 * @param FP_Rewrite $wp_rewrite Current FP_Rewrite instance (passed by reference).
		 */
	 ...
	 *
	 * The difference between this method and FP_Rewrite::rewrite_rules() is that
	...
	public function mod_rewrite_rules() {
		...
		// Add in the rules that don't redirect to FP's index.php (and thus shouldn't be handled by FP at all).
	 ...
	 * query strings. The new permastruct is added to the FP_Rewrite::$extra_permastructs array.
	 *
	 * When the rewrite rules are built by FP_Rewrite::rewrite_rules(), all of these extra
	 * permastructs are passed to FP_Rewrite::generate_rewrite_rules() which transforms them
	 * into the regular expressions that many love to hate.
	 *
	 * The `$args` parameter gives you control over how FP_Rewrite::generate_rewrite_rules()
	 ...
	 *     Default empty array.
	 *
	 *     @type bool $with_front  Whether the structure should be prepended with `FP_Rewrite::$front`.
	 *                             Default true.
	 ...
	 *
	 * Calls FP_Rewrite::wp_rewrite_rules() after removing the 'rewrite_rules' option.
	 ...
	 * FP_Rewrite::init() after the option is updated.
	 *
	 ...
	 * the current category base and the parameter value. Calls FP_Rewrite::init()
	 ...
	 * current tag base and the parameter value. Calls FP_Rewrite::init() after
